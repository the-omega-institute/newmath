...
FLINDERS_PROPHAGE_DIR = RM_CACHE_DIR / "flinders_prophage"
FLINDERS_PHISPY_BULK_PATH = FLINDERS_PROPHAGE_DIR / "phispy_all_predictions.tsv.gz"
FLINDERS_HOST_PROPHAGE_PATH = FLINDERS_PROPHAGE_DIR / "host_prophages.tsv"
FLINDERS_HOST_PROPHAGE_META_PATH = FLINDERS_PROPHAGE_DIR / "host_prophages.meta.json"
...
def assembly_accession_core(accession: str) -> str:
    match = re.search(r"\b(?:GC[AF])_(\d+\.\d+)\b", accession.strip())
...
def esummary_assembly_id(assembly_id: str, deadline: float | None = None) -> tuple[dict[str, object] | None, dict[str, object]]:
    ...
    ftp_refseq = node.findtext("FtpPath_RefSeq") or ""
    ftp_genbank = node.findtext("FtpPath_GenBank") or ""
    ftp = ftp_refseq or ftp_genbank
    row = {
        ...
        "ftp_path": ftp,
        "ftp_path_refseq": ftp_refseq,
        "ftp_path_genbank": ftp_genbank,
...
def genbank_summary_for_accession(accession: str, deadline: float | None = None) -> tuple[dict[str, object] | None, dict[str, object]]:
    cache_path = RM_CACHE_DIR / f"{safe_name(accession)}_genbank_assembly_summary.json"
    cached = cached_json(cache_path)
    if isinstance(cached, dict) and cached.get("ftp_path"):
        return cached, {"on_disk_cache_hit": True, "cache_path": str(cache_path)}
    ids, search_contact = esearch_assembly_ids(accession, retmax=1, deadline=deadline)
    if not ids:
        return None, {"search": search_contact, "error": "assembly_search_failed", "accession": accession}
    row, summary_contact = esummary_assembly_id(ids[0], deadline=deadline)
    if not row:
        return None, {"search": search_contact, "summary": summary_contact, "error": "assembly_summary_failed", "accession": accession}
    genbank_ftp = str(row.get("ftp_path_genbank") or row.get("ftp_path") or "")
    if not genbank_ftp:
        return None, {"search": search_contact, "summary": summary_contact, "error": "genbank_ftp_missing", "accession": accession}
    gb_row = dict(row)
    gb_row["assembly_accession"] = accession
    gb_row["ftp_path"] = genbank_ftp
    write_json(cache_path, gb_row)
    return gb_row, {"search": search_contact, "summary": summary_contact, "on_disk_cache_hit": False, "cache_path": str(cache_path)}
...
def gtdb_lookup_from_rows(limit_genera: int = 200000, deadline: float | None = None) -> tuple[dict[str, dict[str, str]], dict[str, object]]:
    ...
    for domain in ("Bacteria", "Archaea"):
        ...
        for row in rows:
            ...
            genus = str(row.get("gtdb_genus") or row.get("genus") or "")
            tax = {
            ...
            if accession:
                lookup[accession] = tax
                lookup.setdefault(f"core:{assembly_accession_core(accession)}", tax)
            if genus:
                key = f"genus:{genus.lower()}"
                if key not in lookup:
                    genus_hits += 1
                lookup.setdefault(key, tax)
    n_accession_keys = sum(1 for key in lookup if key.startswith(("GCF_", "GCA_")))
    return lookup, {"contacts": contacts, "n_gtdb_accessions": n_accession_keys, "n_gtdb_genus_keys": genus_hits}
...
def infer_family_genus(assembly_row: dict[str, object], gtdb_lookup: dict[str, dict[str, str]]) -> dict[str, str]:
    ...
    core_key = f"core:{assembly_accession_core(accession)}" if accession else ""
    if core_key and core_key in gtdb_lookup:
        return gtdb_lookup[core_key]
    organism = str(assembly_row.get("organism") or assembly_row.get("species_name") or "")
    genus = organism.split()[0] if organism.split() else ""
    genus_key = f"genus:{genus.lower()}" if genus else ""
    if genus_key and genus_key in gtdb_lookup:
        return gtdb_lookup[genus_key]
...
def flinders_source_meta() -> dict[str, object]:
    if not FLINDERS_PHISPY_BULK_PATH.exists():
        return {"source_path": str(FLINDERS_PHISPY_BULK_PATH), "error": "flinders_phispy_bulk_missing"}
    stat = FLINDERS_PHISPY_BULK_PATH.stat()
    return {
        "source_path": str(FLINDERS_PHISPY_BULK_PATH),
        "source_size": stat.st_size,
        "source_mtime_ns": stat.st_mtime_ns,
    }


def load_flinders_host_prophage_meta() -> dict[str, object]:
    cached = cached_json(FLINDERS_HOST_PROPHAGE_META_PATH)
    return cached if isinstance(cached, dict) else {}


def flinders_cache_covers(host_accessions: set[str]) -> bool:
    if not FLINDERS_HOST_PROPHAGE_PATH.exists() or FLINDERS_HOST_PROPHAGE_PATH.stat().st_size == 0:
        return False
    meta = load_flinders_host_prophage_meta()
    source = flinders_source_meta()
    if source.get("error"):
        return False
    if meta.get("source_size") != source.get("source_size") or meta.get("source_mtime_ns") != source.get("source_mtime_ns"):
        return False
    requested = {assembly_accession_core(accession) for accession in host_accessions if accession}
    cached_cores = set(str(core) for core in meta.get("host_cores", []) if core)
    return requested.issubset(cached_cores)


def scan_flinders_host_prophages(host_accessions: set[str]) -> dict[str, object]:
    source = flinders_source_meta()
    if source.get("error"):
        FLINDERS_PROPHAGE_DIR.mkdir(parents=True, exist_ok=True)
        FLINDERS_HOST_PROPHAGE_PATH.write_text(
            "host_assembly\tflinders_genomeid\tcontig\tstart\tstop\tlength\tmatch_mode\n",
            encoding="utf-8",
        )
        write_json(FLINDERS_HOST_PROPHAGE_META_PATH, {**source, "host_cores": sorted(host_accessions), "n_rows": 0})
        return source

    exact_to_host = {accession: accession for accession in host_accessions if accession}
    core_to_hosts: dict[str, list[str]] = defaultdict(list)
    for accession in sorted(host_accessions):
        core_to_hosts[assembly_accession_core(accession)].append(accession)

    rows_written = 0
    kept_seen = 0
    matched_hosts: set[str] = set()
    FLINDERS_PROPHAGE_DIR.mkdir(parents=True, exist_ok=True)
    with gzip.open(FLINDERS_PHISPY_BULK_PATH, "rt", encoding="utf-8", newline="") as handle, FLINDERS_HOST_PROPHAGE_PATH.open(
        "w", encoding="utf-8", newline=""
    ) as out:
        reader = csv.DictReader(handle, delimiter="\t")
        writer = csv.DictWriter(
            out,
            fieldnames=["host_assembly", "flinders_genomeid", "contig", "start", "stop", "length", "match_mode"],
            delimiter="\t",
            lineterminator="\n",
        )
        writer.writeheader()
        for row in reader:
            if row.get("Decision") != "Kept":
                continue
            kept_seen += 1
            genomeid = str(row.get("GENOMEID") or "").strip()
            if not genomeid:
                continue
            if genomeid in exact_to_host:
                hosts = [exact_to_host[genomeid]]
                match_mode = "assembly_exact_flinders_genomeid"
            else:
                hosts = core_to_hosts.get(assembly_accession_core(genomeid), [])
                match_mode = "assembly_numeric_core_flinders_genomeid"
            if not hosts:
                continue
            try:
                start = int(str(row.get("Start") or "0"))
                stop = int(str(row.get("Stop") or "0"))
                length = int(str(row.get("Length") or "0"))
            except ValueError:
                continue
            ...
            for host in hosts:
                writer.writerow(
                    {
                        "host_assembly": host,
                        "flinders_genomeid": genomeid,
                        "contig": str(row.get("Contig") or "").strip(),
                        "start": start,
                        "stop": stop,
                        "length": length,
    ...
    meta = {
        **source,
        "host_cores": sorted({assembly_accession_core(accession) for accession in host_accessions if accession}),
        "n_requested_hosts": len(host_accessions),
        "n_matched_hosts": len(matched_hosts),
        "n_host_prophage_rows": rows_written,
        "n_kept_rows_seen": kept_seen,
        "cache_path": str(FLINDERS_HOST_PROPHAGE_PATH),
    }
    write_json(FLINDERS_HOST_PROPHAGE_META_PATH, meta)
    return meta


def load_flinders_host_prophage_rows(host_accessions: set[str]) -> tuple[dict[str, list[dict[str, object]]], dict[str, object]]:
    if not flinders_cache_covers(host_accessions):
        meta = scan_flinders_host_prophages(host_accessions)
        cache_hit = False
    else:
        meta = load_flinders_host_prophage_meta()
        cache_hit = True
    by_host: dict[str, list[dict[str, object]]] = defaultdict(list)
    if FLINDERS_HOST_PROPHAGE_PATH.exists():
        with FLINDERS_HOST_PROPHAGE_PATH.open("r", encoding="utf-8", newline="") as handle:
            reader = csv.DictReader(handle, delimiter="\t")
            for row in reader:
                host = str(row.get("host_assembly") or "")
                if host not in host_accessions:
                    continue
                try:
                    start = int(str(row.get("start") or "0"))
                    stop = int(str(row.get("stop") or "0"))
    ...
    matched_hosts = {host for host, rows in by_host.items() if rows}
    return by_host, {
        **meta,
        "on_disk_cache_hit": cache_hit,
        "n_hosts_requested_this_run": len(host_accessions),
        "n_hosts_matched_this_run": len(matched_hosts),
        "n_regions_this_run": sum(len(rows) for rows in by_host.values()),
    }


def prophage_regions_for_host(assembly_accession: str) -> list[tuple[str, int, int, int]]:
    rows, _meta = load_flinders_host_prophage_rows({assembly_accession})
    return [
        (str(row["contig"]), int(row["start"]), int(row["stop"]), int(row["length"]))
        for row in rows.get(assembly_accession, [])
    ]


def region_signature(regions: list[dict[str, object]]) -> str:
    payload = json.dumps(
        [
            [
                str(row.get("flinders_genomeid") or ""),
                str(row.get("contig") or ""),
                int(row.get("start") or 0),
                int(row.get("stop") or 0),
...
def prophage_inventory_for_host(
    accession: str,
    ftp_path: str,
    regions: list[dict[str, object]],
    deadline: float | None = None,
) -> tuple[dict[str, object], dict[str, object]]:
    signature = region_signature(regions)
    cache_path = RM_CACHE_DIR / f"{safe_name(accession)}_prophage_inventory.json"
    cached = cached_json(cache_path)
    if isinstance(cached, dict) and cached.get("host_accession") == accession and cached.get("region_signature") == signature:
        return cached, {"on_disk_cache_hit": True, "cache_path": str(cache_path)}
    if not regions:
        row = {
            "host_accession": accession,
            "target_type": "prophage",
            "num_targets": 0,
            "total_target_bp": 0,
            "median_target_len": 0,
            "target_accessions": [],
        ...
        write_json(cache_path, row)
        return row, {"cache_path": str(cache_path), "skipped": True, "reason": "no_flinders_kept_regions"}
    flinders_genomeids = sorted({str(region.get("flinders_genomeid") or "") for region in regions if region.get("flinders_genomeid")})
    fetch_accession = accession
    fetch_ftp_path = ftp_path
    fetch_mode = "mapped_host_assembly"
    if flinders_genomeids:
        gb_row, gb_contact = genbank_summary_for_accession(flinders_genomeids[0], deadline=deadline)
        if gb_row and gb_row.get("ftp_path"):
            fetch_accession = flinders_genomeids[0]
            fetch_ftp_path = str(gb_row["ftp_path"])
            fetch_mode = "flinders_genbank_assembly"
        else:
            fetch_mode = "mapped_host_assembly_after_genbank_lookup_failed"
    else:
        gb_contact = {"skipped": True, "reason": "regions_without_flinders_genomeid"}
    text, contact = cached_assembly_file(fetch_ftp_path, "genomic.fna.gz", fetch_accession, deadline=deadline)
    if not text:
        row = {
            "host_accession": accession,
            "target_type": "prophage",
        ...
        write_json(cache_path, row)
        return row, {"fetch": contact, "genbank_lookup": gb_contact, "cache_path": str(cache_path)}
    fasta = {header.split()[0]: seq for header, seq in parse_fasta_full(text)}
    lengths = []
    target_accessions = []
    extracted_bp = 0
    ...
    for region in regions:
        ...
        seq = fasta.get(contig)
        if seq is None:
            extraction_errors["contig_not_found"] += 1
            continue
        if start <= 0 or stop < start or stop > len(seq):
            extraction_errors["coordinate_out_of_bounds"] += 1
            continue
        subseq = re.sub(r"[^ACGT]", "", seq[start - 1 : stop].upper())
        if not subseq:
            extraction_errors["empty_or_ambiguous_sequence"] += 1
            continue
        lengths.append(declared_length if declared_length > 0 else len(subseq))
        extracted_bp += len(subseq)
        target_accessions.append(f"{contig}:{start}-{stop}")
    row = {
        "host_accession": accession,
        "target_type": "prophage",
        "num_targets": len(lengths),
        ...
        "target_accessions": target_accessions[:50],
        "region_signature": signature,
        "extracted_target_bp": extracted_bp,
        "extraction_errors": dict(extraction_errors),
        "fetch_accession": fetch_accession,
        "fetch_mode": fetch_mode,
        "flinders_genomeids": flinders_genomeids,
    }
    if extraction_errors and not lengths:
        row["error"] = "prophage_sequence_extraction_failed"
    write_json(cache_path, row)
    return row, {"fetch": contact, "genbank_lookup": gb_contact, "cache_path": str(cache_path)}


def prophage_target_inventory(
    mapped_carriers: list[dict[str, object]],
    deadline: float | None = None,
) -> tuple[list[dict[str, object]], dict[str, object]]:
    hosts: dict[str, str] = {}
    for row in mapped_carriers:
        ...
        if accession and ftp_path:
            hosts[accession] = ftp_path
    regions_by_host, flinders_meta = load_flinders_host_prophage_rows(set(hosts))
    inventory = []
    errors = Counter()
    for accession, ftp_path in sorted(hosts.items()):
        if deadline_expired(deadline):
            errors["deadline_skipped"] += 1
            break
        row, _contact = prophage_inventory_for_host(accession, ftp_path, regions_by_host.get(accession, []), deadline=deadline)
        if row.get("error"):
            errors[str(row["error"])] += 1
        inventory.append(row)
    return inventory, {
        "n_hosts_checked": len(inventory),
        "n_hosts_with_prophage": sum(1 for row in inventory if int(row.get("num_targets") or 0) > 0),
        "errors": dict(errors),
        "assembly_cache_dir": str(ASD_CACHE_DIR),
