#!/usr/bin/env python3
"""B*_Q6 residual-basis concentration in the H(3,4) E3 eigenspace.
...
are labeled "*".  The verdict is scoped to excess E3 concentration relative
to a fixed-seed matched synonymous contrast null.
"""
...
EXPERIMENT_ID = "bstar_q6_e3_spectral_concentration"
CLAIM_ID = "bridge.genetic_code.bstar_q6_e3_spectral_concentration"
...
SECONDARY_NULL_SEED = "bstar_q6_e3_spectral_concentration.secondary.via.fixed.hash.seed"
...
def secondary_random_vector(
    template: dict[str, object],
    codons: list[str],
    index: dict[str, int],
    eligible_families: list[list[str]],
    rng: random.Random,
) -> list[float]:
    vector = [0.0 for _ in codons]
    groups = template["fiber_groups"]
    ...
    for group in groups:
        ...
        candidates = [
            family_id
            for family_id, family_codons in enumerate(eligible_families)
            if len(family_codons) >= count and family_id not in used_family_ids
        ]
        if not candidates:
            candidates = [
                family_id
                for family_id, family_codons in enumerate(eligible_families)
                if len(family_codons) >= count
            ]
        family_id = rng.choice(candidates)
        used_family_ids.add(family_id)
        support = rng.sample(eligible_families[family_id], count)
        values = zero_sum_random_values(count, rng)
        for codon, value in zip(support, values):
            vector[index[codon]] = value
    return scale_to_norm(vector, float(template["norm"]))
...
def null_t3_values(
    ...
    for _ in range(N_NULL):
        ...
        values.append(sum(basis_projection_mass(q, e3_basis) for q in q_basis) / len(q_basis))
    return values


def secondary_null_t3_values(
    templates: list[dict[str, object]],
    codons: list[str],
    index: dict[str, int],
    ...
    eligible_families = [
        sorted(family_codons, key=lambda codon: index[codon])
        for aa, family_codons in families.items()
        if aa != "*" and len(family_codons) >= 2
    ]
    values = []
    for _ in range(N_NULL):
        columns = [
            secondary_random_vector(template, codons, index, eligible_families, rng)
    ...
    try:
        ...
        if not all(row["ok"] for row in checks):
            emit(
                ...
                upper_p=None,
                lower_p=None,
                secondary_null_mean=None,
                secondary_upper_p=None,
                n_null=N_NULL,
                alpha=ALPHA,
                checks=checks,
        ...
        secondary_rng = random.Random(stable_seed(SECONDARY_NULL_SEED))
        null_values = null_t3_values(templates, codons, index, bases_by_level[3], primary_rng)
        secondary_values = secondary_null_t3_values(templates, codons, index, families, bases_by_level[3], secondary_rng)

        t3 = t_profile[3]
        null_mean = mean(null_values)
        null_std = sample_std(null_values, null_mean)
        null_z = (t3 - null_mean) / null_std if null_std > 0.0 else math.inf
        excess_d = t3 - null_mean
        upper_p, lower_p = tail_p_values(null_values, t3)
        secondary_mean = mean(secondary_values)
        secondary_upper_p, _ = tail_p_values(secondary_values, t3)

        if upper_p <= ALPHA and excess_d > 0.0:
            status = "certified"
        elif upper_p > ALPHA and lower_p > ALPHA:
            status = "coincidence"
        elif lower_p <= ALPHA:
            status = "refuted"
        else:
        ...
        reason = (
            "Scoped verdict for B*_Q6 E3-concentration extremality vs a matched-synonymous null: "
            "the null preserves each template's synonymous family/support/norm constraints, so the "
            "reported signal is excess-over-generic synonymous contrast structure rather than raw E3 mass."
        )
        ...
        emit(
            ...
            excess_D=round_float(excess_d),
            upper_p=round_float(upper_p),
            lower_p=round_float(lower_p),
            secondary_null_mean=round_float(secondary_mean),
            secondary_upper_p=round_float(secondary_upper_p),
            n_null=N_NULL,
            alpha=ALPHA,
            checks=checks,
            reason=reason,
