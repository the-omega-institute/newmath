...
def build_cds_codon_counts(repo: pathlib.Path, config: dict[str, str]) -> tuple[dict[str, object] | None, str | None]:
    ...
    out_path = repo / f"tools/bio_reality/data/cds_codon_counts_{organism}.json"
    if out_path.exists():
    ...
    try:
        ...
        payload = {
            "schema_version": "bio_reality_cds_codon_counts_v1",
            "organism": organism,
...
def main() -> None:
    ...
    try:
        ...
        for config in ORGANISMS_TO_FETCH:
            organism = config["organism"]
            trna_path = repo / f"tools/bio_reality/data/gtrnadb_trna_all_copy_{organism}.json"
        ...
        for config in EXISTING_FULL:
            ...
            trna_path = repo / f"tools/bio_reality/data/gtrnadb_trna_all_copy_{config['trna_organism']}.json"
