...
def fetch_url(url: str) -> bytes:
    ...
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": "BioReality-Codex-UniProtProteinFeatures/1.0",
            "Connection": "close",
        },
    )
...
def protein_feature_data_path(repo: pathlib.Path, slug: str) -> pathlib.Path:
    return repo / f"tools/bio_reality/data/uniprot_protein_features_{slug}.json"
...
def load_or_fetch_protein_features(repo: pathlib.Path, pair: dict[str, str]) -> tuple[dict[str, object] | None, dict[str, object]]:
    ...
    try:
        ...
        parsed["provenance"] = {
            ...
            "fetch_user_agent": "BioReality-Codex-UniProtProteinFeatures/1.0",
