...
def trna_path(repo: pathlib.Path, organism: str) -> pathlib.Path:
    return repo / f"tools/bio_reality/data/gtrnadb_trna_all_copy_{trna_organism_for(organism)}.json"


def proteomics_n_proteins(repo: pathlib.Path, organism: str) -> int | None:
    path = repo / f"tools/bio_reality/data/proteomics_abundance_{organism}.json"
    if not path.exists():
        return None
...
def main() -> None:
    ...
    required += [f"tools/bio_reality/data/cds_codon_abundance_{organism}.json" for organism in ORGANISMS]
    required += [f"tools/bio_reality/data/proteomics_abundance_{organism}.json" for organism in ORGANISMS]
