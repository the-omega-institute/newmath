#!/usr/bin/env python3
"""Measured TE as an H-candidate for the modeled-tAI complement residual.

This is a descriptive statistical projection audit. It tests whether measured
ribo-seq TE absorbs the B*_Q6 abundance component left after modeled tAI, not a
causal mediation or mechanism claim.
"""
...
EXPERIMENT_ID = "b_star_q6_h_candidate_measured_te_powered"
CLAIM_ID = "h3.cross_layer_relation.h_candidate_measured_te.b_star_q6_nontranslation_residual_powered"
CONJECTURE_ID = "q6.h-candidate-measured-te.non-translation-residual.cross-layer"
...
def h_candidate_rows(
    *,
    ...
    return x_rows, t_mod_rows, t_meas_rows, y_rows, z_rows, summary
...
def h_candidate_decomposition(
    *,
    ...
    absorbed_vector, tmeas_coeff = project_onto_design(t_meas_tilde, p_q_perp_tmod)
    combined_design = append_columns(t_mod_tilde, t_meas_tilde)
    ...
    absorbed_by_meas_te = None if p_q_perp_tmod_norm2 <= SURVIVAL_EPS else bounded_unit(absorbed_norm2 / p_q_perp_tmod_norm2)
    ...
    return {
        ...
        "P_Q_perp_Tmod_absorbed_by_Tmeas_norm2": absorbed_norm2,
        "P_Q_perp_combined_norm2": p_q_perp_combined_norm2,
        "d_modeled": d_modeled,
        "absorbed_by_measTE": absorbed_by_meas_te,
        "d_combined": d_combined,
        "coefficients": {
            "Q_to_P": q_coeff,
            "Tmod_on_P_Q": tmod_coeff,
            "Tmeas_on_P_Q_perp_Tmod": tmeas_coeff,
            "Tmod_Tmeas_on_P_Q": combined_coeff,
        },
    }


def interpretation_label(absorbed_by_meas_te: float | None) -> str:
    if absorbed_by_meas_te is None:
        return "no_modeled_complement_energy_to_classify"
    if absorbed_by_meas_te >= ABSORPTION_LABEL_THRESHOLD:
        return "residual_is_missed_translation"
    return "residual_is_genuinely_non_translation"


def cannot_claim() -> list[str]:
    return [
        "statistical projection, not causal",
        "measured TE single/condition-specific dataset",
        "absorbed fraction is descriptive; not a causal mediator test",
        "if absorbed low, the non-translation residual remains UNEXPLAINED - H still open, needs mRNA/turnover/localization",
    ]
...
def conclusion_from_absorption(absorptions: list[float]) -> dict[str, object]:
    ...
    if len(high) == len(absorptions):
        ...
        verdict = "measured TE is a plausible H candidate for this residual in this descriptive projection"
        residual_read = "the modeled-tAI residual is mostly compatible with missed translation information"
    elif len(low) == len(absorptions):
        consistency = "consistently_low"
        verdict = "measured TE is not sufficient as H for this residual"
        residual_read = "the residual behaves as genuinely non-translation relative to this measured TE readout"
    else:
        consistency = "mixed"
        verdict = "measured TE is not a stable standalone H across organisms"
        residual_read = "some residual may be missed translation, but the cross-organism pattern is not consistent"
    return {
        "threshold_for_descriptive_label_only": ABSORPTION_LABEL_THRESHOLD,
        "high_count": len(high),
        "low_count": len(low),
        "consistency": consistency,
        "measured_TE_H_candidate_conclusion": verdict,
        "nontranslation_residual_conclusion": residual_read,
    }
...
def main() -> None:
    ...
    for pair in ORGANISM_PAIRS:
        ...
        required.append(f"tools/bio_reality/data/ribosome_te_{organism}.json")
        required.append(f"tools/bio_reality/data/proteomics_abundance_{organism}.json")
        required.append(f"tools/bio_reality/data/cds_codon_abundance_{organism}.json")
        required.append(f"tools/bio_reality/data/gtrnadb_trna_all_copy_{trna_organism}.json")
    missing = [path for path in required if not (repo / path).exists()]
    if missing:
        emit("needs_data", missing_required_data=missing, reason="required local measured TE, modeled tAI, PAXdb abundance, and CDS codon data not present")
    ...
    try:
        ...
        for pair in ORGANISM_PAIRS:
            ...
            te_payload = load_json(repo / f"tools/bio_reality/data/ribosome_te_{organism}.json")
            ...
            x_rows, t_mod_rows, t_meas_rows, y_rows, z_rows, data_summary = h_candidate_rows(
            ...
            absorbed = decomposition["absorbed_by_measTE"]
            ...
            t_meas_rank = explained_by_design(t_meas_tilde, y_tilde)[1]
            combined_rank = explained_by_design(append_columns(t_mod_tilde, t_meas_tilde), y_tilde)[1]
            ...
            organism_residualized_ok = (
                ...
                and len(t_meas_tilde) == n_proteins
            ...
            alignment_actual[organism] = {
                ...
                "interpretation_if_failed": "negative self-check: measured-TE availability changes the protein_id join used by the H-candidate projection",
            }
            ...
            per_organism[organism] = {
                ...
                **tai_summary,
                ...
                "norms": {
                    ...
                    "P_Q_perp_Tmod_absorbed_by_Tmeas_norm2": decomposition["P_Q_perp_Tmod_absorbed_by_Tmeas_norm2"],
                    "P_Q_perp_combined_norm2": decomposition["P_Q_perp_combined_norm2"],
                },
                "rank_condition_diagnostics": {
                    "rank_Q_e": x_rank,
                    "rank_Tmod": t_mod_rank,
                    "rank_Tmeas": t_meas_rank,
                    "rank_Tmod_Tmeas": combined_rank,
                    "rank_Z": rank_z,
                    ...
                    "condition_number_Tmeas_gram": condition_number_from_gram(gram_matrix(t_meas_tilde)),
                    "condition_number_Tmod_Tmeas_gram": condition_number_from_gram(gram_matrix(append_columns(t_mod_tilde, t_meas_tilde))),
                },
            }
        ...
        checks = [
            {
                ...
                "actual": input_actual,
                "expected": f"4 organisms with n >= {MIN_PROTEINS_PER_ORGANISM} after measured TE x modeled tAI x PAXdb abundance x CDS protein_id join",
            },
            {
                ...
                "expected": "Q_e, T_mod, T_meas, and P_e residualized against the same Z controls",
            },
            {
            ...
            },
            {
                ...
                "interpretation_if_failed": "reported honestly; the H-candidate projection uses the measured-TE protein_id intersection, while the upstream complement experiment used the full abundance/CDS join",
            },
            {
                "name": "measured_te_absorption_computed",
                "passed": all_absorption_ok,
                "actual": {
                    organism: {
                        "absorbed_by_measTE": result["absorbed_by_measTE"],
                        "d_combined": result["d_combined"],
                    }
                    for organism, result in per_organism.items()
                    if isinstance(result, dict)
                },
                "expected": "absorbed_by_measTE and d_combined are finite values in [0,1]",
            },
            {
        ...
        if status == "passed" and not all_alignment_ok:
            reason = "computed on the measured-TE all-readout join; d_modeled does not match the full-join complement reference within 0.02 for every organism"
        if status != "passed":
        ...
        emit(
            ...
            result={
                ...
                "statement": "Measured ribo-seq TE is tested as a first H-candidate by projecting the modeled-tAI complement abundance residual P_hat_{Q perp Tmod} onto residualized measured TE.",
                ...
                "decomposition": {
                    ...
                    "T_meas": "residualized log10 measured ribo-seq TE after controls Z",
                    "P_e": "residualized log10 PAXdb abundance after controls Z",
                    "P_hat_Q": "Pi_{Q_e} P_e",
                    "P_hat_Q_perp_Tmod": "(I - Pi_{Tmod}) P_hat_Q",
                    "absorbed_by_measTE": "||Pi_{Tmeas} P_hat_{Q perp Tmod}||^2 / ||P_hat_{Q perp Tmod}||^2",
                    "P_hat_Q_perp_Tmod_Tmeas": "(I - Pi_{[Tmod,Tmeas]}) P_hat_Q",
                    "d_combined": "||P_hat_Q_perp_Tmod_Tmeas||^2 / ||P_hat_Q||^2",
                    "ridge_epsilon": RIDGE_EPS,
                },
                "coordinates": q_names,
                "per_organism": per_organism,
                "cross_organism": {
                    "absorbed_by_measTE_distribution": {
                        "values_by_organism": {
                            organism: result["absorbed_by_measTE"]
                            for organism, result in per_organism.items()
                            if isinstance(result, dict)
                        },
                    ...
                    **conclusion,
                    "ordered_by_absorbed_by_measTE": sorted(
                        [
                            {
                                "organism": organism,
                                "absorbed_by_measTE": float(result["absorbed_by_measTE"]),
                            ...
                            for organism, result in per_organism.items()
                            if isinstance(result, dict)
                            and isinstance(result.get("absorbed_by_measTE"), float)
                            and isinstance(result.get("d_modeled"), float)
                            and isinstance(result.get("d_combined"), float)
                        ],
                        key=lambda row: row["absorbed_by_measTE"],
                        reverse=True,
                    ),
                },
                "honest": {
        ...
        emit("failed", checks=[], error=str(exc), reason="invalid or unreadable measured-TE H-candidate input")
