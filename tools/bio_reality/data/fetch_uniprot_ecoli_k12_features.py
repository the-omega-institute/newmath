#!/usr/bin/env python3
"""Fetch E. coli K-12 UniProt reviewed feature data for BioReality."""
...
COMPLEX_PATTERNS = ["complex", "component of", "heteromer", "homodimer", "oligomer"]
SOURCE_URL = (
...
def main() -> int:
    ...
    parsed["provenance"] = {
        ...
        "source_name": "uniprot",
        "accession_or_id": "taxonomy_83333_reviewed_features_string_511145",
    ...
    parsed["raw_payload_text"] = raw.decode("utf-8", "replace")
    parsed["raw_payload_note"] = "full UniProt TSV response reconstructed from REST pages; sha256 is over this UTF-8 byte stream"

    out_path = DATA_DIR / "uniprot_protein_features_escherichia_coli_k12_mg1655.json"
    manifest_path = MANIFEST_DIR / out_path.name
        ...
        json.dumps(
            {
                ...
                "sha256": sha,
                "byte_size": len(raw),
                "n_string_feature_records": parsed["n_string_feature_records"],
            },
            sort_keys=True,
        )
    )
...
if __name__ == "__main__":
    try:
        ...
        print(f"fetch_uniprot_ecoli_k12_features failed: {exc}", file=sys.stderr)
        raise SystemExit(1)
