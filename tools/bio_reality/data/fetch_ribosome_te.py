...
BioReality protein_id universe.
"""
...
def build_yeast() -> dict[str, object]:
    ...
    return {
        ...
        "id_mapping_method": "Exact join from BioReality protein_id suffix 4932.<systematic_ORF> to GEO quant feature name; UTR/intron features are ignored because only exact ORF feature names are accepted.",
...
def build_ecoli() -> dict[str, object]:
    ...
    return {
        ...
        "te_definition": "mean CDS footprint wiggle coverage on the annotated coding strand divided by mean CDS mRNA wiggle coverage on the same strand; CDS intervals and b-number joins come from the existing RefSeq NC_000913.3 BioReality CDS file",
        "id_mapping_method": "Exact join from BioReality protein_id suffix 511145.<b-number> to RefSeq CDS locus_tag b-number; coverage is summed over the matching CDS genomic interval and strand.",
        "n_genes_with_te": len(genes),
        "join_hit_rate": len(genes) / float(len(universe)) if universe else 0.0,
        "join_universe_n": len(universe),
        "genes": genes,
