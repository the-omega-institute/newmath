...
DOMAIN_PROFILE = SCRIPT_DIR / "cellstate_context_refresh_ladder.json"
...
def bootstrap_research_memory(store: BioRealityStore) -> dict[str, Any]:
    ...
    profile_id = str(profile.get("profile_id") or "cellstate_context_refresh")
    contacts = [
        {
...
def run_gate_lane(store: BioRealityStore) -> dict[str, Any]:
    summary = cellstate_reality_loop.run_once(store)
...
def run_oracle_server_lane(store: BioRealityStore) -> dict[str, Any]:
    try:
        ...
        server_script = repo_root / "tools" / "cellstate_reality" / "oracle" / "cellstate_reality_oracle_server.py"
        log_path = repo_root / "tools" / "cellstate_reality" / "state" / "oracle_server.log"
...
def _maybe_run_bio_g_oracle(store: BioRealityStore) -> dict[str, Any]:
    ...
    if not pdf_path.exists():
        pdf_path = None
    persist_dir = _resolve_repo_path(repo_root, config.get("persist_dir") or "tools/cellstate_reality/state/oracle_sessions")
    server_url = str(config.get("server_url") or "http://127.0.0.1:8770")
...
def _load_claims_document(path: Path) -> dict[str, Any]:
    try:
        ...
        return {"version": "cellstate-reality-claims-v1", "claims": []}
...
def _load_experiments_document(path: Path) -> dict[str, Any]:
    try:
        ...
        return {"version": "cellstate-reality-experiments-v1", "experiments": []}
    if not isinstance(data, dict):
        return {"version": "cellstate-reality-experiments-v1", "experiments": []}
...
def _frontier_vision_excerpt(store: BioRealityStore, limit: int = 10000) -> str:
    path = store.paths.vision_dir / "cellstate-context-refresh-boundary.md"
    try:
        text = path.read_text(encoding="utf-8")
    except OSError:
        return ""
...
def _load_writeback_heal_config() -> dict[str, Any]:
    ...
    return {
        ...
        "path_whitelist_prefix": str(config.get("path_whitelist_prefix") or "papers/cellstate_reality/"),
    }
...
def _bios_codex_resolve_prompt(rel_path: str, conflicted_content: str, upstream_sha: str) -> str:
    return "\n".join([
        ...
        "Local branch: feat/cellstate_reality-deepening (BioReality research direction).",
        ...
        "- BioReality content (papers/cellstate_reality/, tools/cellstate_reality/, vision/) → prefer local side",
...
def _bios_codex_resolve_merge(
    ...
    take_ours_bioreality = bool(cfg.get("papers_cellstate_reality_take_ours", True))

    def _conflict_side(rel: str) -> str | None:
        # 按域定权威方向:
        #  - papers/bedc/ 是 Loning 域 → auto-dev 权威 → 取 theirs.
        #  - papers/cellstate_reality/parts/ 是我方域 → 我方 daemon 生成的 rich 内容权威;
        ...
        if take_theirs_bedc and rel.startswith("papers/bedc/"):
            return "theirs"
        if take_ours_bioreality and rel.startswith("papers/cellstate_reality/parts/"):
            return "ours"
        return None
...
def run_keep_lane(store: BioRealityStore) -> dict[str, Any]:
    ...
    branch_default = str(config.get("branch") or "feat/cellstate_reality-deepening")
    ...
    remote = str(config.get("remote") or "origin")
    branch = str(config.get("branch") or "feat/cellstate_reality-deepening")
    try:
...
def run_merge_back_lane(store: BioRealityStore) -> dict[str, Any]:
    """Merge feat/cellstate_reality-deepening back to upstream (auto-dev) as fast-forward.
    ...
    remote = str(config.get("remote") or "origin")
    feat_branch = str(config.get("branch") or "feat/cellstate_reality-deepening")
...
def run_dev_rollup_lane(store: BioRealityStore) -> dict[str, Any]:
    ...
    repo_root = store.paths.root.parent.parent
    source = str(rollup.get("head_branch") or config.get("branch") or "feat/cellstate_reality-deepening")
...
def _passed_conjectures(store: BioRealityStore) -> list[dict[str, Any]]:
    ...
    probes = store.load_probes()
    mismatches = store.load_mismatches()
    results = cellstate_reality_loop.gate_all(conjectures, contacts, probes, mismatches)
    passed = {
...
def _temp_paths(base: Path) -> BioRealityPaths:
    return BioRealityPaths(
        ...
        paper_part=base / "paper" / "parts" / "cellstate_context_refresh_boundary.tex",
    )
...
def _bio_w_codex_writer_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "log_dir": str(writer.get("log_dir", "tools/cellstate_reality/state/writeback_codex_logs")),
...
def render_namecert_with_codex(
    ...
    *,
    ...
    corrective_feedback: list[str] | None = None,
    log_dir: str | Path = "tools/cellstate_reality/state/writeback_codex_logs",
) -> dict[str, Any] | None:
...
def _last_cellstate_reality_tex_file(output: str) -> str:
    matches = re.findall(r"\(\.?/?(parts/[^()\s]*\.tex|papers/cellstate_reality/[^()\s]*\.tex)", output)
    if not matches:
        matches = re.findall(r"(\.?/?parts/[^:\s()]*\.tex|\bpapers/cellstate_reality/[^:\s()]*\.tex)", output)
    if not matches:
        return ""
    rel = matches[-1].lstrip("./")
    if rel.startswith("papers/cellstate_reality/"):
        return rel
    return f"papers/cellstate_reality/{rel}"
...
def _parse_writeback_heal_error(output: str, paper_dir: Path) -> dict[str, str]:
    ...
    if "Unicode character" in combined and "not set up for use with LaTeX" in combined:
        ...
        category = "other"
    rel_file = _last_cellstate_reality_tex_file(combined)
...
def run_writeback_heal_lane(store: BioRealityStore) -> dict[str, Any]:
    try:
        ...
        for attempt in range(1, int(config["max_attempts"]) + 1):
            ...
            # 如果 target file 没 unicode (file detection 错), fallback 扫描 ALL cellstate_reality namecerts
...
def run_writeback_lane(store: BioRealityStore) -> dict[str, Any]:
    ...
    # Auto-build the PDF so reviewers can open papers/cellstate_reality/main.pdf
...
def _bedc_codex_writer_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "log_dir": str(writer.get("log_dir", config.get("codex_log_dir") or "tools/cellstate_reality/state/bedc_writeback_logs")),
...
def render_chapter_with_codex(
    ...
    *,
    timeout_seconds: int = 600,
    log_dir: str | Path = "tools/cellstate_reality/state/bedc_writeback_logs",
    corrective_feedback: list[str] | None = None,
) -> dict[str, Any]:
...
def self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="cellstate-reality-lanes-") as tmp:
        base = Path(tmp)
        ...
        keep_selected, keep_dropped = _filter_paths(
            [
                (" M", "tools/cellstate_reality/state/foo.jsonl"),
                (" M", "tools/cellstate_reality/experiments/run_X.py"),
                ("??", "tools/cellstate_reality/vision/index.md"),
            ],
        ...
        if keep_selected != ["tools/cellstate_reality/experiments/run_X.py"]:
            print(json.dumps({"selected": keep_selected, "dropped": keep_dropped}, indent=2), file=sys.stderr)
            return 1
        if "tools/cellstate_reality/state/foo.jsonl" not in keep_dropped:
            print(json.dumps({"selected": keep_selected, "dropped": keep_dropped}, indent=2), file=sys.stderr)
            return 1
        ...
        bedc_aggregator = base / "bedc_target" / "cellstate_reality_module.tex"
        bedc_mappings = [
            {
        ...
        bedc_auto_aggregator = base / "bedc_auto_target" / "cellstate_reality_module.tex"
        bedc_auto_mappings = [
            {
        ...
        bedc_skip_aggregator = base / "bedc_skip_target" / "cellstate_reality_module.tex"
        ...
        bedc_codex_aggregator = base / "bedc_author_target" / "cellstate_reality_module.tex"
        bedc_codex_mapping = {
            ...
            [
                {
                    ...
                    "source_kind": "genetic_code_table",
                    "source_ref": "curated standard genetic-code bridge",
                    "source_snapshot": "cellstate_context_refresh",
                    "observed_fact": "A curated standard-code table assigns codons to amino-acid or stop labels.",
                    "resolution": "codon label readback",
                    "known_noise_or_bias": "The table is a reality bridge for code labels, not a mechanism or universality proof.",
                    "can_test": ["code_read", "local codon tile label split"],
            ...
            json.dumps(
                {
                    ...
                    "experiments": [
                        {
                            "experiment_id": "self_missing_script",
                            "script_path": "tools/cellstate_reality/experiments/run_self_missing_script.py",
                            "claim_id": "self.claim",
                            "required_data": [],
                            "timeout_seconds": 5,
        ...
        try:
            ...
            oracle_plan_paths.experiments_registry.write_text(
                json.dumps({"version": "cellstate-reality-experiments-v1", "experiments": []}, indent=2),
                encoding="utf-8",
            )
        ...
        if not any(event.get("event_kind") == "claim_redesign_proposed" and event.get("subject_id") == "x" for event in stuck_events):
            ...
            return 1
    print("[cellstate-reality-lanes] self-test ok")
    return 0
