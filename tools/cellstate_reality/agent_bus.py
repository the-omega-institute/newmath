...
AGENTS = {
    "bio-researcher": {
        "lane": "bio-R",
        "writes": [
            "tools/cellstate_reality/inbox/conjectures.jsonl",
            "tools/cellstate_reality/inbox/reality_contacts.jsonl",
            "tools/cellstate_reality/inbox/probes.jsonl",
            "tools/cellstate_reality/inbox/mismatches.jsonl",
        ],
    ...
    "bio-gate-curator": {
        "lane": "bio-Q",
        "writes": [
            "tools/cellstate_reality/deepening_gates.py",
            "tools/cellstate_reality/*.schema.json",
            "tools/cellstate_reality/pipeline_config.json",
        ],
    ...
    "bio-experimentalist": {
        "lane": "bio-R",
        "writes": [
            "tools/cellstate_reality/experiments/run_*.py",
            "tools/cellstate_reality/data/*.json",
            "tools/cellstate_reality/registries/experiments.json",
        ],
    ...
    "bio-data-fetcher": {
        ...
        "writes": [
            "tools/cellstate_reality/data/*.json",
            "tools/cellstate_reality/data/*.py",
            "tools/cellstate_reality/data/manifests/*.json",
        ],
        "mission": "fetch curated public biological reality data (NCBI / UniProt / Ensembl / RefSeq / EBI) via urllib.request, store as versioned JSON under tools/cellstate_reality/data/ with provenance, never write paper or experiment scripts",
    },
    "bio-planner": {
        "lane": "bio-R",
        "writes": [
            "tools/cellstate_reality/registries/claims.json",
            "tools/cellstate_reality/registries/experiments.json",
            "tools/cellstate_reality/experiments/run_*.py",
            "tools/cellstate_reality/data/*.json",
        ],
    ...
    "bio-namer": {
        "lane": "bio-R",
        "writes": [
            "tools/cellstate_reality/state/namecert_proposals/*.md",
        ],
    ...
    "bio-oracle-consumer": {
        ...
        "writes": [
            "tools/cellstate_reality/registries/claims.json",
            "tools/cellstate_reality/registries/experiments.json",
            "tools/cellstate_reality/state/oracle_refinements/*.json",
        ],
...
ORACLE_ESCALATION_RULES = [
    ...
    '    python3 tools/cellstate_reality/oracle/oracle_client.py --query "<concise question>" \\',
...
def _load_claims_document(path: Path) -> dict[str, Any]:
    data = _read_json_object(path, {"version": "cellstate-reality-claims-v1", "claims": []})
...
def _load_experiments_document(path: Path) -> dict[str, Any]:
    data = _read_json_object(path, {"version": "cellstate-reality-experiments-v1", "experiments": []})
...
def render_prompt(event: dict[str, Any], agent_id: str, action: str) -> str:
    ...
    if agent_id == "bio-experimentalist":
        extra_rules = [
            ...
            "- subject_id is the experiment_id; find the matching script_path in tools/cellstate_reality/registries/experiments.json.",
            "",
            ...
            "",
            'Step 0d. After repair, run the script again and confirm it now produces status="passed". If the failure is theoretically real and not a script bug, Do NOT fake passed: leave it failed and instead propose adding a new experiment with a different statistic; in this case write a notes file tools/cellstate_reality/state/proposed_experiments/<id>.md describing what new experiment should be added, but you yourself may not add it.',
            "",
            "You may only edit script files matching tools/cellstate_reality/experiments/run_*.py or data files under tools/cellstate_reality/data/. The script must end by printing a single-line JSON to stdout with fields experiment_id, claim_id, status, checks, result, started_at, completed_at. Python 3 stdlib only.",
        ]
        ...
        extra_rules = [
            ...
            "1. Save raw payload to tools/cellstate_reality/data/<source_name>_<accession_or_id>.json (or appropriate extension)",
            "2. Compute sha256 of the raw response bytes",
            "3. Save provenance to tools/cellstate_reality/data/manifests/<basename>.json with:",
            "   {",
            '     "fetched_at": "<iso-utc>",',
            ...
            "- Inspect event payload to learn which claim's needs_data triggered this. Read tools/cellstate_reality/registries/claims.json to find the claim; read tools/cellstate_reality/registries/experiments.json to find required_data list for the experiment.",
            ...
            "- If no suitable public source is reachable (private DB, paywall, custom dataset), write the diagnosis to tools/cellstate_reality/state/proposed_data_sources/<claim_id>.md and STOP.",
            "",
            ...
            "",
            "Strict no-go:",
            "- Never write any .py file outside tools/cellstate_reality/data/",
        ...
        extra_rules = [
            ...
            "  layer in cellstate_context_refresh_ladder.json (e.g. orf_eligibility may be",
            ...
            "  tools/cellstate_reality/state/proposed_experiments/<claim_id>.md and DO",
        ...
        extra_rules = [
            ...
            "  Read tools/cellstate_reality/registries/claims.json and find the matching claim;",
            "  read tools/cellstate_reality/inbox/conjectures.jsonl and find the matching",
            "  conjecture; read the latest experiment_runs.jsonl entry for the claim.",
            "",
            "Step 1. Read tools/cellstate_reality/state/research_notes/loning_writeback_pattern.md",
            ...
            "  tools/cellstate_reality/state/namecert_proposals/<claim_id>.md with EXACTLY these",
...
def dispatch_codex(task: dict[str, Any], *, execute: bool) -> dict[str, Any]:
    ...
    if task.get("agent_id") == "bio-gate-curator" and dispatch["dispatch_status"] == "completed":
        self_test = subprocess.run(
            ["python3", "tools/cellstate_reality/deepening_gates.py", "--self-test"],
            cwd=repo_root,
            capture_output=True,
            text=True,
            timeout=60,
...
def self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="cellstate-reality-agent-bus-") as tmp:
        base = Path(tmp)
        ...
        _write_json_object(
            ...
            {
                "version": "cellstate-reality-claims-v1",
                "claims": [
                    {
                        "claim_id": "h0.seed",
                        "hypothesis_level": "H0",
        ...
        _write_json_object(
            ...
            {
                "version": "cellstate-reality-experiments-v1",
                "experiments": [
                    {
                        "experiment_id": "seed_experiment",
                        "claim_id": "h0.seed",
        ...
        if failed_events[0]["status"] != "archived":
            ...
            return 1
        if not _allowed_path("tools/cellstate_reality/inbox/conjectures.jsonl", ["tools/cellstate_reality/inbox/*.jsonl"]):
            print("expected inbox jsonl glob to match", file=sys.stderr)
            return 1
        if _allowed_path("lean4/Foo.lean", ["tools/cellstate_reality/inbox/*.jsonl"]):
            print("expected lean4 path not to match inbox glob", file=sys.stderr)
            return 1
    print("[cellstate-reality-agent-bus] self-test ok")
    return 0
