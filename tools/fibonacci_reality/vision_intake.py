#!/usr/bin/env python3
"""Vision intake for FibonacciReality unattended loops."""
...
try:
    ...
    from store import FibonacciRealityPaths, FibonacciRealityStore, read_jsonl
...
def evaluate_vision(path: Path, store: FibonacciRealityStore) -> dict[str, Any]:
...
def run_vision_lane(store: FibonacciRealityStore, *, write: bool = True) -> dict[str, Any]:
...
def self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="fibonacci-reality-vision-") as tmp:
        base = Path(tmp)
        ...
        if last_row.get("decision") != "already_materialized" or last_row.get("skip_reason") != "already_materialized":
            ...
            return 1
    print("[fibonacci-reality-vision] self-test ok")
    return 0


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Scan FibonacciReality vision files into events")
    ...
    summary = run_vision_lane(FibonacciRealityStore(paths), write=not args.no_write)
