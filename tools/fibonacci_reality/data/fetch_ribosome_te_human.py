#!/usr/bin/env python3
"""Fetch human measured ribosome-profiling translation efficiency data.

The emitted TE value is the processed RPFdb CDS translation-efficiency value.
Ribo-seq and RNA-seq rawcount tables from the same study are retained as
positive support columns, but are not used to replace the direct TE value.
"""
...
HUMAN_URLS = {
    "translation_efficiency_cds": f"{RPFDB_BASE}/data/TranslationEff/H.sapiens_{STUDY}_TE_CDS.txt.gz",
    "riboseq_rawcount": f"{RPFDB_BASE}/data/Rawcount/H.sapiens_{STUDY}_rawcount.metatable.gz",
    "rnaseq_rawcount": f"{RPFDB_BASE}/data/Rawcount_RNAseq/H.sapiens_{STUDY}_rawcount.metatable.gz",
}

CANNOT_CLAIM = [
    "measured TE from one human ribo-seq dataset; condition-specific; still not causal",
    "RPFdb processed CDS TE is used as a direct measured TE readout, not as a universal human translation-efficiency constant",
    ...
    "gene-level Ensembl IDs are mapped to one longest-CDS ENSP representative; unmapped genes and non-positive measurements are discarded",
]
...
def parse_mean_positive_table(raw_gz: bytes, expected_first_header: str) -> dict[str, float]:
    ...
    for line in lines[1:]:
        ...
        if not gene_key.startswith("ENSG"):
            continue
...
def load_human_cds_representatives() -> tuple[dict[str, dict[str, object]], int]:
    with (DATA_DIR / "cds_codon_abundance_homo_sapiens.json").open("r", encoding="utf-8") as f:
        payload = json.load(f)
    joined = payload["joined"]
    representatives: dict[str, dict[str, object]] = {}
    for row in joined:
        header = str(row.get("cds_header", ""))
        match = re.search(r"gene:(ENSG[0-9]+)(?:\.[0-9]+)?", header)
        if not match:
            continue
...
def build_human() -> dict[str, object]:
    payloads = {key: fetch_payload(url) for key, url in HUMAN_URLS.items()}
    ...
    representatives, universe_n = load_human_cds_representatives()
    ...
    return {
        ...
        "source_url": HUMAN_URLS["translation_efficiency_cds"],
        "source_urls": [HUMAN_URLS[key] for key in sorted(HUMAN_URLS)],
        "study_ref": "RPFdb processed Homo sapiens GSE56887, Wolfe et al. 2014 Nature, eIF4A-dependent oncogene translation in cancer",
        "payload_sha256": combined_payload_sha(payloads),
        "payload_sha256_by_url": payload_archive(payloads, HUMAN_URLS),
        "te_definition": "direct positive RPFdb CDS translation-efficiency value from H.sapiens_GSE56887_TE_CDS.txt.gz, averaged across positive TE columns for each Ensembl gene; footprint and mRNA fields are mean positive rawcounts from the matched Ribo-seq and RNA-seq RPFdb rawcount tables for the same study and gene",
        "id_mapping_method": "RPFdb Ensembl gene IDs are stripped of version suffix and matched exactly to the gene:ENSG field in the existing FibonacciReality Homo sapiens Ensembl CDS headers; when multiple CDS records share one Ensembl gene, the longest CDS representative protein_id is emitted; all unmapped or non-positive genes are skipped.",
...
def main() -> None:
    ...
    write_json("ribosome_te_homo_sapiens.json", result)
    print(
        result["organism"],
        "n=",
