// ==UserScript==
// @name         FibonacciReality Oracle Bridge (macOS, multi-turn)
// @namespace    omega-fibonacci-reality
// @version      2.3
// @description  FibonacciReality-pipeline ChatGPT bridge bio-2.0 with automath-stable waiting and BEDC project/PDF routing. Talks to fibonacci_reality_oracle_server.py on :8771.
...
(function () {
  ...
  const SERVER = "http://127.0.0.1:8771";
  ...
  const FIBONACCI_REALITY_PROJECT_PREFIX = "/g/g-p-6a098a6e69688191a6afd91978c585ef-ge-ben-ha-gen-zhi-lu";
  const FIBONACCI_REALITY_PROJECT_HOME = `https://chatgpt.com${FIBONACCI_REALITY_PROJECT_PREFIX}/project`;

  function isInsideFibonacciRealityProject() {
    return window.location.pathname.startsWith(FIBONACCI_REALITY_PROJECT_PREFIX);
  }
  ...
  function projectEntryUrl(flag = bioFlagFromUrl()) {
    return `${FIBONACCI_REALITY_PROJECT_HOME}${flag ? `?bio=${encodeURIComponent(flag)}` : ""}`;
  }
  ...
  let active = (() => {
    ...
    try {
      ...
      return storedActive && (urlOptIn || isInsideFibonacciRealityProject());
    } catch {
      return urlOptIn;
    }
  })();
  ...
  function updatePanel() {
    ...
    panel.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px">
        <b style="color:#d7fff2">[bio] FibonacciReality Oracle ${SCRIPT_VERSION}</b>
  ...
  function pinToProject(url) {
    ...
    try {
      ...
      if (!FIBONACCI_REALITY_PROJECT_PREFIX) return u.toString();
      // bare /c/<id> — splice in the FibonacciReality project prefix
      const m = u.pathname.match(/^\/c\/[a-f0-9-]{6,}/);
      if (m) {
        u.pathname = `${FIBONACCI_REALITY_PROJECT_PREFIX}${m[0]}`;
        return u.toString();
      }
  ...
  function ensureInProject() {
    ...
    if (!FIBONACCI_REALITY_PROJECT_PREFIX || window.location.pathname.startsWith(FIBONACCI_REALITY_PROJECT_PREFIX)) return false;
  ...
  async function processTask(task) {
    ...
    if (!isInsideFibonacciRealityProject()) {
    ...
    try {
      ...
      if (needNavToFresh) {
        ...
        if (ok) {
          ...
          const fallbackUrl = `${FIBONACCI_REALITY_PROJECT_HOME}${bioFlag ? `?bio=${encodeURIComponent(bioFlag)}` : ""}`;
  ...
  function navigateTaskBackToProject(task, reason) {
    ...
    setTaskPhase("navigating");
    log(`${reason}; navigating to FibonacciReality chat ${target.slice(-80)}`);
    busy = false;
    updatePanel();
  ...
  function _readActive() {
    try { return sessionStorage.getItem("bio_active") === "1" && isInsideFibonacciRealityProject(); }
    catch { return false; }
  }

  function enforceProjectBeforePolling() {
    if (isInsideFibonacciRealityProject()) return false;
    if (window.location.search.includes("bio=")) {
      const target = projectEntryUrl();
      log(`FibonacciReality chat required; navigating to ${target}`);
      window.location.href = target;
      return true;
    }
    try { sessionStorage.setItem("bio_active", "0"); } catch {}
    active = false;
    log("Outside FibonacciReality chat; polling paused");
    updatePanel();
    return true;
  }
  ...
  // ── Bootstrap ────────────────────────────────────────────────────────
  async function init() {
    log(`FibonacciReality Oracle Bridge ${SCRIPT_VERSION} loaded — ${active ? "ACTIVE" : "PAUSED"} — agent=${agentId()}`);
    if (active) {
      startKeepAlive();
      resumeKeepAlive();
    }
    ...
    if (urlHasFlag && !isInsideFibonacciRealityProject()) {
      const target = projectEntryUrl();
      log(`FibonacciReality chat required; navigating to ${target}`);
      window.location.href = target;
      return;
