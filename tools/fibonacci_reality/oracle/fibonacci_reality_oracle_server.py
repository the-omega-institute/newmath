#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Forked from tools/bedc-deep/bedc_oracle_server.py v1.21; differences: port 8771,
# sessions under tools/fibonacci_reality/state/oracle_sessions/, namespace bio_*.
"""FibonacciReality Oracle bridge server — multi-turn deep reasoning for FibonacciReality targets.
...
The FibonacciReality oracle supports a *session* (= one ChatGPT conversation) into which
...
Differences vs the paper oracle:
  - Port 8771 (paper oracle is 8765). Run them side by side.
  ...
  - Sessions persist to disk at tools/fibonacci_reality/state/oracle_sessions/
    so server restart doesn't lose the conversation thread.
  - No PDF support (FibonacciReality claim-packet reviews work on inline research.md text); kept as
    optional pass-through if ever needed later.

Usage:
    python3 tools/fibonacci_reality/oracle/fibonacci_reality_oracle_server.py

    # Open ChatGPT.com tab(s) with fibonacci_reality_oracle_macos.user.js installed,
...
Hard rules:
  ...
  - Server never auto-publishes anything. Results land in sessions/ and are
    consumed by tools/fibonacci_reality/oracle/oracle_client.py state JSON.
"""
...
DEFAULT_PORT = 8771
PORT = DEFAULT_PORT
FIBONACCI_REALITY_DIR = Path(__file__).resolve().parents[1]
ORACLE_STATE_DIR = FIBONACCI_REALITY_DIR / "state"
...
FIBONACCI_REALITY_CHATGPT_URL = "https://chatgpt.com/"
FIBONACCI_REALITY_PROJECT_URL = FIBONACCI_REALITY_CHATGPT_URL
...
class FibonacciRealityOracleHandler(BaseHTTPRequestHandler):
        ...
        if parsed.path == "/task":
            ...
            with _lock:
                if not in_project:
                    ...
                    if agent_id in pending_tasks:
                        cancelled_id = _cancel_pending_for_agent(
                            agent_id,
                            reason=f"outside FibonacciReality Project poll ({poll_metrics['page_url'][-80:]})",
                        )
                    else:
                        recent_agents.pop(agent_id, None)
                    self._send_json({
                        ...
                        "required_project_url": FIBONACCI_REALITY_PROJECT_URL,
                        "reason": "agent outside FibonacciReality Project",
                    })
                    return
                ...
                if not _page_is_bio_conversation(poll_metrics["page_url"]):
                    self._send_json({
                        "status": "idle",
                        "reason": "agent on FibonacciReality Project root; waiting for conversation page before dispatch",
                    })
                    return
                if task_queue and len(pending_tasks) < MAX_AGENTS:
                    task = task_queue.popleft()
    ...
    def _handle_pin_conv_url(self, data: dict):
        ...
        if chatgpt_url and not _page_in_bio_project(chatgpt_url):
            self._send_json({
                ...
                "task_id": task_id,
                "required_project_url": FIBONACCI_REALITY_PROJECT_URL,
                "reason": "chatgpt_url outside FibonacciReality Project",
            })
            return
        with _lock:
...
def self_test() -> int:
    ...
    _ensure_dirs()
    server = HTTPServer(("127.0.0.1", 0), FibonacciRealityOracleHandler)
    ...
    try:
        ...
        saved = _http_json(
            ...
            {"response": "mock FibonacciReality oracle response", "agent_id": "bio_self"},
        )
        ...
        if fetched.get("status") != "completed" or fetched.get("response") != "mock FibonacciReality oracle response":
...
def main(argv: list[str] | None = None) -> int:
    global PORT
    parser = argparse.ArgumentParser(description="Run the FibonacciReality oracle bridge server")
    ...
    _ensure_dirs()
    _hydrate_sessions()
    server = HTTPServer(("127.0.0.1", PORT), FibonacciRealityOracleHandler)
    ...
    for i in range(1, MAX_AGENTS + 1):
        print(f"  Tab {i}: {FIBONACCI_REALITY_PROJECT_URL}?bio={i}")
