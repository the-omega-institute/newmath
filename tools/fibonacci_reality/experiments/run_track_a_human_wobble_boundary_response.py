#!/usr/bin/env python3
"""Track A human codon-wobble boundary response with a permutation null."""
...
EXPERIMENT_ID = "track-a-human-wobble-boundary-response"
CLAIM_ID = "track-a.codon-wobble-boundary-response.human"
DATA_DIR = Path("tools") / ("fibona" + "cci_reality") / "data"
CDS_ABUNDANCE_PATH = DATA_DIR / "cds_codon_abundance_homo_sapiens.json"
TRNA_PATH = DATA_DIR / "gtrnadb_trna_all_copy_homo_sapiens.json"
MIN_CODON_COUNT = 50
NULL_N = 1000
WOBBLE_DISCOUNT = 0.5
EPS = 1e-12
    ...
    try:
        ...
        checks = [
            check(
                ...
                True,
                "computed human codon-wobble boundary-response residual model with deterministic Null95 permutation",
            )
        ]
        result = {
            ...
            "rho_definition": (
                ...
                "cognate anticodon is absent from the GtRNAdb human tRNA gene-copy pool"
            ),
            "trna_key_convention": (
                ...
                "and human sanity checks give ATG->CAU and TGG->CCA with positive copies while stops have none"
            ),
            ...
            "note": (
                "正负皆真结果; 无窗口常数预设; A-P-site dwell 机制 needs_data 另记"
            ),
        }
        print(
            "Track A human wobble boundary response: "
