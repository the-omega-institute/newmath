#!/usr/bin/env python3
"""Ordered orthogonal residual decomposition for B*_Q6 to protein abundance.

The experiment is descriptive. It partitions projection energy after local
controls and ranks residual H readouts by absorption above a deterministic
label-permutation null with description-length and join-shift penalties.
"""
...
EXPERIMENT_ID = "b_star_q6_residual_decomposition_powered"
CLAIM_ID = "h3.cross_layer_relation.residual_decomposition.b_star_q6_abundance_ordered_powered"
CONJECTURE_ID = "q6.residual-decomposition.abundance-ordered.cross-layer"
...
def score_candidate(
    *,
    ...
    score = a_h - float(null["null95"]) - LAMBDA_DL * dl - RHO_JOIN * join_shift
    return {
        "readout": candidate,
        "status": "computed",
        "n_join": n_join,
...
def verdict_for(result: dict[str, object]) -> str:
    ...
    return f"{lead}: T={t_fraction:.3f}, S={s_fraction:.3f}, H={h_names}, unexplained_U={unexplained:.3f}; descriptive ordered partition only."


def cannot_claim() -> list[str]:
    return [
        "ordered orthogonal variance-partition, NOT causal pathway proof",
        "readout order T->S->H is a modeling choice; different order gives different attribution",
        "per-organism single datasets (TE/abundance/half-life/features from different studies, platforms differ)",
        "H-candidates are residual absorbers not mechanisms",
        "join-sensitive readouts (danio/human feature joins) flagged via join_shift penalty",
        "descriptive minimal dictionary per organism, not a universal mechanism",
    ]
...
def checks_for(organisms: dict[str, dict[str, object]]) -> list[dict[str, object]]:
    ...
    return [
        {
            "name": "per_organism_P_Q_computed",
        ...
        {
            "name": "ordered_T_S_decomposition",
        ...
        },
        {
            ...
            "expected": "each computed H has BEDC score = A_H - Null95(H) - lambda*DL(H) - rho*Deltajoin(H), with 1000 deterministic label permutations and join_shift",
        },
        {
            "name": "decomposition_sums_to_one",
            "passed": bool(computed) and sum_ok,
            "actual": {key: {"sum": result.get("decomposition_sum"), "sum_error": result.get("decomposition_sum_error")} for key, result in computed.items()},
            "expected": f"T + S + sum(H_kept) + U equals 1 within {SUM_TOL}",
        },
        {
...
def main() -> None:
    ...
    try:
        ...
        reason = None if status == "passed" else "no organism satisfied every honest gate for ordered residual decomposition"
        emit(
            status,
            reason=reason,
            checks=checks,
            result={
                ...
                "statement": "For each organism, P_Q = Pi_{Q_e} P_e is decomposed in the fixed order measured-TE, mRNA-stability where available, then a greedy scored H-dictionary. This is an ordered orthogonal projection partition, not causal pathway proof.",
                "decomposition_definition": {
                    ...
                    "BEDC_score": "A_H - Null95(H) - lambda*DL(H) - rho*Deltajoin(H)",
