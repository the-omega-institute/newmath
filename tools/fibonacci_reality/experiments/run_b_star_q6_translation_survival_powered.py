...
def manifest_organisms(repo: pathlib.Path) -> list[str]:
    path = repo / "tools/fibonacci_reality/data/multi_organism_campaign_manifest.json"
    raw = load_json(path)
...
def codon_frequency_residual(
    ...
    raw = load_json(repo / f"tools/fibonacci_reality/data/codon_usage_{organism}.json")
    if not isinstance(raw, dict):
...
def modeled_tai_residual(
    ...
    raw = load_json(repo / f"tools/fibonacci_reality/data/gtrnadb_trna_all_copy_{organism}.json")
    if not isinstance(raw, dict):
    ...
    try:
        ...
        for organism in organisms:
            for prefix in ["codon_usage_", "gtrnadb_trna_all_copy_"]:
                path = repo / f"tools/fibonacci_reality/data/{prefix}{organism}.json"
                if not path.exists():
