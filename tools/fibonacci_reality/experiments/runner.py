#!/usr/bin/env python3
"""Subprocess runner for FibonacciReality experiment scripts."""
...
SCRIPT_ROOT = Path("tools/fibonacci_reality/experiments")
...
def load_experiments_registry(repo_root: Path) -> list[dict[str, Any]]:
    data = _safe_load_json(repo_root / "tools/fibonacci_reality/registries/experiments.json")
...
def load_claims_registry(repo_root: Path) -> dict[str, dict[str, Any]]:
    data = _safe_load_json(repo_root / "tools/fibonacci_reality/registries/claims.json")
...
def run_experiment(experiment_spec: dict[str, Any], *, timeout_seconds: int = 300, repo_root: Path) -> dict[str, Any]:
    """
    Run one registered FibonacciReality experiment and normalize its JSON result.
