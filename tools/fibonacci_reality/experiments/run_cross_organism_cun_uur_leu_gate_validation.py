...
def organism_codons(repo, organism):
    path = repo / f"tools/fibonacci_reality/data/kazusa_codon_usage_{organism}.json"
    raw = load_json(path)
...
def organism_trna(repo, organism):
    path = repo / f"tools/fibonacci_reality/data/gtrnadb_trna_leu_copy_{organism}.json"
    raw = load_json(path)
