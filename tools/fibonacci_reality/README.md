# FibonacciReality Deepening Pipeline
...
- `forced_window_to_bedc_ladder.json` records the first domain profile for the
...
- `lanes.py` defines the `bio-V`, `bio-P`, `bio-G`, `bio-R`, `bio-W`, `bio-Q`, and
  `bio-A` local lanes.
- `fibonacci_reality_loop.py` runs one deterministic deepening cycle and plans the
  next tasks.
...
- `papers/fibonacci_reality/main.tex` and `papers/fibonacci_reality/parts/` are the
...
- `tools/fibonacci_reality/inbox/`
- `tools/fibonacci_reality/out/`
- `tools/fibonacci_reality/state/`
- `tools/fibonacci_reality/cache/`
...
packet is internally admissible for the next FibonacciReality cycle. It does not
...
```text
tools/fibonacci_reality/out/gate_results.jsonl
tools/fibonacci_reality/out/deepening_tasks.jsonl
tools/fibonacci_reality/out/review_queue.jsonl
tools/fibonacci_reality/out/packet_targets.jsonl
tools/fibonacci_reality/out/events.jsonl
tools/fibonacci_reality/out/agent_tasks.jsonl
tools/fibonacci_reality/out/agent_reviews.jsonl
tools/fibonacci_reality/out/hardening_targets.jsonl
tools/fibonacci_reality/out/lane_dashboard.md
```
...
- `bio-V` is the vision lane. It scans `tools/fibonacci_reality/vision/*.md`, appends
...
- `bio-W` is the writeback lane. It reads gate-passed research memory and writes
  the standalone FibonacciReality paper with explicit external-reality, internal
...
- `bio-K` is the keep lane. It runs at the end of every cycle, filters
  ...
  pushes useful spec changes to `origin/feat/fibonacci_reality-deepening`.
...
independent checkpoint in `tools/fibonacci_reality/state/loop_state.json`, and
...
pipeline is the FibonacciReality paper surface named above.
...
```bash
python3 tools/fibonacci_reality/deepening_gates.py --self-test
python3 tools/fibonacci_reality/fibonacci_reality_loop.py --self-test
python3 tools/fibonacci_reality/vision_intake.py --self-test
python3 tools/fibonacci_reality/agent_bus.py --self-test
python3 tools/fibonacci_reality/lanes.py --self-test
python3 tools/fibonacci_reality/supervisor.py --once
```
