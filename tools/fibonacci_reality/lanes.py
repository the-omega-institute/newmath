#!/usr/bin/env python3
"""P/G/A lane helpers for FibonacciReality automation."""
...
try:
    ...
    from store import FibonacciRealityPaths, FibonacciRealityStore, append_jsonl, read_jsonl, write_jsonl
...
DOMAIN_PROFILE = SCRIPT_DIR / "forced_window_to_bedc_ladder.json"
...
def _quality_lab_exports_path(store: FibonacciRealityStore) -> Path:
...
def _consume_quality_lab_export_for_hardening(store: FibonacciRealityStore) -> dict[str, int]:
...
def run_vision_lane(store: FibonacciRealityStore) -> dict[str, Any]:
    return vision_intake.run_vision_lane(store)


def bootstrap_research_memory(store: FibonacciRealityStore) -> dict[str, Any]:
    """Create the first self-generated FibonacciReality packets when memory is empty."""
    ...
    profile_id = str(profile.get("profile_id") or "forced_window_to_bedc")
    contacts = [
        {
...
def promote_next_layer(store: FibonacciRealityStore) -> dict[str, Any]:
...
def run_packet_lane(store: FibonacciRealityStore) -> dict[str, Any]:
    packet_config = _load_pipeline_config().get("packet_lane")
    if isinstance(packet_config, dict) and packet_config.get("enabled") is False:
        store.write_packet_targets([])
        render_lane_dashboard(store.paths, [])
        return {
            "lane": "bio-P",
            "status": "disabled",
            "conjectures": len(store.load_conjectures()),
            "contacts": len(store.load_contacts()),
            "probes": len(store.load_probes()),
            "mismatches": len(store.load_mismatches()),
            "packet_targets": 0,
        }
...
def run_gate_lane(store: FibonacciRealityStore) -> dict[str, Any]:
    summary = fibonacci_reality_loop.run_once(store)
...
def run_oracle_server_lane(store: FibonacciRealityStore) -> dict[str, Any]:
    try:
        ...
        server_script = repo_root / "tools" / "fibonacci_reality" / "oracle" / "fibonacci_reality_oracle_server.py"
        log_path = repo_root / "tools" / "fibonacci_reality" / "state" / "oracle_server.log"
...
def _repo_root_from_store(store: FibonacciRealityStore) -> Path:
...
def _oracle_state_path(store: FibonacciRealityStore) -> Path:
...
def _read_oracle_state(store: FibonacciRealityStore) -> dict[str, Any]:
    return _read_json_object(_oracle_state_path(store))


def _write_oracle_state(store: FibonacciRealityStore, state: dict[str, Any]) -> None:
...
def _oracle_linked_records_for_conjecture(store: FibonacciRealityStore, conjecture: dict[str, Any]) -> dict[str, Any]:
    contact_refs = {str(item) for item in conjecture.get("reality_contact_refs", []) if isinstance(item, str)}
    probe_refs = {str(item) for item in conjecture.get("probe_refs", []) if isinstance(item, str)}
...
def _namecert_proposal_text(store: FibonacciRealityStore, claim_id: str) -> str:
...
def _select_bio_g_rotation_candidate(
    ...
    store: FibonacciRealityStore | None = None,
) -> dict[str, Any] | None:
...
def _maybe_run_bio_g_oracle(store: FibonacciRealityStore) -> dict[str, Any]:
    config = _load_oracle_integration_config()
...
def _load_claims_document(path: Path) -> dict[str, Any]:
    try:
        ...
        return {"version": "fibonacci-reality-claims-v1", "claims": []}
...
def _load_experiments_document(path: Path) -> dict[str, Any]:
    try:
        ...
        return {"version": "fibonacci-reality-experiments-v1", "experiments": []}
    if not isinstance(data, dict):
        return {"version": "fibonacci-reality-experiments-v1", "experiments": []}
...
def _namecert_log(store: FibonacciRealityStore, event: str, details: dict[str, Any]) -> None:
...
def _loning_naming_hint(claim: dict[str, Any], conjecture: dict[str, Any]) -> str:
    ...
    return "FibonacciReality"


def _proposal_exists(store: FibonacciRealityStore, claim_id: str) -> bool:
...
def _frontier_vision_excerpt(store: FibonacciRealityStore, limit: int = 10000) -> str:
...
def _frontier_prompt(
    ...
    return "\n".join(
        [
            "You are proposing exactly one gated FibonacciReality frontier conjecture.",
...
def _validate_frontier_conjecture(store: FibonacciRealityStore, record: dict[str, Any]) -> list[str]:
    contacts = store.load_contacts()
    probes = store.load_probes()
    contact_by_id = {
...
def run_plan_lane(store: FibonacciRealityStore) -> dict[str, Any]:
    """Detect phase-advance and stuck-claim signals, emit events for bio-R."""
    plan_config = _load_pipeline_config().get("plan_lane")
    if isinstance(plan_config, dict) and plan_config.get("enabled") is False:
        return {
            "lane": "bio-Plan",
            "status": "disabled",
            "phase_advance_events": 0,
            "stuck_redesign_events": 0,
...
def run_execute_lane(store: FibonacciRealityStore) -> dict[str, Any]:
    claims_document = _load_claims_document(store.paths.claims_registry)
    experiments_document = _load_experiments_document(store.paths.experiments_registry)
...
def run_namecert_lane(store: FibonacciRealityStore) -> dict[str, Any]:
    claims_document = _load_claims_document(store.paths.claims_registry)
    claims = [item for item in claims_document.get("claims", []) if isinstance(item, dict)]
    conjectures = store.load_conjectures()
...
def run_assimilation_lane(paths: FibonacciRealityPaths) -> dict[str, Any]:
...
def _load_writeback_heal_config() -> dict[str, Any]:
    ...
    return {
        ...
        "path_whitelist_prefix": str(config.get("path_whitelist_prefix") or "papers/fibonacci_reality/"),
    }
...
def _append_sync_log(store: FibonacciRealityStore, event: str, details: dict[str, Any]) -> None:
    record = {"ts": now_iso(), "event": event, "details": details}
    try:
...
def _append_keep_log(store: FibonacciRealityStore, event: str, details: dict[str, Any]) -> None:
    record = {"ts": now_iso(), "event": event, "details": details}
    try:
...
def _write_sync_state(store: FibonacciRealityStore, state: dict[str, Any]) -> None:
...
def _bios_codex_resolve_prompt(rel_path: str, conflicted_content: str, upstream_sha: str) -> str:
    return "\n".join([
        "You are resolving a single git merge conflict in the FibonacciReality / BEDC repository.",
        "Local branch: feat/fibonacci_reality-deepening (FibonacciReality research direction).",
        ...
        "- FibonacciReality discipline: every cross-layer claim needs a reality contact at its layer + falsifiable perturbation prediction",
        ...
        "- FibonacciReality content (papers/fibonacci_reality/, tools/fibonacci_reality/, vision/) → prefer local side",
...
def _bios_resolve_signature_path(store: FibonacciRealityStore) -> Path:
    return store.paths.sync_lane_state.parent / "bios_resolve_signatures.jsonl"


def _bios_resolve_recent_count(store: FibonacciRealityStore, signature: str, window_seconds: int) -> int:
...
def _bios_resolve_record(store: FibonacciRealityStore, record: dict[str, Any]) -> None:
    path = _bios_resolve_signature_path(store)
...
def _bios_codex_resolve_merge(
    ...
    take_ours_bioreality = bool(cfg.get("papers_fibonacci_reality_take_ours", True))

    def _conflict_side(rel: str) -> str | None:
        # 按域定权威方向:
        #  - papers/bedc/ 是 Loning 域 → auto-dev 权威 → 取 theirs.
        #  - papers/fibonacci_reality/parts/ 是我方域 → 我方 daemon 生成的 rich 内容权威;
        ...
        if take_theirs_bedc and rel.startswith("papers/bedc/"):
            return "theirs"
        if take_ours_bioreality and rel.startswith("papers/fibonacci_reality/parts/"):
            return "ours"
        return None
...
def run_sync_lane(store: FibonacciRealityStore) -> dict[str, Any]:
...
def _keep_lane_recently_pushed(store: FibonacciRealityStore, min_seconds: float) -> tuple[bool, float]:
...
def _run_keep_gates(store: FibonacciRealityStore, repo_root: Path, gates: list[Any]) -> tuple[bool, list[str] | None]:
    for gate in gates:
...
def _push_ahead_only(store: FibonacciRealityStore, repo_root: Path, remote: str, branch: str) -> tuple[bool, str]:
...
def _keep_lane_commit_message(store: FibonacciRealityStore, selected: list[str]) -> str:
    ...
    title = f"FibonacciReality cycle {now_iso()}: {len(selected)} file(s)"
    body = [
        "",
        "Selected files:",
...
def run_keep_lane(store: FibonacciRealityStore) -> dict[str, Any]:
    try:
    ...
    branch_default = str(config.get("branch") or "feat/fibonacci_reality-deepening")
    ...
    remote = str(config.get("remote") or "origin")
    branch = str(config.get("branch") or "feat/fibonacci_reality-deepening")
    try:
...
def run_merge_back_lane(store: FibonacciRealityStore) -> dict[str, Any]:
    """Merge feat/fibonacci_reality-deepening back to upstream (auto-dev) as fast-forward.
    ...
    remote = str(config.get("remote") or "origin")
    feat_branch = str(config.get("branch") or "feat/fibonacci_reality-deepening")
...
def run_dev_rollup_lane(store: FibonacciRealityStore) -> dict[str, Any]:
    ...
    repo_root = store.paths.root.parent.parent
    source = str(rollup.get("head_branch") or config.get("branch") or "feat/fibonacci_reality-deepening")
...
def _passed_conjectures(store: FibonacciRealityStore) -> list[dict[str, Any]]:
    conjectures = store.load_conjectures()
    contacts = store.load_contacts()
    probes = store.load_probes()
    mismatches = store.load_mismatches()
    results = fibonacci_reality_loop.gate_all(conjectures, contacts, probes, mismatches)
    passed = {
...
def _temp_paths(base: Path) -> FibonacciRealityPaths:
    return FibonacciRealityPaths(
        root=SCRIPT_DIR,
        ...
        paper_part=base / "paper" / "parts" / "forced_window_structure.tex",
    )
...
def _render_paper_main(paths: FibonacciRealityPaths, namecert_slugs: list[str]) -> str:
    lines = [
        ...
        "",
        r"% BEDC-style macros stubbed for the standalone FibonacciReality paper.",
        r"\newcommand{\origin}[1]{}",
        ...
        r"",
        r"\title{Fibonacci Reality: Forced Window Structure}",
        ...
        "",
        r"\section{Scope}",
        "FibonacciReality is a standalone research pipeline for finite forced-window structure. "
        "The paper starts empty: the orchestrator supplies anchors, conjectures, contacts, and promotion boundaries after the mechanical engine fork is live.",
        "",
        r"\input{parts/forced_window_structure}",
        "",
    ]
    if namecert_slugs:
        lines.extend(
...
def _bio_w_codex_writer_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "log_dir": str(writer.get("log_dir", "tools/fibonacci_reality/state/writeback_codex_logs")),
...
def render_namecert_with_codex(
    ...
    *,
    ...
    corrective_feedback: list[str] | None = None,
    log_dir: str | Path = "tools/fibonacci_reality/state/writeback_codex_logs",
) -> dict[str, Any] | None:
...
def _writeback_heal_state_path(store: FibonacciRealityStore) -> Path:
    return store.paths.root / "state" / "writeback_heal_signatures.jsonl"


def _last_fibonacci_reality_tex_file(output: str) -> str:
    matches = re.findall(r"\(\.?/?(parts/[^()\s]*\.tex|papers/fibonacci_reality/[^()\s]*\.tex)", output)
    if not matches:
        matches = re.findall(r"(\.?/?parts/[^:\s()]*\.tex|\bpapers/fibonacci_reality/[^:\s()]*\.tex)", output)
    if not matches:
        return ""
    rel = matches[-1].lstrip("./")
    if rel.startswith("papers/fibonacci_reality/"):
        return rel
    return f"papers/fibonacci_reality/{rel}"
...
def _parse_writeback_heal_error(output: str, paper_dir: Path) -> dict[str, str]:
    ...
    if "Unicode character" in combined and "not set up for use with LaTeX" in combined:
        ...
        category = "other"
    rel_file = _last_fibonacci_reality_tex_file(combined)
...
def _append_writeback_heal_record(store: FibonacciRealityStore, record: dict[str, Any]) -> None:
    append_jsonl(_writeback_heal_state_path(store), [{"ts": now_iso(), **record}])


def _writeback_heal_recent_count(store: FibonacciRealityStore, signature: str, window_seconds: int) -> int:
    cutoff = time.time() - window_seconds
    count = 0
...
def _writeback_heal_prompt(rel_file: str, content: str, error_tail: str) -> str:
    return "\n".join(
        [
            "# Task",
            "Repair exactly one LaTeX compile error in a FibonacciReality paper source file.",
            "",
            "# Project constraints",
...
def run_writeback_heal_lane(store: FibonacciRealityStore) -> dict[str, Any]:
    try:
        ...
        for attempt in range(1, int(config["max_attempts"]) + 1):
            ...
            # 如果 target file 没 unicode (file detection 错), fallback 扫描 ALL fibonacci_reality namecerts
...
def run_writeback_lane(store: FibonacciRealityStore) -> dict[str, Any]:
    ...
    part_lines = [
        r"\section{Forced Window Structure}",
        r"\label{sec:forced-window-structure}",
        "",
        "This section records gate-passed FibonacciReality research memory. "
    ...
    # Auto-build the PDF so reviewers can open papers/fibonacci_reality/main.pdf
...
def _append_bedc_writeback_log(store: FibonacciRealityStore, event: str, details: dict[str, Any]) -> None:
    record = {"ts": now_iso(), "event": event, "details": details}
    try:
...
def _auto_discovered_bedc_mappings(proposal_dir: Path, mapped_claim_ids: set[str]) -> list[dict[str, Any]]:
    ...
    for proposal_path in sorted(proposal_dir.glob("*.md"), key=lambda path: path.stem):
        ...
        carrier_match = re.search(r"\b(FibonacciReality\w+|RealityBound\w+|Empirical\w+|Curated\w+)\b", carrier_section)
...
def _bedc_codex_writer_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "log_dir": str(writer.get("log_dir", config.get("codex_log_dir") or "tools/fibonacci_reality/state/bedc_writeback_logs")),
...
def render_chapter_with_codex(
    ...
    *,
    timeout_seconds: int = 600,
    log_dir: str | Path = "tools/fibonacci_reality/state/bedc_writeback_logs",
    corrective_feedback: list[str] | None = None,
) -> dict[str, Any]:
...
def run_bedc_writeback_lane(store: FibonacciRealityStore) -> dict[str, Any]:
    try:
...
def render_lane_dashboard(paths: FibonacciRealityPaths, targets: list[dict[str, Any]], signals: dict[str, Any] | None = None) -> None:
    lines = [
        "# FibonacciReality lane dashboard",
        "",
...
def self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="fibonacci-reality-lanes-") as tmp:
        base = Path(tmp)
        ...
        try:
            disabled_summary = run_sync_lane(FibonacciRealityStore(paths))
        finally:
        ...
        keep_selected, keep_dropped = _filter_paths(
            [
                (" M", "tools/fibonacci_reality/state/foo.jsonl"),
                (" M", "tools/fibonacci_reality/experiments/run_X.py"),
                ("??", "tools/fibonacci_reality/vision/index.md"),
            ],
        ...
        if keep_selected != ["tools/fibonacci_reality/experiments/run_X.py"]:
            print(json.dumps({"selected": keep_selected, "dropped": keep_dropped}, indent=2), file=sys.stderr)
            return 1
        if "tools/fibonacci_reality/state/foo.jsonl" not in keep_dropped:
            print(json.dumps({"selected": keep_selected, "dropped": keep_dropped}, indent=2), file=sys.stderr)
            return 1
        ...
        bedc_aggregator = base / "bedc_target" / "fibonacci_reality_module.tex"
        bedc_mappings = [
            {
                ...
                "carrier_name": "FibonacciRealityQSixMedianClosureLedger",
            ...
            },
            {
                ...
                "carrier_name": "FibonacciRealityQSixMedianShapeSignificance",
            ...
            },
            {
                ...
                "carrier_name": "FibonacciRealityQSixReassignedCodonLedger",
            ...
            },
            {
                ...
                "carrier_name": "FibonacciRealityQSixEmpiricalCodonSpectrum",
        ...
        try:
            bedc_summary = run_bedc_writeback_lane(FibonacciRealityStore(bedc_paths))
        finally:
            globals()["PIPELINE_CONFIG"] = original_pipeline_config
        ...
        bedc_auto_aggregator = base / "bedc_auto_target" / "fibonacci_reality_module.tex"
        bedc_auto_mappings = [
            {
                ...
                "carrier_name": "FibonacciRealityMappedSeed",
        ...
        (bedc_auto_paths.namecert_proposals_dir / "extra.md").write_text(
            "\n".join(
                [
                    ...
                    "## 2. Carrier",
                    "",
                    "Carrier: FibonacciRealityExtraSeedUp.",
                ]
            ),
            encoding="utf-8",
        )
        ...
        try:
            bedc_auto_summary = run_bedc_writeback_lane(FibonacciRealityStore(bedc_auto_paths))
        finally:
            globals()["PIPELINE_CONFIG"] = original_pipeline_config
        ...
        (bedc_auto_paths.namecert_proposals_dir / "blocked.auto.md").write_text(
            "\n".join(
                [
                    ...
                    "## 2. Carrier",
                    "",
                    "Carrier: FibonacciRealityBlockedSeedUp.",
                ]
            ),
            encoding="utf-8",
        )
        ...
        bedc_skip_aggregator = base / "bedc_skip_target" / "fibonacci_reality_module.tex"
        ...
        try:
            bedc_skip_summary = run_bedc_writeback_lane(FibonacciRealityStore(bedc_auto_paths))
        finally:
            globals()["PIPELINE_CONFIG"] = original_pipeline_config
        if (
        ...
        bedc_codex_aggregator = base / "bedc_author_target" / "fibonacci_reality_module.tex"
        bedc_codex_mapping = {
            ...
            "carrier_name": "FibonacciRealityAuthorWriter",
        ...
        def _mock_render_chapter_with_codex(
            ...
            *,
            ...
            carrier = str(target.get("carrier_name") or "FibonacciRealityAuthorWriter")
        ...
        try:
            bedc_codex_summary = run_bedc_writeback_lane(FibonacciRealityStore(bedc_codex_paths))
        finally:
        ...
        try:
            bio_w_codex_summary = run_writeback_lane(FibonacciRealityStore(bio_w_codex_paths))
        finally:
        ...
        try:
            fallback_summary = run_writeback_lane(FibonacciRealityStore(bio_w_fallback_paths))
        finally:
        ...
        promote_store = FibonacciRealityStore(promote_paths)
            ...
            [
                {
                    ...
                    "source_kind": "genetic_code_table",
                    "source_ref": "curated standard genetic-code bridge",
                    "source_snapshot": "forced_window_to_bedc",
                    "observed_fact": "A curated standard-code table assigns codons to amino-acid or stop labels.",
                    "resolution": "codon label readback",
                    "known_noise_or_bias": "The table is a reality bridge for code labels, not a mechanism or universality proof.",
                    "can_test": ["code_read", "local codon tile label split"],
            ...
            json.dumps(
                {
                    ...
                    "experiments": [
                        {
                            "experiment_id": "self_missing_script",
                            "script_path": "tools/fibonacci_reality/experiments/run_self_missing_script.py",
                            "claim_id": "self.claim",
                            "required_data": [],
                            "timeout_seconds": 5,
        ...
        execute_store = FibonacciRealityStore(execute_paths)
        ...
        namecert_store = FibonacciRealityStore(namecert_paths)
        ...
        writeback_empty_fact_store = FibonacciRealityStore(writeback_empty_fact_paths)
        original_pipeline_config = PIPELINE_CONFIG
        globals()["PIPELINE_CONFIG"] = writeback_disabled_config_path
        try:
        ...
        writeback_fact_store = FibonacciRealityStore(writeback_fact_paths)
        original_pipeline_config = PIPELINE_CONFIG
        globals()["PIPELINE_CONFIG"] = writeback_disabled_config_path
        try:
        ...
        try:
            ...
            oracle_gate_store = FibonacciRealityStore(oracle_gate_paths)
            ...
            oracle_plan_paths.experiments_registry.write_text(
                json.dumps({"version": "fibonacci-reality-experiments-v1", "experiments": []}, indent=2),
                encoding="utf-8",
            )
            oracle_plan_store = FibonacciRealityStore(oracle_plan_paths)
        ...
        plan_phase_store = FibonacciRealityStore(plan_phase_paths)
        original_pipeline_config = PIPELINE_CONFIG
        globals()["PIPELINE_CONFIG"] = oracle_disabled_config_path
        try:
        ...
        stuck_store = FibonacciRealityStore(stuck_paths)
        original_pipeline_config = PIPELINE_CONFIG
        globals()["PIPELINE_CONFIG"] = oracle_disabled_config_path
        try:
        ...
        if not any(event.get("event_kind") == "claim_redesign_proposed" and event.get("subject_id") == "x" for event in stuck_events):
            ...
            return 1
    print("[fibonacci-reality-lanes] self-test ok")
    return 0


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run FibonacciReality V/P/G/X/N/R/W/B/Q/A lanes")
