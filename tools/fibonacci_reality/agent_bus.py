#!/usr/bin/env python3
"""Event-driven agent dispatch contracts for FibonacciReality."""
...
try:
    from store import FibonacciRealityPaths, FibonacciRealityStore, read_jsonl, write_jsonl
...
AGENTS = {
    "bio-researcher": {
        "lane": "bio-R",
        "writes": [
            "tools/fibonacci_reality/inbox/conjectures.jsonl",
            "tools/fibonacci_reality/inbox/reality_contacts.jsonl",
            "tools/fibonacci_reality/inbox/probes.jsonl",
            "tools/fibonacci_reality/inbox/mismatches.jsonl",
        ],
        "mission": "extend FibonacciReality research memory under external-reality and cannot-claim discipline",
    },
    "bio-gate-curator": {
        "lane": "bio-Q",
        "writes": [
            "tools/fibonacci_reality/deepening_gates.py",
            "tools/fibonacci_reality/*.schema.json",
            "tools/fibonacci_reality/pipeline_config.json",
        ],
    ...
    "bio-experimentalist": {
        "lane": "bio-R",
        "writes": [
            "tools/fibonacci_reality/experiments/run_*.py",
            "tools/fibonacci_reality/data/*.json",
            "tools/fibonacci_reality/registries/experiments.json",
        ],
        "mission": "implement or repair FibonacciReality experiment scripts that produce a single JSON result with checks and status",
    },
    "bio-data-fetcher": {
        "lane": "bio-R",
        "writes": [
            "tools/fibonacci_reality/data/*.json",
            "tools/fibonacci_reality/data/*.py",
            "tools/fibonacci_reality/data/manifests/*.json",
        ],
        "mission": "fetch curated public biological reality data (NCBI / UniProt / Ensembl / RefSeq / EBI) via urllib.request, store as versioned JSON under tools/fibonacci_reality/data/ with provenance, never write paper or experiment scripts",
    },
    "bio-planner": {
        "lane": "bio-R",
        "writes": [
            "tools/fibonacci_reality/registries/claims.json",
            "tools/fibonacci_reality/registries/experiments.json",
            "tools/fibonacci_reality/experiments/run_*.py",
            "tools/fibonacci_reality/data/*.json",
        ],
        "mission": "decompose passed FibonacciReality content into next-step BEDC claim and experiment skeletons, or redesign stuck claims with stronger statistics",
    },
    "bio-namer": {
        "lane": "bio-R",
        "writes": [
            "tools/fibonacci_reality/state/namecert_proposals/*.md",
        ],
        "mission": "translate a stable FibonacciReality claim into a Loning-format NameCert proposal markdown with explicit external-reality fields and avoid Loning's already-occupied namespaces",
    },
    "bio-oracle-consumer": {
        ...
        "writes": [
            "tools/fibonacci_reality/registries/claims.json",
            "tools/fibonacci_reality/registries/experiments.json",
            "tools/fibonacci_reality/state/oracle_refinements/*.json",
        ],
...
ORACLE_ESCALATION_RULES = [
    ...
    "  for decomposing a stable FibonacciReality finding, or you cannot identify a stronger",
    "  null model that respects the existing acceptance criteria, you may consult",
    "  the FibonacciReality oracle via:",
    '    python3 tools/fibonacci_reality/oracle/oracle_client.py --query "<concise question>" \\',
...
def _load_claims_document(path: Path) -> dict[str, Any]:
    data = _read_json_object(path, {"version": "fibonacci-reality-claims-v1", "claims": []})
...
def _load_experiments_document(path: Path) -> dict[str, Any]:
    data = _read_json_object(path, {"version": "fibonacci-reality-experiments-v1", "experiments": []})
    if not isinstance(data.get("experiments"), list):
        data["experiments"] = []
    return data


def _load_claims_registry(store: FibonacciRealityStore) -> dict[str, dict[str, Any]]:
...
def _claim_status_by_experiment(store: FibonacciRealityStore) -> dict[str, str]:
...
def _oracle_sessions_dir(store: FibonacciRealityStore) -> Path:
...
def _oracle_refinements_dir(store: FibonacciRealityStore) -> Path:
    events_parent = store.paths.events.parent
    if events_parent.name == "out":
...
def _resolve_oracle_transcript(event: dict[str, Any], store: FibonacciRealityStore | None = None) -> tuple[Path | None, str]:
...
def _backfill_oracle_consultation_events(store: FibonacciRealityStore, events: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def build_events(store: FibonacciRealityStore) -> list[dict[str, Any]]:
...
def render_prompt(event: dict[str, Any], agent_id: str, action: str) -> str:
    ...
    if agent_id == "bio-oracle-consumer":
        ...
        return "\n".join(
            [
                "You are a FibonacciReality oracle-consumer agent inside the newmath repository.",
                f"Agent: {agent_id}",
    ...
    if agent_id == "bio-experimentalist":
        extra_rules = [
            ...
            "- subject_id is the experiment_id; find the matching script_path in tools/fibonacci_reality/registries/experiments.json.",
            "",
            ...
            "",
            'Step 0d. After repair, run the script again and confirm it now produces status="passed". If the failure is theoretically real and not a script bug, Do NOT fake passed: leave it failed and instead propose adding a new experiment with a different statistic; in this case write a notes file tools/fibonacci_reality/state/proposed_experiments/<id>.md describing what new experiment should be added, but you yourself may not add it.',
            "",
            "You may only edit script files matching tools/fibonacci_reality/experiments/run_*.py or data files under tools/fibonacci_reality/data/. The script must end by printing a single-line JSON to stdout with fields experiment_id, claim_id, status, checks, result, started_at, completed_at. Python 3 stdlib only.",
        ]
        ...
        extra_rules = [
            ...
            "- urllib.request timeout=60s; set User-Agent to FibonacciReality-data-fetcher/1.0",
            "",
            "Mandatory provenance for every fetched artifact:",
            "1. Save raw payload to tools/fibonacci_reality/data/<source_name>_<accession_or_id>.json (or appropriate extension)",
            "2. Compute sha256 of the raw response bytes",
            "3. Save provenance to tools/fibonacci_reality/data/manifests/<basename>.json with:",
            "   {",
            '     "fetched_at": "<iso-utc>",',
            ...
            "- Inspect event payload to learn which claim's needs_data triggered this. Read tools/fibonacci_reality/registries/claims.json to find the claim; read tools/fibonacci_reality/registries/experiments.json to find required_data list for the experiment.",
            ...
            "- If no suitable public source is reachable (private DB, paywall, custom dataset), write the diagnosis to tools/fibonacci_reality/state/proposed_data_sources/<claim_id>.md and STOP.",
            "",
            ...
            "",
            "Strict no-go:",
            "- Never write any .py file outside tools/fibonacci_reality/data/",
        ...
        extra_rules = [
            ...
            "  layer in forced_window_to_bedc_ladder.json (e.g. orf_eligibility may be",
            ...
            "  tools/fibonacci_reality/state/proposed_experiments/<claim_id>.md and DO",
            ...
            "- All edits must be atomic and pass the FibonacciReality self-tests before",
            "  you finish.",
            *ORACLE_ESCALATION_RULES,
        ]
        ...
        extra_rules = [
            ...
            "  Read tools/fibonacci_reality/registries/claims.json and find the matching claim;",
            "  read tools/fibonacci_reality/inbox/conjectures.jsonl and find the matching",
            "  conjecture; read the latest experiment_runs.jsonl entry for the claim.",
            "",
            "Step 1. Read tools/fibonacci_reality/state/research_notes/loning_writeback_pattern.md",
            "  if it exists. It records Loning's already-occupied terms",
            "  (Codon64, Q_6, M^star, StopCodonZeckendorfSuffix, etc.) and recommended",
            "  prefixes (FibonacciRealityQSix, RealityBoundCodonTopology, CuratedGeneticTableLedger,",
            "  EmpiricalCodonSpectrum). Avoid Loning's occupied names; use the recommended",
            "  prefixes.",
            "",
            "Step 2. Compose a NameCert proposal markdown at",
            "  tools/fibonacci_reality/state/namecert_proposals/<claim_id>.md with EXACTLY these",
            ...
            "  using a FibonacciReality-namespaced concept (must not clash with Loning).",
            "",
            "  ## 2. Carrier",
            "  Carrier name (CamelCase, must start with FibonacciReality or RealityBound or Empirical",
            ...
            "  BEDC.Derived.FibonacciRealityQSixMedianClosureUp.TasteGate carrier alignment).",
    ...
    return "\n".join(
        [
            "You are a FibonacciReality automation agent inside the newmath repository.",
            f"Agent: {agent_id}",
            f"Mission: {agent['mission']}",
            f"Action: {action}",
            "",
            ...
            "- Run the relevant FibonacciReality gates before reporting completion.",
        ...
        targets.append(
            {
                ...
                "reason": review.get("reason"),
                "next_action": "tighten FibonacciReality gate/schema" if decision == "requires_gate_hardening" else "dispatch bio-researcher with event prompt",
            }
        )
...
def run_agent_lane(store: FibonacciRealityStore, *, execute_codex: bool = True, max_dispatch: int = 1) -> dict[str, Any]:
...
def run_quality_lane(store: FibonacciRealityStore) -> dict[str, Any]:
...
def self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="fibonacci-reality-agent-bus-") as tmp:
        base = Path(tmp)
        ...
        if (
            ...
            return 1
        filter_paths = FibonacciRealityPaths(
            root=SCRIPT_DIR,
        ...
        filtered_events = build_events(FibonacciRealityStore(filter_paths))
        ...
        if len(filtered_experiment_failed) != 1 or filtered_experiment_failed[0].get("subject_id") != "null_model_uniform":
            ...
            return 1
        latest_paths = FibonacciRealityPaths(
            root=SCRIPT_DIR,
        ...
        latest_events = build_events(FibonacciRealityStore(latest_paths))
        ...
        for required_text in (
            ...
            "FibonacciReality or RealityBound",
        ...
        oracle_paths = FibonacciRealityPaths(
            root=SCRIPT_DIR,
        ...
        _write_json_object(
            ...
            {
                "version": "fibonacci-reality-claims-v1",
                "claims": [
                    {
                        "claim_id": "h0.seed",
                        "hypothesis_level": "H0",
        ...
        _write_json_object(
            ...
            {
                "version": "fibonacci-reality-experiments-v1",
                "experiments": [
                    {
                        "experiment_id": "seed_experiment",
                        "claim_id": "h0.seed",
        ...
        oracle_store = FibonacciRealityStore(oracle_paths)
        ...
        backfill_paths = FibonacciRealityPaths(
            root=SCRIPT_DIR,
        ...
        backfilled_events = build_events(FibonacciRealityStore(backfill_paths))
        if not any(
        ...
        if failed_events[0]["status"] != "archived":
            ...
            return 1
        if not _allowed_path("tools/fibonacci_reality/inbox/conjectures.jsonl", ["tools/fibonacci_reality/inbox/*.jsonl"]):
            print("expected inbox jsonl glob to match", file=sys.stderr)
            return 1
        if _allowed_path("lean4/Foo.lean", ["tools/fibonacci_reality/inbox/*.jsonl"]):
            print("expected lean4 path not to match inbox glob", file=sys.stderr)
            return 1
    print("[fibonacci-reality-agent-bus] self-test ok")
    return 0


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Plan and review FibonacciReality agent tasks from events")
