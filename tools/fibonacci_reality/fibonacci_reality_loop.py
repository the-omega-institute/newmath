#!/usr/bin/env python3
"""Run one deterministic FibonacciReality conjecture-deepening loop."""
...
try:
    ...
    from store import FibonacciRealityPaths, FibonacciRealityStore, write_jsonl
...
def run_once(store: FibonacciRealityStore) -> dict[str, Any]:
...
def self_test() -> int:
    ...
    summary = run_once(FibonacciRealityStore(paths))
    tasks = FibonacciRealityStore(paths).paths.deepening_tasks.read_text(encoding="utf-8")
    if summary["blocked"] != 0 or "ready_for_operator_review" not in tasks:
        print(json.dumps(summary, indent=2), file=sys.stderr)
        return 1
    print("[fibonacci-reality-loop] self-test ok")
    return 0


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run one FibonacciReality deepening loop")
    parser.add_argument("--conjectures", default=str(FibonacciRealityPaths.conjectures), help="conjecture JSONL")
    parser.add_argument("--contacts", default=str(FibonacciRealityPaths.contacts), help="reality contact JSONL")
    parser.add_argument("--probes", default=str(FibonacciRealityPaths.probes), help="probe JSONL")
    parser.add_argument("--mismatches", default=str(FibonacciRealityPaths.mismatches), help="mismatch JSONL")
    parser.add_argument("--gate-results", default=str(FibonacciRealityPaths.gate_results), help="gate result JSONL")
    parser.add_argument("--deepening-tasks", default=str(FibonacciRealityPaths.deepening_tasks), help="deepening task JSONL")
    parser.add_argument("--review-queue", default=str(FibonacciRealityPaths.review_queue), help="review queue JSONL")
    ...
    try:
        ...
        print(f"[fibonacci-reality-loop] error: {exc}", file=sys.stderr)
        return 1
    print(
        "[fibonacci-reality-loop] "
