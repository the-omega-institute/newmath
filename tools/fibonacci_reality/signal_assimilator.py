#!/usr/bin/env python3
"""Classify FibonacciReality loop outputs into local pipeline signals."""
...
try:
    from store import FibonacciRealityPaths, read_jsonl
except ModuleNotFoundError:  # pragma: no cover
    from tools.fibonacci_reality.store import FibonacciRealityPaths, read_jsonl


SCRIPT_DIR = Path(__file__).resolve().parent
STATE_DIR = SCRIPT_DIR / "state"
SIGNAL_JOURNAL = STATE_DIR / "fibonacci_reality_signals.jsonl"
LATEST_PATH = STATE_DIR / "fibonacci_reality_signals_latest.md"
VERSION = "fibonacci-reality-pipeline-signals"
...
def summarize(paths: FibonacciRealityPaths) -> dict[str, Any]:
...
def render_latest(summary: dict[str, Any]) -> str:
    lines = [
        "# FibonacciReality pipeline signals latest",
        "",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Summarize FibonacciReality loop outputs into local signals")
    parser.add_argument("--gate-results", default=str(FibonacciRealityPaths.gate_results))
    parser.add_argument("--deepening-tasks", default=str(FibonacciRealityPaths.deepening_tasks))
    parser.add_argument("--review-queue", default=str(FibonacciRealityPaths.review_queue))
    parser.add_argument("--events", default=str(FibonacciRealityPaths.events))
    parser.add_argument("--agent-tasks", default=str(FibonacciRealityPaths.agent_tasks))
    parser.add_argument("--dispatch-results", default=str(FibonacciRealityPaths.dispatch_results))
    parser.add_argument("--hardening-targets", default=str(FibonacciRealityPaths.hardening_targets))
