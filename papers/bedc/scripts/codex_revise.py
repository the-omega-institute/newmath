        ...
        try:
                ...
                with push_lock_cm:
                    with _git_lock:
                        ...
                        if r3.returncode == 0:
                            ...
                            status_output = (rs.stdout or "").strip()
                            if status_output:
                                logger.warning(
                                ...
                                recovery_prompt = (
                                    ...
                                    .replace("<STATUS_OUTPUT>", status_output[:2000] or "(empty)")
            ...
            if recovery_prompt:
                ...
                try:
                    ...
                    logger.warning(f"[origin-sync] push lock timeout after codex recovery: {exc}")
        except Exception as exc:
...
def _sync_local_with_origin(*, model: Optional[str] = None) -> bool:
    ...
    try:
        ...
        logger.warning(f"_sync_local_with_origin: push lock timeout during cleanup: {exc}")
        return False
...
def request_recovery(wt: WorktreeInfo) -> None:
    ...
    try:
        ...
        stem = wt.lease.ticket_stem if wt.lease else f"legacy_paper_revise_{wt.round_number}"
...
def merge_worktree_to_base(wt: WorktreeInfo, *, model: Optional[str] = None) -> bool:
    ...
    Concurrency: shared main-checkout git sequences use the cross-process
    `acquire_push_lock(BASE_BRANCH)` advisory flock outside `_git_lock`.
    Codex conflict recovery and hard gates run outside both locks.
    """
    from contextlib import nullcontext
    import sys as _sys
    import random
    ...
    try:
        ...
        logger.warning(f"[P{wt.round_number}] push-lock import failed: {exc}; using nullcontext")
        _pl = None
    ...
    if not _sync_local_with_origin(model=model):
        logger.error(
            f"[P{wt.round_number}] local {BASE_BRANCH} could not sync "
            f"with origin/{BASE_BRANCH}; aborting"
        )
        return False

    # Phase 1b: worktree-local work (per-worker, NO _git_lock)
    merge = run_cmd(
        ["git", "merge", "--no-ff", "--no-edit", BASE_BRANCH],
        cwd=wt.path, timeout=180,
    )
    if merge.returncode != 0:
        logger.warning(f"[P{wt.round_number}] Merge conflict, invoking codex")
    ...
    if not any(own_prefix in l for l in merged_lines):
        logger.error(
            ...
            f"{BASE_BRANCH}; refusing to report merge success"
        )
        return False
    ...
    if not ok:
        ...
        if not ok:
            logger.error(
                f"[P{wt.round_number}] Audit still failing after codex "
                "resolution; refusing to merge"
            )
    ...
    for attempt in range(1, MAX_PUSH_ATTEMPTS + 1):
        ...
        try:
            with push_lock_cm, _git_lock:
                ...
                current_base_sha = run_cmd(
                    ["git", "rev-parse", f"origin/{BASE_BRANCH}"], cwd=REPO_ROOT
                ).stdout.strip()

                if current_base_sha != captured_base_sha:
                    logger.info(
                        f"[P{wt.round_number}] base sha moved "
                    ...
                    if ok:
                        ...
                        logger.warning(
                            f"[P{wt.round_number}] push rejected attempt {attempt}: "
                            f"{push.stderr[:200]}"
                        )
                    else:
                        logger.warning(
        ...
        if attempt == MAX_PUSH_ATTEMPTS:
            logger.error(f"[P{wt.round_number}] push attempts exhausted ({MAX_PUSH_ATTEMPTS})")
            return False

        backoff = min(5 * (2 ** (attempt - 1)) + random.uniform(0, 5), 60.0)
        logger.info(
            f"[P{wt.round_number}] backing off {backoff:.1f}s before push attempt {attempt + 1}"
        )
        time.sleep(backoff)
