...
This chapter records the discipline by which a mature mathematical object's claim is read. The discipline reports three independent axes. The \emph{theoretical closure axis} records what the paper has proved about the object's $\NameCert$, scope, exactness, ledger, stability, and bridge; it is read entirely from the manuscript, with no input from any proof assistant. The \emph{external correspondence axis} records how the package is compared with a declared outside reference theory and scope. The \emph{formal verification axis} records the independent audit state of the corresponding Lean targets; it strengthens reliability and reproducibility but assigns no theoretical content. A claim and its audit are separate artefacts that compose only via $\VerifiedGate$; external correspondence is reported alongside them and does not alter the theory or verification grade.

Read against \coreboundaryref{rem:three-axioms-gate-meaning}{the axiom-purity gate reading}, this chapter is the positive face of the project's discipline: positive content admitted only if its closure is exhibited by paper-side witnesses, while machine verification is reported as an external audit layer that never substitutes for closure.
...
\section{External correspondence axis}
\label{sec:external-correspondence-axis}

\begin{definition}[External correspondence grades]
\label{def:external-correspondence-grades}
The \emph{external correspondence axis} is the four-element set
$$
  \ExternalCorrespondenceStatus
  \;=\;
  \{\, \notApplicable,\ \referencedExternal,\ \bridgedExternal,\ \calibratedReconstruction \,\}
$$
ordered by $\notApplicable < \referencedExternal < \bridgedExternal < \calibratedReconstruction$.
\end{definition}

\begin{definition}[External correspondence interpretations]
\label{def:external-correspondence-interpretations}
The grades are read as follows.
\begin{description}
  \item[\textbf{$\notApplicable$:}] the package is stated internally, with no declared outside reference theory for comparison.
  \item[\textbf{$\referencedExternal$:}] an outside reference theory $R$ is named, and the comparison scope $S$ is stated.
  \item[\textbf{$\bridgedExternal$:}] the chapter states the object and main-theorem correspondences between $R$ and the BEDC construction over $S$.
  \item[\textbf{$\calibratedReconstruction$:}] relative to a declared and commit-pinned outside reference theory $R$ and a declared scope $S$, the package closes the theory-narrative question of constructive content over $S$: it supplies an internal constructive theory $C$ over $S$, states the $R \leftrightarrow C$ object and main-theorem correspondences, declares coverage and exclusions, and exhausts $S$ as the disjoint union of the BEDC reconstructed constructive kernel and the measured or refused classical boundary, with no hidden third region. This grade does not assert kernel checking, axiom purity, or footprint cleanliness; those are reported only by the formal verification and bridge-status axes.
\end{description}
\end{definition}

\begin{principle}[Calibrated reconstruction requires boundary exhaustion]
\label{prin:calibrated-reconstruction-boundary-exhaustion}
A closurestatus block may report \texttt{\textbackslash externalcorrespondence\{\textbackslash calibratedReconstruction\}} only when it also supplies \texttt{\textbackslash boundaryexhaustive\{<label>\}} pointing to a finite decomposition of the declared scope. Each row of that decomposition must be marked either as BEDC reconstructed constructive kernel, with local Lean targets or definitions, or as measured/refused classical boundary, with the external footprint explicitly named.
\end{principle}

\begin{remark}[External correspondence is not bridged closure]
\label{rem:external-correspondence-not-bridged-closure}
The external correspondence axis is additive. It does not rewrite $\bridgedClosure$ and it does not promote a package along the theoretical closure ladder. A package can have $\calibratedReconstruction$ for an external corpus while its theory closure and formal verification grades remain exactly the values recorded by $\theoryclosure$ and $\formalstatus$.
\end{remark}
...
\begin{closurestatus}{\<X>Up}
  \constructivestory{A closure claim is generated bottom-up by first naming the finite source field, then exposing the visible carrier rows, classifier rows, ledger rows, and stability rows that make the claim readable. At the lowest layer the packet is still a BHist-shaped artefact: hsame supplies rowwise sameness, Cont supplies the displayed continuation routes, and Pkg records which source certificates are being consumed. The classifier and ledger align because every accepted row is also the row a downstream consumer may inspect, while the source specification says where each row came from. Nonemptiness is witnessed by the displayed seed packet, and reflexivity, symmetry, and transitivity are inherited componentwise from the hsame comparisons. The closurestatus block therefore records how the object is built before it reports any verification grade.}
  \theoryclosure{\scopedClosure}
  \scopeclosed{<text describing exactly what the chapter closes>}
  \externalcorrespondence{\notApplicable | \referencedExternal
    | \bridgedExternal | \calibratedReconstruction}
  \reference{<external reference theory and commit, when applicable>}
  \reconstructionscope{<external comparison scope>}
  \boundaryexhaustive{<label of finite decomposition table>}
