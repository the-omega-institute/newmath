...
The BEDC project as a development artefact is identifiable not because there is a fixed "the BEDC project" object underlying every commit, but because the commit chain forms a connected monotonic accumulation of $\mathsf{Obs}$ records. Reversion or replacement of an early commit would, in the strict closed-system reading, produce a different observer; the policy in \autoref{ch:theory-amendment-policy} requiring current-state presentation is the operational form of preserving the project's observer identity.
\end{example}
