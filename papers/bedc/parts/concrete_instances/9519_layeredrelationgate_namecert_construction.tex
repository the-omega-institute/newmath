...
\end{enumerate}
\end{theorem}
\leanchecked{BEDC.Derived.LayeredRelationGateUp.LayeredRelationGateNameCertObligations}
\begin{proof}
...
\end{theorem}
\leanchecked{BEDC.Derived.LayeredRelationGateUp.LayeredRelationGate\_same\_schema\_nonidentity}
\begin{proof}
...
\end{proof}
\leanchecked{BEDC.Derived.LayeredRelationGateUp.LayeredRelationGate\_not\_preserved\_row\_determinacy}
...
\end{theorem}
\leanchecked{BEDC.Derived.LayeredRelationGateUp.LayeredRelationGateConsumerRoute}
\leanvariant{BEDC.Derived.LayeredRelationGateUp.LayeredRelationGate\_consumer\_route}
\begin{proof}
...
\begin{theorem}[LayeredRelationGate carrier obligation]
\label{thm:layered-relation-gate-carrier-obligation}
Every $\LayeredRelationGateUp$ obligation read must expose the carrier rows $A,B,L,P,N,F,G,H,C,\Pi$ before any verdict, consumer route, or same-schema export is available.
\end{theorem}
\leanchecked{BEDC.Derived.LayeredRelationGateUp.LayeredRelationGateCarrierObligation}
\begin{proof}
Use the carrier of \autoref{def:layered-relation-gate-carrier}. Its finite tuple lists exactly the two source rows, layer-list row, preserved row, not-preserved row, refusal ledger, verdict row, transport row, continuation row, and package-name row.

Apply the NameCert obligations of \autoref{thm:layered-relation-gate-namecert-obligations}. Carrier exposure is the first obligation, so the downstream route cannot read $G$ or a same-schema consumer before $A,B,L,P,N,F,G,H,C,\Pi$ have all been displayed.
\end{proof}

\begin{theorem}[LayeredRelationGate classifier obligation]
\label{thm:layered-relation-gate-classifier-obligation}
A $\LayeredRelationGateUp$ classifier read may export a passing verdict only after comparing the preserved row $P$ with the not-preserved row $N$ over the same source, layer, and refusal rows $A,B,L,F$. The verdict row $G$ is therefore not a classifier for identity between $A$ and $B$.
\end{theorem}
\leanchecked{BEDC.Derived.LayeredRelationGateUp.LayeredRelationGateClassifier\_obligation}
\leanvariant{BEDC.Derived.LayeredRelationGateUp.LayeredRelationGateCarrier\_classifier\_obligation}
\begin{proof}
Start from \autoref{thm:layered-relation-gate-namecert-obligations}. The preserved and not-preserved disclosures are separate obligations, and both are upstream of verdict export.

Now apply \autoref{thm:layered-relation-gate-same-schema-nonidentity}. It states that a passing verdict licenses relation use while refusing object identity, empirical law, proof transfer, subject-substance transfer, global-frame content, and bridge strength. Hence the classifier must compare $P$ and $N$ before $G$ can be read as a same-schema verdict.
\end{proof}

\begin{theorem}[LayeredRelationGate ledger obligation]
\label{thm:layered-relation-gate-ledger-obligation}
Every same-schema consumer of $\LayeredRelationGateUp$ must keep the refusal ledger $F$ visible. If $F$ is not displayed, the consumer route cannot distinguish a permitted same-schema relation from a hidden identity, empirical-law, proof-transfer, subject-substance, global-frame, or bridge-strength claim.
\end{theorem}
\begin{proof}
Use the consumer route of \autoref{thm:layered-relation-gate-consumer-route}. The route reads the relation-certificate source, concrete certificate surface, force inter-history relation surface, and axiom-refusal ledger before it reaches the local packet.

Apply \autoref{thm:layered-relation-gate-namecert-obligations} and \autoref{thm:layered-relation-gate-same-schema-nonidentity}. They keep $F$ as the displayed failure-boundary row and place the refused readings in $N$ rather than in the exported relation. Without $F$, the same-schema consumer has no visible ledger separating the accepted relation from the forbidden readings.
\end{proof}
\leanchecked{BEDC.Derived.LayeredRelationGateUp.LayeredRelationGate\_ledger\_obligation}

\begin{theorem}[LayeredRelationGate obligation closure]
\label{thm:layered-relation-gate-obligation-closure}
The carrier of \autoref{def:layered-relation-gate-carrier}, the NameCert obligations of \autoref{thm:layered-relation-gate-namecert-obligations}, same-schema non-identity from \autoref{thm:layered-relation-gate-same-schema-nonidentity}, the consumer route of \autoref{thm:layered-relation-gate-consumer-route}, and the carrier, classifier, and ledger obligations of \autoref{thm:layered-relation-gate-carrier-obligation}, \autoref{thm:layered-relation-gate-classifier-obligation}, and \autoref{thm:layered-relation-gate-ledger-obligation} form the complete finite obligation surface for $\LayeredRelationGateUp$. The assembled surface exports only the same-schema relation verdict over the displayed rows $A,B,L,P,N,F,G,H,C,\Pi$ and refuses object identity, empirical law, proof transfer, subject-substance transfer, global-frame theorem, bridge strength, hidden choice, Lean verification, and bridge checking.
\end{theorem}
\begin{proof}
The carrier theorem fixes the finite packet as $A,B,L,P,N,F,G,H,C,\Pi$. There is no additional row from which identity, empirical law, proof transfer, subject-substance, global-frame structure, bridge strength, hidden choice, verification status, or bridge status could be read.

The NameCert obligation theorem exposes the ordered rows for source domains, layer list, preserved content, not-preserved content, refusal ledger, verdict, transport, continuation, package, and local certificate. Same-schema non-identity then identifies the verdict as relation language rather than object equality.

The consumer route sends every downstream read through the relation-certificate source, concrete certificate surface, force inter-history relation surface, and axiom-refusal ledger. The carrier, classifier, and ledger obligations close the remaining local readings by requiring carrier exposure, preserved/not-preserved comparison, and visible refusal-ledger replay.

Thus every allowed read is already listed by the finite surface, and every refused reading remains in $N$ or $F$ rather than in the exported verdict. The assembled obligation surface is complete for the chapter's current NameCert claim and carries no Lean verification or bridge-checking assertion.
\end{proof}
\leanchecked{BEDC.Derived.LayeredRelationGateUp.LayeredRelationGate\_obligation\_closure}

\begin{theorem}[LayeredRelationGate obligation scope]
\label{thm:layered-relation-gate-obligation-scope}
The carrier, classifier, ledger, same-schema nonidentity, and consumer-route obligations of $\LayeredRelationGateUp$ are scoped by the single-carrier TasteGate witness for the finite packet $A,B,L,P,N,F,G,H,C,\Pi$. Consequently the accepted verdict cannot be read as host identity between the compared domains; it is only a same-schema relation export over the displayed rows.
\end{theorem}
\leanchecked{BEDC.Derived.LayeredRelationGateUp.LayeredRelationGateTasteGate\_single\_carrier\_alignment}
\begin{proof}
Read the carrier of \autoref{def:layered-relation-gate-carrier}. Its rows form one displayed packet, so every obligation in \autoref{thm:layered-relation-gate-carrier-obligation}, \autoref{thm:layered-relation-gate-classifier-obligation}, and \autoref{thm:layered-relation-gate-ledger-obligation} is evaluated on the same finite surface.

The TasteGate witness supplies the single-carrier alignment for that surface. It keeps source rows, preserved rows, not-preserved rows, ledger rows, verdict rows, transport, replay, and provenance in one packet rather than allowing an external identification between the compared domains.

Apply same-schema non-identity and the consumer route, \autoref{thm:layered-relation-gate-same-schema-nonidentity} and \autoref{thm:layered-relation-gate-consumer-route}. The consumer may export the displayed relation after reading the packet, but no row converts the verdict into host identity, empirical law, proof transfer, subject-substance transfer, global-frame content, bridge strength, or hidden choice.
\end{proof}
...
\closureat{LayeredRelationGateUp}{obligationStr}
...
\begin{closurestatus}{\LayeredRelationGateUp}
  ...
  \scopeclosed{\autoref{def:layered-relation-gate-carrier}, \autoref{thm:layered-relation-gate-namecert-obligations}, \autoref{thm:layered-relation-gate-same-schema-nonidentity}, \autoref{thm:layered-relation-gate-consumer-route}, \autoref{thm:layered-relation-gate-obligation-closure}, and \autoref{thm:layered-relation-gate-obligation-scope} bind the displayed $\BHist$ carrier, componentwise $\hsame$ transport, $\Cont$ replay, $\Pkg$ provenance, local $\NameCert$ row, preserved and not-preserved disclosure, refusal-ledger replay, and same-schema consumer-handoff scope for the finite LayeredRelationGate packet.}
