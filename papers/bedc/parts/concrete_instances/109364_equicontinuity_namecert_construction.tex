...
\begin{closurestatus}{\EquicontinuityUp}
  \constructivestory{An $\EquicontinuityUp$ packet is generated from compact metric source rows, continuous-family ledger rows, tolerance and shared-radius rows, modulus handoff rows, componentwise hsame transport, displayed Cont replay, Pkg provenance, and a local NameCert row.}
  \theoryclosure{\seedClosure}
  \scopeclosed{\autoref{def:equicontinuity-carrier}, \autoref{thm:equicontinuity-namecert-obligations}, \autoref{thm:equicontinuity-compact-metric-route}, and \autoref{thm:equicontinuity-nonescape} seed the finite compact-source family-modulus route for equicontinuity consumers.}
  \formalstatus{\unformalizedV}
  \bridgestatus{none}
  \notclaimed{This chapter does not close compactness, selected finite nets, selected subsequences, Arzela--Ascoli convergence, host-level continuity, pointwise-only radius choice, Lean verification, or bridge checking for $\EquicontinuityUp$.}
  \upgradepath{Obligation closure requires checked carrier admission, compact-source compatibility, continuous-family ledger transport, shared-radius exactness, modulus handoff, replay, provenance, and local naming for BEDC.Derived.EquicontinuityUp.}
\end{closurestatus}
