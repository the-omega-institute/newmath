...
\begin{closurestatus}{\EquicontinuityUp}
  \constructivestory{An $\EquicontinuityUp$ packet is generated from compact metric source rows, finite continuous-family rows, tolerance and shared-radius rows, modulus handoff, componentwise hsame transport, displayed Cont replay, Pkg provenance, and a local NameCert row.}
  \theoryclosure{\seedClosure}
  \scopeclosed{\autoref{def:equicontinuity-carrier}, \autoref{thm:equicontinuity-namecert-obligations}, \autoref{thm:equicontinuity-compact-metric-route}, and \autoref{thm:equicontinuity-nonescape} seed the finite family-modulus route through compact source, continuous family, tolerance, shared radius, and modulus handoff rows.}
  \formalstatus{\unformalizedV}
  \bridgestatus{none}
  \notclaimed{This chapter does not close compactness, selected finite nets, selected subsequences, Arzela--Ascoli convergence, host-level continuity, pointwise-only radius choice, ambient $\RealUp$ completeness, Lean verification, or bridge checking.}
  \upgradepath{Obligation closure requires checked carrier admission, shared-modulus exactness, compact-source compatibility, nonescape, transport, replay, provenance, and local naming for BEDC.Derived.EquicontinuityUp.}
\end{closurestatus}
