...
\begin{theorem}[RegularCauchySum L10 four-face exactness]
\label{thm:regular-cauchy-sum-l10-four-face-exactness}
Let $S=(A,B,W_A,W_B,D_A,D_B,D,E,R,H,C,P,N)$ be an accepted $\RegularCauchySumUp$ packet. Every Real-and-Completion consumer route for $S$ factors through exactly four displayed L10 faces: first the $\DyadicRatCoreUp$ endpoint-addition ledger $D$ with source rows $D_A,D_B$, then the aligned finite $\StreamNameUp$ windows $W_A,W_B$, then the $\RegSeqRatUp$ readback row $R$, and finally the $\RealUp$ seal route of \autoref{thm:regular-cauchy-sum-real-seal-route}.

The visible evidence carried along this route is precisely the finite $\FiniteErrorBudgetUp$ row $E$, the componentwise $\hsame$ transports $H$, the displayed $\Cont$ routes $C$, the $\Pkg$ provenance row $P$, and the local $\NameCert$ rows $N$. No Real-and-Completion consumer can read a quotient stream, selected host limit, ambient completeness coordinate, host real equality, or independent completed-real addition law outside these four faces.
\end{theorem}
\begin{proof}
First expose the finite endpoint and window surface. The window-budget theorem \autoref{thm:regular-cauchy-sum-window-budget} supplies the source endpoint rows $D_A,D_B$, the endpoint-addition ledger $D$, the aligned windows $W_A,W_B$, and the finite budget row $E$ at the shared request.

Next pass from windows to regular readback. The $\RegSeqRatUp$ handoff of \autoref{thm:regular-cauchy-sum-regseqrat-handoff} repacks the same endpoint-addition ledger, aligned windows, budget row, transports, continuations, provenance, and naming rows as the readback row $R$.

Now route the completed-real read. The Real seal route \autoref{thm:regular-cauchy-sum-real-seal-route} places every $\RealUp$-facing consumer downstream of the two source regular rows, the aligned windows, the dyadic ledger, the finite budget row, the shared-tail-meet handoff, the downstream seal handoff, and the visible local certificate rows.

Finally apply the non-escape boundary of \autoref{thm:regular-cauchy-sum-finite-budget-nonescape}. Since that boundary excludes quotient streams, host equality, selected limits, ambient completeness, and independent tail or addition oracles, the four displayed faces exhaust the Real-and-Completion phase route exactly.
\end{proof}
