...
\item $P$ is the $\Pkg$ provenance over $\CantorCompletenessUp$, $\NestedIntervalCompactnessUp$, $\NestedIntervalUp$, $\LocatedIntervalUp$, $\CauchyTailDiameterUp$, $\StreamNameUp$, $\RegSeqRatUp$, $\DyadicRatCoreUp$, $\RealUp$, $\BHist$, $\hsame$, $\Cont$, and $\NameCert$.
