...
\begin{theorem}[ComputableReal public effective NameCert export]
\label{thm:computable-real-public-effective-namecert-export}
The public $\NameCert_{\ComputableRealUp}$ export of an accepted
$\ComputableRealUp$ packet is the finite effective-real surface consisting of
the source packet, classifier transport, modulus-window stability,
$\RegSeqRatUp$ seal factorization, $\RealUp$ handoff, ledger coverage, and
bridge-boundary exclusions. Every downstream consumer may read only the
displayed $\mathsf{BHist}$, $\hsame$, $\Cont$, $\mathsf{Pkg}$,
$\StreamNameUp$, $\CauchyModulusUp$, $\DyadicRatCoreUp$, $\RegSeqRatUp$, and
$\RealUp$ rows.

The export exposes no quotient real equality, no selected limit, no classical
completeness principle, and no hidden standard bridge datum outside the finite
ledger.
\end{theorem}
\begin{proof}
...
Use \autoref{thm:computable-real-classifier-transport} to transport the
classifier across the visible coordinates and
\autoref{thm:computable-real-modulus-window-stability} to keep finite windows
stable under restriction, enlargement, and refinement. Then apply
\autoref{thm:computable-real-regseqrat-seal-factorization} and
\autoref{thm:computable-real-realup-handoff} to identify the only
regular-sequence and real-facing exits.

Finally, \autoref{thm:computable-real-ledger-coverage} exhausts the public
consumer rows, and \autoref{thm:computable-real-bridge-boundary} records the
objects not projected by the effective-real boundary. Since those excluded
objects are absent from the finite ledger, a downstream chapter receives exactly
the listed $\NameCert_{\ComputableRealUp}$ surface.
\end{proof}
