...
\begin{closurestatus}{\InterHistRateBoundaryUp}
  \constructivestory{An $\InterHistRateBoundaryUp$ packet is generated from paired observer-history boundary rows, a cross-history causal route, a maximum-rate gate, rate row, observer-symmetry row, no-global-frame refusal, inter-Hist invariant readback, componentwise $\hsame$ transport, probe stability, displayed $\Cont$ routes, provenance, and local naming. The carrier makes the rate visible only through the gate and refusal rows. Its classifier records a bounded inter-Hist relation: the consumer reads route, gate, symmetry, refusal, and invariant data, while $\hsame$ and $\Cont$ preserve exactly those fields without importing a host metric or shared clock.}
  \theoryclosure{\seedClosure}
  \scopeclosed{This chapter binds the inter-Hist rate-boundary carrier, obligations, gated consumer theorem, non-escape theorem, and paper-Lean package comparison.}
  \formalstatus{\unformalizedV}
  \bridgestatus{none}
  \origin{ai}
  \notclaimed{It does not claim host-side speed, global time, metric tensor, Lorentz frame, selected synchronization map, or detached rate primitive.}
  \upgradepath{To reach obligationClosure, promote the displayed TasteGate-aligned carrier into explicit checked obligations for all route, gate, refusal, and invariant rows.}
\end{closurestatus}
