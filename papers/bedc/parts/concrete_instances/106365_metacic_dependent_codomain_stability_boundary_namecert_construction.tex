...
\item $L$ is the stability ledger: it records the exact row that a later bundle witness must fill to make the dependent codomain readback stable under the application-argument step.
...
\item $P$ is the $\Pkg$ provenance over $\MetaCICDependentCodomainStabilityBoundaryUp$, $\DependentCodomainInversionBoundaryUp$, $\DependentCodomainClosurePreservationUp$, $\MetaCICSubjectReductionBoundaryUp$, $\ClosedNormalConsistencyBoundaryUp$, $\SubjectReductionDischargeBundle$, $\BHist$, $\hsame$, $\Cont$, and $\NameCert$.
\item $N$ is the local $\NameCert_{\MetaCICDependentCodomainStabilityBoundaryUp}$ row.
\end{enumerate}
The classifier compares accepted carriers only through $A,D,R,I,V,Q,L,H,C,P,N$. It has no coordinate for unconditional subject reduction, conversion completeness, closed consistency, ambient normalization, or a hidden discharge witness.
\end{definition}
...
\end{theorem}
\leanchecked{BEDC.Derived.MetaCICDependentCodomainStabilityBoundaryUp.TasteGate.MetaCICDependentCodomainStabilityBoundaryTasteGate\_single\_carrier\_alignment}
\begin{proof}
...
Finally $H,C,P,N$ transport, replay, source, and name the same finite packet. The required Lean `Nontrivial` and `FieldFaithful` instances belong to the later bundle witness because no hidden instance coordinate occurs in this carrier.
\end{proof}
...
$$
The handoff exposes the stability ledger that a later bundle witness must fill. It does not turn the dependent-codomain application case into unconditional subject reduction and does not promote closed-normal consistency to full closed consistency.
\end{theorem}
\begin{proof}
...
The ledger $L$ is then the only exported positive handoff. It names the dependent-codomain stability row that a later bundle witness must supply, including the instance obligations recorded in the carrier theorem.
...
\begin{theorem}[AppArgTypeStable boundary row]
\label{thm:metacic-dependent-codomain-stability-boundary-app-arg-type-stable-row}
For an accepted $\MetaCICDependentCodomainStabilityBoundaryUp$ carrier $S=(A,D,R,I,V,Q,L,H,C,P,N)$, the ledger row $L$ is the AppArgTypeStable-facing boundary: any application-argument reduction read must display the pre-step and post-step codomain observations in $D$ before it can feed the subject-reduction discharge request $R$.
\end{theorem}
\begin{proof}
Project the rows $A,D,R,L$ from the accepted carrier. The row $A$ identifies the read as an application-argument reduction, while $D$ displays the dependent codomain before and after that step.

Apply \autoref{thm:metacic-dependent-codomain-stability-boundary-handoff}. The only positive row exported toward the subject-reduction request is $L$, and the handoff reaches it only after $A,D,R$ have been displayed.

Thus $L$ names the AppArgTypeStable-facing boundary for the later discharge witness. It records the required codomain-stability row without claiming unconditional subject reduction.
\end{proof}
...
\begin{closurestatus}{\MetaCICDependentCodomainStabilityBoundaryUp}
  ...
  \upgradepath{Obligation closure requires checked application-argument admission, dependent-codomain readback visibility, inversion-boundary routing, closure-preservation transport, obstruction recording, stability-ledger exactness, and the required witness instances for BEDC.Derived.MetaCICDependentCodomainStabilityBoundaryUp.}
\end{closurestatus}
