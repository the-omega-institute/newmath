...
\begin{closurestatus}{\RegularCauchyDifferenceUp}
  ...
  \scopeclosed{The obligation scope is bound by \autoref{thm:regular-cauchy-difference-obligation-package}, \autoref{thm:regular-cauchy-difference-schedule-exactness}, and \autoref{thm:regular-cauchy-difference-consumer-exhaustion}: carrier admission, classifier transport, regularity closure, tolerance-ledger exactness, null-sequence consumption, scheduled pointwise-difference reads, and $\RealUp$ seal routing for the same finite packet.}
  ...
  \upgradepath{Scoped closure requires a public completed-real subtraction surface or a checked bridge connecting the displayed source rows, subtraction schedule, tolerance ledger, null-sequence consumer, RealUp seal route, and $\mathsf{BHist}$, $\hsame$, $\Cont$, $\Pkg$, and $\NameCert$ dependencies.}
\end{closurestatus}
