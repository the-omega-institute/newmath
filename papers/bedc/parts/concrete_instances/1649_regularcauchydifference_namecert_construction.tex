...
The two source rows are read through \autoref{ch:concrete-instances-regular-cauchy-name-namecert}, and the zero-shrinkage consumer row is read through \autoref{ch:concrete-instances-nullsequence-namecert}. The $\RegularCauchyDifferenceUp$ packet consumes those sibling surfaces to form a pointwise subtraction packet; it introduces no quotient stream equality, host real equality, selected limits, or ambient completeness.
...
\end{proof}
\leanchecked{BEDC.Derived.RegularCauchyDifferenceUp.RegularCauchyDifferenceCarrier\_regularity\_closure}
...
\end{proof}
\leanchecked{BEDC.Derived.RegularCauchyDifferenceUp.RegularCauchyDifferenceCarrier\_tolerance\_ledger\_exactness}
...
\end{proof}
\leanchecked{BEDC.Derived.RegularCauchyDifferenceUp.RegularCauchyDifferenceCarrier\_nullsequence\_consumption}
...
\begin{theorem}[RegularCauchyDifference obligation package]
\label{thm:regular-cauchy-difference-obligation-package}
For an accepted $\RegularCauchyDifferenceUp$ carrier
$$
D=(X,Y,S,T,Z,H,C,P,N),
$$
the local obligation package is exactly the finite package that combines carrier admission for $D$, classifier transport for the source rows, aligned windows, dyadic subtraction rows, tolerance ledger, and optional null-sequence row, regularity closure of the displayed pointwise difference, tolerance-ledger exactness for $T$, $\NullSequenceUp$ consumption through $Z$, and the $\RealUp$ seal route through the same displayed rows.
\end{theorem}
\begin{proof}
Carrier admission is \autoref{def:regular-cauchy-difference-carrier} together with \autoref{thm:regular-cauchy-difference-namecert-obligations}. These identify the accepted object as the finite tuple $D$ and list the source rows, schedule, ledger, consumer row, transport rows, continuation rows, package provenance, and local naming row.

Classifier transport and regularity closure are the next two obligations. Apply \autoref{thm:regular-cauchy-difference-classifier-transport} to move the source rows, aligned windows, subtraction rows, ledger, and optional consumer row componentwise. Apply \autoref{thm:regular-cauchy-difference-regularity-closure} and \autoref{thm:regular-cauchy-difference-difference-carrier-closure} to repack the transported observations as the displayed regular rational difference row.

The consumer obligations are exhausted by the ledger and seal theorems. Use \autoref{thm:regular-cauchy-difference-tolerance-ledger-exactness} for the tolerance row, \autoref{thm:regular-cauchy-difference-nullsequence-consumption} for zero-shrinkage reads, and \autoref{thm:regular-cauchy-difference-real-seal-route} for Real-facing reads. The non-escape boundary of \autoref{thm:regular-cauchy-difference-non-escape-boundary} shows that no quotient stream, selected limit, host real equality, or ambient completeness row is available outside this package.
\end{proof}

\begin{theorem}[RegularCauchyDifference schedule exactness]
\label{thm:regular-cauchy-difference-schedule-exactness}
Every pointwise difference read exported by an accepted $\RegularCauchyDifferenceUp$ carrier $D=(X,Y,S,T,Z,H,C,P,N)$ is generated by the aligned $\StreamNameUp$ windows named by $S$, the $\DyadicRatCoreUp$ subtraction rows named by $S$, and the transported tolerance ledger $T$. In particular, a difference read has no alternative source outside the displayed source rows $X,Y$, schedule $S$, and ledger $T$.
\end{theorem}
\begin{proof}
Begin with the displayed difference read. By \autoref{thm:regular-cauchy-difference-difference-carrier-closure}, its windows are the aligned $\StreamNameUp$ windows named by $S$, its rational observations are the $\DyadicRatCoreUp$ subtraction rows named by $S$, and its tolerances are the entries of $T$.

The exactness of the tolerance coordinate is \autoref{thm:regular-cauchy-difference-tolerance-ledger-exactness}. It forces every output tolerance request through the transported ledger rather than an external modulus or host equality witness.

Finally apply \autoref{thm:regular-cauchy-difference-non-escape-boundary}. Since the carrier exposes no quotient stream, selected limit, host real equality, or ambient completeness object, every exported pointwise difference read is precisely the scheduled finite read through $X,Y,S,T$.
\end{proof}

\begin{theorem}[RegularCauchyDifference consumer exhaustion]
\label{thm:regular-cauchy-difference-consumer-exhaustion}
Every $\RegSeqRatUp$, $\NullSequenceUp$, and $\RealUp$ consumer of an accepted $\RegularCauchyDifferenceUp$ carrier factors through the displayed packet
$$
D=(X,Y,S,T,Z,H,C,P,N).
$$
The regular consumer reads the displayed difference row through $X,Y,S,T$; the null-sequence consumer reads the zero-shrinkage surface through $Z$ and the same ledger; and the Real-facing consumer reads only the seal route supplied by those carrier rows.
\end{theorem}
\begin{proof}
For the $\RegSeqRatUp$ consumer, apply \autoref{thm:regular-cauchy-difference-regularity-closure} and \autoref{thm:regular-cauchy-difference-difference-carrier-closure}. The consumer receives the displayed pointwise difference row through the two source rows, aligned windows, dyadic subtraction rows, and tolerance ledger.

For the $\NullSequenceUp$ consumer, apply \autoref{thm:regular-cauchy-difference-nullsequence-consumption}. The zero-shrinkage row $Z$ consumes the same displayed difference and the same tolerance ledger before exposing any downstream evidence.

For the $\RealUp$ consumer, apply \autoref{thm:regular-cauchy-difference-real-seal-handoff} and \autoref{thm:regular-cauchy-difference-real-seal-route}. Both routes factor through the displayed regular row, optional null-sequence row, transport rows, continuation rows, package provenance, and local $\NameCert$ row. The non-escape boundary then rules out any consumer path outside $D$.
\end{proof}

\closureat{RegularCauchyDifferenceUp}{seedStr}
\closureat{RegularCauchyDifferenceUp}{obligationStr}
\closureat{RegularCauchyDifferenceUp}{obligationStr}[BEDC.Derived.RegularCauchyDifferenceUp.RegularCauchyDifferenceCarrier\_tolerance\_ledger\_exactness]
\closureat{RegularCauchyDifferenceUp}{scopedStr}[BEDC.Derived.RegularCauchyDifferenceUp.RegularCauchyDifferenceCarrier\_tolerance\_ledger\_exactness]
\closureat{RegularCauchyDifferenceUp}{publicStr}[BEDC.Derived.RegularCauchyDifferenceUp.RegularCauchyDifferenceCarrier\_tolerance\_ledger\_exactness]
\closureat{RegularCauchyDifferenceUp}{matureStr}[BEDC.Derived.RegularCauchyDifferenceUp.RegularCauchyDifferenceCarrier\_tolerance\_ledger\_exactness]
...
\begin{closurestatus}{\RegularCauchyDifferenceUp}
  ...
  \scopeclosed{The scoped dependency surface is bound by \autoref{thm:regular-cauchy-difference-obligation-package}, \autoref{thm:regular-cauchy-difference-schedule-exactness}, and \autoref{thm:regular-cauchy-difference-consumer-exhaustion}: all $\mathsf{BHist}$ carrier rows, $\hsame$ transports, $\Cont$ routes, $\Pkg$ provenance, local $\NameCert$ data, $\NullSequenceUp$ consumption, $\RegSeqRatUp$ difference reads, and $\RealUp$ seal routing factor through the same finite packet.}
  \formalstatus{\axiomCleanV}
  \leantarget{BEDC.Derived.RegularCauchyDifferenceUp.RegularCauchyDifferenceCarrier\_tolerance\_ledger\_exactness}
  ...
  \upgradepath{No theory-axis upgrade remains; bridge-checked verification requires a checked standard bridge for the same subtraction schedule and tolerance ledger.}
\end{closurestatus}
