...
The packet is consumed by \autoref{ch:concrete-instances-finitenetminimumfold-namecert} for the positive rational lower-bound fold and by \autoref{ch:concrete-instances-uniformmodulus-namecert} for the final tolerance-only modulus handoff. Its finite-net source is the compact metric surface of \autoref{ch:concrete-instances-compactmetric-namecert}, so the cover row is a horizontal dependency in the concrete dossier rather than a kernel-only packet.
...
\end{proof}
\leanchecked{BEDC.Derived.CompactModulusCoverUp.CompactModulusCoverCarrier\_namecert\_obligations}
...
\end{proof}
\leanchecked{BEDC.Derived.CompactModulusCoverUp.CompactModulusCoverCarrier\_real\_completion\_handoff}
...
\end{proof}
\leanchecked{BEDC.Derived.CompactModulusCoverUp.CompactModulusCoverCarrier\_obligation\_triad}
...
\closureat{CompactModulusCoverUp}{obligationStr}
\closureat{CompactModulusCoverUp}{publicStr}[BEDC.Derived.CompactModulusCoverUp.CompactModulusCoverCarrier\_obligation\_triad]
\closureat{CompactModulusCoverUp}{bridgedStr}[BEDC.Derived.CompactModulusCoverUp.CompactModulusCoverCarrier\_obligation\_triad]
\closureat{CompactModulusCoverUp}{matureStr}[BEDC.Derived.CompactModulusCoverUp.CompactModulusCoverCarrier\_obligation\_triad]
...
\begin{closurestatus}{\CompactModulusCoverUp}
  ...
  \scopeclosed{\autoref{def:compact-modulus-cover-carrier}, \autoref{thm:compact-modulus-cover-namecert-obligations}, \autoref{thm:compact-modulus-cover-finite-net-source-exhaustion}, \autoref{thm:compact-modulus-cover-real-completion-handoff}, and \autoref{thm:compact-modulus-cover-obligation-triad} bind the carrier, NameCert obligations, finite-net exhaustion, real-completion handoff, and obligation-triad rows to the displayed compact-net, pointwise-modulus, lower-bound-fold, and uniform-modulus scope.}
  \formalstatus{\axiomCleanV}
  \leanchecked{BEDC.Derived.CompactModulusCoverUp.taste\_gate}
  \leantarget{BEDC.Derived.CompactModulusCoverUp.CompactModulusCoverCarrier\_namecert\_obligations}
  \bridgestatus{none}
  \origin{ai}
  ...
  \upgradepath{The paper axis is closed at the finite compact-net, pointwise-modulus, rational-radius, lower-bound-fold, and uniform-modulus consumer surface; only bridge checking remains on the formal axis.}
\end{closurestatus}
