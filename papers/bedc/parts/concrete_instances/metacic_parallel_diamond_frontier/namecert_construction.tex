...
\begin{theorem}[MetaCIC parallel-diamond frontier seed-record exhaustion]
\label{thm:metacic-parallel-diamond-frontier-seed-record-exhaustion}
Seed-facing confluence reads of an accepted $\MetaCICParallelDiamondFrontierUp$ packet are exhausted by
$$
P,K,J,R,S,C,B,O,H,T,G,N .
$$
They cannot promote the residual-substitution premise, subject reduction, Church--Rosser, evaluator equality, quotient equality, or kernel consistency.
\end{theorem}
\begin{proof}
Project the carrier of \autoref{def:metacic-parallel-diamond-frontier-carrier}. Its seed-facing rows are exactly $P,K,J,R,S,C,B,O,H,T,G,N$.

Use the obligation surface and handoff theorem, \autoref{thm:metacic-parallel-diamond-frontier-obligations} and \autoref{thm:metacic-parallel-diamond-frontier-handoff}. They keep the residual-substitution premise visible and expose only the conditional frontier route.

Finally cite premise retention, residual-substitution scope, and discharge obstruction. These retain $P$, scope the candidate frontier, and keep the refused confluence, typing, evaluator, quotient, and kernel endpoints outside the seed record.
\end{proof}
...
\end{theorem}
\leanchecked{BEDC.Derived.MetaCICParallelDiamondFrontierUp.MetacicParallelDiamondFrontierRoute}
\begin{proof}
...
\end{proof}
\leanchecked{BEDC.Derived.MetaCICParallelDiamondFrontierUp.MetacicParallelDiamondFrontierResidualSubstitutionScope}
...
\begin{theorem}[MetaCIC parallel-diamond public export]
\label{thm:metacic-parallel-diamond-frontier-public-export}
For an accepted $\MetaCICParallelDiamondFrontierUp$ carrier $F=(P,K,J,R,S,C,B,O,H,T,G,N)$, the public confluence-facing export is exactly the route through residual-substitution premise $P$, critical-pair row $K$, candidate-join row $J$, residual-frontier row $R$, closed-checker row $S$, closed-normal fragment row $C$, bounded-check row $B$, obstruction row $O$, and structural rows $H,T,G,N$. The export may feed MetaCIC critical-path and confluence-audit consumers, but it does not prove full Church--Rosser, subject reduction, evaluator equality, quotient equality, or kernel consistency.
\end{theorem}
\begin{proof}
Begin with the obligation surface of \autoref{thm:metacic-parallel-diamond-frontier-obligations}. It admits the carrier only through the displayed rows $P,K,J,R,S,C,B,O,H,T,G,N$.

Use critical-pair exhaustion, obstruction retention, and local-join scope: \autoref{thm:metacic-parallel-diamond-critical-pair-exhaustion}, \autoref{thm:metacic-parallel-diamond-frontier-obstruction-triad}, and \autoref{thm:metacic-parallel-diamond-frontier-local-join-scope}. These keep the critical pair, candidate join, residual frontier, bounded check, and obstruction rows visible before any confluence-facing read.

Next apply the normalization handoff and candidate-mediated socket, \autoref{thm:metacic-parallel-diamond-normalization-frontier-handoff} and \autoref{thm:metacic-parallel-diamond-frontier-candidate-mediated-sn-socket}. They route normalization-facing consumers through the same finite packet without exporting global normalization or confluence.

Finally invoke residual-join induction, \autoref{thm:metacic-parallel-diamond-frontier-residual-join-induction}. The structural rows $H,T,G,N$ transport, replay, source, and name the same packet, so the public export is the listed route and none of the refused endpoints follows.
\end{proof}
...
\begin{theorem}[MetaCIC parallel-diamond scoped closure status]
\label{thm:metacic-parallel-diamond-frontier-scoped-closure-status}
The seed-readiness surface, retained-premise handoff, obstruction row, residual-substitution premise, critical-path consumer route, and normalization-confluence handoff of an accepted $\MetaCICParallelDiamondFrontierUp$ packet together scope the chapter to finite candidate-mediated local-diamond evidence. The scoped surface is exactly the displayed route through $P,K,J,R,S,C,B,O,H,T,G,N$ and does not assert subject reduction, unrestricted confluence, evaluator equality, quotient term equality, global normalization, residual-substitution discharge, or kernel consistency.
\end{theorem}
\begin{proof}
Read the seed surface through \autoref{thm:metacic-parallel-diamond-frontier-seed-readiness} and \autoref{thm:metacic-parallel-diamond-frontier-seed-closure-status}. These results expose the carrier rows and keep the finite local-diamond evidence inside the displayed packet.

Apply the retained-premise handoff and residual-substitution scope, \autoref{thm:metacic-parallel-diamond-frontier-critical-path-retained-premise-handoff} and \autoref{thm:metacic-parallel-diamond-frontier-residual-substitution-scope}. They keep the premise row $P$ visible for every critical-path read instead of discharging it as a substitution theorem.

Finally use the obstruction, public export, and normalization-frontier handoff rows of \autoref{thm:metacic-parallel-diamond-frontier-obstruction-triad}, \autoref{thm:metacic-parallel-diamond-frontier-public-export}, and \autoref{thm:metacic-parallel-diamond-normalization-frontier-handoff}. The resulting scope is finite, candidate-mediated, and local to $P,K,J,R,S,C,B,O,H,T,G,N$, so none of the refused global endpoints is exported.
\end{proof}
