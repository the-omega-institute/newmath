...
\begin{theorem}[MetaCIC parallel diamond frontier candidate-mediated handoff]
\label{thm:metacic-parallel-diamond-frontier-candidate-mediated-handoff}
Every accepted $\MetaCICParallelDiamondFrontierUp$ packet that routes a local candidate join through the critical-pair row $K$, closed-checker seal $S$, closed-normal fragment $C$, bounded-check row $B$, and residual-substitution premise row $P$ may export only a conditional closed-star confluence handoff until a separate residual-substitution compatibility certificate is cited. In carrier terms, the export remains the retained route through $P,K,J,R,S,C,B,O,H,T,G,N$, with $P$ preserved as a premise rather than discharged as a theorem.
\end{theorem}
\begin{proof}
Project the carrier of \autoref{def:metacic-parallel-diamond-frontier-carrier}. The candidate-mediated data are the critical-pair row $K$, candidate-join row $J$, residual-frontier row $R$, closed-checker row $S$, closed-normal row $C$, bounded-check row $B$, and residual-substitution premise row $P$.

Apply the local-diamond frontier theorem, \autoref{thm:metacic-local-diamond-frontier}. It allows the candidate join to be read only through the displayed residual-frontier and closed-fragment rows, with the obstruction row still visible.

Apply residual-substitution scope and premise retention, \autoref{thm:metacic-parallel-diamond-frontier-residual-substitution-scope} and \autoref{thm:metacic-parallel-diamond-frontier-premise-retention}. These theorems keep $P$ in the consumer-visible route through the critical-pair, residual-frontier, closed-checker, and closed-normal rows.

Apply closed-checker retention and the confluence handoff theorem, \autoref{thm:metacic-parallel-diamond-closed-checker-retention} and \autoref{thm:metacic-parallel-diamond-confluence-handoff}. The handoff reaches the candidate-normalization consumer with $P,S,C,B,O$ still displayed, so it is conditional closed-star confluence data rather than an unconditional Church--Rosser result.

The TasteGate sentence below this theorem records the expected nontriviality and field-faithfulness coverage for the same rows $P,K,J,R,S,C,B,O,H,T,G,N$. This paper theorem keeps that coverage boundary visible and makes no Lean verification claim.
\end{proof}
