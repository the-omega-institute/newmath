...
\begin{theorem}[Real L10 public pullback defect localization]
\label{thm:real-l10-public-pullback-defect-localization}
Let $\mathcal{P}$ be a finite consumer packet presented on the Real L10 route
at the public Phase Real and Completion boundary. If $\mathcal{P}$ does not
satisfy the four-face pullback criterion, then the defect is witnessed by
disagreement or absence in at least one of the displayed object faces
$\DyadicRatCoreUp$, $\StreamNameUp$, $\RegSeqRatUp$, and $\RealUp$, or in at
least one displayed support row among $\BHist$, $\hsame$, $\Cont$, $\Pkg$,
and $\NameCert$. The defect cannot be repaired by quotient stream equality,
selected host limits, countable-choice selectors, ambient completeness,
metric-completeness rows, aggregate formalstatus rows, or bridge tokens.
\end{theorem}
\begin{proof}
Assume no displayed object face and no displayed support row witnesses a
defect. Then the $\DyadicRatCoreUp$, $\StreamNameUp$, $\RegSeqRatUp$, and
$\RealUp$ faces are present and agree with the public boundary data, and the
same holds for the $\BHist$, $\hsame$, $\Cont$, $\Pkg$, and $\NameCert$
support rows.

Apply \autoref{thm:real-l10-four-face-public-pullback-criterion}. The absence
of a facewise or support-row defect is exactly the componentwise agreement
required by its reverse direction, so $\mathcal{P}$ is the public four-face
pullback. This contradicts the assumed failure of the criterion.

It remains to locate the support on the Real L10 route rather than in an
external substitute. Apply
\autoref{thm:real-l10-route-cover-phase-exit-exactness} to identify the
public boundary with the four route faces and the same-route exact-boundary
witness.

Next use
\autoref{thm:streamname-regseqrat-realup-finite-window-pullback-criterion},
\autoref{thm:regseqrat-seal-route-visible-source-normal-form}, and
\autoref{thm:realup-open-phase-four-face-pullback}. These results transport
the StreamName finite windows through RegSeqRat readback and local Real seal
data, normalize the visible source rows, and repack the Real-facing packet as
the four-face pullback. The listed exclusions are preserved throughout that
composition, so quotient stream equality, selected host limits, countable
choice, ambient or metric completeness, aggregate formalstatus rows, and
bridge tokens cannot remove the localized defect.
\end{proof}
