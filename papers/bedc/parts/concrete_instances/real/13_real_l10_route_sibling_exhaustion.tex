...
\begin{theorem}[Real L10 route-cover threshold intersection exactness]
\label{thm:real-l10-route-cover-threshold-intersection-exactness}
Let $\mathcal{U}$ and $\mathcal{V}$ be finite covers of accepted current Phase
Real and Completion terminal presentations on the Real L10 route. Suppose that
every member and every overlap in both covers exposes the same ordered four
faces
$$
\DyadicRatCoreUp,\quad \StreamNameUp,\quad \RegSeqRatUp,\quad \RealUp
$$
with their local closurestatus and formalstatus records, and suppose that the
Real-facing support is carried by the shared rows
$$
\BHist,\quad \hsame,\quad \Cont,\quad \Pkg,\quad \NameCert.
$$
Then the common refinement $\mathcal{U}\cap\mathcal{V}$ is accepted exactly
when each refined cell inherits, from both covers, the dyadic threshold, the
finite $\StreamNameUp$ window, the $\RegSeqRatUp$ readback, the $\RealUp$ seal
row, and the displayed support rows.

Equivalently, finite cover threshold evidence on the Real L10 route is stable
under common refinement: no refined cell may replace any inherited face or
support row by a quotient stream, selected limit, countable-choice selector,
ambient completeness claim, metric-completeness row, aggregate formalstatus
row, or bridge token. Those objects are not admissible substitutes for the
dyadic threshold, finite StreamName window, RegSeqRat readback, Real seal row,
or shared $\BHist$, $\hsame$, $\Cont$, $\Pkg$, and $\NameCert$ support.
\end{theorem}
\begin{proof}
First read both covers through
\autoref{thm:real-l10-route-cover-phase-exit-converse}. The accepted public
Phase Real and Completion exit of each cover pulls back to the same ordered
four faces and to the same displayed Real L10 support rows. Hence every
intersection cell has only those inherited rows available for its terminal
presentation.

Next restrict the local threshold packets to a refined cell. The hypotheses
give the dyadic threshold, finite StreamName window, RegSeqRat readback, Real
seal row, and the $\BHist$, $\hsame$, $\Cont$, $\Pkg$, and $\NameCert$ support
from each side of the intersection. Apply
\autoref{thm:real-l10-finite-cover-exit-threshold-gluing} to glue the two
local packets along their common visible faces, and apply
\autoref{thm:real-l10-finite-cover-exit-threshold-descent} to descend the
glued packet to the refined cell.

For the forward direction, apply
\autoref{thm:real-l10-route-cover-phase-exit-exactness} to the common
refinement. Exactness identifies any accepted refined cell with the four
roadmap faces, the same-route boundary witness, and the displayed support
rows. The descent step then says that the cell inherits precisely the local
threshold data listed above from both covers.

For the reverse direction, assume every refined cell carries those inherited
threshold and support rows. The gluing step assembles the cells into a finite
Real-facing replay-cover, and the descent step verifies that no off-route row
is needed. Applying
\autoref{thm:real-l10-route-cover-phase-exit-exactness} yields the accepted
Phase Real and Completion exit read for the common refinement.

The converse theorem removes any attempted witness not already present on the
two covers, while the exactness theorem reads accepted refined cells only
through the four visible faces and their support. The gluing and descent
results cited above have the same exclusion clauses, so quotient streams,
selected limits, countable choice, ambient completeness, metric completeness,
aggregate formalstatus rows, and bridge tokens cannot serve as replacement
evidence under the refinement.
\end{proof}
