...
\leanvariant{BEDC.Derived.CauchyDifferenceCriterionUp.CauchyDifferenceCriterionCarrier\_kernel\_scope}
...
\end{theorem}
\leanchecked{BEDC.Derived.CauchyDifferenceCriterionUp.CauchyDifferenceCriterionObligationExport}
\begin{proof}
...
\end{theorem}
\leanchecked{BEDC.Derived.CauchyDifferenceCriterionUp.CauchyDifferenceCriterionScopePackage}
\begin{proof}
...
\end{proof}
\leanchecked{BEDC.Derived.CauchyDifferenceCriterionUp.CauchyDifferenceCriterionScopedKernelBinding}
...
\end{proof}
\leanchecked{BEDC.Derived.CauchyDifferenceCriterionUp.CauchyDifferenceCriterionPublicExport}

\begin{theorem}[Cauchy difference criterion public exactness]
\label{thm:cauchy-difference-criterion-public-exactness}
Every public $\RealUp$ equality consumer of an accepted $\CauchyDifferenceCriterionUp$ carrier receives exactly the following two routes. First, the displayed $\RegularCauchyDifferenceUp$ row with $\NullSequenceUp$ evidence implies the $\RegularCauchyZeroDistanceUp$ handoff and the terminal zero-distance $\RealUp$ seal. Second, any zero-distance $\RealUp$ seal exported by the carrier reconstructs the same displayed null-difference route through $\RegularCauchyDifferenceUp$ and $\NullSequenceUp$.

Both directions pass only through the displayed $\DyadicRatCoreUp$, $\StreamNameUp$, $\RegSeqRatUp$, $\RegularCauchyDifferenceUp$, $\RegularCauchyZeroDistanceUp$, $\RealUp$, $\BHist$, $\hsame$, $\Cont$, $\Pkg$, and $\NameCert$ rows. The public exactness surface has no coordinate for quotient stream equality, selected limits, host real equality, trichotomy, countable choice, or ambient $\RealUp$ completeness.
\end{theorem}
\begin{proof}
For the forward route, start with the displayed $\RegularCauchyDifferenceUp$ row and $\NullSequenceUp$ evidence. Apply the bidirectional exactness theorem \autoref{thm:cauchy-difference-criterion-bidirectional-exactness}; it routes the null difference through the $\RegularCauchyZeroDistanceUp$ row and then through the terminal $\RealUp$ seal. The public export theorem \autoref{thm:cauchy-difference-criterion-public-export} keeps this route on the displayed $\RegSeqRatUp$, $\StreamNameUp$, $\DyadicRatCoreUp$, and structural rows.

For the reverse route, start with a zero-distance $\RealUp$ seal exported by the accepted carrier. The null-route converse \autoref{thm:cauchy-difference-criterion-null-route-converse} reconstructs the displayed $\RegularCauchyDifferenceUp$ and $\NullSequenceUp$ rows behind that seal. The zero-distance correspondence \autoref{thm:cauchy-difference-criterion-zero-distance-correspondence} identifies the reconstructed route with the same $\RegularCauchyZeroDistanceUp$ handoff.

The route is stable under finite window changes by \autoref{thm:cauchy-difference-criterion-shift-invariance}. Thus the exactness claim does not depend on a selected tail representative or quotient stream equality; finite $\StreamNameUp$ shifts preserve the displayed $D,Z,Q,E$ chain.

Finally apply the scoped kernel binding \autoref{thm:cauchy-difference-criterion-scoped-kernel-binding}. It binds both directions to the finite kernel rows $X,Y,D,Z,Q,W,T,E,H,C,P,N$, so the public equality consumer receives exactly the two displayed routes and no host equality, selected limit, trichotomy, countable-choice schedule, or ambient-completeness witness.
\end{proof}
\leanchecked{BEDC.Derived.CauchyDifferenceCriterionUp.CauchyDifferenceCriterionPublicExactness}

\closureat{CauchyDifferenceCriterionUp}{publicStr}[BEDC.Derived.CauchyDifferenceCriterionUp.CauchyDifferenceCriterionPublicExactness]
...
\begin{closurestatus}{\CauchyDifferenceCriterionUp}
  ...
  \upgradepath{Bridged closure requires the standard bridge or bridge-facing theorem for the public exactness route.}
