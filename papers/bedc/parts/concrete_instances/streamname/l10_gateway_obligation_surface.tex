...
\begin{theorem}[StreamName L10 face-window realization]
\label{thm:streamname-l10-face-window-realization}
Every accepted $\StreamNameUp$ L10 gateway packet consumed as Phase Real and Completion evidence realizes each finite request window through four displayed faces: the $\StreamNameUp$ finite section, the $\DyadicRatCoreUp$ tolerance row, the $\RegSeqRatUp$ readback row, and the terminal $\RealUp$ seal row. These four faces are carried together with componentwise $\hsame$ transport, displayed $\Cont$ replay, $\Pkg$ provenance, and local $\NameCert$ rows.

The realization exports no quotient stream, selected limit, quotient equality of Cauchy presentations, countable choice, ambient $\RealUp$ completeness, or host completion coordinate.
\end{theorem}
\begin{proof}
Start with the L10 gateway obligation surface of \autoref{thm:streamname-l10-gateway-namecert-obligation-surface}. It pins every gateway request to finite StreamName carrier rows, finite request sections, dyadic tolerance data, regular-rational readback, terminal Real locks, componentwise transport, replay, provenance, and local naming.

Apply the reindex-stability theorem, \autoref{thm:streamname-l10-gateway-window-reindex-stability}. Certified reindex routes preserve the endpoint rows, pointwise classifier rows, $\BHist$ support, $\hsame$ comparisons, $\Cont$ routes, $\Pkg$ provenance, and local $\NameCert$ rows, so the same finite request window can be read through its transported public section.

Finally use the Real-RegSeqRat handoff of \autoref{thm:streamname-l10-gateway-real-regseqrat-handoff}. It orders the request through the StreamName finite section, RegSeqRat readback, DyadicRatCore budget, and Real seal before any Phase Real or Completion consumer sees evidence. Since each cited route refuses quotient streams, selected limits, countable choice, and ambient Real completeness, the four-face realization has the same refusal surface.
\end{proof}
