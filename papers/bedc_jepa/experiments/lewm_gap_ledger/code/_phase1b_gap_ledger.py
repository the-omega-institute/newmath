...
NPZ_PATH = ROOT / "tworooms_latent.npz"
REPORT_DIR = ROOT / "reports"
JSON_PATH = REPORT_DIR / "lewm_gap_ledger_head.json"
MD_PATH = REPORT_DIR / "lewm_gap_ledger_head.md"
FINDINGS_PATH = ROOT / "PHASE1B_FINDINGS.md"
...
def main() -> None:
    ...
    debts.append(
        {
            ...
            "detail": "single LeWM tworooms latent export with 37 episodes/642 transitions; bootstrap CI does not replace >=30 independent world seeds",
        }
    )
    ...
    x_gap, min_abs_margin = build_gap_features(rows["emb"], probe_prob, probe_logit, probe_pred, quality_feature)

    tau_err = float(np.percentile(rows["mse"][train_mask], 75))
    low_margin_tau = float(np.percentile(min_abs_margin[train_mask], 30))
    ...
    y_low_margin = (min_abs_margin <= low_margin_tau).astype(np.int8)
    ...
    # vanilla: no learned head, constant train base rates.
    train_rates = labels[train_mask].mean(axis=0)
    vanilla_scores = np.tile(train_rates.reshape(1, -1), (n_rows, 1))
    arm_scores["vanilla"] = {"scores": vanilla_scores}

    # learned gap heads.
    learned_scores = np.zeros((n_rows, len(GAP_CHANNELS)), dtype=np.float64)
    learned_fits: dict[str, Any] = {}
    for j, ch in enumerate(GAP_CHANNELS):
        head, info = fit_logistic_head(x_gap[train_mask], labels[train_mask, j], steps=500, lr=0.14, l2=1e-4)
        learned_scores[:, j] = predict_logistic_head(head, x_gap)[0]
        learned_fits[ch] = info
    arm_scores["learned_gap_head_on_h"] = {"scores": learned_scores, "fits": learned_fits}

    # matched-random: same features/capacity/budget, seed deterministic train-label permutation.
    ...
    for j, ch in enumerate(GAP_CHANNELS):
        ...
        rng.shuffle(y_perm)
        head, info = fit_logistic_head(x_gap[train_mask], y_perm, steps=500, lr=0.14, l2=1e-4)
        random_scores[:, j] = predict_logistic_head(head, x_gap)[0]
        random_fits[ch] = {**info, "permutation_seed": MATCHED_RANDOM_SEED, "train_rows": int(len(train_idx))}
    arm_scores["matched_random_gap_head"] = {"scores": random_scores, "fits": random_fits}

    for arm_name, pack in arm_scores.items():
        scores = pack["scores"]
        gap_score = scores[:, 0]
        critical_score = np.max(scores, axis=1)
        metrics_eval = bootstrap_metrics(
        ...
        arms[arm_name] = {
            "metrics_eval_bootstrap": metrics_eval,
    ...
    learned = arms["learned_gap_head_on_h"]["metrics_eval_bootstrap"]
    matched = arms["matched_random_gap_head"]["metrics_eval_bootstrap"]
    vanilla = arms["vanilla"]["metrics_eval_bootstrap"]
    auroc_separates = ci_separation(learned, matched, "failure_detection_auroc")
    uer_beats_vanilla = learned["unlogged_error_rate"]["ci95_high"] < vanilla["unlogged_error_rate"]["ci95_low"]
    conclusion = {
        "claim": "positive" if auroc_separates and uer_beats_vanilla else "negative_or_inconclusive",
        "learned_auroc_ci_low_gt_matched_ci_high": bool(auroc_separates),
        "learned_uer_ci_high_lt_vanilla_ci_low": bool(uer_beats_vanilla),
        "summary": (
            "learned gap head significantly beats matched-random on native prediction-error AUROC and beats vanilla on UER"
            if auroc_separates and uer_beats_vanilla
            else "fail-closed: current eval bootstrap CI does not establish both required inequalities"
        ),
    }
    ...
    pred_def_report = {
        ...
        "loning_probe_error": {
            "definition": "max over distinctions of probe prediction != truth",
            "rates": {
    ...
    report = {
        "protocol": "Phase 1b gap ledger head on real LeWM latent/tworooms",
        ...
        "gap_feature_columns": {
            "width": int(x_gap.shape[1]),
            "components": {
                "emb": 192,
                "probe_probabilities": len(DISTINCTIONS),
                "probe_logits": len(DISTINCTIONS),
                "probe_predictions": len(DISTINCTIONS),
                "probe_signed_margins": len(DISTINCTIONS),
                "min_abs_margin": 1,
        ...
        "thresholds": {
            ...
            "low_margin_train_p30": low_margin_tau,
        },
        ...
        "bootstrap": {
            ...
            "unit": "eval transition rows",
            "seed": BOOTSTRAP_SEED,
            "warning": "row bootstrap underestimates uncertainty from episode/world-seed variation",
        },
    }
    ...
    md_lines = [
        ...
        "",
        f"- valid transitions: {n_rows}; episodes: {data['emb'].shape[0]}",
        f"- split transitions: train={int(train_mask.sum())}, calibration={int(cal_mask.sum())}, eval={int(eval_mask.sum())}",
    ...
    for arm_name in ("vanilla", "learned_gap_head_on_h", "matched_random_gap_head"):
        m = arms[arm_name]["metrics_eval_bootstrap"]
        md_lines.append(
    ...
    findings = [
        ...
        "",
        "核心判据：",
        f"- learned AUROC: {fmt_ci(learned['failure_detection_auroc'])}",
        f"- matched-random AUROC: {fmt_ci(matched['failure_detection_auroc'])}",
        f"- vanilla UER: {fmt_ci(vanilla['unlogged_error_rate'])}",
        f"- learned UER: {fmt_ci(learned['unlogged_error_rate'])}",
        "",
        "## 关键数据",
        "",
        f"- 输入 latent: `tworooms_latent.npz`，{n_rows} valid transitions / {data['emb'].shape[0]} episodes。",
        f"- episode split: train={int(train_mask.sum())} transitions, calibration={int(cal_mask.sum())}, eval={int(eval_mask.sum())}。",
    ...
    findings.extend(
        [
            ...
            "## 下一步",
            "",
            "- 用更多独立 world seeds/exports 替代当前 transition-row bootstrap，关闭 coverage debt。",
            "- 若 off-target channel 退化，定义 target-specific action perturbation 或从环境动作语义构造更干净的 intervention family。",
            "- 对 native prediction-error threshold 做 calibration split 稳健性扫描，并报告 episode-level bootstrap。",
            "",
            "产物：`reports/lewm_gap_ledger_head.json` 和 `reports/lewm_gap_ledger_head.md`。",
            "",
        ]
    )
