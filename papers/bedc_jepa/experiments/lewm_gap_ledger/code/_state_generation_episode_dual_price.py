...
DEFAULT_JSON = REPORT_DIR / "state_generation_episode_dual_price.json"
DEFAULT_MD = REPORT_DIR / "state_generation_episode_dual_price.md"
DEFAULT_OUT = REPORT_DIR / "state_generation_episode_dual_price_predictions.npz"
...
def episode_groups(episode: np.ndarray) -> list[np.ndarray]:
    ...
    for value in np.unique(episode.astype(np.int64)):
        groups.append(np.flatnonzero(episode.astype(np.int64) == int(value)).astype(np.int64))
    return groups


def lambda_grid(option_error: np.ndarray) -> np.ndarray:
    spread = float(np.percentile(np.abs(option_error.astype(np.float64) - np.median(option_error.astype(np.float64))), 95))
    radius = max(0.05, 4.0 * spread)
    return np.linspace(-radius, radius, 801, dtype=np.float64)


def shadow_prices(split: dict[str, np.ndarray], depths: np.ndarray) -> np.ndarray:
    prices = np.zeros(len(split["episode"]), dtype=np.float64)
    grid = lambda_grid(split["option_error"])
    centered_depth = depths.astype(np.float64) - 3.0
    for idxs in episode_groups(split["episode"]):
        errors = split["option_error"][idxs].astype(np.float64)
        best_key = (float("inf"), float("inf"), 0.0)
        best_lambda = 0.0
        for lam in grid:
            choice = np.argmin(errors + float(lam) * centered_depth[None, :], axis=1)
            mean_depth = float(np.mean(depths[choice].astype(np.float64)))
            chosen_error = float(np.mean(errors[np.arange(len(idxs)), choice]))
            key = (abs(mean_depth - 3.0), chosen_error, abs(float(lam)))
            if key < best_key:
                best_key = key
                best_lambda = float(lam)
        prices[idxs] = best_lambda
    return prices


def dual_targets(split: dict[str, np.ndarray], depths: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    prices = shadow_prices(split, depths)
    centered_depth = depths.astype(np.float64) - 3.0
    target = split["option_error"].astype(np.float64) + prices[:, None] * centered_depth[None, :]
    target = target - np.mean(target, axis=1, keepdims=True)
    return target.astype(np.float32), prices.astype(np.float32)


def train_model(
    train_x: np.ndarray,
    train_dual_target: np.ndarray,
    ...
    *,
    ...
    target_t = torch.from_numpy(train_dual_target.astype(np.float32)).to(device)
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, len(order), batch):
            ...
            loss = 0.45 * reg + 0.25 * rank + 0.20 * ce + 0.10 * balance
        ...
        cal_eval = episode_alloc.evaluate(cal_split, cal_score, seed=seed + 2800 + epoch)
...
def write_markdown(path: Path, report: dict[str, Any]) -> None:
    rows = report["eval"]
    lines = [
        "# State-Generation Episode Dual Price",
        "",
    ...
    for name in ["dual_price", "episode_allocation", "allocation_native", "oracle_true_error"]:
        row = rows[name]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train an episode-level dual-price allocation objective")
    ...
    train_target, train_price = dual_targets(train, structured.DEPTHS)
    cal_target, cal_price = dual_targets(cal, structured.DEPTHS)
    ...
    eval_score = episode_alloc.predict(model, eval_x, device, int(args.batch))
    ...
    rows = {
        "dual_price": episode_alloc.evaluate(eval_split, eval_score, seed=int(args.seed) + 1101),
        "episode_allocation": episode_alloc.evaluate(eval_split, episode_score, seed=int(args.seed) + 1101),
        "allocation_native": episode_alloc.evaluate(eval_split, allocation_native_score, seed=int(args.seed) + 1101),
        "oracle_true_error": episode_alloc.evaluate(eval_split, eval_split["option_error"], seed=int(args.seed) + 1101),
    }
    report = {
        "status": "ok",
        "schema_id": "bedc_jepa.state_generation_episode_dual_price",
        "device": str(device),
        "config": {
            "epochs": int(args.epochs),
            "hidden": int(args.hidden),
            ...
            "batch": int(args.batch),
            "lr": float(args.lr),
            "loss": "0.45*dual_price_smooth_l1 + 0.25*dual_order + 0.20*oracle_choice_ce + 0.10*expected_depth",
            "shadow_price": "per-episode grid search minimizing absolute mean-depth deviation, then chosen error, then absolute lambda",
        },
        "counts": {
        ...
        "shadow_price_summary": {
            "train_abs_median": clean_float(float(np.median(np.abs(train_price)))),
            "train_abs_p90": clean_float(float(np.percentile(np.abs(train_price), 90))),
            "calibration_abs_median": clean_float(float(np.median(np.abs(cal_price)))),
            "calibration_abs_p90": clean_float(float(np.percentile(np.abs(cal_price), 90))),
        },
        "selection": selection,
        "eval": rows,
        "diagnosis": {
            "allocation_closed": bool(float(rows["dual_price"]["allocation_delta"]["high"]) < 0.0),
            "beats_episode_objective_observed": bool(float(rows["dual_price"]["allocation_delta"]["observed"]) < float(rows["episode_allocation"]["allocation_delta"]["observed"])),
            "beats_episode_objective_ci_high": bool(float(rows["dual_price"]["allocation_delta"]["high"]) < float(rows["episode_allocation"]["allocation_delta"]["high"])),
            "beats_allocation_native_observed": bool(float(rows["dual_price"]["allocation_delta"]["observed"]) < float(rows["allocation_native"]["allocation_delta"]["observed"])),
            "beats_allocation_native_ci_high": bool(float(rows["dual_price"]["allocation_delta"]["high"]) < float(rows["allocation_native"]["allocation_delta"]["high"])),
            "oracle_closed": bool(float(rows["oracle_true_error"]["allocation_delta"]["high"]) < 0.0),
        },
        "leakage_attestation": {
            "features": "aligned export x only",
            "targets": "train option_error is converted to per-episode shadow-price scores and exact-budget oracle choices; eval option_error used only after score generation for metrics",
    ...
    np.savez_compressed(
        ...
        dual_price_score=eval_score.astype(np.float64),
