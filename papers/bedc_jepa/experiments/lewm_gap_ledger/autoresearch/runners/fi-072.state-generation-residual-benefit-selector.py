...
DEFAULT_REPORT = REPORT_DIR / "state_generation_residual_benefit_selector.json"
HYPOTHESIS_ID = "fi-072.state-generation-residual-benefit-selector"
...
def fail_closed(reason: str) -> dict:
    return {
        ...
        "metric": "residual_benefit_selector_valid",
        "metric_value": 0.0,
        "ci": {"low": 0.0, "high": 0.0},
        "status": "fail-closed",
        "measured_scope": ["residual_benefit_selector", "state_generation_allocation_boundary", "world_model_gate"],
        "reported_claim": f"residual-benefit selector validation fail-closed: {reason}",
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate residual-benefit selector")
    ...
    if not report_path.exists():
        ...
        schema_ok = str(report.get("schema_id") or "") == "bedc_jepa.state_generation_residual_benefit_selector"
        ...
        selected = rows.get("benefit_selector", {}) if isinstance(rows.get("benefit_selector"), dict) else {}
        ...
        payload = {
            ...
            "measured_scope": [
                "residual_benefit_selector",
                "budget_priority_residual",
                "state_generation_allocation_boundary",
                "world_model_gate",
            ],
            "reported_claim": (
                "Residual-benefit selector is evaluated against the priority-residual row: "
