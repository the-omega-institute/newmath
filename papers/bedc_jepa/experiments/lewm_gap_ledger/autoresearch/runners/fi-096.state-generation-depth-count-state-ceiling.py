...
DEFAULT_REPORT = REPORT_DIR / "state_generation_depth_count_state_ceiling.json"
HYPOTHESIS_ID = "fi-096.state-generation-depth-count-state-ceiling"
...
def fail_closed(reason: str) -> dict:
    return {
        ...
        "metric": "depth_count_state_ceiling_valid",
        "metric_value": 0.0,
        "ci": {"low": 0.0, "high": 0.0},
        "status": "fail-closed",
        "measured_scope": ["depth_count_state_ceiling", "state_generation_allocation_boundary", "world_model_gate"],
        "reported_claim": f"depth-count state ceiling audit fail-closed: {reason}",
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate depth-count state ceiling audit")
    ...
    if not report_path.exists():
        ...
        schema_ok = str(report.get("schema_id") or "") == "bedc_jepa.state_generation_depth_count_state_ceiling"
        status_ok = str(report.get("status") or "") == "ok"
        diagnosis = report.get("diagnosis") if isinstance(report.get("diagnosis"), dict) else {}
        closes = bool(diagnosis.get("depth_count_state_closes", False))
        selected = str(diagnosis.get("selected_source", ""))
        hard_capture = float(diagnosis.get("depth_count_hard_mean_capture_ratio", 0.0))
        hard_min = float(diagnosis.get("depth_count_hard_min_capture_ratio", 0.0))
        ...
        payload = {
            ...
            "measured_scope": [
                "depth_count_state_ceiling",
                "oracle_depth_count_state",
            ...
            "reported_claim": (
                "Depth-count state ceiling: "
                f"selected={selected}, hard_capture={hard_capture:.6g}, hard_min={hard_min:.6g}, "
            ...
            "diagnostics": {
                ...
                "depth_count_hard_mean_capture_ratio": hard_capture,
                "depth_count_hard_min_capture_ratio": hard_min,
