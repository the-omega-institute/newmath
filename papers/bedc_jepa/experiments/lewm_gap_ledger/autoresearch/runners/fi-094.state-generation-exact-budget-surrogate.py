...
DEFAULT_REPORT = REPORT_DIR / "state_generation_exact_budget_surrogate.json"
HYPOTHESIS_ID = "fi-094.state-generation-exact-budget-surrogate"
...
def fail_closed(reason: str) -> dict:
    return {
        ...
        "metric": "exact_budget_surrogate_valid",
        "metric_value": 0.0,
        "ci": {"low": 0.0, "high": 0.0},
        "status": "fail-closed",
        "measured_scope": ["exact_budget_surrogate", "state_generation_allocation_boundary", "world_model_gate"],
        "reported_claim": f"exact-budget surrogate audit fail-closed: {reason}",
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate exact-budget surrogate audit")
    ...
    if not report_path.exists():
        ...
        schema_ok = str(report.get("schema_id") or "") == "bedc_jepa.state_generation_exact_budget_surrogate"
        status_ok = str(report.get("status") or "") == "ok"
        diagnosis = report.get("diagnosis") if isinstance(report.get("diagnosis"), dict) else {}
        closes = bool(diagnosis.get("exact_budget_surrogate_closes", False))
        selected = str(diagnosis.get("selected_source", ""))
        hard_capture = float(diagnosis.get("surrogate_hard_mean_capture_ratio", 0.0))
        hard_min = float(diagnosis.get("surrogate_hard_min_capture_ratio", 0.0))
        ...
        payload = {
            ...
            "diagnostics": {
                ...
                "surrogate_hard_mean_capture_ratio": hard_capture,
                "surrogate_hard_min_capture_ratio": hard_min,
