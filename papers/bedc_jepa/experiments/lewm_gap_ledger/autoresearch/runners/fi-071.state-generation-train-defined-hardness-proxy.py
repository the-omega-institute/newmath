...
DEFAULT_REPORT = REPORT_DIR / "state_generation_train_defined_hardness_proxy.json"
HYPOTHESIS_ID = "fi-071.state-generation-train-defined-hardness-proxy"
...
def fail_closed(reason: str) -> dict:
    return {
        ...
        "metric": "train_defined_hardness_proxy_valid",
        "metric_value": 0.0,
        "ci": {"low": 0.0, "high": 0.0},
        "status": "fail-closed",
        "measured_scope": ["train_defined_hardness_proxy", "state_generation_allocation_boundary", "world_model_gate"],
        "reported_claim": f"train-defined hardness proxy validation fail-closed: {reason}",
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate train-defined hardness proxy")
    ...
    if not report_path.exists():
        ...
        schema_ok = str(report.get("schema_id") or "") == "bedc_jepa.state_generation_train_defined_hardness_proxy"
        ...
        selected = rows.get("selected_proxy", {}) if isinstance(rows.get("selected_proxy"), dict) else {}
        ...
        payload = {
            ...
            "measured_scope": [
                "train_defined_hardness_proxy",
                "budget_priority_residual",
                "state_generation_allocation_boundary",
                "world_model_gate",
            ],
            "reported_claim": (
                "Train-defined hardness proxy is evaluated against the priority-residual row: "
