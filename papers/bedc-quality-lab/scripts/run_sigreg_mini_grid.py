#!/usr/bin/env python3
"""Run the SIGReg mini-grid backend evidence producer."""
...
DEFAULT_RUN_ID = "sigreg-mini-grid"
...
REPORT_ARTIFACT = "reports/canonical/sigreg-mini-grid.md"
MIXING_ALIASES = {
    "spiral": "spiral",
...
def run_single_arm(
    *,
    ...
    envelope = run_gaussian_ou_lejepa.run_experiment(
        ...
        run_id=f"sigreg-mini-grid-arm-{seed}",
        envelope_artifact="reports/runs/sigreg-mini-grid/arm-envelope.json",
        report_artifact="reports/runs/sigreg-mini-grid/arm-report.md",
    )
    ...
    return {
        ...
        "covariance_proxy": float(score.get("cov_to_identity_fro", envelope.metrics.get("covariance_deviation", 0.0))),
...
def _deterministic_record(
    ...
    *,
    ...
    return {
        ...
        "covariance_proxy": 0.19 + 0.01 * rho_rank + 0.002 * lambda_rank,
        "linear_identifiability_r2": 0.40 + 0.10 * rho_rank,
        "actual_recovery_mse": 0.42 - 0.03 * rho_rank + 0.01 * lambda_rank,
        "theorem3_bound_mse": 1.0 + 0.02 * lambda_rank,
        "collapse_rate": 0.02 + 0.03 * lambda_rank,
        "quality_q": 1.0 + 0.12 * rho_rank - 0.08 * lambda_rank - mixing_penalty - seed_jitter,
        "source_run_id": f"deterministic-sigreg-mini-grid-arm-{seed}",
    }
...
def write_artifacts(projection: Mapping[str, Any], *, root: Path, json_artifact: str = JSON_ARTIFACT, report_artifact: str = REPORT_ARTIFACT) -> None:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    projection = build_projection(
        ...
        deterministic=not args.real_backend,
