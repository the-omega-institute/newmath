...
def fussCatalanExport : FussCatalanExportWitness where
  ...
  nat_choose_div_apply := toNat_eq_nat_choose_div
...
theorem fussCatalanCount_eq_nat_choose_div (m n : Nat) :
  ...
  by
    change toNat m n = Nat.choose (m * n) n / ((m - 1) * n + 1)
    exact BedcMathlibBridge.Constructive.FussCatalan.toNat_eq_nat_choose_div m n
