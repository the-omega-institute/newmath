...
|---|---|---|
| Cook 2009 Figure 6 | `Ē` produces `C2` from `A4` in the construction case only when `Ē` is `up 5` from `A4`; the resulting `C2` is distance `0` from the creating `Ē`; `Ē` crosses `A4` iff it is `up 0`; incoming `A4` is `up 5` from regenerated `Ē`; consecutive `A4`s are `up 5`; after an invisible `Ē`, `k=0` produces `C2` and `k=1` crosses; after moving data, `k=4` produces `C2` and `k=5` crosses (from caption and figure visual; PDF page 13, lines 639-654). | DISCREPANCY: `cook_ossifier.c` lines 8-10, 24-32 emit four `A`-phase objects per group, four groups total, with group gap `6k+5`; it does not encode the Figure 6 single-`A4` crossing/ossifying branch or the `k=0/1/4/5` data-conditioned cases. |
| Cook 2009 Figure 7 | Acceptor/rejector processing uses local measurement labels `(i) 0,0,3,5`, `(j) 0,2`, `(k) 0,4,3,5`, `(l) 0,2`; acceptor converts primary/standard components into moving data and rejector deletes components; cluster measurements use the closest glider touching the measured ether (from caption and figure visual; PDF page 14, lines 693-700). | DISCREPANCY: `cook_leader.c` lines 24-38 use fixed accept layout `3,3,3` and reject layout `2,4,5`; lines 20-21 and 24-38 leave `a_count=0`, so the figure-level acceptor/rejector products are not emitted as `A` objects. |
| Cook 2009 Figure 8 | Invisibles from a prepared leader have alignment `k+5`; after moving data, acceptor preparation gives `k=0` and invisible alignment `5`; after rejected invisibles, `k = 4+1+(2c-1)*5+2 = 10c+2`, and `c` multiple of 6 gives `k=2`, hence invisible alignment `1`; the diagram also shows local labels `3,2,k`, `0,0,k`, and `4,1,5,2,k` with the `2c-1` span (from caption and figure visual; PDF page 15, lines 759-769). | DISCREPANCY: `cook_leader.c` lines 24-38 and 197-291 emit fixed `Ebar`/`C2` packets; the `c`-dependent rejection formula and `k+5` branch are not represented. |
| Cook 2009 Figure 9 | Raw leaders are `up 0` from the previous component; with no previous component, raw leader is `up 0` from the previous short leader; raw leaders after short leaders give prepared leaders with `k=2`; raw short leaders are `up +3` higher through the `E^n`s than raw regular leaders. Diagram labels are `(p) 3,3`, `(q) 0,0`, `(r) 0,0,0,2,k=2` (from caption and figure visual; PDF page 16, lines 808-814). | DISCREPANCY: `cook_encode.c` lines 58-65 store the block-level `K/L/H/I/J` recipes, but `cook_leader.c` lines 24-38 does not distinguish raw regular leaders from raw short leaders or encode the `up +3` placement rule. |
...
| Cook 2004 Figure 12 | Leader/tape interaction uses eight `E` gliders, mostly `Ē`, hitting four vertical `C2`s; tape `N` produces rejector `A3`; tape `Y` produces acceptor `A4 A1 A`; both cases emit two invisible `Ē`s to the left; the first `Ē` reacts with four `C2`s, emits two `A`s, and becomes an invisible `Ē`; the center-`C2` spacing difference changes the emitted-`A` timing (from caption/body; PDF page 25-26, lines 1090-1112). Body text gives tape `Y = C2 18 C2 18 C2 14 C2` and tape `N = C2 18 C2 10 C2 14 C2` (PDF page 30, lines 1252-1257). | Partial match plus DISCREPANCY: `cook_leader.c` lines 9-11 matches eight `Ebar`, four `C2`, and two invisible counts, but lines 20-38 do not emit `A3` or `A4 A1 A`. `cook_data_block.c` lines 143-144 matches `Y={18,18,14}` but uses `N={28,10,14}` instead of paper `N={18,10,14}`; `cook_decode.c` lines 257-258 uses the same `28,10,14` classifier. |
