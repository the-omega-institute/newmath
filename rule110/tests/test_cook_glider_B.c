...
static void test_glider_B_moves(void) {
    ...
    free(initial);
    free(cells);
    printf("  glider_B_moves: PASS\n");
}

static void test_glider_B_ether_unaffected(void) {
    const size_t period_count = 20;
    const size_t len = COOK_ETHER_WIDTH * period_count;
    const size_t pos = 100;
    uint8_t *cells = (uint8_t *)malloc(len);
    ...
    for (size_t i = pos + COOK_GLIDER_B_WIDTH + 51; i < len - 20; i++) {
        assert(cells[i] == ether_cell_at_step(i, 50));
    }

    free(cells);
    printf("  glider_B_ether_unaffected: PASS\n");
}

int main(void) {
    printf("== test_cook_glider_B ==\n");
    test_glider_B_moves();
    test_glider_B_ether_unaffected();
    printf("ALL test_cook_glider_B tests passed\n");
    return 0;
}
