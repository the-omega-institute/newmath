...
static void test_gliderD1_moves(void) {
    ...
    bits = glider_phase("D1", "A", 1, &phase_len);
    assert(bits != NULL);
    assert(phase_len == COOK_GLIDER_D1_WIDTH);
    ...
    r110_run_n_steps(after, tape_len, COOK_GLIDER_D1_PERIOD);
    ...
    free(after);
    free(before);
    printf("  gliderD1_moves: PASS\n");
}

static void test_gliderD1_ether_unaffected(void) {
    const char *bits = NULL;
    const size_t repeat_count = 9;
    const int displacement = 2;
    size_t phase_len = 0;
    ...
    assert(cook_glider_D1_emit_phase_exact(cells, center, len, "A", 1) == 0);

    r110_run_n_steps(cells, len, COOK_GLIDER_D1_PERIOD);
    ...
    free(cells);
    free(before);
    printf("  gliderD1_ether_unaffected: PASS\n");
}

int main(void) {
    printf("== test_cook_glider_D1 ==\n");
    test_gliderD1_moves();
    test_gliderD1_ether_unaffected();
    printf("ALL test_cook_glider_D1 tests passed\n");
    return 0;
}
