...
static void test_glider_E_moves(void) {
    ...
    cook_glider_E_emit(cells, pos, len);

    count0 = diff_span_range(cells, pos - 40, pos + COOK_GLIDER_E_WIDTH + 40,
                             0, &first0, &last0);
    assert(count0 > 0);
    assert(first0 >= pos);
    assert(last0 < pos + COOK_GLIDER_E_WIDTH);
    ...
    free(cells);
    printf("  glider_E_moves: PASS\n");
}

static void test_glider_E_ether_unaffected(void) {
    const size_t period_count = 120;
    const size_t len = COOK_ETHER_WIDTH * period_count;
    const size_t pos = 840;
    uint8_t *cells = (uint8_t *)malloc(len);
    ...
    free(cells);
    printf("  glider_E_ether_unaffected: PASS\n");
}

int main(void) {
    printf("== test_cook_glider_E ==\n");
    test_glider_E_moves();
    test_glider_E_ether_unaffected();
    printf("ALL test_cook_glider_E tests passed\n");
    return 0;
}
