...
- [!] L3.4: Cook encoder for arbitrary cyclic-tag programs now has `docs/cook_encoder_design.md`, a `cook_encode_phase_exact` C entry point, layout/resource accounting, and structural composition through the phase-exact component APIs. Actual emission remains blocked on the L3.1 `B` through `H` glider catalog and L3.3 phase-exact leader/ossifier/data-block bodies.
