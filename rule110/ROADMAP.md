...
- [!] L5.7: `GroundCompiler` (the channel-encoding pipeline that rule110 itself uses; meta-circular). Enum mirror covers flow round-trip rows, BHist injectivity rows, and the six C-level reject reasons in `manifests/ground_compiler/`; `tests/test_ground_compiler.c` validates those rows against the locked GroundCompiler encoder/decoder. Boundary: bounded enumeration only, with no universal cyclic-tag recognizer for all legal streams and no Lean cross-check registration for this family.
