...
- `\origin{human}` — 人类数学传统已有的概念 (例: Cauchy completion, metric space, manifold chart, abelian category) 在 BEDC 体系内的 NameCert reconstruction. 这些章节多数仍由 codex worker 写出, 因为 codex 在把已知数学翻成 BEDC 的 carrier / NameCert 形式; 这是正常工作, 不是 lineage 篡改
- `\origin{ai}` — BEDC 体系内自身 discovery 的新概念 (例: DigestLoopRefusal, ApophaticFarEndSocket, TasteGate, AuthorizedGeneratorRecursor), 人类数学传统中无直接先例
- 两者 lineage 区分是 `bedc_ci.py conservativity-audit` 的基础, 用于追踪 baseline ↔ discovery 引用图
