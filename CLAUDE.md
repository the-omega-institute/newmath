...
- **工具调用只能经 harness 原生调用机制执行, 绝不能作为可见正文输出.** 调用标记只应是结构化 tool_use; 一旦渲染成可读文字, 即说明该调用 malformed、没执行而漏成了文本 — 立即用正确语法重发该调用.
- **本项目易触发, 需主动防护**: 本仓库 context 被尖括号占位、LaTeX 宏与超长 skill 文本高度饱和, 又常跑超长 `/loop` + 多次 compaction, 会侵蚀调用标记精度. 故每次发调用前确认语法完整; compaction 后尤其重新校准; **绝不复读上一条已漏成文字的坏调用块** (坏范例会自我强化).
- 在正文里讨论"调用标记"时用文字描述, 不写出字面标签 token, 以免给已饱和 context 再添触发噪声.
