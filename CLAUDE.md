...
Schema field policy: artifact schema identity stays stable across additive field growth. Later non-safety fields must be reader-compatible: missing values produce a visible warning, default, or downgrade, not a hard failure. Only identity, owner, authority, provenance, gate, publication-risk, pointer, or trust-boundary fields may enter a named hard-fail required set.
