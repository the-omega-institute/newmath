...
The 33/33 paper cross-check means the table's claims are what the paper printed. The 26/33 strict detector PASS means that for 26 of those rows the direct simulation reproduces the claimed outcome under the detector's recognition rule. The remaining 7 are "detector cannot name the output glider" — the classifier's tag set doesn't have a phase entry for one of the produced gliders, so the detector can see something arrived at a given offset but can't assign it a name.

This is what classifier disagreement looks like when it is exposed. It is not a fault. It is an honest reading of where the catalog's tag set stops covering what the substrate produces. The relevant value of the project's audit setup is precisely that this reading is *visible* — the audit prints `INFO N audit finding(s)`, the boundary doesn't get smoothed out by a softer recognition rule.

Compare what would happen without the disagreement boundary. The detector could be made more permissive — "well, this output kind of looks like Ē, call it Ē". The strict count would go up. The catalog's actual coverage would not change at all. The classifier discipline would have been silently weakened.

## What the BEDC kernel disagreement boundary looks like

The kernel has the same kind of boundary in the same kind of place. When BEDC tries to apply its `closed-normal consistency` theorem to a closed term whose codomain depends on a typed sub-term, it stops. The kernel doesn't pretend that case is settled; it names four `subject-reduction` discharge hypotheses and tracks them as `\formalstatus{formalTargetV}` rather than `\theoremCheckedV`. The closed-codomain case has 17 strict obligations PASS in the project's exhaustiveness audit; the dependent-codomain case is parked behind the four hypotheses.
