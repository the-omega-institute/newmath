...
::: {.evidence}
**Lean:** [`BEDC.FKernel.Cont`](https://github.com/the-omega-institute/newmath/blob/dev/lean4/BEDC/FKernel/Cont.lean) — the relation $Cont : BHist \to BMark \to BHist \to Prop$.
...
$hsame : BHist \to BHist \to Prop$ is the only identity relation in the kernel. It identifies *traces*, not the agents that left them.
:::

If you tried to add an observer-substance — a separate type `Observer` with $Cont : Observer \to BHist \to BMark \to BHist \to Prop$ — the `cont_assoc`, `cont_left_unit`, `cont_right_unit` proofs that the kernel already ships with would no longer go through. The two-loops architecture (ground loop closes, meta loop does not close) would break: positing an observer-as-substance generates a Tarski-style fixed-point that has no closed inductive home.
...
**Lean roadmap:** [`BEDC.Derived.InterHist`](https://github.com/the-omega-institute/newmath/blob/dev/papers/bedc/parts/visions/inter_hist_locality.tex) namespace — currently schema-only. The closed witnesses await multi-Hist semantics in future BEDC chapters.
:::
...
- [`observer_hist_identity.tex`](https://github.com/the-omega-institute/newmath/blob/dev/papers/bedc/parts/visions/observer_hist_identity.tex) — single-Hist commitments BEDC already proves, read under the new identification
- [`inter_hist_locality.tex`](https://github.com/the-omega-institute/newmath/blob/dev/papers/bedc/parts/visions/inter_hist_locality.tex) — multi-Hist roadmap; mostly schema-only, but with the structural argument for constant `MaxRate` made formal
:::
