...
The literal reading of the Lean code does not say that. `e0` and `e1` are constructor names, but they could just as well have been called $\alpha$ and $\beta$, or `left` and `right`. What the code actually says is:
...
After 72 hours of AI-driven formalisation: provisionally yes, for 38 chapters covering numbers, sequences, real-stream classifiers, continuous functions, metric spaces, compact subsets, $S^{1}$, categories, functors, natural transformations. With 1837 mechanically verified theorems, mathlib-free, axiom-pure.
...
- Constructive compactness via located $\varepsilon$-nets (26 theorems)
