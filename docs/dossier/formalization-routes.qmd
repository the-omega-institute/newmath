...
date: 2026-05-07
description: 'A map of 11+ formalization routes, sorted by three axes: classical/constructive, wholesale/minimalist, kernel-extension yes/no. BEDC sits at (constructive $\times$ minimalist $\times$ no kernel extension $\times$ cannot-claim + AI parallelism). That cell is still empty otherwise.'
---
...
Once everything is placed, the cell BEDC occupies — **constructive $\times$ minimalist $\times$ no kernel extension $\times$ AI parallelism for productivity** — is, as of 2026-04-29, **still occupied by BEDC alone**.
...
Family A's shared trust posture: **axiom debt is not debt; host primitives are not a burden**. They bet on person-month efficiency $\times$ engineering culture to win the **porting race**. Mathlib in 2026 is the peak of this family — 9 years, ~150K theorems, time-averaged ~45 / day.
...
- **Style**: a mathematician can write spaces directly — `data Circle where base : Circle; loop : base == base` — write quotients via HITs, write equality via `ua`.
...
- **Relation to BEDC**: direct ancestor. BEDC's "0 three axioms," "scope-bound mature," and "cannot-claim registry" all draw their spirit from Bishop. Differences: Bishop has no formal system, no closure $\times$ verification two-axis ledger, no generative kernel ($b_{0}, b_{1} \to Hist \to ...$). BEDC is Bishop **formalized + with one extra discipline layer**.
...
- **Relation to BEDC**: **the closest cousin, but one discipline layer thinner than BEDC**.
  ...
  - Different: CoRN has no generative kernel ($Mark \to Hist \to ... \to NameCert$); it starts directly from Coq's `Set`/`Prop`/Cauchy sequences.
  - Different: CoRN has no closure $\times$ verification two-axis ledger.
