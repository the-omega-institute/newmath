...
当前机制: 当 `theory_extension` 候选有 `cross_chapter_unification = 2` (引用 ≥3 个不同章节的 leanchecked 名, 且结论是等价/iff/桥梁), 它对普通候选获得绝对优先. 它的 `paper_files[0]` 落在 `papers/bedc/parts/visions/`. 桥梁定理本身是个 Lean 定理, 不是 prose 段落.
