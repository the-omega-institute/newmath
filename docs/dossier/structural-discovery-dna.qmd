...
| Fingerprint | Role |
|---|---|
| `structural_fingerprint` | A faithful handle for the elaborated structure |
| `reduced_fingerprint` | A normalizing handle after beta, eta, zeta, and transparent unfolding normalization |
...
The structural reconstruction gate is negative. It is triggered by sincere discovery declarations, not by topic, chapter origin, filename, or other heuristics. The two declaration surfaces are:
...
The relation layer gives positive provenance evidence for a sound fragment. A candidate classifier of the shape `fun x => ... ∧ P ∧ ...` can refine a prior classifier `fun x => P` by adding conjunctive conditions. The semantic direction is projection: from `A ∧ B` the kernel can eliminate `And` and recover `A`.
