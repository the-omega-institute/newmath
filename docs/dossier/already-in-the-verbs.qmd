...
This is the scaffolding plan we did *not* execute. Four new bridge chapters in `parts/visions/` say something different:
...
Plotkin's *Structural Operational Semantics* (1981) defines program execution as a relation $\langle c, \sigma\rangle \to \langle c', \sigma'\rangle$ between configurations: pair of code and state. A single transition is one execution step. A program *runs* by chaining transitions until it reaches a final configuration.
...
::: {.evidence}
**Termination** is $\neg\exists m h', Cont h m h'$ — a history that admits no continuation. This is the BEDC reading of *normal form*.
...
In BEDC, every classifier closure law is **exactly** a proof of its closure. The closure law $\forall x y, P(x) \land P(y) \to P(combine(x, y))$ is, simultaneously:
...
::: {.evidence}
**Compositional compilation**: a multi-pass compiler is a composition of NameCert morphisms. The composition lemma — $(g \circ f) preserves hsame iff f and g each do$ — is a one-line proof in BEDC, not a separate engineering achievement.

**Optimisation correctness**: an optimisation pass is a $NameCert_S \to NameCert_S$ morphism (source to source). It is correct iff it preserves `hsame_S`. Same framework, same proof obligation.
