...
`axiom-purity` 用 `#print axioms` 检测每条 BEDC 定理的传递依赖, 默认禁止 `Classical.choice` (LEM/选择公理) 和 `Quot.sound` (商类型公理). `propext` 默认允许 (simp/rewrite 内置使用, 哲学上无争议). 加 `--strict` 可同时禁 propext.
