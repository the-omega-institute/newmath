...
omit [AskSetup] P in
theorem packageTokenPolicy_classification_roundtrip [AskSetup] [PackageSetup]
    {bundle : ProbeBundle ProbeName} (policy : PackageTokenPolicy bundle)
    {s t : BHist} {p q : Pkg} :
    TokIntro bundle s p → TokIntro bundle t q →
      (psame bundle p q → hsame s t) ∧ (hsame s t → psame bundle p q) := by
...
omit [AskSetup] P in
theorem packageTokenPolicy_psame_two_step_chain_pair [AskSetup] [PackageSetup]
    {bundle : ProbeBundle ProbeName} (policy : PackageTokenPolicy bundle)
    {a b c : BHist} {p q r : Pkg} :
    TokIntro bundle a p → TokIntro bundle b q → TokIntro bundle c r →
      psame bundle p q → psame bundle q r → hsame a c ∧ psame bundle p r := by
  intro left middle right leftSame rightSame
  have leftHist : hsame a b := policy.reflection left middle leftSame
  have rightHist : hsame b c := policy.reflection middle right rightSame
  have outerHist : hsame a c := hsame_trans leftHist rightHist
  exact And.intro outerHist (policy.soundness left right outerHist)
...
omit [AskSetup] P in
theorem packageTokenPolicy_psame_three_step_on_introduced [AskSetup] [PackageSetup]
    {bundle : ProbeBundle ProbeName} (policy : PackageTokenPolicy bundle)
    {a b c d : BHist} {p q r u : Pkg} :
    TokIntro bundle a p → TokIntro bundle b q → TokIntro bundle c r →
      TokIntro bundle d u → psame bundle p q → psame bundle q r →
        psame bundle r u → psame bundle p u := by
...
omit [AskSetup] P in
theorem packageTokenPolicy_psame_three_step_reflect_on_introduced [AskSetup] [PackageSetup]
    {bundle : ProbeBundle ProbeName} (policy : PackageTokenPolicy bundle)
    {a b c d : BHist} {p q r u : Pkg} :
    TokIntro bundle a p → TokIntro bundle b q → TokIntro bundle c r →
      TokIntro bundle d u → psame bundle p q → psame bundle q r →
  ...
  exact policy.reflection left right
    (packageTokenPolicy_psame_three_step_on_introduced policy left mid1 mid2 right
...
theorem packageTokenPolicy_closure_fields_on_introduced {bundle : ProbeBundle ProbeName}
    (policy : PackageTokenPolicy bundle) :
  ...
  constructor
  · intro s p tok
    exact packageTokenPolicy_psame_refl_on_introduced policy tok
  · constructor
    · intro s t p q left right samePkg
      exact packageTokenPolicy_psame_symm_on_introduced policy left right samePkg
    · intro a b c p q r left middle right leftSame rightSame
      exact packageTokenPolicy_psame_trans_on_introduced policy left middle right leftSame rightSame
