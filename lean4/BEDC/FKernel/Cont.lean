...
theorem cont_step_rules_inversion_pair :
  ...
  constructor
  · intro h k r hcont
    exact BHist.e0.inj hcont
  · intro h k r hcont
    exact BHist.e1.inj hcont
...
theorem cont_common_suffix_cancellation {h h' k r r' : BHist} :
...
theorem cont_e0_result_iff {h k r : BHist} :
    Cont h k (BHist.e0 r) ↔
      (k = BHist.Empty ∧ hsame h (BHist.e0 r)) ∨
        (∃ k0 : BHist, k = BHist.e0 k0 ∧ Cont h k0 r) := by
  constructor
  · exact cont_e0_result_inversion
  · intro data
    cases data with
    | inl base =>
        cases base with
...
theorem continuation_associativity_up_to_hsame {h k l u v w z : BHist} :
    Cont h k u -> Cont u l v -> Cont k l w -> Cont h w z -> hsame v z := by
  intro hku hulv klw hwz
  cases hku
  cases hulv
...
theorem cont_assoc_four {a b c d ab abc cd bcd left right : BHist} :
    Cont a b ab → Cont ab c abc → Cont c d cd → Cont b cd bcd →
      Cont abc d left → Cont a bcd right → hsame left right := by
  intro hab habc hcd hbcd hleft hright
  cases hab
  cases habc
  cases hcd
  cases hbcd
  cases hleft
  cases hright
  exact (append_assoc (append a b) c d).trans (append_assoc a b (append c d))
