...
theorem external_append_middle_cancel_hsame {a b c d : BWord} :
    hsame (append a (append b c)) (append a (append b d)) -> hsame c d := by
  intro same
  have reassociated :
      hsame (append (append a b) c) (append (append a b) d) := by
    exact Eq.trans (external_append_assoc a b c)
      (Eq.trans same (external_append_assoc a b d).symm)
  exact external_append_left_cancel_hsame reassociated

theorem external_append_double_cancel_hsame {a b c d : BWord} :
    hsame (append a (append b c)) (append a (append d c)) -> hsame b d := by
  intro same
  have suffixSame : hsame (append b c) (append d c) :=
...
theorem external_append_cancel_hsame_directions_by_induction :
    (∀ {a b c : BWord}, hsame (append a c) (append b c) → hsame a b) ∧
      (∀ {a b c : BWord}, hsame (append c a) (append c b) → hsame a b) := by
  constructor
  · intro a b c same
    exact external_append_right_cancel_hsame same
  · intro a b c same
    exact external_append_left_cancel_by_induction same
