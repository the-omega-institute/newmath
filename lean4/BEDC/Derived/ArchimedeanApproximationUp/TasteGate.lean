...
theorem ArchimedeanApproximationCarrier_window_readback_obligations [AskSetup] [PackageSetup]
    {bound rational dyadic tolerance window readback sealRow transportRow replayRow provenance
      localName windowRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    ArchimedeanApproximationCarrier bound rational dyadic tolerance window readback sealRow
        transportRow replayRow provenance localName bundle pkg →
      Cont bound dyadic tolerance →
            ...
            SemanticNameCert
                ...
                (fun row : BHist =>
                  hsame row bound ∨ hsame row dyadic ∨ hsame row tolerance ∨
                    hsame row window ∨ hsame row windowRead)
                (fun row : BHist =>
                  UnaryHistory row ∧ Cont bound dyadic tolerance ∧
                    Cont tolerance window windowRead ∧ PkgSig bundle provenance pkg)
                hsame ∧ UnaryHistory windowRead := by
  ...
  intro carrier boundDyadic toleranceWindow provenancePkg
  obtain ⟨_boundUnary, _rationalUnary, _dyadicUnary, toleranceUnary, windowUnary,
  ...
  have cert :
    ...
    core := {
      carrier_inhabited :=
        Exists.intro windowRead ⟨hsame_refl windowRead, windowReadUnary⟩
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
    ...
    ledger_sound := by
      intro _row sourceRow
      exact ⟨sourceRow.right, boundDyadic, toleranceWindow, provenancePkg⟩
  }
  exact ⟨cert, windowReadUnary⟩

theorem ArchimedeanApproximationCarrier_dyadic_readback [AskSetup] [PackageSetup]
    {bound rational dyadic tolerance window readback sealRow transportRow replayRow provenance
      localName : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    ArchimedeanApproximationCarrier bound rational dyadic tolerance window readback sealRow
          ...
          PkgSig bundle provenance pkg →
            PkgSig bundle localName pkg →
              SemanticNameCert
                  ...
                  (fun row : BHist =>
                    hsame row dyadic ∨ hsame row tolerance ∨ hsame row window ∨
                      hsame row readback)
                  (fun row : BHist =>
                    UnaryHistory row ∧ Cont bound dyadic tolerance ∧
...
theorem ArchimedeanApproximationCarrier_real_seal_nonescape [AskSetup] [PackageSetup]
    {bound rational dyadic tolerance window readback sealRow transportRow replayRow provenance
      localName realRead noescapeRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    ArchimedeanApproximationCarrier bound rational dyadic tolerance window readback sealRow
        transportRow replayRow provenance localName bundle pkg ->
      Cont bound dyadic tolerance ->
        Cont tolerance window readback ->
          Cont readback sealRow realRead ->
            Cont realRead localName noescapeRead ->
              PkgSig bundle provenance pkg ->
                PkgSig bundle localName pkg ->
                  SemanticNameCert
                      (fun row : BHist => hsame row noescapeRead /\ UnaryHistory row)
                      (fun row : BHist =>
                        hsame row dyadic \/ hsame row window \/ hsame row readback \/
                          hsame row sealRow \/ hsame row realRead \/ hsame row noescapeRead)
                      (fun row : BHist =>
                        UnaryHistory row /\ Cont bound dyadic tolerance /\
                          Cont tolerance window readback /\ Cont readback sealRow realRead /\
                            Cont realRead localName noescapeRead /\
                              PkgSig bundle provenance pkg /\ PkgSig bundle localName pkg)
                      hsame /\ UnaryHistory realRead /\ UnaryHistory noescapeRead := by
  -- BEDC touchpoint anchor: ArchimedeanApproximationCarrier BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier boundDyadic toleranceWindow readbackSeal realNonescape provenancePkg namePkg
  obtain ⟨boundUnary, _rationalUnary, dyadicUnary, _toleranceUnary, windowUnary,
    _readbackUnary, sealUnary, _transportUnary, _replayUnary, _provenanceUnary,
    localNameUnary, _carrierBoundDyadic, _carrierToleranceWindow, _carrierProvenancePkg,
  ...
  have realReadUnary : UnaryHistory realRead :=
    unary_cont_closed readbackUnary sealUnary readbackSeal
  have noescapeReadUnary : UnaryHistory noescapeRead :=
    unary_cont_closed realReadUnary localNameUnary realNonescape
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row noescapeRead /\ UnaryHistory row)
          (fun row : BHist =>
          ...
          (fun row : BHist =>
            UnaryHistory row /\ Cont bound dyadic tolerance /\
              Cont tolerance window readback /\ Cont readback sealRow realRead /\
                Cont realRead localName noescapeRead /\ PkgSig bundle provenance pkg /\
                  PkgSig bundle localName pkg)
          hsame := {
    core := {
      carrier_inhabited :=
        Exists.intro noescapeRead ⟨hsame_refl noescapeRead, noescapeReadUnary⟩
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
    ...
    ledger_sound := by
      intro _row sourceRow
      exact
        ⟨sourceRow.right, boundDyadic, toleranceWindow, readbackSeal, realNonescape,
