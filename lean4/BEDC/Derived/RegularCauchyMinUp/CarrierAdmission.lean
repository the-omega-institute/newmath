...
theorem RegularCauchyMinCarrier_apartness_selector_scope [AskSetup] [PackageSetup]
                            ...
                            RegularCauchyMinCarrier A B W DA DB J S R E H C P N ∧
                              Cont W DA leftSelector ∧ Cont W DB rightSelector ∧
                                Cont leftSelector J S ∧ Cont S R readbackRead ∧
                                  Cont readbackRead E sealRead ∧
                                    PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
                        hsame ∧ UnaryHistory leftSelector ∧ UnaryHistory rightSelector ∧
                      UnaryHistory readbackRead ∧ UnaryHistory sealRead := by
  -- BEDC touchpoint anchor: RegularCauchyMinUp BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier leftRoute rightRoute selectorRoute readbackRoute sealRoute pPkg nPkg
  have carrierOriginal : RegularCauchyMinCarrier A B W DA DB J S R E H C P N := carrier
  ...
  have selectedUnary : UnaryHistory S :=
    unary_cont_closed leftUnary jUnary selectorRoute
  have readbackUnary : UnaryHistory readbackRead :=
    unary_cont_closed selectedUnary rUnary readbackRoute
  have sealUnary : UnaryHistory sealRead :=
    unary_cont_closed readbackUnary eUnary sealRoute
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ RegularCauchyMinCarrier A B W DA DB J S R E H C P N ∧
              Cont W DA leftSelector ∧ Cont W DB rightSelector ∧ Cont leftSelector J S ∧
                Cont S R readbackRead ∧ Cont readbackRead E sealRead ∧
                  PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, carrierOriginal, leftRoute, rightRoute, selectorRoute,
          readbackRoute, sealRoute, pPkg, nPkg⟩
  }
  exact ⟨cert, leftUnary, rightUnary, readbackUnary, sealUnary⟩
