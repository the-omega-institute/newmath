...
theorem RegularCauchyMinCarrier_obligation_closure_route [AskSetup] [PackageSetup]
    {A B W DA DB J S R E H C P N selectedRead readbackRead sealRead
                        ...
                        UnaryHistory row ∧
                          RegularCauchyMinCarrier A B W DA DB J S R E H C P N ∧
                            Cont J S selectedRead ∧ Cont selectedRead R readbackRead ∧
                              Cont readbackRead E sealRead ∧
                                Cont sealRead C classifierRead ∧ PkgSig bundle P pkg ∧
                                  PkgSig bundle N pkg)
                      hsame ∧
  ...
  intro carrier selectedRoute readbackRoute sealRoute classifierRoute provenancePkg namePkg
  ...
  obtain ⟨_aUnary, _bUnary, _wUnary, _daUnary, _dbUnary, jUnary, sUnary, rUnary,
    eUnary, _hUnary, cUnary, _pUnary, _nUnary⟩ := carrier
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, carrierOriginal, selectedRoute, readbackRoute, sealRoute,
          classifierRoute, provenancePkg, namePkg⟩
  }
