...
theorem EpigraphLowerSetTransport [AskSetup] [PackageSetup]
    {D V L O H C P N lowerRead transportedRead replayRead namedRead : BHist}
  ...
  intro carrier lowerRoute transportRoute replayRoute nameRoute provenancePkg namePkg
  obtain ⟨_dUnary, _vUnary, lUnary, oUnary, hUnary, cUnary, _pUnary, nUnary,
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont L O lowerRead ∧ Cont lowerRead H transportedRead ∧
              Cont transportedRead C replayRead ∧ Cont replayRead N namedRead ∧
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, lowerRoute, transportRoute, replayRoute, nameRoute,
          provenancePkg, namePkg⟩
  }
  exact ⟨cert, lowerUnary, transportedUnary, replayUnary, namedUnary⟩
