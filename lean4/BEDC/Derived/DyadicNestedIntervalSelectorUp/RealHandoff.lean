...
theorem DyadicNestedIntervalSelectorRealHandoff [AskSetup] [PackageSetup]
                    ...
                    SemanticNameCert
                      ...
                      (fun row : BHist =>
                        hsame row I ∨ hsame row D ∨ hsame row S ∨ hsame row R ∨
                          hsame row E ∨ Cont H C replayRead)
                      (fun row : BHist =>
                        hsame row realRead ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
                      hsame := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert
  intro fieldRows _nestedRoute _dyadicRoute _streamRoute realRoute replayRoute provenancePkg
    namePkg
  cases fieldRows
  exact {
    core := {
