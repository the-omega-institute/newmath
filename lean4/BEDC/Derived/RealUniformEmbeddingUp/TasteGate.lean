...
private theorem RealUniformEmbeddingTasteGate_single_carrier_alignment_decode :
    ∀ h : BHist, realUniformEmbeddingDecodeBHist (realUniformEmbeddingEncodeBHist h) = h := by
  -- BEDC touchpoint anchor: BHist BMark
  intro h
  induction h with
  | Empty => rfl
...
def realUniformEmbeddingToEventFlow : RealUniformEmbeddingUp → EventFlow :=
  -- BEDC touchpoint anchor: BHist BMark
  fun x => (realUniformEmbeddingFields x).map realUniformEmbeddingEncodeBHist

private def realUniformEmbeddingEventAtDefault : Nat → EventFlow → RawEvent
  -- BEDC touchpoint anchor: BHist BMark
  | Nat.zero, [] => []
...
private theorem RealUniformEmbeddingTasteGate_single_carrier_alignment_round_trip :
...
private theorem RealUniformEmbeddingTasteGate_single_carrier_alignment_toEventFlow_injective
...
private theorem RealUniformEmbeddingTasteGate_single_carrier_alignment_fields :
    ∀ x y : RealUniformEmbeddingUp, realUniformEmbeddingFields x = realUniformEmbeddingFields y → x = y := by
  -- BEDC touchpoint anchor: BHist BMark
  intro x y hfields
  cases x with
...
theorem RealUniformEmbeddingTasteGate_single_carrier_alignment :
  ...
  exact
    ⟨RealUniformEmbeddingTasteGate_single_carrier_alignment_decode,
      ...
      (fun _ _ heq =>
        RealUniformEmbeddingTasteGate_single_carrier_alignment_toEventFlow_injective heq),
      rfl⟩
