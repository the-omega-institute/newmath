...
theorem BoundedSetCarrier_uniform_space_boundary [AskSetup] [PackageSetup]
    ...
    BoundedSetCarrier X S center radius ball transport replay provenance nameRow bundle pkg ->
      Cont S center memberRead ->
        Cont memberRead radius ballRead ->
          Cont ballRead transport uniformRead ->
            PkgSig bundle uniformRead pkg ->
              SemanticNameCert
                  (fun row : BHist => hsame row uniformRead ∧ UnaryHistory row)
                  (fun row : BHist =>
                    hsame row X ∨ hsame row S ∨ hsame row ballRead ∨
                      hsame row uniformRead ∨ Cont ballRead transport uniformRead)
                  (fun row : BHist =>
                    PkgSig bundle provenance pkg ∧ PkgSig bundle uniformRead pkg ∧
                      hsame row uniformRead)
                  hsame ∧
                UnaryHistory memberRead ∧ UnaryHistory ballRead ∧ UnaryHistory uniformRead ∧
                  Cont S center memberRead ∧ Cont memberRead radius ballRead ∧
                    Cont ballRead transport uniformRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro carrier subsetCenter memberRadius ballTransport uniformPkg
  ...
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row uniformRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row X ∨ hsame row S ∨ hsame row ballRead ∨ hsame row uniformRead ∨
              Cont ballRead transport uniformRead)
          (fun row : BHist =>
            PkgSig bundle provenance pkg ∧ PkgSig bundle uniformRead pkg ∧
              hsame row uniformRead)
  ...
  exact
    ⟨cert, memberUnary, ballReadUnary, uniformUnary, subsetCenter, memberRadius,
      ballTransport⟩
