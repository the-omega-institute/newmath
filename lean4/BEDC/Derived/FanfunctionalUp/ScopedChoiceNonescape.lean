...
theorem FanFunctionalScopedChoiceNonescape [AskSetup] [PackageSetup]
    {C F eps B D W M H K P N prefixRead barRead compactRead modulusRead
        ...
        Cont prefixRead B barRead →
          Cont barRead W compactRead →
            Cont compactRead M modulusRead →
              Cont modulusRead H structuralRead →
                PkgSig bundle P pkg →
                  PkgSig bundle N pkg →
                    SemanticNameCert
                        ...
                        (fun row : BHist =>
                          UnaryHistory row ∧ Cont C F prefixRead ∧
                            Cont prefixRead B barRead ∧ Cont barRead W compactRead ∧
                              Cont compactRead M modulusRead ∧ Cont modulusRead H structuralRead ∧
                                PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
                        hsame ∧
                      UnaryHistory modulusRead ∧ UnaryHistory structuralRead := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro carrier prefixRoute barRoute compactRoute modulusRoute structuralRoute provenancePkg
    namePkg
  obtain ⟨cUnary, fUnary, _epsUnary, bUnary, _dUnary, wUnary, mUnary, hUnary, _kUnary,
  ...
  have structuralUnary : UnaryHistory structuralRead :=
    unary_cont_closed modulusUnary hUnary structuralRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row structuralRead ∧ UnaryHistory row)
          (fun row : BHist =>
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, prefixRoute, barRoute, compactRoute, modulusRoute, structuralRoute,
          provenancePkg, namePkg⟩
  }
  exact ⟨cert, modulusUnary, structuralUnary⟩
