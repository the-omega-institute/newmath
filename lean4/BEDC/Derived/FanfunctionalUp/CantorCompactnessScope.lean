...
theorem FanFunctionalCantorCompactnessScope [AskSetup] [PackageSetup]
    {C F eps B D W M H K P N prefixRead barRead compactRead namedRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    FanFunctionalCarrierSurface C F eps B D W M H K P N bundle pkg ->
      Cont C F prefixRead -> Cont prefixRead B barRead ->
        Cont barRead K compactRead -> Cont compactRead N namedRead ->
          PkgSig bundle P pkg -> PkgSig bundle N pkg ->
            SemanticNameCert
                (fun row : BHist => hsame row namedRead ∧ UnaryHistory row)
                (fun row : BHist =>
                  hsame row C ∨ hsame row F ∨ hsame row B ∨ hsame row D ∨
                    hsame row W ∨ hsame row K ∨ hsame row namedRead)
                (fun row : BHist =>
  ...
  intro carrier prefixRoute barRoute compactRoute namedRoute provenancePkg namePkg
  obtain ⟨cUnary, fUnary, _epsUnary, bUnary, _dUnary, _wUnary, _mUnary, _hUnary, kUnary,
  ...
  have compactUnary : UnaryHistory compactRead :=
    unary_cont_closed barUnary kUnary compactRoute
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row C ∨ hsame row F ∨ hsame row B ∨ hsame row D ∨ hsame row W ∨
              hsame row K ∨ hsame row namedRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont C F prefixRead ∧ Cont prefixRead B barRead ∧
              Cont barRead K compactRead ∧ Cont compactRead N namedRead ∧
                PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, prefixRoute, barRoute, compactRoute, namedRoute, provenancePkg,
          namePkg⟩
  }
  exact ⟨cert, prefixUnary, barUnary, compactUnary, namedUnary⟩
