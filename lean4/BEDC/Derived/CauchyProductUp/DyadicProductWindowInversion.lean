...
theorem CauchyProductPacket_dyadic_product_window_selector_certificate
    [AskSetup] [PackageSetup]
    {sourceA sourceB windowA windowB radiusA radiusB observationA observationB product
      classifier transport routes ledger name selectorWindow selectorDyadic selectorRegular
      productSeal : BHist}
            ...
            UnaryHistory selectorWindow ->
              UnaryHistory selectorDyadic ->
                SemanticNameCert
                    (fun row : BHist =>
                      hsame row windowA ∨ hsame row windowB ∨ hsame row product ∨
                        hsame row selectorRegular ∨ hsame row productSeal)
                    (fun row : BHist =>
                      hsame row windowA ∨ hsame row windowB ∨ hsame row product ∨
                        hsame row selectorRegular ∨ hsame row productSeal)
                    (fun row : BHist =>
                  ...
                  UnaryHistory windowA ∧ UnaryHistory windowB ∧ UnaryHistory observationA ∧
                    UnaryHistory observationB ∧ UnaryHistory product ∧
                      UnaryHistory selectorRegular ∧ UnaryHistory productSeal ∧
                        Cont observationA observationB product ∧
                          Cont selectorWindow selectorDyadic selectorRegular ∧
                            Cont product selectorRegular productSeal := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert
  intro packet selectorRegularRoute productSealRoute selectorRegularPkg selectorWindowUnary
    selectorDyadicUnary
  obtain ⟨_sourceAUnary, _sourceBUnary, windowAUnary, windowBUnary, _radiusAUnary,
  ...
  have productSealUnary : UnaryHistory productSeal :=
    unary_cont_closed productUnary selectorRegularUnary productSealRoute
  have sourceAtProductSeal :
      (fun row : BHist =>
        hsame row windowA ∨ hsame row windowB ∨ hsame row product ∨
          hsame row selectorRegular ∨ hsame row productSeal) productSeal := by
    exact Or.inr (Or.inr (Or.inr (Or.inr (hsame_refl productSeal))))
  have cert :
      SemanticNameCert
          (fun row : BHist =>
            hsame row windowA ∨ hsame row windowB ∨ hsame row product ∨
          ...
          hsame := {
    core := {
      carrier_inhabited := Exists.intro productSeal sourceAtProductSeal
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
      ...
      carrier_respects_equiv := by
        ...
        | inr tail =>
            ...
            | inr tail =>
                ...
                | inr tail =>
                    cases tail with
                    | inl rowSelectorRegular =>
                        exact
                          Or.inr (Or.inr (Or.inr (Or.inl
                            (hsame_trans (hsame_symm sameRows) rowSelectorRegular))))
                    | inr rowProductSeal =>
                        exact
                          Or.inr (Or.inr (Or.inr (Or.inr
                            (hsame_trans (hsame_symm sameRows) rowProductSeal))))
    }
    pattern_sound := by
      intro _row source
      exact source
    ledger_sound := by
      intro _row source
      exact ⟨selectorRegularPkg, source⟩
  }
  exact
    ⟨cert, windowAUnary, windowBUnary, observationAUnary, observationBUnary, productUnary,
      selectorRegularUnary, productSealUnary, productRoute, selectorRegularRoute,
      productSealRoute⟩
