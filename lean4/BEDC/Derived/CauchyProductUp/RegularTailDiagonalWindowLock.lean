...
theorem CauchyProductRegularTailDiagonalWindowLock [AskSetup] [PackageSetup]
    {sourceA sourceB windowA windowB radiusA radiusB observationA observationB product
      classifier transport routes ledger name budgetClassifier budgetSeal diagonalTail
      realSeal : BHist}
    ...
    CauchyProductPacket sourceA sourceB windowA windowB radiusA radiusB observationA
      ...
      Cont classifier routes budgetClassifier ->
        Cont budgetClassifier ledger budgetSeal ->
          Cont budgetSeal transport diagonalTail ->
            Cont diagonalTail routes realSeal ->
              PkgSig bundle realSeal pkg ->
                SemanticNameCert
                    (fun row : BHist => hsame row realSeal ∧ UnaryHistory row)
                    (fun row : BHist =>
                      hsame row classifier ∨ hsame row budgetClassifier ∨
                        hsame row budgetSeal ∨ hsame row diagonalTail ∨ hsame row realSeal)
                    (fun row : BHist => UnaryHistory row ∧ PkgSig bundle realSeal pkg)
                    hsame ∧
                  UnaryHistory budgetClassifier ∧ UnaryHistory budgetSeal ∧
                    UnaryHistory diagonalTail ∧ UnaryHistory realSeal := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro packet classifierBudget budgetSealRoute budgetTransportTail tailRoutesSeal realSealPkg
  ...
  have diagonalTailUnary : UnaryHistory diagonalTail :=
    unary_cont_closed budgetSealUnary transportUnary budgetTransportTail
  have realSealUnary : UnaryHistory realSeal :=
    unary_cont_closed diagonalTailUnary routesUnary tailRoutesSeal
  have sourceSeal :
  ...
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row realSeal ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row classifier ∨ hsame row budgetClassifier ∨ hsame row budgetSeal ∨
              hsame row diagonalTail ∨ hsame row realSeal)
          (fun row : BHist => UnaryHistory row ∧ PkgSig bundle realSeal pkg)
          hsame := {
    core := {
  ...
  exact
    ⟨cert, budgetClassifierUnary, budgetSealUnary, diagonalTailUnary, realSealUnary⟩
