...
theorem CauchyProductPacket_real_multiplication_seal_handoff [AskSetup] [PackageSetup]
  ...
  intro packet classifierBudget budgetSealRoute realSealRoute multiplicationRoute multiplicationPkg
  ...
  have multiplicationUnary : UnaryHistory multiplicationRead :=
      ...
      carrier_respects_equiv := by
        ...
        | inr tail =>
            ...
            | inr tail =>
                cases tail with
                | inl rowRealSeal =>
                    exact Or.inr (Or.inr (Or.inl (hsame_trans (hsame_symm sameRows) rowRealSeal)))
                | inr rowMultiplication =>
                    exact Or.inr
                      (Or.inr (Or.inr (hsame_trans (hsame_symm sameRows) rowMultiplication)))
    }
    pattern_sound := by
      intro _row source
      exact source
  ...
  exact
    ⟨cert, productUnary, classifierUnary, budgetClassifierUnary, budgetSealUnary,
      realSealUnary, multiplicationUnary, classifierRoute, classifierBudget, budgetSealRoute,
      realSealRoute, multiplicationRoute, namePkg, multiplicationPkg⟩
