...
theorem UniformCauchyCriterionPacket_obligation_assembly [AskSetup] [PackageSetup]
          ...
          UnaryHistory regseqRead ∧ UnaryHistory realRead ∧ UnaryHistory publicRead ∧
            Cont index windows modulus ∧ Cont modulus tolerance tail ∧
              Cont index tail regseqRead ∧ Cont tail sealRow realRead ∧
                Cont regseqRead realRead publicRead ∧ PkgSig bundle name pkg ∧
  ...
  exact
    ⟨indexUnary, windowsUnary, modulusUnary, toleranceUnary, tailUnary, sealRowUnary,
      regseqUnary, realUnary, publicUnary, indexWindowsModulus, modulusToleranceTail,
      indexTailRegseq, tailSealReal, regseqRealPublic, namePkg, publicPkg⟩
