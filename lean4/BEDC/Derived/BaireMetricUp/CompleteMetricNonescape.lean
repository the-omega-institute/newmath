...
theorem BaireMetricCarrier_root_complete_metric_nonescape [AskSetup] [PackageSetup]
    {B W D R U S H C P N prefixRead radiusRead metricRead completeRead : BHist}
                    ...
                    (fun row : BHist =>
                      UnaryHistory row ∧ Cont S B prefixRead ∧
                        Cont prefixRead W radiusRead ∧ Cont radiusRead D metricRead ∧
                          Cont metricRead R completeRead ∧ PkgSig bundle completeRead pkg)
                    hsame ∧ UnaryHistory prefixRead ∧ UnaryHistory radiusRead ∧
                  UnaryHistory metricRead ∧ UnaryHistory completeRead := by
  -- BEDC touchpoint anchor: BaireMetricCarrier BHist Cont ProbeBundle PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier prefixRoute radiusRoute metricRoute completeRoute completePkg
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, prefixRoute, radiusRoute, metricRoute, completeRoute, completePkg⟩
  }
  exact ⟨cert, prefixUnary, radiusUnary, metricUnary, completeUnary⟩
