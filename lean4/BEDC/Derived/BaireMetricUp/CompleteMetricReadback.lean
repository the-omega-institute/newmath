...
theorem BaireMetricCompleteMetricReadback [AskSetup] [PackageSetup]
    {B W D R U S H C P N prefixRead radiusRead metricRead completeRead namedRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    BaireMetricCarrier B W D R U S H C P N bundle pkg →
      Cont S B prefixRead →
        Cont prefixRead W radiusRead →
          Cont radiusRead D metricRead →
            Cont metricRead R completeRead →
              Cont completeRead N namedRead →
                      ...
                      (fun row : BHist =>
                        UnaryHistory row ∧ Cont S B prefixRead ∧
                          Cont prefixRead W radiusRead ∧ Cont radiusRead D metricRead ∧
                            Cont metricRead R completeRead ∧ Cont completeRead N namedRead ∧
                              PkgSig bundle namedRead pkg)
                      hsame ∧ UnaryHistory prefixRead ∧ UnaryHistory radiusRead ∧
                    UnaryHistory metricRead ∧ UnaryHistory completeRead ∧
                      UnaryHistory namedRead := by
  -- BEDC touchpoint anchor: BaireMetricCarrier BHist Cont ProbeBundle PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier prefixRoute radiusRoute metricRoute completeRoute namedRoute namedPkg
  obtain ⟨bUnary, wUnary, dUnary, rUnary, _uUnary, sUnary, _hUnary, _cUnary, _pUnary,
    nUnary, _carrierSBW, _carrierWDR, _carrierRUC, _carrierCNP, _carrierPkg,
  ...
  have completeUnary : UnaryHistory completeRead :=
    unary_cont_closed metricUnary rUnary completeRoute
  have namedUnary : UnaryHistory namedRead :=
    unary_cont_closed completeUnary nUnary namedRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row namedRead ∧ UnaryHistory row)
          (fun row : BHist =>
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont S B prefixRead ∧ Cont prefixRead W radiusRead ∧
              Cont radiusRead D metricRead ∧ Cont metricRead R completeRead ∧
                Cont completeRead N namedRead ∧ PkgSig bundle namedRead pkg)
          hsame := {
    core := {
      carrier_inhabited :=
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, prefixRoute, radiusRoute, metricRoute, completeRoute,
          namedRoute, namedPkg⟩
  }
  exact ⟨cert, prefixUnary, radiusUnary, metricUnary, completeUnary, namedUnary⟩
