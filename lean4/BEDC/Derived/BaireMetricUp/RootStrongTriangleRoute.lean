...
theorem BaireMetricRootStrongTriangleRoute [AskSetup] [PackageSetup]
                    ...
                    (fun row : BHist =>
                      UnaryHistory row ∧ Cont S B prefixRead ∧
                        Cont prefixRead W radiusRead ∧ Cont radiusRead D metricRead ∧
                          Cont metricRead U strongRead ∧
                            PkgSig bundle strongRead pkg)
                    hsame ∧ UnaryHistory strongRead := by
  -- BEDC touchpoint anchor: BaireMetricCarrier BHist ProbeBundle Pkg Cont SemanticNameCert hsame
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row S ∨ hsame row B ∨ hsame row W ∨ hsame row D ∨ hsame row R ∨
              hsame row U ∨ hsame row strongRead)
          (fun row : BHist =>
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, prefixRoute, radiusRoute, metricRoute, strongRoute,
          strongPkg⟩
  }
  exact ⟨cert, strongUnary⟩
