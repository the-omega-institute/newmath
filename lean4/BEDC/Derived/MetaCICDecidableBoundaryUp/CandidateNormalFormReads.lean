...
theorem MetaCICDecidableBoundary_candidate_normal_form_reads [AskSetup] [PackageSetup]
    {T S B F R H C P N candidateRead replayRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    MetaCICDecidableBoundaryCarrier T S B F R H C P N bundle pkg ->
      Cont B F candidateRead ->
        Cont candidateRead R replayRead ->
          PkgSig bundle replayRead pkg ->
            SemanticNameCert
                (fun row : BHist => hsame row replayRead ∧ UnaryHistory row)
                (fun row : BHist =>
                  hsame row T ∨ hsame row S ∨ hsame row B ∨ hsame row F ∨
                    hsame row R ∨ hsame row candidateRead ∨ hsame row replayRead)
                (fun row : BHist =>
                  UnaryHistory row ∧ PkgSig bundle P pkg ∧ PkgSig bundle replayRead pkg)
                hsame ∧
              UnaryHistory candidateRead ∧ UnaryHistory replayRead := by
  -- BEDC touchpoint anchor: MetaCICDecidableBoundaryCarrier BHist Cont ProbeBundle PkgSig SemanticNameCert
  intro carrier candidateRoute replayRoute replayPkg
  obtain ⟨tUnary, sUnary, fUnary, rUnary, _hUnary, _cUnary, _pUnary, _nUnary,
  ...
  have replayUnary : UnaryHistory replayRead :=
    unary_cont_closed candidateUnary rUnary replayRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row replayRead ∧ UnaryHistory row)
          (fun row : BHist =>
  ...
  exact ⟨cert, candidateUnary, replayUnary⟩
