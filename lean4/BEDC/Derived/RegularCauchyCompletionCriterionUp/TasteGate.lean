...
theorem RegularCauchyCompletionCriterionCarrier_ledger_refusal
    {R W D M L Q H C P N windowRead toleranceRead boundaryRead ledgerRead : BHist} :
    UnaryHistory W →
      UnaryHistory D →
        UnaryHistory M →
          UnaryHistory Q →
                          ...
                          (fun row : BHist =>
                            hsame row R ∨ hsame row W ∨ hsame row D ∨ hsame row M ∨
                              hsame row L ∨ hsame row Q ∨ hsame row H ∨ hsame row C ∨
                                hsame row P ∨ hsame row N ∨ hsame row ledgerRead)
                          (fun row : BHist =>
                            UnaryHistory row ∧ Cont W D windowRead ∧
                              Cont windowRead M toleranceRead ∧
                                Cont toleranceRead Q boundaryRead ∧
  ...
  -- BEDC touchpoint anchor: BHist Cont hsame SemanticNameCert UnaryHistory
  intro wUnary dUnary mUnary qUnary nUnary windowRoute toleranceRoute boundaryRoute
    ledgerRoute
  have windowUnary : UnaryHistory windowRead :=
    unary_cont_closed wUnary dUnary windowRoute
  have toleranceUnary : UnaryHistory toleranceRead :=
  ...
  have ledgerUnary : UnaryHistory ledgerRead :=
    unary_cont_closed boundaryUnary nUnary ledgerRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row ledgerRead ∧ UnaryHistory row)
          (fun row : BHist =>
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont W D windowRead ∧ Cont windowRead M toleranceRead ∧
              Cont toleranceRead Q boundaryRead ∧ Cont boundaryRead N ledgerRead)
          hsame := {
    core := {
      carrier_inhabited :=
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, windowRoute, toleranceRoute, boundaryRoute, ledgerRoute⟩
  }
