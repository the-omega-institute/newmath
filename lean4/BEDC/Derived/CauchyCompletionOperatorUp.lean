...
theorem CauchyCompletionOperatorIdempotentCompleteCarrier [AskSetup] [PackageSetup]
    {M B U S R D Q E H C P N boundaryRead finiteWindow separatedRead sealRead secondWindow
      secondSeal : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    CauchyCompletionOperatorLedgerPacket M B U S R D Q E H C P N bundle pkg ->
      Cont M U boundaryRead ->
        Cont B S finiteWindow ->
          Cont D Q separatedRead ->
            Cont separatedRead E sealRead ->
              Cont finiteWindow R secondWindow ->
                Cont secondWindow E secondSeal ->
                    ...
                    SemanticNameCert
                        ...
                        (fun row : BHist =>
                          hsame row M ∨ hsame row B ∨ hsame row U ∨ hsame row S ∨
                            hsame row R ∨ hsame row D ∨ hsame row Q ∨ hsame row E ∨
                              hsame row H ∨ hsame row C ∨ hsame row P ∨ hsame row N ∨
                                hsame row secondWindow ∨ hsame row secondSeal)
                        (fun row : BHist =>
                          UnaryHistory row ∧ Cont M U boundaryRead ∧
                              ...
                              Cont separatedRead E sealRead ∧
                                Cont finiteWindow R secondWindow ∧
                                  Cont secondWindow E secondSeal ∧
                                    PkgSig bundle secondSeal pkg)
                        hsame ∧
                      UnaryHistory secondWindow ∧ UnaryHistory secondSeal := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro packet metricUniformBoundary boundaryWindow dyadicSeparated separatedSeal
    secondWindowRoute secondSealRoute secondSealPkg
  ...
  have secondWindowUnary : UnaryHistory secondWindow :=
    unary_cont_closed finiteUnary regularUnary secondWindowRoute
  have secondSealUnary : UnaryHistory secondSeal :=
    unary_cont_closed secondWindowUnary realSealUnary secondSealRoute
  have sourceSeal :
      (fun row : BHist => hsame row secondSeal ∧ UnaryHistory row) secondSeal := by
    exact ⟨hsame_refl secondSeal, secondSealUnary⟩
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row secondSeal ∧ UnaryHistory row)
          (fun row : BHist =>
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont M U boundaryRead ∧ Cont B S finiteWindow ∧
              Cont D Q separatedRead ∧ Cont separatedRead E sealRead ∧
                Cont finiteWindow R secondWindow ∧ Cont secondWindow E secondSeal ∧
                  PkgSig bundle secondSeal pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro secondSeal sourceSeal
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
    ...
    ledger_sound := by
      intro _row sourceRow
      exact
        ⟨sourceRow.right, metricUniformBoundary, boundaryWindow, dyadicSeparated,
          separatedSeal, secondWindowRoute, secondSealRoute, secondSealPkg⟩
  }
