...
theorem CauchyRateBudgetCarrier_public_export
    {R W Q D E H C P N rateWindow readbackRead toleranceRead sealRead replayRead : BHist} :
    UnaryHistory R ->
      UnaryHistory W ->
        UnaryHistory Q ->
          UnaryHistory D ->
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, rateRoute, readbackRoute, toleranceRoute, sealRoute, replayRoute⟩
  }
  exact ⟨cert, rateUnary, readbackUnary, toleranceUnary, sealUnary, replayUnary⟩
