...
theorem CauchyRateBudgetCarrier_tail_monotonicity
    {R W Q D E H C P N rateWindow readbackRead toleranceRead sealRead tailRead : BHist} :
    UnaryHistory R ->
      UnaryHistory W ->
        UnaryHistory Q ->
          UnaryHistory D ->
  ...
  intro rUnary wUnary qUnary dUnary eUnary cUnary rateRoute readbackRoute toleranceRoute
    sealRoute tailRoute
  ...
  have tailUnary : UnaryHistory tailRead :=
    unary_cont_closed sealUnary cUnary tailRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row tailRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row R ∨ hsame row W ∨ hsame row Q ∨ hsame row D ∨ hsame row E ∨
              hsame row H ∨ hsame row C ∨ hsame row P ∨ hsame row N ∨ hsame row tailRead)
          (fun row : BHist =>
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, rateRoute, readbackRoute, toleranceRoute, sealRoute, tailRoute⟩
  }
  exact ⟨cert, rateUnary, readbackUnary, toleranceUnary, sealUnary, tailUnary⟩
