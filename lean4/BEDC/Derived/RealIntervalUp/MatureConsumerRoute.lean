...
theorem RealIntervalMatureConsumerRoute [AskSetup] [PackageSetup]
    {L U E D W R S H C P N endpointRead windowRead bridgeRead densityRead matureRead :
      BHist}
    ...
    UnaryHistory L → UnaryHistory U → UnaryHistory E → UnaryHistory D → UnaryHistory W →
      UnaryHistory H → UnaryHistory P → UnaryHistory N → Cont L U endpointRead →
        Cont D W R → Cont R E S → Cont S H C → Cont C P windowRead →
          Cont windowRead N bridgeRead → Cont D R densityRead →
            Cont bridgeRead densityRead matureRead → PkgSig bundle N pkg →
              SemanticNameCert
                      ...
                      hsame row W ∨ hsame row R ∨ hsame row S ∨ hsame row H ∨
                        hsame row C ∨ hsame row P ∨ hsame row N ∨
                          hsame row endpointRead ∨ hsame row windowRead ∨
                            hsame row bridgeRead ∨ hsame row densityRead ∨
                ...
                UnaryHistory endpointRead ∧ UnaryHistory R ∧ UnaryHistory S ∧
                  UnaryHistory C ∧ UnaryHistory windowRead ∧ UnaryHistory bridgeRead ∧
                    UnaryHistory densityRead ∧ UnaryHistory matureRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert
  intro lUnary uUnary eUnary dUnary wUnary hUnary pUnary nUnary endpointRoute windowRoute
    sealRoute replayRoute publicRoute bridgeRoute densityRoute matureRoute namePkg
  ...
  have densityUnary : UnaryHistory densityRead :=
    unary_cont_closed dUnary readbackUnary densityRoute
  have matureUnary : UnaryHistory matureRead :=
    unary_cont_closed bridgeUnary densityUnary matureRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row matureRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row L ∨ hsame row U ∨ hsame row E ∨ hsame row D ∨ hsame row W ∨
              hsame row R ∨ hsame row S ∨ hsame row H ∨ hsame row C ∨ hsame row P ∨
                hsame row N ∨ hsame row endpointRead ∨ hsame row windowRead ∨
                  hsame row bridgeRead ∨ hsame row densityRead ∨ hsame row matureRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont L U endpointRead ∧ Cont D W R ∧ Cont R E S ∧
              Cont S H C ∧ Cont C P windowRead ∧ Cont windowRead N bridgeRead ∧
                Cont D R densityRead ∧ Cont bridgeRead densityRead matureRead ∧
                  PkgSig bundle N pkg)
          hsame := {
    core := {
