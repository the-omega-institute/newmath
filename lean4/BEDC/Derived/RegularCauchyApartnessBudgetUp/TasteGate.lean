...
def RegularCauchyApartnessBudgetCarrier [AskSetup] [PackageSetup]
    (X A M W D R E H C P N : BHist) (bundle : ProbeBundle ProbeName) (pkg : Pkg) :
    Prop :=
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig UnaryHistory
  UnaryHistory X ∧ UnaryHistory A ∧ UnaryHistory M ∧ UnaryHistory W ∧
    UnaryHistory D ∧ UnaryHistory R ∧ UnaryHistory E ∧ UnaryHistory H ∧
      UnaryHistory C ∧ UnaryHistory P ∧ UnaryHistory N ∧ Cont A M W ∧
        Cont W D R ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg

theorem RegularCauchyApartnessBudgetLowerBoundDeterminacy [AskSetup] [PackageSetup]
    {X X' A M W D R E E' H C P N : BHist}
    ...
    RegularCauchyApartnessBudgetCarrier X A M W D R E H C P N bundle pkg ->
      RegularCauchyApartnessBudgetCarrier X' A M W D R E' H C P N bundle pkg ->
        SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont A M W ∧ Cont W D R ∧ PkgSig bundle P pkg ∧
              PkgSig bundle N pkg)
          hsame ∧ UnaryHistory D := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert
  intro leftCarrier rightCarrier
  obtain ⟨_xUnary, aUnary, mUnary, wUnary, dUnaryLeft, rUnary, _eUnary, _hUnary,
    _cUnary, _pUnary, _nUnary, apartnessModulusWindow, windowLowerReadback, pkgP,
    pkgN⟩ := leftCarrier
  obtain ⟨_xPrimeUnary, _aUnary', _mUnary', _wUnary', dUnary, _rUnary', _ePrimeUnary,
    _hUnary', _cUnary', _pUnary', _nUnary', _apartnessModulusWindow',
    _windowLowerReadback', _pkgP', _pkgN'⟩ := rightCarrier
  have sourceAtD : hsame D D ∧ UnaryHistory D :=
    ⟨hsame_refl D, dUnary⟩
  have cert :
      SemanticNameCert
        (fun row : BHist => hsame row D ∧ UnaryHistory row)
        ...
        hsame := {
    core := {
      carrier_inhabited := Exists.intro D sourceAtD
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, apartnessModulusWindow, windowLowerReadback, pkgP, pkgN⟩
  }
  exact ⟨cert, dUnaryLeft⟩
