...
theorem PaperLeanDriftWitness_marker_inventory_kind_lock [AskSetup] [PackageSetup]
    {M A L I R H C P N markerRead inventoryRead verdictRead : BHist}
              ...
              SemanticNameCert
                  ...
                  (fun row : BHist =>
                    UnaryHistory row ∧ Cont M A markerRead ∧
                      Cont markerRead I inventoryRead ∧
                        Cont inventoryRead R verdictRead ∧ PkgSig bundle verdictRead pkg)
                  hsame ∧
  ...
  intro carrier markerRoute inventoryRoute verdictRoute verdictPkg
  obtain ⟨mUnary, aUnary, _lUnary, iUnary, rUnary, _hUnary, _cUnary, _pUnary,
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont M A markerRead ∧ Cont markerRead I inventoryRead ∧
              Cont inventoryRead R verdictRead ∧ PkgSig bundle verdictRead pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, markerRoute, inventoryRoute, verdictRoute, verdictPkg⟩
  }
  exact ⟨cert, markerUnary, inventoryUnary, verdictUnary⟩
