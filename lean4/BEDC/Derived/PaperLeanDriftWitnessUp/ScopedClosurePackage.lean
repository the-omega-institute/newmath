import BEDC.Derived.PaperLeanDriftWitnessUp

namespace BEDC.Derived.PaperLeanDriftWitnessUp

open BEDC.FKernel.Ask
open BEDC.FKernel.Bundle
open BEDC.FKernel.Cont
open BEDC.FKernel.Hist
open BEDC.FKernel.NameCert
open BEDC.FKernel.Package
open BEDC.FKernel.Unary

theorem PaperLeanDriftWitness_scoped_closure_package [AskSetup] [PackageSetup]
    {M A L I R H C P N verdictRead dependencyRead scopedRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    PaperLeanDriftWitnessCarrier M A L I R H C P N bundle pkg ->
      Cont R H verdictRead ->
        Cont C P dependencyRead ->
          Cont dependencyRead N scopedRead ->
            PkgSig bundle scopedRead pkg ->
              SemanticNameCert
                (fun row : BHist => hsame row scopedRead ∧ UnaryHistory row)
                (fun row : BHist =>
                  hsame row M ∨ hsame row A ∨ hsame row L ∨ hsame row I ∨
                    hsame row R ∨ hsame row dependencyRead ∨ hsame row scopedRead)
                (fun row : BHist => hsame row scopedRead ∧ PkgSig bundle scopedRead pkg)
                hsame ∧ UnaryHistory verdictRead ∧ UnaryHistory dependencyRead ∧
                  UnaryHistory scopedRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg SemanticNameCert hsame Cont
  intro carrier verdictRoute dependencyRoute scopedRoute scopedPkg
  obtain ⟨mUnary, _aUnary, _lUnary, _iUnary, rUnary, hUnary, cUnary, pUnary, nUnary,
    _markerNameLedger, _ledgerInventoryVerdict, _verdictTransportConsumer, _namePkg⟩ :=
    carrier
  have verdictUnary : UnaryHistory verdictRead :=
    unary_cont_closed rUnary hUnary verdictRoute
  have dependencyUnary : UnaryHistory dependencyRead :=
    unary_cont_closed cUnary pUnary dependencyRoute
  have scopedUnary : UnaryHistory scopedRead :=
    unary_cont_closed dependencyUnary nUnary scopedRoute
  have sourceScoped :
      (fun row : BHist => hsame row scopedRead ∧ UnaryHistory row) scopedRead := by
    exact ⟨hsame_refl scopedRead, scopedUnary⟩
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row scopedRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row M ∨ hsame row A ∨ hsame row L ∨ hsame row I ∨
              hsame row R ∨ hsame row dependencyRead ∨ hsame row scopedRead)
          (fun row : BHist => hsame row scopedRead ∧ PkgSig bundle scopedRead pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro scopedRead sourceScoped
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
        intro _row _other sameRows
        exact hsame_symm sameRows
      equiv_trans := by
        intro _row _middle _other sameLeft sameRight
        exact hsame_trans sameLeft sameRight
      carrier_respects_equiv := by
        intro _row _other sameRows source
        exact
          ⟨hsame_trans (hsame_symm sameRows) source.left,
            unary_transport source.right sameRows⟩
    }
    pattern_sound := by
      intro _row source
      exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr source.left)))))
    ledger_sound := by
      intro _row source
      exact ⟨source.left, scopedPkg⟩
  }
  exact ⟨cert, verdictUnary, dependencyUnary, scopedUnary⟩

end BEDC.Derived.PaperLeanDriftWitnessUp
