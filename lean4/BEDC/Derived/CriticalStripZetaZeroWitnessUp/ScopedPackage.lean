...
theorem CriticalStripZetaZeroWitnessPacket_scoped_package [AskSetup] [PackageSetup]
      ...
      Cont strip zero sourceRead ->
        Cont sourceRead line lineRead ->
          Cont line boundary boundaryRead ->
            Cont sourceRead boundaryRead consumerRead ->
              PkgSig bundle boundaryRead pkg ->
                SemanticNameCert
                    (fun row : BHist => hsame row consumerRead ∧ UnaryHistory row)
                    (fun row : BHist =>
                      hsame row sourceRead ∨ hsame row lineRead ∨
                        hsame row boundaryRead ∨ hsame row consumerRead)
                    (fun row : BHist =>
                      hsame row consumerRead ∧ PkgSig bundle boundaryRead pkg)
                    hsame ∧
                  UnaryHistory strip ∧ UnaryHistory zero ∧ UnaryHistory line ∧
                    UnaryHistory boundary ∧ UnaryHistory sourceRead ∧ UnaryHistory lineRead ∧
                      UnaryHistory boundaryRead ∧ UnaryHistory consumerRead ∧
                        Cont strip zero transport ∧ Cont strip zero sourceRead ∧
                          Cont sourceRead line lineRead ∧ Cont line boundary route ∧
                            Cont line boundary boundaryRead ∧
                              Cont sourceRead boundaryRead consumerRead ∧
                                PkgSig bundle endpoint pkg ∧ PkgSig bundle boundaryRead pkg := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro packet sourceReadRoute lineReadRoute boundaryReadRoute consumerReadRoute boundaryPkg
  ...
  have consumerReadUnary : UnaryHistory consumerRead :=
    unary_cont_closed sourceReadUnary boundaryReadUnary consumerReadRoute
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row sourceRead ∨ hsame row lineRead ∨
              hsame row boundaryRead ∨ hsame row consumerRead)
          (fun row : BHist => hsame row consumerRead ∧ PkgSig bundle boundaryRead pkg)
          hsame := {
    core := {
  ...
  exact
    ⟨cert, stripUnary, zeroUnary, lineUnary, boundaryUnary, sourceReadUnary,
      lineReadUnary, boundaryReadUnary, consumerReadUnary, stripZeroTransport,
      sourceReadRoute, lineReadRoute, lineBoundaryRoute, boundaryReadRoute,
      consumerReadRoute, endpointPkg, boundaryPkg⟩
