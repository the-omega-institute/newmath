...
theorem ContinuationMorphism_comp_left_visible_tail_empty_absurd {a b c m : BHist}
    ...
    (hsame left.tail (BHist.e0 m) ->
      hsame (ContinuationMorphism_comp_closed left right).tail BHist.Empty -> False) ∧
    (hsame left.tail (BHist.e1 m) ->
      hsame (ContinuationMorphism_comp_closed left right).tail BHist.Empty -> False) := by
  constructor
  · intro sameLeftTail compositeEmpty
    cases left with
    | mk leftTail leftRel =>
        cases right with
        | mk rightTail rightRel =>
            cases sameLeftTail
            exact not_hsame_e0_empty (append_eq_empty_iff.mp compositeEmpty).left
  · intro sameLeftTail compositeEmpty
    cases left with
    | mk leftTail leftRel =>
        cases right with
        | mk rightTail rightRel =>
            cases sameLeftTail
            exact not_hsame_e1_empty (append_eq_empty_iff.mp compositeEmpty).left
