...
theorem CategoryHomCarrier_comp_left_right_identity_public_readback
    {a b idA idB f left result : BHist} :
    CategoryHomCarrier a a idA -> CategoryHomCarrier a b f -> CategoryHomCarrier b b idB ->
      Cont idA f left -> Cont left idB result ->
        CategoryHomCarrier a b result ∧
          (∀ {g : BHist}, CategoryHomCarrier a b g -> hsame result g) := by
  intro leftIdentity homCarrier rightIdentity leftRel resultRel
  have emptyLeftIdentity : CategoryHomCarrier a a BHist.Empty :=
    CategoryHomCarrier_empty_identity leftIdentity.left
  have idAEmpty : hsame idA BHist.Empty :=
    CategoryHomCarrier_morphism_deterministic leftIdentity emptyLeftIdentity
  ...
  have leftCarrier : CategoryHomCarrier a b left := by
    cases leftSame
    exact homCarrier
  have emptyRightIdentity : CategoryHomCarrier b b BHist.Empty :=
    CategoryHomCarrier_empty_identity rightIdentity.left
  have idBEmpty : hsame idB BHist.Empty :=
    CategoryHomCarrier_morphism_deterministic rightIdentity emptyRightIdentity
  have resultSame : hsame result left := by
    cases idBEmpty
    exact cont_deterministic resultRel (cont_right_unit left)
  have resultCarrier : CategoryHomCarrier a b result := by
    cases resultSame
    exact leftCarrier
  exact And.intro resultCarrier (fun displayed =>
    CategoryHomCarrier_morphism_deterministic resultCarrier displayed)
...
theorem CategoryHomCarrier_comp_middle_identity_public_readback
    {a b c f id g fid result : BHist} :
    CategoryHomCarrier a b f -> CategoryHomCarrier b b id -> CategoryHomCarrier b c g ->
      Cont f id fid -> Cont fid g result ->
        CategoryHomCarrier a c result ∧
          (∀ {fg : BHist}, CategoryHomCarrier a c fg -> hsame result fg) := by
  intro left identity right fidRel resultRel
  have emptyIdentity : CategoryHomCarrier b b BHist.Empty :=
    CategoryHomCarrier_empty_identity identity.left
  have idEmpty : hsame id BHist.Empty :=
    CategoryHomCarrier_morphism_deterministic identity emptyIdentity
  have fidSame : hsame fid f := by
    cases idEmpty
    exact cont_deterministic fidRel (cont_right_unit f)
  have fidCarrier : CategoryHomCarrier a b fid := by
    cases fidSame
    exact left
  have resultCarrier : CategoryHomCarrier a c result :=
    CategoryHomCarrier_comp_closed fidCarrier right resultRel
  exact And.intro resultCarrier (fun displayed =>
    CategoryHomCarrier_morphism_deterministic resultCarrier displayed)
