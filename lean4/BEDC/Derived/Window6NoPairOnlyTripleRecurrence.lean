namespace BEDC.Derived.Window6NoPairOnlyTripleRecurrence

/-- Balanced five-term linear form over the current transfer state. -/
def lin (a b c d e p q r s t : Nat) : Nat := (((p * a + q * b) + (r * c + s * d)) + t * e)

/-- Nonnegative five-state transfer for no-pair-only ordered triples. -/
def c : Nat → Nat × Nat × Nat × Nat × Nat
  ...
  | n + 1 =>
      ...
      let d := (c n).2.2.2.1
      let e := (c n).2.2.2.2
      (a + b + c2 + d + e, a + c2 + d, a + b + d, a + b + c2, a)

def N (m : Nat) : Nat :=
  ((((c m).1 + (c m).2.1) + (c m).2.2.1) + (c m).2.2.2.1) + (c m).2.2.2.2

theorem N_zero : N 0 = 1 := rfl
theorem N_one : N 1 = 5 := rfl
theorem N_two : N 2 = 15 := rfl
theorem N_three : N 3 = 53 := rfl
theorem N_four : N 4 = 179 := rfl
...
theorem c_step_two (n : Nat) :
    (c (n + 1)).2.1 = ((c n).1 + (c n).2.2.1) + (c n).2.2.2.1 := rfl

theorem c_step_three (n : Nat) :
    (c (n + 1)).2.2.1 = ((c n).1 + (c n).2.1) + (c n).2.2.2.1 := rfl

theorem c_step_four (n : Nat) :
    (c (n + 1)).2.2.2.1 = ((c n).1 + (c n).2.1) + (c n).2.2.1 := rfl
...
theorem c_one_two (n : Nat) :
    (c (n + 1)).2.1 =
      lin (c n).1 (c n).2.1 (c n).2.2.1 (c n).2.2.2.1 (c n).2.2.2.2 1 0 1 1 0 := by
  rw [c_step_two n]
  unfold lin
  rw [Nat.one_mul, Nat.zero_mul, Nat.one_mul, Nat.one_mul, Nat.zero_mul,
      Nat.add_zero, Nat.add_zero, Nat.add_assoc (c n).1 (c n).2.2.1 (c n).2.2.2.1]
...
theorem c_one_three (n : Nat) :
  ...
  rw [Nat.one_mul, Nat.one_mul, Nat.zero_mul, Nat.one_mul, Nat.zero_mul,
      Nat.add_zero, Nat.zero_add (c n).2.2.2.1]
...
theorem c_two_one (n : Nat) :
    (c (n + 2)).1 =
      lin (c n).1 (c n).2.1 (c n).2.2.1 (c n).2.2.2.1 (c n).2.2.2.2 5 3 3 3 1 := by
...
theorem c_two_four (n : Nat) :
    (c (n + 2)).2.2.2.1 =
      lin (c n).1 (c n).2.1 (c n).2.2.1 (c n).2.2.2.1 (c n).2.2.2.2 3 2 2 3 1 := by
  rw [show n + 2 = (n + 1) + 1 by rfl]
...
theorem c_three_one (n : Nat) :
    (c (n + 3)).1 =
      lin (c n).1 (c n).2.1 (c n).2.2.1 (c n).2.2.2.1 (c n).2.2.2.2 15 11 11 11 5 := by
...
theorem c_three_two (n : Nat) :
    (c (n + 3)).2.1 =
      lin (c n).1 (c n).2.1 (c n).2.2.1 (c n).2.2.2.1 (c n).2.2.2.2 11 7 8 8 3 := by
  rw [show n + 3 = (n + 2) + 1 by rfl]
...
theorem c_three_three (n : Nat) :
    (c (n + 3)).2.2.1 =
      lin (c n).1 (c n).2.1 (c n).2.2.1 (c n).2.2.2.1 (c n).2.2.2.2 11 8 7 8 3 := by
  rw [show n + 3 = (n + 2) + 1 by rfl]
...
theorem c_three_four (n : Nat) :
    (c (n + 3)).2.2.2.1 =
      lin (c n).1 (c n).2.1 (c n).2.2.1 (c n).2.2.2.1 (c n).2.2.2.2 11 8 8 7 3 := by
  rw [show n + 3 = (n + 2) + 1 by rfl]
...
theorem N_three_lin (n : Nat) :
    N (n + 3) =
      lin (c n).1 (c n).2.1 (c n).2.2.1 (c n).2.2.2.1 (c n).2.2.2.2 53 37 37 37 15 := by
  unfold N
  rw [c_three_one n, c_three_two n, c_three_three n, c_three_four n, c_three_five n,
      lin_add, lin_add, lin_add, lin_add]

theorem no_pair_only_triple_recurrence (n : Nat) :
    N (n + 3) + 2 * N n = 3 * N (n + 2) + 2 * N (n + 1) := by
  rw [N_three_lin n, N_base_lin n, N_two_lin n, N_one_lin n,
      lin_two (c n).1 (c n).2.1 (c n).2.2.1 (c n).2.2.2.1 (c n).2.2.2.2 1 1 1 1 1,
      lin_add,
      lin_scale (c n).1 (c n).2.1 (c n).2.2.1 (c n).2.2.2.1 (c n).2.2.2.2 3 15 11 11 11 5,
      lin_two (c n).1 (c n).2.1 (c n).2.2.1 (c n).2.2.2.1 (c n).2.2.2.2 5 3 3 3 1,
      lin_add]

end BEDC.Derived.Window6NoPairOnlyTripleRecurrence
