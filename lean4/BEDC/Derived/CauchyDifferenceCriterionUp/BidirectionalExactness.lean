...
theorem CauchyDifferenceCriterionBidirectionalExactness [AskSetup] [PackageSetup]
                        ...
                        SemanticNameCert
                            ...
                            (fun row : BHist =>
                              hsame row X ∨ hsame row Y ∨ hsame row D ∨ hsame row Z ∨
                                hsame row Q ∨ hsame row E ∨ Cont zeroRead E sealRead)
                            (fun row : BHist =>
      ...
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row X ∨ hsame row Y ∨ hsame row D ∨ hsame row Z ∨ hsame row Q ∨
              hsame row E ∨ Cont zeroRead E sealRead)
          (fun row : BHist =>
...
theorem CauchyDifferenceCriterionNullRouteConverse [AskSetup] [PackageSetup]
    {X Y D Z Q W T E H C P N nullRead zeroRead sealRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    cauchyDifferenceCriterionFields (CauchyDifferenceCriterionUp.mk X Y D Z Q W T E H C P N) =
        [X, Y, D, Z, Q, W, T, E, H, C, P, N] ->
                  ...
                  Cont zeroRead E sealRead ->
                    PkgSig bundle P pkg ->
                      PkgSig bundle N pkg ->
                        SemanticNameCert
                            (fun row : BHist =>
                              (hsame row Z ∨ hsame row sealRead) ∧ UnaryHistory row)
                            (fun row : BHist =>
                              hsame row D ∨ hsame row Z ∨ hsame row Q ∨ hsame row E ∨
                                hsame row sealRead ∨ Cont D Z nullRead ∨
                                  Cont nullRead Q zeroRead ∨ Cont zeroRead E sealRead)
                            (fun row : BHist =>
                              UnaryHistory row ∧ Cont D Z nullRead ∧
                                Cont nullRead Q zeroRead ∧ Cont zeroRead E sealRead ∧
                                  PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
                            hsame ∧
                          UnaryHistory nullRead ∧ UnaryHistory zeroRead ∧
                            UnaryHistory sealRead := by
  -- BEDC touchpoint anchor: CauchyDifferenceCriterionUp BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro fields dUnary zUnary qUnary eUnary nullRoute zeroRoute sealRoute pPkg nPkg
  have _acceptedFields :
  ...
  have cert :
      SemanticNameCert
          (fun row : BHist => (hsame row Z ∨ hsame row sealRead) ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row D ∨ hsame row Z ∨ hsame row Q ∨ hsame row E ∨
              hsame row sealRead ∨ Cont D Z nullRead ∨ Cont nullRead Q zeroRead ∨
                Cont zeroRead E sealRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont D Z nullRead ∧ Cont nullRead Q zeroRead ∧
              Cont zeroRead E sealRead ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
    ...
    core := {
      carrier_inhabited :=
        Exists.intro Z ⟨Or.inl (hsame_refl Z), zUnary⟩
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
      ...
      carrier_respects_equiv := by
        ...
        · cases source.left with
          | inl sameZ =>
              exact Or.inl (hsame_trans (hsame_symm sameRows) sameZ)
          | inr sameSeal =>
  ...
  exact ⟨cert, nullUnary, zeroUnary, sealUnary⟩
