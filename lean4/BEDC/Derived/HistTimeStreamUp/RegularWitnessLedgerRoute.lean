...
theorem HistTimeStreamCarrier_regular_witness_ledger_route [AskSetup] [PackageSetup]
    {source schedule start replay transport provenance name streamWindow regseqRead witnessLedger
      realRead : BHist}
    ...
    HistTimeStreamCarrier source schedule start replay transport provenance name bundle pkg ->
      Cont source replay streamWindow ->
        Cont streamWindow transport regseqRead ->
          Cont regseqRead name witnessLedger ->
            Cont witnessLedger provenance realRead ->
              PkgSig bundle realRead pkg ->
                SemanticNameCert
                  ...
                  UnaryHistory streamWindow ∧ UnaryHistory regseqRead ∧
                    UnaryHistory witnessLedger ∧ UnaryHistory realRead ∧
                      PkgSig bundle provenance pkg ∧ PkgSig bundle realRead pkg := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle PkgSig SemanticNameCert hsame
  intro carrier sourceReplayWindow windowTransportRegseq regseqNameWitness
    witnessProvenanceReal realPkg
  ...
  have witnessLedgerUnary : UnaryHistory witnessLedger :=
    unary_cont_closed regseqReadUnary nameUnary regseqNameWitness
  have realReadUnary : UnaryHistory realRead :=
    unary_cont_closed witnessLedgerUnary provenanceUnary witnessProvenanceReal
  have certCore :
      NameCert
        (fun row : BHist =>
          HistTimeStreamCarrier source schedule start replay transport provenance name bundle pkg ∧
      ...
      SemanticNameCert
          ...
          (fun _row : BHist =>
            Cont source replay streamWindow ∧
              Cont streamWindow transport regseqRead ∧
                Cont regseqRead name witnessLedger ∧
                  Cont witnessLedger provenance realRead ∧ PkgSig bundle provenance pkg ∧
                    PkgSig bundle realRead pkg)
          hsame := by
    exact {
      core := certCore
      ...
      ledger_sound := by
        ...
        exact
          ⟨sourceReplayWindow, windowTransportRegseq, regseqNameWitness,
            witnessProvenanceReal, provenancePkg, realPkg⟩
    }
  exact
    ⟨semantic, streamWindowUnary, regseqReadUnary, witnessLedgerUnary, realReadUnary,
