...
theorem EquicontinuityCompactFamilyNetReadiness [AskSetup] [PackageSetup]
                ...
                (fun row : BHist =>
                  hsame row K ∨ hsame row F ∨ hsame row eps ∨ hsame row rho ∨
                    hsame row M ∨ hsame row T ∨ hsame row R ∨ hsame row P ∨
                      hsame row N ∨ hsame row finiteNetRead)
                (fun row : BHist =>
                  UnaryHistory row ∧ Cont K F radiusRead ∧
                    Cont radiusRead rho handoffRead ∧
                      Cont handoffRead M finiteNetRead ∧ PkgSig bundle finiteNetRead pkg)
                hsame ∧
  ...
  intro carrier unaryM handoffFinite finiteNetPkg
  ...
  have finiteNetUnary : UnaryHistory finiteNetRead :=
    unary_cont_closed handoffUnary unaryM handoffFinite
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row finiteNetRead ∧ UnaryHistory row)
          (fun row : BHist =>
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, radiusRoute, handoffRoute, handoffFinite, finiteNetPkg⟩
  }
