...
theorem EquicontinuityModulusHandoffAdmission [AskSetup] [PackageSetup]
                ...
                SemanticNameCert
                    ...
                    (fun row : BHist =>
                      hsame row eps ∨ hsame row rho ∨ hsame row M ∨ hsame row T ∨
                        hsame row handoffRead ∨ hsame row modulusRead ∨
                          hsame row finiteNetRead)
                    (fun row : BHist =>
                      UnaryHistory row ∧ Cont radiusRead rho handoffRead ∧
                        Cont handoffRead M modulusRead ∧
                          Cont modulusRead T finiteNetRead ∧
                            PkgSig bundle finiteNetRead pkg)
                    hsame ∧ UnaryHistory modulusRead ∧ UnaryHistory finiteNetRead := by
  ...
  obtain ⟨radiusUnary, handoffUnary, _radiusRoute, handoffRoute, _pkgP, _pkgN⟩ :=
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row eps ∨ hsame row rho ∨ hsame row M ∨ hsame row T ∨
              hsame row handoffRead ∨ hsame row modulusRead ∨ hsame row finiteNetRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont radiusRead rho handoffRead ∧
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, handoffRoute, modulusRoute, finiteNetRoute, finiteNetPkg⟩
  }
  exact ⟨cert, modulusUnary, finiteNetUnary⟩
