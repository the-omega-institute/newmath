...
theorem EquicontinuityRootUnblockFiniteNetConsumerBoundary [AskSetup] [PackageSetup]
                  ...
                  (fun row : BHist =>
                    UnaryHistory row ∧ Cont K F radiusRead ∧
                      Cont radiusRead rho handoffRead ∧ Cont handoffRead M finiteNetRead ∧
                        Cont finiteNetRead N boundaryRead ∧ PkgSig bundle P pkg ∧
                          PkgSig bundle N pkg)
                  hsame ∧
                UnaryHistory finiteNetRead ∧ UnaryHistory boundaryRead := by
  -- BEDC touchpoint anchor: EquicontinuityCarrier BHist ProbeBundle PkgSig Cont hsame SemanticNameCert UnaryHistory
  intro carrier unaryM finiteNetRoute boundaryRoute _finiteNetPkg
  obtain ⟨unaryK, unaryF, unaryRho, _unaryR, unaryN, radiusRoute, handoffRoute, pkgP,
    pkgN⟩ := carrier
  ...
  have boundaryUnary : UnaryHistory boundaryRead :=
    unary_cont_closed finiteNetUnary unaryN boundaryRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row boundaryRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row K ∨ hsame row F ∨ hsame row eps ∨ hsame row rho ∨ hsame row M ∨
              hsame row P ∨ hsame row N ∨ hsame row finiteNetRead ∨
                hsame row boundaryRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont K F radiusRead ∧ Cont radiusRead rho handoffRead ∧
              Cont handoffRead M finiteNetRead ∧ Cont finiteNetRead N boundaryRead ∧
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, radiusRoute, handoffRoute, finiteNetRoute, boundaryRoute, pkgP, pkgN⟩
  }
  exact ⟨cert, finiteNetUnary, boundaryUnary⟩
