...
theorem EquicontinuityArzelaAscoliConsumerRoute [AskSetup] [PackageSetup]
            ...
            Cont handoffRead M modulusRead ->
              Cont modulusRead T finiteNetRead ->
                Cont finiteNetRead R arzelaRead ->
                  PkgSig bundle arzelaRead pkg ->
                    SemanticNameCert
                        (fun row : BHist => hsame row arzelaRead ∧ UnaryHistory row)
                        (fun row : BHist =>
                          hsame row K ∨ hsame row F ∨ hsame row eps ∨ hsame row rho ∨
                            hsame row M ∨ hsame row T ∨ hsame row R ∨
                              hsame row finiteNetRead ∨ hsame row arzelaRead)
                        (fun row : BHist =>
                      ...
                      UnaryHistory modulusRead ∧ UnaryHistory finiteNetRead ∧
                        UnaryHistory arzelaRead := by
  -- BEDC touchpoint anchor: EquicontinuityCarrier BHist ProbeBundle Pkg Cont hsame SemanticNameCert
  intro carrier unaryM unaryT unaryR modulusRoute finiteNetRoute arzelaRoute arzelaPkg
      ...
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row K ∨ hsame row F ∨ hsame row eps ∨ hsame row rho ∨ hsame row M ∨
              hsame row T ∨ hsame row R ∨ hsame row finiteNetRead ∨
                hsame row arzelaRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont K F radiusRead ∧ Cont radiusRead rho handoffRead ∧
              Cont handoffRead M modulusRead ∧ Cont modulusRead T finiteNetRead ∧
                Cont finiteNetRead R arzelaRead ∧ PkgSig bundle arzelaRead pkg)
          hsame := {
    core := {
      carrier_inhabited :=
        Exists.intro arzelaRead ⟨hsame_refl arzelaRead, arzelaUnary⟩
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, radiusRoute, handoffRoute, modulusRoute, finiteNetRoute,
          arzelaRoute, arzelaPkg⟩
  }
  exact ⟨cert, modulusUnary, finiteNetUnary, arzelaUnary⟩
