...
theorem EquicontinuityRootObligationNonescape [AskSetup] [PackageSetup]
    {K F eps rho M T R P N radiusRead handoffRead finiteNetRead uniformRead : BHist}
                ...
                SemanticNameCert
                    ...
                    (fun row : BHist =>
                      hsame row K ∨ hsame row F ∨ hsame row eps ∨ hsame row rho ∨
                        hsame row M ∨ hsame row finiteNetRead ∨ hsame row uniformRead ∨
                    ...
                    (fun row : BHist =>
                      UnaryHistory row ∧ Cont K F radiusRead ∧
                        Cont radiusRead rho uniformRead ∧ Cont uniformRead M handoffRead ∧
                          Cont handoffRead M finiteNetRead ∧
                            PkgSig bundle finiteNetRead pkg)
                    hsame ∧ UnaryHistory finiteNetRead ∧ UnaryHistory uniformRead := by
  -- BEDC touchpoint anchor: EquicontinuityCarrier BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier unaryM finiteNetRoute radiusUniform uniformHandoff finiteNetPkg
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row K ∨ hsame row F ∨ hsame row eps ∨ hsame row rho ∨ hsame row M ∨
              hsame row finiteNetRead ∨ hsame row uniformRead ∨ hsame row handoffRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont K F radiusRead ∧ Cont radiusRead rho uniformRead ∧
              Cont uniformRead M handoffRead ∧ Cont handoffRead M finiteNetRead ∧
                PkgSig bundle finiteNetRead pkg)
          hsame := {
    core := {
      carrier_inhabited :=
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, radiusRoute, radiusUniform, uniformHandoff, finiteNetRoute,
          finiteNetPkg⟩
  }
  exact ⟨cert, finiteNetUnary, uniformUnary⟩
