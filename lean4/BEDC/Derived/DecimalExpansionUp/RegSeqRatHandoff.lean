...
theorem DecimalExpansionRegSeqRatHandoff [AskSetup] [PackageSetup]
  ...
  intro dUnary wUnary vUnary qUnary rUnary eUnary digitWindow prefixPlace
    placeDyadic dyadicRegSeq regSeqSeal provenancePkg namePkg
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont placeRead Q dyadicRead ∧
              Cont dyadicRead R regseqRead ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
          hsame := {
    core := {
      carrier_inhabited :=
        Exists.intro regseqRead ⟨hsame_refl regseqRead, regseqUnary⟩
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, placeDyadic, dyadicRegSeq, provenancePkg, namePkg⟩
  }
  exact ⟨cert, prefixUnary, placeUnary, dyadicUnary, regseqUnary, sealUnary⟩
