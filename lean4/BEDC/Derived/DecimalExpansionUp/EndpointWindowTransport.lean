...
theorem DecimalExpansionEndpointWindowTransport [AskSetup] [PackageSetup]
    {D W V Q R E H C P N prefixRead placeRead dyadicRead regseqRead realSeal
      transportedWindow : BHist} {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    UnaryHistory D →
      UnaryHistory W →
        UnaryHistory V →
          UnaryHistory Q →
                    ...
                    Cont placeRead Q dyadicRead →
                      Cont dyadicRead R regseqRead →
                        Cont regseqRead E realSeal →
                          hsame transportedWindow prefixRead →
                            PkgSig bundle P pkg →
                              PkgSig bundle transportedWindow pkg →
                                SemanticNameCert
                                    (fun row : BHist =>
                                      hsame row transportedWindow ∧ UnaryHistory row)
                                    (fun row : BHist =>
                                      hsame row D ∨ hsame row W ∨ hsame row prefixRead ∨
                                        hsame row transportedWindow ∨ hsame row realSeal)
                                    (fun row : BHist =>
                                      UnaryHistory row ∧ Cont D W prefixRead ∧
                                        Cont regseqRead E realSeal ∧ PkgSig bundle P pkg ∧
                                          PkgSig bundle transportedWindow pkg)
                                    hsame ∧
                                  UnaryHistory transportedWindow ∧ UnaryHistory prefixRead ∧
  ...
  intro dUnary wUnary vUnary qUnary rUnary eUnary prefixRoute placeRoute dyadicRoute
    regseqRoute realRoute transportedSamePrefix provenancePkg transportedPkg
  ...
  have transportedUnary : UnaryHistory transportedWindow :=
    unary_transport_symm prefixUnary transportedSamePrefix
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row transportedWindow ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row D ∨ hsame row W ∨ hsame row prefixRead ∨
              hsame row transportedWindow ∨ hsame row realSeal)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont D W prefixRead ∧ Cont regseqRead E realSeal ∧
              PkgSig bundle P pkg ∧ PkgSig bundle transportedWindow pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro transportedWindow
        ⟨hsame_refl transportedWindow, transportedUnary⟩
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, prefixRoute, realRoute, provenancePkg, transportedPkg⟩
  }
  exact
    ⟨cert, transportedUnary, prefixUnary, placeUnary, dyadicUnary, regseqUnary, realUnary⟩
