...
theorem DecimalExpansionPrefixDensityExhaustion [AskSetup] [PackageSetup]
                                                ...
                                                hsame row D ∨ hsame row W ∨ hsame row V ∨
                                                  hsame row Q ∨ hsame row R ∨ hsame row E ∨
                                                    hsame row H ∨ hsame row C ∨ hsame row P ∨
                                                      hsame row N ∨ hsame row densityRead)
                                              (fun row : BHist =>
  ...
  intro prefixRoute placeRoute toleranceRoute readbackRoute sealRoute densityRoute namedRoute
  ...
  have cert :
          ...
          (fun row : BHist =>
            hsame row D ∨ hsame row W ∨ hsame row V ∨ hsame row Q ∨
              hsame row R ∨ hsame row E ∨ hsame row H ∨ hsame row C ∨
                hsame row P ∨ hsame row N ∨ hsame row densityRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont D W prefixRead ∧ Cont prefixRead V placeRead ∧
              Cont placeRead Q toleranceRead ∧ Cont toleranceRead R readbackRead ∧
                Cont readbackRead E sealRead ∧ Cont sealRead H densityRead ∧
                  Cont densityRead C namedRead ∧ PkgSig bundle P pkg ∧
                    PkgSig bundle N pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro row source
      exact
        ⟨source.right, prefixRoute, placeRoute, toleranceRoute, readbackRoute,
          sealRoute, densityRoute, namedRoute, provenancePkg, namePkg⟩
  }
  exact
    ⟨cert, prefixUnary, placeUnary, toleranceUnary, readbackUnary, sealUnary,
      densityUnary, namedUnary⟩
