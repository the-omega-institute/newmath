...
theorem CauchyCompletionMultiplicationCarrier_real_seal_nonescape [AskSetup] [PackageSetup]
    {M O I A S R D E H C P N flattenRead streamRead dyadicRead sealRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    CauchyCompletionMultiplicationCarrier M O I A S R D E H C P N bundle pkg ->
      Cont M O flattenRead ->
        Cont S R streamRead ->
              ...
              PkgSig bundle sealRead pkg ->
                SemanticNameCert
                  ...
                  UnaryHistory S ∧ UnaryHistory R ∧ UnaryHistory D ∧ UnaryHistory E ∧
                    UnaryHistory streamRead ∧ UnaryHistory dyadicRead ∧
                      UnaryHistory sealRead ∧ Cont S R streamRead ∧
                        Cont streamRead D dyadicRead ∧ Cont dyadicRead E sealRead ∧
                          PkgSig bundle sealRead pkg := by
  -- BEDC touchpoint anchor: BHist Cont hsame SemanticNameCert ProbeBundle Pkg PkgSig
  intro carrier _flattenRoute streamRoute dyadicRoute sealRoute sealPkg
  obtain ⟨_mUnary, _oUnary, _iUnary, _aUnary, sUnary, rUnary, dUnary, eUnary,
  ...
  exact
    ⟨cert, sUnary, rUnary, dUnary, eUnary, streamUnary, dyadicUnary, sealUnary,
      streamRoute, dyadicRoute, sealRoute, sealPkg⟩
