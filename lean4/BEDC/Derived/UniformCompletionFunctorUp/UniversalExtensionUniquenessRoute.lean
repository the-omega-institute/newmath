...
theorem UniformCompletionFunctorUniversalExtensionUniquenessRoute [AskSetup] [PackageSetup]
  ...
  obtain ⟨unaryU, unaryF, unaryE, unaryR, unaryW, unaryD, unaryS, _unaryH, _unaryC,
    _unaryP, _unaryN, _carrierSourceRoute, _carrierReadbackRoute, _carrierSealRoute,
      provenancePkg, localNamePkg⟩ := carrier
  have sourceHandoffUnary : UnaryHistory sourceHandoff :=
    unary_cont_closed unaryU unaryF sourceRoute
  have extensionUnary : UnaryHistory extensionRead :=
    unary_cont_closed sourceHandoffUnary unaryE extensionRoute
      ...
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row U ∨ hsame row F ∨ hsame row E ∨ hsame row R ∨ hsame row W ∨
              hsame row D ∨ hsame row S ∨ hsame row sealRead)
          (fun row : BHist =>
            UnaryHistory row ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
          hsame := {
    core := {
  ...
  exact
    ⟨cert, sourceHandoffUnary, extensionUnary, regseqUnary, windowUnary, dyadicUnary,
      sealUnary⟩
