...
theorem UniformCompletionFunctorCauchyFilterNonescape [AskSetup] [PackageSetup]
    ...
    UniformCompletionFunctorCarrier U F E R W D S H C P N bundle pkg ->
      Cont U F sourceRead -> Cont sourceRead E extensionRead ->
              ...
              SemanticNameCert
                  ...
                  (fun row : BHist =>
                    hsame row U ∨ hsame row F ∨ hsame row E ∨ hsame row R ∨
                      hsame row W ∨ hsame row D ∨ hsame row S ∨ hsame row sealRead)
                  (fun row : BHist =>
                    UnaryHistory row ∧ Cont U F sourceRead ∧
                      Cont sourceRead E extensionRead ∧ Cont extensionRead R regseqRead ∧
                        Cont regseqRead W windowRead ∧ Cont windowRead D dyadicRead ∧
                          Cont dyadicRead S sealRead ∧ PkgSig bundle P pkg)
                  hsame ∧
  ...
  obtain ⟨unaryU, unaryF, unaryE, unaryR, unaryW, unaryD, unaryS, _unaryH,
    _unaryC, _unaryP, _unaryN, _carrierSourceRoute, _carrierReadbackRoute,
      _carrierSealRoute, _carrierProvenance, _localNamePkg⟩ := carrier
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, sourceRoute, extensionRoute, regseqRoute, windowRoute, dyadicRoute,
          sealRoute, provenancePkg⟩
  }
  exact
    ⟨cert, sourceUnary, extensionUnary, regseqUnary, windowUnary, dyadicUnary, sealUnary⟩
