...
theorem UniformCompletionFunctorReadbackSealExactness [AskSetup] [PackageSetup]
    {U F E R W D S H C P N sourceRead extensionRead regseqRead windowRead sealRead :
      BHist}
              ...
              Cont windowRead S sealRead →
                PkgSig bundle sealRead pkg →
                  SemanticNameCert
                      ...
                      (fun row : BHist =>
                        hsame row R ∨ hsame row W ∨ hsame row S ∨ hsame row sealRead)
                      (fun row : BHist =>
                        UnaryHistory row ∧ Cont extensionRead R regseqRead ∧
                          Cont regseqRead W windowRead ∧ Cont windowRead S sealRead ∧
                            PkgSig bundle sealRead pkg)
                      hsame ∧
                    UnaryHistory regseqRead ∧ UnaryHistory windowRead ∧
                      UnaryHistory sealRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro carrier sourceRoute extensionRoute regseqRoute windowRoute sealRoute sealPkg
  obtain ⟨unaryU, unaryF, unaryE, unaryR, unaryW, _unaryD, unaryS, _unaryH,
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, regseqRoute, windowRoute, sealRoute, sealPkg⟩
  }
  exact ⟨cert, regseqUnary, windowUnary, sealUnary⟩
