...
theorem ParsevalRealSealNonescape [AskSetup] [PackageSetup]
    {fourier source pairing integral readback tolerance sealRow transport replay provenance name
      coefficientRead energyRead sealRead refusalRead : BHist}
    ...
    ParsevalCarrier fourier source pairing integral readback tolerance sealRow transport replay
      ...
      Cont fourier source coefficientRead →
        Cont pairing integral energyRead →
          Cont coefficientRead energyRead sealRead →
            Cont sealRead transport refusalRead →
                    ...
                    (fun row : BHist =>
                      hsame row fourier ∨ hsame row source ∨ hsame row pairing ∨
                        hsame row integral ∨ hsame row coefficientRead ∨
                          hsame row energyRead ∨ hsame row sealRead ∨
  ...
  intro carrier coefficientRoute energyRoute sealRoute refusalRoute refusalPkg
  obtain ⟨fourierUnary, sourceUnary, pairingUnary, integralUnary, _readbackUnary,
    _toleranceUnary, _sealUnary, transportUnary, _replayUnary, _provenanceUnary,
  ...
  have sealReadUnary : UnaryHistory sealRead :=
    unary_cont_closed coefficientUnary energyUnary sealRoute
  have refusalUnary : UnaryHistory refusalRead :=
    unary_cont_closed sealReadUnary transportUnary refusalRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row refusalRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row fourier ∨ hsame row source ∨ hsame row pairing ∨
              hsame row integral ∨ hsame row coefficientRead ∨
                hsame row energyRead ∨ hsame row sealRead ∨ hsame row refusalRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont fourier source coefficientRead ∧
              Cont pairing integral energyRead ∧ Cont coefficientRead energyRead sealRead ∧
                Cont sealRead transport refusalRead ∧ PkgSig bundle provenance pkg ∧
                  PkgSig bundle refusalRead pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, coefficientRoute, energyRoute, sealRoute, refusalRoute,
          provenancePkg, refusalPkg⟩
  }
  exact ⟨cert, coefficientUnary, energyUnary, sealReadUnary, refusalUnary⟩
