...
theorem ParsevalFiniteEnergyInductionReadback [AskSetup] [PackageSetup]
    {fourier source pairing integral readback tolerance sealRow transport replay provenance name
      baseWindow stepWindow partialLedger energyRead : BHist}
      ...
      Cont BHist.Empty fourier baseWindow →
        Cont baseWindow source stepWindow →
          Cont stepWindow tolerance partialLedger →
            Cont partialLedger sealRow energyRead →
              PkgSig bundle energyRead pkg →
                SemanticNameCert
                    (fun row : BHist => hsame row energyRead ∧ UnaryHistory row)
                    (fun row : BHist =>
                      hsame row fourier ∨ hsame row source ∨ hsame row partialLedger ∨
                        hsame row sealRow ∨ hsame row energyRead)
                    (fun row : BHist =>
                      UnaryHistory row ∧ Cont partialLedger sealRow energyRead ∧
                        PkgSig bundle energyRead pkg)
                    hsame ∧ UnaryHistory energyRead ∧
                  Cont partialLedger sealRow energyRead := by
  -- BEDC touchpoint anchor: ParsevalCarrier BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier baseRoute stepRoute partialRoute energyRoute energyPkg
  ...
  have energyUnary : UnaryHistory energyRead :=
    unary_cont_closed partialUnary sealUnary energyRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row energyRead ∧ UnaryHistory row)
          (fun row : BHist =>
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, energyRoute, energyPkg⟩
  }
  exact ⟨cert, energyUnary, energyRoute⟩
