...
theorem ParsevalScopedEnergyRoute [AskSetup] [PackageSetup]
    {F S I E R D L H C P N coefficientRead energyRead toleranceRead sealRead
      scopedRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    ParsevalCarrierSurface F S I E R D L H C P N bundle pkg ->
      Cont F S coefficientRead ->
              ...
              Cont sealRead C scopedRead ->
                PkgSig bundle scopedRead pkg ->
                  SemanticNameCert
                      ...
                      (fun row : BHist =>
                        hsame row F ∨ hsame row S ∨ hsame row I ∨ hsame row E ∨
                          hsame row D ∨ hsame row L ∨ hsame row C ∨
  ...
  -- BEDC touchpoint anchor: ParsevalCarrierSurface BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier coefficientRoute energyRoute toleranceRoute sealRoute scopedRoute scopedPkg
  obtain ⟨fourierUnary, sourceUnary, pairingUnary, integralUnary, _readbackUnary,
    toleranceUnary, sealUnary, _transportUnary, replayUnary, _provenanceUnary,
    _localNameUnary, provenancePkg, _localNamePkg⟩ := carrier
  have coefficientUnary : UnaryHistory coefficientRead :=
    unary_cont_closed fourierUnary sourceUnary coefficientRoute
  have energyUnary : UnaryHistory energyRead :=
  ...
  have scopedReadUnary : UnaryHistory scopedRead :=
    unary_cont_closed sealReadUnary replayUnary scopedRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row scopedRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row F ∨ hsame row S ∨ hsame row I ∨ hsame row E ∨ hsame row D ∨
              hsame row L ∨ hsame row C ∨ hsame row scopedRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont F S coefficientRead ∧ Cont I E energyRead ∧
              Cont energyRead D toleranceRead ∧ Cont toleranceRead L sealRead ∧
                Cont sealRead C scopedRead ∧ PkgSig bundle P pkg ∧
                  PkgSig bundle scopedRead pkg)
          hsame := {
    core := {
  ...
  exact
    ⟨cert, coefficientUnary, energyUnary, toleranceReadUnary, sealReadUnary,
      scopedReadUnary⟩
