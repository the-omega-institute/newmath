...
theorem ParsevalL2EnergyIdentityRoute [AskSetup] [PackageSetup]
        ...
        Cont pairing integral energyRead →
          Cont energyRead tolerance toleranceRead →
            Cont toleranceRead sealRow sealRead →
              Cont sealRead replay namedRead →
                PkgSig bundle namedRead pkg →
                  SemanticNameCert
                      (fun row : BHist => hsame row namedRead ∧ UnaryHistory row)
                      (fun row : BHist =>
                        hsame row fourier ∨ hsame row source ∨ hsame row pairing ∨
                          hsame row integral ∨ hsame row energyRead ∨
  ...
  obtain ⟨fourierUnary, sourceUnary, pairingUnary, integralUnary, _readbackUnary,
    toleranceUnary, sealUnary, _transportUnary, replayUnary, _provenanceUnary, _nameUnary,
  ...
  have namedUnary : UnaryHistory namedRead :=
    unary_cont_closed sealReadUnary replayUnary namedRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row namedRead ∧ UnaryHistory row)
          (fun row : BHist =>
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont fourier source coefficientRead ∧
              Cont pairing integral energyRead ∧ Cont energyRead tolerance toleranceRead ∧
                Cont toleranceRead sealRow sealRead ∧ Cont sealRead replay namedRead ∧
  ...
  exact ⟨cert, coefficientUnary, energyUnary, toleranceReadUnary, sealReadUnary, namedUnary⟩
