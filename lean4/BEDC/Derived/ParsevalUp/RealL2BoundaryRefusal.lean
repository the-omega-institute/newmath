import BEDC.Derived.ParsevalUp.RootEnergyCarrierAdmission
import BEDC.FKernel.NameCert

namespace BEDC.Derived.ParsevalUp

open BEDC.FKernel.Ask
open BEDC.FKernel.Bundle
open BEDC.FKernel.Cont
open BEDC.FKernel.Hist
open BEDC.FKernel.NameCert
open BEDC.FKernel.Package
open BEDC.FKernel.Unary

theorem ParsevalRealL2BoundaryRefusal [AskSetup] [PackageSetup]
    {fourier source pairing integral readback tolerance sealRow transport replay provenance name
      coefficientRead energyRead l2Read sealRead refusalRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    ParsevalCarrier fourier source pairing integral readback tolerance sealRow transport replay
        provenance name bundle pkg →
      Cont fourier source coefficientRead →
        Cont pairing integral energyRead →
          Cont coefficientRead energyRead l2Read →
            Cont l2Read sealRow sealRead →
              Cont sealRead transport refusalRead →
                PkgSig bundle refusalRead pkg →
                  SemanticNameCert
                      (fun row : BHist => hsame row refusalRead ∧ UnaryHistory row)
                      (fun row : BHist =>
                        hsame row fourier ∨ hsame row source ∨ hsame row pairing ∨
                          hsame row integral ∨ hsame row coefficientRead ∨
                            hsame row energyRead ∨ hsame row l2Read ∨
                              hsame row sealRead ∨ hsame row refusalRead)
                      (fun row : BHist =>
                        UnaryHistory row ∧ Cont fourier source coefficientRead ∧
                          Cont pairing integral energyRead ∧
                            Cont coefficientRead energyRead l2Read ∧
                              Cont l2Read sealRow sealRead ∧
                                Cont sealRead transport refusalRead ∧
                                  PkgSig bundle provenance pkg ∧
                                    PkgSig bundle refusalRead pkg)
                      hsame ∧
                    UnaryHistory coefficientRead ∧ UnaryHistory energyRead ∧
                      UnaryHistory l2Read ∧ UnaryHistory sealRead ∧
                        UnaryHistory refusalRead := by
  -- BEDC touchpoint anchor: ParsevalCarrier BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier coefficientRoute energyRoute l2Route sealRoute refusalRoute refusalPkg
  obtain ⟨fourierUnary, sourceUnary, pairingUnary, integralUnary, _readbackUnary,
    _toleranceUnary, sealUnary, transportUnary, _replayUnary, _provenanceUnary, _nameUnary,
    _fourierSourcePairing, _pairingIntegralReadback, _readbackToleranceSeal,
    _sealTransportReplay, provenancePkg, _namePkg⟩ := carrier
  have coefficientUnary : UnaryHistory coefficientRead :=
    unary_cont_closed fourierUnary sourceUnary coefficientRoute
  have energyUnary : UnaryHistory energyRead :=
    unary_cont_closed pairingUnary integralUnary energyRoute
  have l2Unary : UnaryHistory l2Read :=
    unary_cont_closed coefficientUnary energyUnary l2Route
  have sealReadUnary : UnaryHistory sealRead :=
    unary_cont_closed l2Unary sealUnary sealRoute
  have refusalUnary : UnaryHistory refusalRead :=
    unary_cont_closed sealReadUnary transportUnary refusalRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row refusalRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row fourier ∨ hsame row source ∨ hsame row pairing ∨
              hsame row integral ∨ hsame row coefficientRead ∨
                hsame row energyRead ∨ hsame row l2Read ∨ hsame row sealRead ∨
                  hsame row refusalRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont fourier source coefficientRead ∧
              Cont pairing integral energyRead ∧ Cont coefficientRead energyRead l2Read ∧
                Cont l2Read sealRow sealRead ∧ Cont sealRead transport refusalRead ∧
                  PkgSig bundle provenance pkg ∧ PkgSig bundle refusalRead pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro refusalRead ⟨hsame_refl refusalRead, refusalUnary⟩
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
        intro _row _other sameRows
        exact hsame_symm sameRows
      equiv_trans := by
        intro _row _middle _other sameLeft sameRight
        exact hsame_trans sameLeft sameRight
      carrier_respects_equiv := by
        intro _row _other sameRows source
        exact
          ⟨hsame_trans (hsame_symm sameRows) source.left,
            unary_transport source.right sameRows⟩
    }
    pattern_sound := by
      intro _row source
      exact
        Or.inr
          (Or.inr
            (Or.inr
              (Or.inr
                (Or.inr
                  (Or.inr
                    (Or.inr
                      (Or.inr source.left)))))))
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, coefficientRoute, energyRoute, l2Route, sealRoute,
          refusalRoute, provenancePkg, refusalPkg⟩
  }
  exact ⟨cert, coefficientUnary, energyUnary, l2Unary, sealReadUnary, refusalUnary⟩

end BEDC.Derived.ParsevalUp
