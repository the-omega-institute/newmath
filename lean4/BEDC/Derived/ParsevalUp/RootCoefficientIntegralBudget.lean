...
theorem ParsevalRootCoefficientIntegralBudget [AskSetup] [PackageSetup]
    {fourier source pairing integral readback tolerance sealRow transport replay provenance name
      coefficientRead integralRead budgetRead rootRead : BHist}
    ...
    ParsevalCarrier fourier source pairing integral readback tolerance sealRow transport replay
      ...
      Cont fourier source coefficientRead →
        Cont pairing integral integralRead →
          Cont integralRead readback budgetRead →
            Cont budgetRead sealRow rootRead →
              PkgSig bundle rootRead pkg →
                SemanticNameCert
                    (fun row : BHist => hsame row rootRead ∧ UnaryHistory row)
                    (fun row : BHist =>
                      hsame row fourier ∨ hsame row source ∨ hsame row pairing ∨
                        hsame row integral ∨ hsame row integralRead ∨
                          hsame row budgetRead ∨ hsame row rootRead)
                    (fun row : BHist =>
                      UnaryHistory row ∧ Cont fourier source coefficientRead ∧
                        Cont pairing integral integralRead ∧
                          Cont integralRead readback budgetRead ∧
                            Cont budgetRead sealRow rootRead ∧ PkgSig bundle rootRead pkg)
                    hsame ∧ UnaryHistory coefficientRead ∧ UnaryHistory integralRead ∧
                  UnaryHistory budgetRead ∧ UnaryHistory rootRead := by
  -- BEDC touchpoint anchor: ParsevalCarrier BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier coefficientRoute integralRoute budgetRoute rootRoute rootPkg
  ...
  have budgetUnary : UnaryHistory budgetRead :=
    unary_cont_closed integralReadUnary readbackUnary budgetRoute
  have rootUnary : UnaryHistory rootRead :=
    unary_cont_closed budgetUnary sealUnary rootRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row rootRead ∧ UnaryHistory row)
          (fun row : BHist =>
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont fourier source coefficientRead ∧
              Cont pairing integral integralRead ∧ Cont integralRead readback budgetRead ∧
                Cont budgetRead sealRow rootRead ∧ PkgSig bundle rootRead pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, coefficientRoute, integralRoute, budgetRoute, rootRoute, rootPkg⟩
  }
  exact ⟨cert, coefficientUnary, integralReadUnary, budgetUnary, rootUnary⟩
