...
theorem CauchyWitnessLedgerCarrier_streamname_regseq_factorization [AskSetup] [PackageSetup]
    {Q B S K H C P N streamWindow regSeqRead endpoint : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    Cont Q B S →
      Cont B H streamWindow →
        Cont streamWindow S regSeqRead →
          Cont regSeqRead K endpoint →
              ...
              SemanticNameCert
                ...
                (fun row : BHist =>
                  Cont Q B S ∧ Cont B H streamWindow ∧
                    Cont streamWindow S regSeqRead ∧ Cont regSeqRead K endpoint ∧
                      hsame row endpoint)
                (fun row : BHist => hsame row endpoint ∧ PkgSig bundle endpoint pkg)
                hsame := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont SemanticNameCert hsame
  intro budgetRoute streamRoute regSeqRoute endpointRoute endpointPkg
  exact {
    core := {
      carrier_inhabited :=
    ...
    pattern_sound := by
      intro _row source
      exact ⟨budgetRoute, streamRoute, regSeqRoute, endpointRoute, source.left⟩
    ledger_sound := by
      intro _row source
