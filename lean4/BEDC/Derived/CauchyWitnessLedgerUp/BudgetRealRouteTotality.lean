...
theorem CauchyWitnessLedgerCarrier_budget_real_route_totality [AskSetup] [PackageSetup]
    {Q B S K H C P N _budgetRead realRead : BHist}
              ...
              hsame N realRead →
                PkgSig bundle realRead pkg →
                  SemanticNameCert
                    ...
                    (fun row : BHist =>
                      Cont Q B S ∧ Cont S K realRead ∧ hsame row realRead)
                    (fun row : BHist => hsame row realRead ∧ PkgSig bundle realRead pkg)
                    hsame := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont SemanticNameCert hsame
  intro routeBudget routeReal _sameH _sameC _sameP _sameN realPkg
  exact {
    core := {
      carrier_inhabited :=
    ...
    pattern_sound := by
      intro _row source
      exact ⟨routeBudget, routeReal, source.left⟩
    ledger_sound := by
      intro _row source
