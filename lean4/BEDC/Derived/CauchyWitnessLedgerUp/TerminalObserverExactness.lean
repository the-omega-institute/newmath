...
theorem CauchyWitnessLedgerCarrier_terminal_observer_exactness [AskSetup] [PackageSetup]
    {Q B S K H C P N streamWindow dyadicRead regSeqRead realRead terminalRead : BHist}
  ...
  intro routeS routeWindow routeDyadic routeRegSeq routeReal routeTerminal terminalPkg
  exact {
    core := {
      carrier_inhabited :=
    ...
    pattern_sound := by
      intro _row source
      exact
        ⟨routeS, routeWindow, routeDyadic, routeRegSeq, routeReal, routeTerminal,
          source.left⟩
    ledger_sound := by
      intro _row source
