import BEDC.Derived.CauchyWitnessLedgerUp.TasteGate

namespace BEDC.Derived.CauchyWitnessLedgerUp

open BEDC.FKernel.Ask
open BEDC.FKernel.Bundle
open BEDC.FKernel.Cont
open BEDC.FKernel.Hist
open BEDC.FKernel.NameCert
open BEDC.FKernel.Package

theorem CauchyWitnessLedgerCarrier_budget_factorization [AskSetup] [PackageSetup]
    {Q B S K H C P N budgetRead observationRead sealRead classifierRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    Cont Q B observationRead ->
      Cont observationRead S sealRead ->
        Cont sealRead K classifierRead ->
          PkgSig bundle classifierRead pkg ->
            SemanticNameCert
              (fun row : BHist =>
                hsame row classifierRead ∧
                  ∃ packet : CauchyWitnessLedgerUp,
                    packet = CauchyWitnessLedgerUp.mk Q B S K H C P N)
              (fun row : BHist =>
                Cont Q B observationRead ∧ Cont observationRead S sealRead ∧
                  Cont sealRead K classifierRead ∧ hsame row classifierRead)
              (fun row : BHist => hsame row classifierRead ∧ PkgSig bundle classifierRead pkg)
              hsame := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont SemanticNameCert hsame
  intro routeObservation routeSeal routeClassifier classifierPkg
  exact {
    core := {
      carrier_inhabited :=
        Exists.intro classifierRead
          ⟨hsame_refl classifierRead,
            Exists.intro (CauchyWitnessLedgerUp.mk Q B S K H C P N) rfl⟩
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
        intro _row _other same
        exact hsame_symm same
      equiv_trans := by
        intro _row _middle _other sameLeft sameRight
        exact hsame_trans sameLeft sameRight
      carrier_respects_equiv := by
        intro _row _other same source
        exact
          ⟨hsame_trans (hsame_symm same) source.left,
            source.right⟩
    }
    pattern_sound := by
      intro _row source
      exact ⟨routeObservation, routeSeal, routeClassifier, source.left⟩
    ledger_sound := by
      intro _row source
      exact ⟨source.left, classifierPkg⟩
  }

end BEDC.Derived.CauchyWitnessLedgerUp
