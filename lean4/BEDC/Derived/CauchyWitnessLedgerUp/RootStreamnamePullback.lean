...
theorem CauchyWitnessLedgerCarrier_root_streamname_pullback [AskSetup] [PackageSetup]
                    ...
                    (fun row : BHist =>
                      Cont dyadicRoute streamRoute regSeqRoute ∧
                        Cont regSeqRoute realRoute endpoint ∧ Cont Q B S ∧
                          Cont S K endpoint ∧ Cont endpoint N consumer ∧ hsame row consumer)
  ...
  exact {
    ...
    pattern_sound := by
      intro _row source
      exact
        ⟨dyadicStreamReg, regRealEndpoint, budgetRoute, internalRoute, consumerRoute,
          source.left⟩
    ledger_sound := by
      intro _row source
