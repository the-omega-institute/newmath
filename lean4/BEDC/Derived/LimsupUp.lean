...
theorem LimsupTailSealScope [AskSetup] [PackageSetup]
    {S U D T H C P N upperRead lowerRead sealRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    LimsupCarrier S U D T H C P N bundle pkg →
      Cont S U upperRead →
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont S U upperRead ∧ Cont upperRead D lowerRead ∧
              Cont lowerRead T sealRead ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg ∧
                PkgSig bundle sealRead pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro sealRead ⟨hsame_refl sealRead, sealUnary⟩
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, upperRoute, lowerRoute, sealRoute, provenancePkg, namePkg, sealPkg⟩
  }
  exact ⟨cert, upperUnary, lowerUnary, sealUnary⟩
