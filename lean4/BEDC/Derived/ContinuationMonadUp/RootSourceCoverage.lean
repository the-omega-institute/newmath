...
theorem ContinuationMonadCarrier_root_source_coverage [AskSetup] [PackageSetup]
    {A B C f g u H K L N rootRead : BHist}
        ...
        PkgSig bundle rootRead pkg →
          UnaryHistory A ∧ UnaryHistory B ∧ UnaryHistory C ∧ UnaryHistory f ∧
            UnaryHistory g ∧ UnaryHistory u ∧ UnaryHistory K ∧ UnaryHistory L ∧
              UnaryHistory N ∧ UnaryHistory rootRead ∧ Cont A f B ∧ Cont B g C ∧
                Cont f g K ∧ Cont K u L ∧ Cont L N rootRead ∧ hsame N L ∧
                  PkgSig bundle rootRead pkg ∧
                    SemanticNameCert
                      (fun row : BHist => hsame row rootRead ∧ UnaryHistory row)
                      (fun row : BHist => hsame row rootRead)
                      (fun row : BHist =>
                        hsame row rootRead ∧ PkgSig bundle rootRead pkg)
                      hsame := by
  ...
  have rootUnary : UnaryHistory rootRead := unary_cont_closed unaryL unaryN rootRoute
  have cert :
      SemanticNameCert
        (fun row : BHist => hsame row rootRead ∧ UnaryHistory row)
        (fun row : BHist => hsame row rootRead)
  ...
  exact
    ⟨unaryA, unaryB, unaryC, unaryF, unaryG, unaryU, unaryK, unaryL, unaryN,
      rootUnary, routeB, routeC, routeK, routeL, rootRoute, sameEndpoint, rootPkg, cert⟩
