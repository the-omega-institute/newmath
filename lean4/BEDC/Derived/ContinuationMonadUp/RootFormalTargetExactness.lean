import BEDC.Derived.ContinuationMonadUp

namespace BEDC.Derived.ContinuationMonadUp

open BEDC.FKernel.Ask
open BEDC.FKernel.Bundle
open BEDC.FKernel.Cont
open BEDC.FKernel.Hist
open BEDC.FKernel.Package
open BEDC.FKernel.Unary

theorem ContinuationMonadCarrier_root_formal_target_exactness [AskSetup] [PackageSetup]
    {A B C f g u H K L N category generator publicRead formal : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    ContinuationMonadCarrier A B C f g u H K L N ->
      Cont L N category ->
        Cont category K generator ->
          Cont generator N publicRead ->
            Cont publicRead N formal ->
              PkgSig bundle formal pkg ->
                UnaryHistory A ∧ UnaryHistory B ∧ UnaryHistory C ∧ UnaryHistory f ∧
                  UnaryHistory g ∧ UnaryHistory u ∧ UnaryHistory K ∧ UnaryHistory L ∧
                    UnaryHistory N ∧ UnaryHistory category ∧ UnaryHistory generator ∧
                      UnaryHistory publicRead ∧ UnaryHistory formal ∧ Cont A f B ∧
                        Cont B g C ∧ Cont f g K ∧ Cont K u L ∧ Cont L N category ∧
                          Cont category K generator ∧ Cont generator N publicRead ∧
                            Cont publicRead N formal ∧ hsame N L ∧
                              PkgSig bundle formal pkg := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont UnaryHistory hsame
  intro carrier categoryRoute generatorRoute publicRoute formalRoute formalPkg
  obtain ⟨unaryA, unaryF, unaryG, unaryU, routeB, routeC, routeK, routeL,
    sameEndpoint⟩ := carrier
  have unaryB : UnaryHistory B :=
    unary_cont_closed unaryA unaryF routeB
  have unaryC : UnaryHistory C :=
    unary_cont_closed unaryB unaryG routeC
  have unaryK : UnaryHistory K :=
    unary_cont_closed unaryF unaryG routeK
  have unaryL : UnaryHistory L :=
    unary_cont_closed unaryK unaryU routeL
  have unaryN : UnaryHistory N :=
    unary_transport unaryL (hsame_symm sameEndpoint)
  have categoryUnary : UnaryHistory category :=
    unary_cont_closed unaryL unaryN categoryRoute
  have generatorUnary : UnaryHistory generator :=
    unary_cont_closed categoryUnary unaryK generatorRoute
  have publicReadUnary : UnaryHistory publicRead :=
    unary_cont_closed generatorUnary unaryN publicRoute
  have formalUnary : UnaryHistory formal :=
    unary_cont_closed publicReadUnary unaryN formalRoute
  exact
    ⟨unaryA, unaryB, unaryC, unaryF, unaryG, unaryU, unaryK, unaryL, unaryN,
      categoryUnary, generatorUnary, publicReadUnary, formalUnary, routeB, routeC, routeK,
      routeL, categoryRoute, generatorRoute, publicRoute, formalRoute, sameEndpoint,
      formalPkg⟩

end BEDC.Derived.ContinuationMonadUp
