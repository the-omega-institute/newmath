...
theorem ContinuationMonadCarrier_root_kleisli_associativity_scope
        ...
        PkgSig bundle rootRead pkg ->
          UnaryHistory A ∧ UnaryHistory B ∧ UnaryHistory C ∧ UnaryHistory f ∧
            UnaryHistory g ∧ UnaryHistory u ∧ UnaryHistory K ∧ UnaryHistory L ∧
              UnaryHistory rootRead ∧ Cont A f B ∧ Cont B g C ∧ Cont f g K ∧
                Cont K u L ∧ Cont L N rootRead ∧ hsame N L ∧
                  PkgSig bundle rootRead pkg ∧
                    SemanticNameCert
  ...
  have sourceRoot :
    ...
    exact ⟨hsame_refl rootRead, unaryRootRead⟩
  have cert :
      SemanticNameCert
        (fun row : BHist => hsame row rootRead ∧ UnaryHistory row)
        (fun row : BHist => Cont L N row ∧ hsame N L)
  ...
  exact
    ⟨unaryA, unaryB, unaryC, unaryF, unaryG, unaryU, unaryK, unaryL,
      unaryRootRead, routeB, routeC, routeK, routeL, rootRoute, sameEndpoint,
      rootPkg, cert⟩
