...
theorem ContinuationMonadCarrier_namecert_ledger_exactness [AskSetup] [PackageSetup]
    {A B C f g u H K L N unitRead bindRead assocRead ledgerRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    ContinuationMonadCarrier A B C f g u H K L N ->
      Cont u f unitRead ->
        Cont K N bindRead ->
          Cont bindRead L assocRead ->
            Cont assocRead N ledgerRead ->
              PkgSig bundle ledgerRead pkg ->
                SemanticNameCert
                    (fun row : BHist => hsame row ledgerRead ∧ UnaryHistory row)
                    (fun row : BHist =>
                      hsame row ledgerRead ∧ Cont assocRead N ledgerRead)
                    (fun row : BHist =>
                  ...
                  UnaryHistory A ∧ UnaryHistory B ∧ UnaryHistory C ∧
                    UnaryHistory unitRead ∧ UnaryHistory bindRead ∧
                      UnaryHistory assocRead ∧ UnaryHistory ledgerRead ∧ hsame N L := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont SemanticNameCert hsame
  intro carrier unitRoute bindRoute assocRoute ledgerRoute ledgerPkg
  ...
  have unaryAssocRead : UnaryHistory assocRead :=
    unary_cont_closed unaryBindRead unaryL assocRoute
  have unaryLedgerRead : UnaryHistory ledgerRead :=
    unary_cont_closed unaryAssocRead unaryN ledgerRoute
  have sourceLedger :
      (fun row : BHist => hsame row ledgerRead ∧ UnaryHistory row) ledgerRead := by
    exact ⟨hsame_refl ledgerRead, unaryLedgerRead⟩
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row ledgerRead ∧ UnaryHistory row)
          (fun row : BHist => hsame row ledgerRead ∧ Cont assocRead N ledgerRead)
  ...
  exact
    ⟨cert, unaryA, unaryB, unaryC, unaryUnitRead, unaryBindRead, unaryAssocRead,
      unaryLedgerRead, sameEndpoint⟩
