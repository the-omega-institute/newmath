...
theorem ContinuationMonadCarrier_root_kleisli_source_route_exhaustion
    [AskSetup] [PackageSetup]
    {A B C f g u H K L N category generator publicRead consumer : BHist}
                            ...
                            Cont publicRead N consumer ∧ hsame N L ∧
                              PkgSig bundle consumer pkg ∧
                                SemanticNameCert
                                  ...
                                  (fun row : BHist =>
                                    hsame row consumer ∧ Cont L N category ∧
                                      Cont category K generator ∧
                                        Cont generator u publicRead ∧
                                          Cont publicRead N consumer)
                                  (fun row : BHist =>
  ...
  intro carrier categoryRoute generatorRoute publicRoute consumerRoute consumerPkg
  ...
  have cert :
      SemanticNameCert
        (fun row : BHist => hsame row consumer ∧ UnaryHistory row)
        (fun row : BHist =>
          hsame row consumer ∧ Cont L N category ∧ Cont category K generator ∧
            Cont generator u publicRead ∧ Cont publicRead N consumer)
    ...
    exact {
      ...
      pattern_sound := by
        intro _row source
        exact
          ⟨source.left, categoryRoute, generatorRoute, publicRoute, consumerRoute⟩
      ledger_sound := by
        intro _row source
  ...
  exact
    ⟨unaryA, unaryB, unaryC, unaryF, unaryG, unaryU, unaryK, unaryL, unaryN,
      unaryCategory, unaryGenerator, unaryPublicRead, unaryConsumer, routeB, routeC, routeK,
      routeL, categoryRoute, generatorRoute, publicRoute, consumerRoute, sameEndpoint,
      consumerPkg, cert⟩
