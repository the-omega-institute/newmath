...
theorem ContinuationMonadCarrier_root_source_public_contract [AskSetup] [PackageSetup]
  ...
  have cert :
      SemanticNameCert
        ...
        (fun row : BHist =>
          hsame row publicRead ∧ Cont L N category ∧ Cont category K generator ∧
            Cont u N unitRead ∧ Cont K L bindRead ∧ Cont generator unitRead publicRead)
    ...
    exact {
      ...
      pattern_sound := by
        intro _row source
        exact ⟨source.left, categoryRoute, generatorRoute, unitRoute, bindRoute, publicRoute⟩
      ledger_sound := by
        intro _row source
  ...
  exact
    ⟨unaryA, unaryB, unaryC, unaryF, unaryG, unaryU, unaryK, unaryL, unaryN,
      unaryCategory, unaryGenerator, unaryUnitRead, unaryBindRead, unaryPublicRead, routeB,
      routeC, routeK, routeL, categoryRoute, generatorRoute, unitRoute, bindRoute,
      publicRoute, sameEndpoint, publicPkg, cert⟩
