...
theorem ContinuationMonadCarrier_root_category_generator_obligation_surface
                    ...
                    Cont L N category ∧ Cont category K generator ∧ hsame N L ∧
                      PkgSig bundle generator pkg ∧
                        SemanticNameCert
                          ...
                          (fun row : BHist =>
                            hsame row generator ∧ Cont L N category ∧
                              Cont category K generator)
                          (fun row : BHist =>
  ...
  have cert :
    ...
    exact {
      core := {
        carrier_inhabited := Exists.intro generator generatorSource
        equiv_refl := by
          intro row _source
  ...
  exact
    ⟨unaryA, unaryB, unaryC, unaryF, unaryG, unaryU, unaryK, unaryL, unaryN,
      ...
      generatorRoute, sameEndpoint, generatorPkg, cert⟩
