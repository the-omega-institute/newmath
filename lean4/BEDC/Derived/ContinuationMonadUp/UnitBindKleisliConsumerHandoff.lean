...
theorem ContinuationMonadCarrier_unit_bind_kleisli_consumer_handoff
    [AskSetup] [PackageSetup]
    {A B C f g u H K L N terminal consumer : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    ContinuationMonadCarrier A B C f g u H K L N ->
      Cont L N terminal ->
        Cont terminal N consumer ->
          PkgSig bundle consumer pkg ->
            UnaryHistory A ∧ UnaryHistory B ∧ UnaryHistory C ∧ UnaryHistory f ∧
              UnaryHistory g ∧ UnaryHistory u ∧ UnaryHistory K ∧ UnaryHistory L ∧
                UnaryHistory terminal ∧ UnaryHistory consumer ∧ Cont A f B ∧
                  Cont B g C ∧ Cont f g K ∧ Cont K u L ∧ Cont L N terminal ∧
                    Cont terminal N consumer ∧ hsame N L ∧ PkgSig bundle consumer pkg ∧
                      SemanticNameCert
                        (fun row : BHist => hsame row consumer ∧ UnaryHistory row)
                        (fun row : BHist => Cont terminal N row ∧ Cont L N terminal)
                        (fun row : BHist => hsame row consumer ∧ PkgSig bundle consumer pkg)
                        hsame := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont SemanticNameCert hsame
  intro carrier terminalRoute consumerRoute consumerPkg
  ...
  have unaryConsumer : UnaryHistory consumer :=
    unary_cont_closed unaryTerminal unaryN consumerRoute
  have sourceConsumer :
      (fun row : BHist => hsame row consumer ∧ UnaryHistory row) consumer := by
    exact And.intro (hsame_refl consumer) unaryConsumer
  have cert :
      SemanticNameCert
        (fun row : BHist => hsame row consumer ∧ UnaryHistory row)
        (fun row : BHist => Cont terminal N row ∧ Cont L N terminal)
    ...
    exact {
      ...
      pattern_sound := by
        intro _row source
        exact And.intro
          (cont_result_hsame_transport consumerRoute (hsame_symm source.left))
          terminalRoute
      ledger_sound := by
        intro _row source
  ...
  exact
    ⟨unaryA, unaryB, unaryC, unaryF, unaryG, unaryU, unaryK, unaryL, unaryTerminal,
      unaryConsumer, routeB, routeC, routeK, routeL, terminalRoute, consumerRoute,
      sameEndpoint, consumerPkg, cert⟩
