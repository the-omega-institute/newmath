...
theorem ContinuationMonadCarrier_root_unit_bind_exhaustion [AskSetup] [PackageSetup]
              ...
              UnaryHistory g ∧ UnaryHistory u ∧ UnaryHistory K ∧ UnaryHistory L ∧
                UnaryHistory unitRead ∧ UnaryHistory bindRead ∧ Cont A f B ∧
                  Cont B g C ∧ Cont f g K ∧ Cont K u L ∧ Cont u f unitRead ∧
                    Cont K N bindRead ∧ hsame N L ∧ PkgSig bundle bindRead pkg ∧
                      SemanticNameCert
                        (fun row : BHist => hsame row bindRead ∧ UnaryHistory row)
                        (fun row : BHist => hsame row bindRead ∧ Cont K N bindRead)
                        (fun row : BHist =>
                          hsame row bindRead ∧ PkgSig bundle bindRead pkg)
                        hsame := by
  ...
  have cert :
    ...
    exact {
      ...
      pattern_sound := by
        intro _row source
        exact ⟨source.left, bindRoute⟩
      ledger_sound := by
        intro _row source
        exact ⟨source.left, bindPkg⟩
    }
  exact
    ⟨unaryA, unaryB, unaryC, unaryF, unaryG, unaryU, unaryK, unaryL, unaryUnitRead,
      unaryBindRead, routeB, routeC, routeK, routeL, unitRoute, bindRoute, sameEndpoint,
      bindPkg, cert⟩
