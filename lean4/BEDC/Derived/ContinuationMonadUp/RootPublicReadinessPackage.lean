...
theorem ContinuationMonadCarrier_root_public_readiness_package
                      ...
                      UnaryHistory g /\ UnaryHistory u /\ UnaryHistory K /\ UnaryHistory L /\
                        UnaryHistory N /\ UnaryHistory category /\ UnaryHistory generator /\
                          UnaryHistory unitRead /\ UnaryHistory bindRead /\
                            UnaryHistory publicRead /\ UnaryHistory consumer /\ Cont A f B /\
  ...
  have cert :
      SemanticNameCert
        ...
        (fun row : BHist =>
          Cont publicRead N row /\ Cont generator unitRead publicRead /\
            Cont L N category)
    ...
    exact {
      core := {
        carrier_inhabited :=
          Exists.intro consumer (And.intro (hsame_refl consumer) unaryConsumer)
        equiv_refl := by
          intro row _source
  ...
  exact
    ⟨unaryA, unaryB, unaryC, unaryF, unaryG, unaryU, unaryK, unaryL, unaryN,
      ...
      unaryConsumer, routeB, routeC, routeK, routeL, categoryRoute, generatorRoute,
      unitRoute, bindRoute, publicRoute, consumerRoute, sameEndpoint, consumerPkg, cert⟩
