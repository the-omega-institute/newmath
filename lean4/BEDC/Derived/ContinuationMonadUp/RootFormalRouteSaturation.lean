...
theorem ContinuationMonadCarrier_root_formal_audit_factorization [AskSetup] [PackageSetup]
    {A B C f g u H K L N rootRead formalRead auditRead auditConsumer : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    ContinuationMonadCarrier A B C f g u H K L N →
      Cont L N rootRead →
        Cont rootRead N formalRead →
          Cont formalRead K auditRead →
            Cont auditRead N auditConsumer →
              PkgSig bundle auditConsumer pkg →
                UnaryHistory auditRead ∧ UnaryHistory auditConsumer ∧
                  Cont formalRead K auditRead ∧ Cont auditRead N auditConsumer ∧
                    PkgSig bundle auditConsumer pkg ∧
                      SemanticNameCert
                        (fun row : BHist => hsame row auditConsumer ∧ UnaryHistory row)
                        (fun row : BHist =>
                          hsame row auditConsumer ∧ Cont formalRead K auditRead ∧
                            Cont auditRead N auditConsumer)
                        (fun row : BHist =>
                          hsame row auditConsumer ∧ PkgSig bundle auditConsumer pkg)
                        hsame := by
  -- BEDC touchpoint anchor: BHist AskSetup PackageSetup ProbeBundle Pkg Cont hsame
  intro carrier rootRoute formalRoute auditRoute consumerRoute consumerPkg
  obtain ⟨unaryA, unaryF, unaryG, unaryU, routeB, routeC, routeK, routeL,
    sameEndpoint⟩ := carrier
  have unaryB : UnaryHistory B :=
    unary_cont_closed unaryA unaryF routeB
  ...
  have unaryAuditRead : UnaryHistory auditRead :=
    unary_cont_closed unaryFormalRead unaryK auditRoute
  have unaryAuditConsumer : UnaryHistory auditConsumer :=
    unary_cont_closed unaryAuditRead unaryN consumerRoute
  have cert :
      SemanticNameCert
        (fun row : BHist => hsame row auditConsumer ∧ UnaryHistory row)
        (fun row : BHist =>
          hsame row auditConsumer ∧ Cont formalRead K auditRead ∧
            Cont auditRead N auditConsumer)
        (fun row : BHist => hsame row auditConsumer ∧ PkgSig bundle auditConsumer pkg)
        hsame := by
    exact {
      core := {
        carrier_inhabited :=
          Exists.intro auditConsumer
            (And.intro (hsame_refl auditConsumer) unaryAuditConsumer)
        equiv_refl := by
          intro row _source
          exact hsame_refl row
        equiv_symm := by
      ...
      pattern_sound := by
        intro _row source
        exact ⟨source.left, auditRoute, consumerRoute⟩
      ledger_sound := by
        intro _row source
        exact And.intro source.left consumerPkg
    }
  exact ⟨unaryAuditRead, unaryAuditConsumer, auditRoute, consumerRoute, consumerPkg, cert⟩
