...
theorem ContinuationMonadCarrier_root_consumer_standard_row [AskSetup] [PackageSetup]
    {A B C f g u H K L N unitRead bindRead consumer : BHist}
            ...
            PkgSig bundle consumer pkg ->
              UnaryHistory A /\ UnaryHistory B /\ UnaryHistory C /\ UnaryHistory f /\
                UnaryHistory g /\ UnaryHistory u /\ UnaryHistory K /\ UnaryHistory L /\
                  UnaryHistory unitRead /\ UnaryHistory bindRead /\ UnaryHistory consumer /\
                    Cont A f B /\ Cont B g C /\ Cont f g K /\ Cont K u L /\
                      Cont u f unitRead /\ Cont K N bindRead /\ Cont L N consumer /\
                        hsame N L /\ PkgSig bundle consumer pkg /\
                          SemanticNameCert
  ...
  intro carrier unitRoute bindRoute consumerRoute consumerPkg
  ...
  exact
    ⟨unaryA, unaryB, unaryC, unaryF, unaryG, unaryU, unaryK, unaryL, unaryUnitRead,
      unaryBindRead, unaryConsumer, routeB, routeC, routeK, routeL, unitRoute, bindRoute,
