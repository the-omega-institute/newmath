...
theorem ContinuationMonadCarrier_root_public_contract_factorization
    [AskSetup] [PackageSetup]
    {A B C f g u H K L N category generator unitRead bindRead publicRead
      contractRead : BHist}
          ...
          Cont u N unitRead ->
            Cont K L bindRead ->
              Cont generator unitRead publicRead ->
                Cont publicRead N contractRead ->
                  PkgSig bundle contractRead pkg ->
                        ...
                        UnaryHistory K ∧ UnaryHistory L ∧ UnaryHistory N ∧
                          UnaryHistory category ∧ UnaryHistory generator ∧
                            UnaryHistory unitRead ∧ UnaryHistory bindRead ∧
                              UnaryHistory publicRead ∧ UnaryHistory contractRead ∧
                                Cont A f B ∧ Cont B g C ∧ Cont f g K ∧
                                  Cont K u L ∧ Cont L N category ∧
                                    Cont category K generator ∧ Cont u N unitRead ∧
                                      Cont K L bindRead ∧
                                        Cont generator unitRead publicRead ∧
                                          Cont publicRead N contractRead ∧ hsame N L ∧
                                            PkgSig bundle contractRead pkg ∧
                                              SemanticNameCert
                                                (fun row : BHist =>
                                                  hsame row contractRead ∧
                                                    UnaryHistory row)
                                                (fun row : BHist =>
  ...
  intro carrier categoryRoute generatorRoute unitRoute bindRoute publicRoute contractRoute
    contractPkg
  ...
  have unaryContractRead : UnaryHistory contractRead :=
    unary_cont_closed unaryPublicRead unaryN contractRoute
  have sourceContract :
      (fun row : BHist => hsame row contractRead ∧ UnaryHistory row) contractRead := by
    exact ⟨hsame_refl contractRead, unaryContractRead⟩
  have cert :
      SemanticNameCert
        (fun row : BHist => hsame row contractRead ∧ UnaryHistory row)
        (fun row : BHist =>
          Cont publicRead N row ∧ Cont generator unitRead publicRead ∧
            Cont L N category ∧ Cont category K generator)
        (fun row : BHist => hsame row contractRead ∧ PkgSig bundle contractRead pkg)
        hsame := by
    exact {
      core := {
        carrier_inhabited := Exists.intro contractRead sourceContract
        equiv_refl := by
          intro row _source
      ...
      pattern_sound := by
        intro _row source
        exact
          ⟨cont_result_hsame_transport contractRoute (hsame_symm source.left),
            publicRoute, categoryRoute, generatorRoute⟩
      ledger_sound := by
        intro _row source
        exact ⟨source.left, contractPkg⟩
    }
  exact
    ⟨unaryA, unaryB, unaryC, unaryF, unaryG, unaryU, unaryK, unaryL, unaryN,
      ...
      unaryContractRead, routeB, routeC, routeK, routeL, categoryRoute, generatorRoute,
      unitRoute, bindRoute, publicRoute, contractRoute, sameEndpoint, contractPkg, cert⟩
