...
theorem ContinuationMonadCarrier_root_bind_completion_classifier [AskSetup] [PackageSetup]
    {A B C f g u H K L N terminal : BHist} {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
                ...
                Cont K u L ∧ Cont L N terminal ∧ hsame N L ∧
                  PkgSig bundle terminal pkg ∧
                    SemanticNameCert
                      ...
                      (fun row : BHist => Cont L N row ∧ hsame N L)
  ...
  have cert :
    ...
    exact {
      ...
      pattern_sound := by
        intro _row source
        cases source.left
        exact ⟨terminalRoute, sameEndpoint⟩
      ledger_sound := by
        intro _row source
  ...
  exact
    ⟨unaryA, unaryB, unaryC, unaryF, unaryG, unaryU, unaryK, unaryL, unaryTerminal,
      routeB, routeC, routeK, routeL, terminalRoute, sameEndpoint, terminalPkg, cert⟩
