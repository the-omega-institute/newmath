...
theorem ContinuationMonadCarrier_generator_category_lockdown [AskSetup] [PackageSetup]
              ...
              Cont generator unitRead publicRead ->
                PkgSig bundle publicRead pkg ->
                  SemanticNameCert
                      ...
                      hsame ∧
                    UnaryHistory category ∧ UnaryHistory generator ∧ UnaryHistory unitRead ∧
                      UnaryHistory bindRead ∧ UnaryHistory publicRead ∧ Cont L N category ∧
  ...
  have cert :
    ...
    exact {
      ...
      pattern_sound := by
        intro _row _source
        exact ⟨categoryRoute, generatorRoute, publicRoute⟩
      ledger_sound := by
        intro _row source
        exact And.intro source.left publicPkg
    }
  exact
    ⟨cert, unaryCategory, unaryGenerator, unaryUnitRead, unaryBindRead, unaryPublicRead,
      categoryRoute, generatorRoute, unitRoute, bindRoute, publicRoute, publicPkg⟩
