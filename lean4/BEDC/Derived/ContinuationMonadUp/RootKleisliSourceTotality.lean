...
theorem ContinuationMonadCarrier_root_kleisli_source_totality [AskSetup] [PackageSetup]
                ...
                Cont f g K ∧ Cont K u L ∧ Cont L N rootRead ∧ hsame N L ∧
                  PkgSig bundle rootRead pkg ∧
                    SemanticNameCert
                      ...
                      (fun row : BHist => hsame row rootRead ∧ Cont L N rootRead)
  ...
  have cert :
    ...
    exact {
      core := {
        carrier_inhabited :=
          Exists.intro rootRead (And.intro (hsame_refl rootRead) unaryRootRead)
        equiv_refl := by
          intro row _source
      ...
      pattern_sound := by
        intro _row source
        exact And.intro source.left rootRoute
      ledger_sound := by
        intro _row source
  ...
  exact
    ⟨unaryA, unaryB, unaryC, unaryF, unaryG, unaryU, unaryK, unaryL, unaryN,
      unaryRootRead, routeB, routeC, routeK, routeL, rootRoute, sameEndpoint, rootPkg,
      cert⟩
