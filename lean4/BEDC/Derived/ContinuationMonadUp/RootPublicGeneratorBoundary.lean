...
theorem ContinuationMonadCarrier_root_public_generator_boundary [AskSetup] [PackageSetup]
              ...
              UnaryHistory A ∧ UnaryHistory B ∧ UnaryHistory C ∧ UnaryHistory f ∧
                UnaryHistory g ∧ UnaryHistory u ∧ UnaryHistory K ∧ UnaryHistory L ∧
                  UnaryHistory N ∧ UnaryHistory unitRead ∧ UnaryHistory bindRead ∧
                    UnaryHistory generator ∧ Cont A f B ∧ Cont B g C ∧
                      Cont f g K ∧ Cont K u L ∧ Cont u f unitRead ∧
                        Cont K N bindRead ∧ Cont bindRead N generator ∧ hsame N L ∧
                          PkgSig bundle generator pkg ∧
                            SemanticNameCert
                              (fun row : BHist => hsame row generator ∧ UnaryHistory row)
                              (fun row : BHist =>
                                hsame row generator ∧ Cont u f unitRead ∧
                                  Cont K N bindRead ∧ Cont bindRead N generator)
                              (fun row : BHist =>
                                hsame row generator ∧ PkgSig bundle generator pkg)
                              hsame := by
  ...
  have cert :
      SemanticNameCert
        (fun row : BHist => hsame row generator ∧ UnaryHistory row)
        (fun row : BHist =>
          hsame row generator ∧ Cont u f unitRead ∧ Cont K N bindRead ∧
            Cont bindRead N generator)
    ...
    exact {
      core := {
        carrier_inhabited :=
          Exists.intro generator (And.intro (hsame_refl generator) unaryGenerator)
        equiv_refl := by
          intro row _source
      ...
      pattern_sound := by
        intro _row source
        exact ⟨source.left, unitRoute, bindRoute, generatorRoute⟩
      ledger_sound := by
        intro _row source
  ...
  exact
    ⟨unaryA, unaryB, unaryC, unaryF, unaryG, unaryU, unaryK, unaryL, unaryN,
      ...
      unitRoute, bindRoute, generatorRoute, sameEndpoint, generatorPkg, cert⟩
