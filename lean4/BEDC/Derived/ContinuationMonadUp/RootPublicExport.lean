...
theorem ContinuationMonadCarrier_root_public_export [AskSetup] [PackageSetup]
    {A B C f g u H K L N category generator unitRead bindRead publicRead : BHist}
  ...
  intro carrier categoryRoute generatorRoute unitRoute bindRoute publicRoute publicPkg
  ...
  exact {
    ...
    pattern_sound := by
      intro _row source
      exact
        ⟨source.left, categoryRoute, generatorRoute, unitRoute, bindRoute,
          publicRoute⟩
    ledger_sound := by
      intro _row source
