...
theorem ZetaContinuationApplicationPublicExport [AskSetup] [PackageSetup]
  ...
  have cert :
    ...
    pattern_sound := by
      intro _row _source
      exact
        ⟨etaGammaRead, gammaApplicationRead, applicationReplayRead, provenanceNameLedger,
          operationLedgerPublic⟩
    ledger_sound := by
      intro _row source
      cases source with
      | inl sameEta =>
          exact ⟨unary_transport etaReadUnary (hsame_symm sameEta), publicReadPkg⟩
      | inr rest =>
          cases rest with
          | inl sameGamma =>
              exact ⟨unary_transport gammaReadUnary (hsame_symm sameGamma), publicReadPkg⟩
          | inr rest =>
              cases rest with
              | inl sameOperation =>
                  exact
              ...
              | inr rest =>
                  ...
                  | inl sameLedger =>
                      exact
                        ⟨unary_transport ledgerReadUnary (hsame_symm sameLedger),
                          publicReadPkg⟩
                  | inr samePublic =>
                      exact
                        ⟨unary_transport publicReadUnary (hsame_symm samePublic),
                          publicReadPkg⟩
  }
