...
theorem CauchyContinuousMap_completion_factorization [AskSetup] [PackageSetup]
    ...
    CauchyContinuousMapPacket M.windows M.imageReadback M.toleranceLedger
      ...
      Cont M.windows M.imageReadback imageRead →
        Cont imageRead M.realSealHandoff sealRead →
          Cont sealRead M.replay boundaryRead →
            Cont boundaryRead M.localName completionRead →
  ...
  have completionUnary : UnaryHistory completionRead :=
    unary_cont_closed boundaryUnary localNameUnary completionRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row completionRead ∧ UnaryHistory row)
          (fun row : BHist =>
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, imageRoute, sealRoute, boundaryRoute, completionRoute,
          completionPkg⟩
  }
