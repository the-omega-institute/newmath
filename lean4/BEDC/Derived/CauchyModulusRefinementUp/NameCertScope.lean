...
theorem CauchyModulusRefinementCarrier_namecert_scope [AskSetup] [PackageSetup]
  ...
  exact {
    ...
    pattern_sound := by
      ...
      exact
        ⟨m0m1u, uvt, twq, qeh,
          cont_result_hsame_transport endpointRoute (hsame_symm source.right),
          endpointPkg⟩
    ledger_sound := by
      intro _row source
...
theorem CauchyModulusRefinementCarrier_carrier_admission_row [AskSetup] [PackageSetup]
      ...
      SemanticNameCert
        (fun row : BHist =>
          CauchyModulusRefinementCarrier m0 m1 u v t w q e h c p n bundle pkg ∧
            hsame row n)
        (fun _row : BHist =>
          Cont m0 m1 u ∧ Cont u v t ∧ Cont t w q ∧ Cont q e h ∧ hsame h n)
  ...
  exact {
    ...
    pattern_sound := by
      intro _row _source
      exact ⟨m0m1u, uvt, twq, qeh, hn⟩
    ledger_sound := by
      intro _row source
      exact ⟨unary_transport nUnary (hsame_symm source.right), pPkg⟩
  }
