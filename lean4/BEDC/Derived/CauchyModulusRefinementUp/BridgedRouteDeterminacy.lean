...
theorem CauchyModulusRefinementBridgedRouteDeterminacy [AskSetup] [PackageSetup]
    {m0 m1 u v t w q e h c p n publicRead publicReadPrime : BHist}
            ...
            PkgSig bundle publicRead pkg →
              PkgSig bundle publicReadPrime pkg →
                SemanticNameCert
                  ...
                  (fun row : BHist =>
                    Cont q e h ∧ Cont h c row ∧ PkgSig bundle publicRead pkg)
  ...
  intro carrier hPublicRead hPublicReadPrime samePublicReads publicPkg _publicPrimePkg
  rcases carrier with
  ...
  exact {
    ...
    pattern_sound := by
      intro _row source
      exact
        ⟨qeh, cont_result_hsame_transport transportedPrime (hsame_symm source.right),
          publicPkg⟩
    ledger_sound := by
      intro _row source
