...
theorem CauchyModulusRefinementCarrier_tailmeet_real_seal_normal_form [AskSetup]
    [PackageSetup]
    {tailBudget observation limiter entry m0 m1 u v t w q e h c p n selected readback sealRead
      publicRead : BHist}
  ...
  intro tailUnary observationUnary tailObservationLimiter limiterEntry carrier tWSelected
    selectedQReadback readbackESeal sealHPublic publicPkg
  rcases carrier with
  ...
  exact {
    ...
    pattern_sound := by
      intro _row source
      exact
        ⟨tailObservationLimiter, limiterEntry, m0m1u, uvt, tWSelected,
          selectedQReadback, readbackESeal,
