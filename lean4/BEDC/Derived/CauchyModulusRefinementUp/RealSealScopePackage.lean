...
theorem CauchyModulusRefinementCarrier_real_seal_scope_package [AskSetup] [PackageSetup]
  ...
  exact {
    ...
    ledger_sound := by
      intro row source
      exact
        ⟨unary_transport endpointUnary (hsame_symm source.right), pPkg, endpointPkg⟩
  }
