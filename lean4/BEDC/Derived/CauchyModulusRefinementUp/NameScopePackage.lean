...
theorem CauchyModulusRefinementCarrier_namescope_package [AskSetup] [PackageSetup]
        ...
        PkgSig bundle endpoint pkg ->
          SemanticNameCert
            (fun row : BHist =>
              CauchyModulusRefinementCarrier m0 m1 u v t w q e h c p n bundle pkg ∧
                hsame row endpoint)
            (fun row : BHist => Cont h c row ∧ PkgSig bundle endpoint pkg)
  ...
  exact {
    ...
    pattern_sound := by
      intro _row source
      exact
        ⟨cont_result_hsame_transport endpointRoute (hsame_symm source.right), endpointPkg⟩
    ledger_sound := by
      intro _row source
