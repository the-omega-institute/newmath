...
theorem CauchyModulusRefinementCarrier_real_seal_pullback_nonescape
        ...
        PkgSig bundle endpoint pkg ->
          SemanticNameCert
            (fun row : BHist =>
              CauchyModulusRefinementCarrier m0 m1 u v t w q e h c p n bundle pkg ∧
                hsame row endpoint)
            (fun row : BHist => Cont u v t ∧ Cont t w q ∧ Cont q e h ∧ Cont h c row)
  ...
  exact {
    ...
    pattern_sound := by
      intro _row source
      exact
        ⟨uvt, twq, qeh, cont_result_hsame_transport hEndpoint (hsame_symm source.right)⟩
    ledger_sound := by
      intro _row source
