...
theorem CauchyModulusRefinementCarrier_root_real_seal_exhaustion
  ...
  intro carrier qSealRead sealPkg
  rcases carrier with
  ...
  exact {
    ...
    pattern_sound := by
      ...
      exact
        ⟨m0m1u, uvt, twq,
          cont_result_hsame_transport qSealRead (hsame_symm source.right), sealPkg⟩
    ledger_sound := by
      intro _row source
      exact And.intro (unary_transport sealReadUnary (hsame_symm source.right)) sealPkg
  }
