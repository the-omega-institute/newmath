...
theorem CauchyModulusRefinementRealCompletenessHandoff_semantic_name_certificate
    [AskSetup] [PackageSetup]
    {m0 m1 u v t w q e h c p n completionRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    CauchyModulusRefinementCarrier m0 m1 u v t w q e h c p n bundle pkg ->
      Cont h c completionRead ->
          ...
          SemanticNameCert
            ...
            (fun row : BHist =>
              Cont m0 m1 u ∧ Cont u v t ∧ Cont t w q ∧ Cont q e h ∧
                Cont h c row ∧ PkgSig bundle p pkg)
  ...
  have completionUnary : UnaryHistory completionRead :=
    unary_cont_closed hUnary cUnary completionRoute
  exact {
    core := {
      carrier_inhabited :=
    ...
    pattern_sound := by
      ...
      exact
        ⟨m0m1u, uvt, twq, qeh,
          cont_result_hsame_transport completionRoute (hsame_symm source.right), pPkg⟩
    ledger_sound := by
      intro _row source
