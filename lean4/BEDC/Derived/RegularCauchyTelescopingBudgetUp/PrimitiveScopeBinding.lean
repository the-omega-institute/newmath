...
theorem RegularCauchyTelescopingBudgetCarrier_primitive_scope_binding [AskSetup]
    [PackageSetup]
    {E W D R S T H C P N precisionRead ledgerRead telescopingRead handoffRead sealRead
      replayRead scopeRead : BHist}
  ...
  intro eUnary wUnary dUnary tUnary rUnary sUnary hUnary cUnary precisionRoute ledgerRoute
    telescopingRoute handoffRoute sealRoute replayRoute scopeRoute provenancePkg namePkg
  ...
  have scopeUnary : UnaryHistory scopeRead :=
    unary_cont_closed replayUnary hUnary scopeRoute
  have cert :
      SemanticNameCert
            ...
            UnaryHistory row ∧ Cont E W precisionRead ∧ Cont precisionRead D ledgerRead ∧
              Cont ledgerRead T telescopingRead ∧ Cont telescopingRead R handoffRead ∧
                Cont handoffRead S sealRead ∧ Cont sealRead C replayRead ∧
                  Cont replayRead H scopeRead ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, precisionRoute, ledgerRoute, telescopingRoute, handoffRoute,
          sealRoute, replayRoute, scopeRoute, provenancePkg, namePkg⟩
  }
  exact
    ⟨cert, precisionUnary, ledgerUnary, telescopingUnary, handoffUnary, sealUnary,
      replayUnary, scopeUnary⟩

theorem RegularCauchyTelescopingBudgetCarrier_public_export [AskSetup]
    [PackageSetup]
    {E W D R S T H C P N precisionRead ledgerRead telescopingRead handoffRead sealRead
      replayRead scopeRead publicRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    UnaryHistory E →
      UnaryHistory W →
        UnaryHistory D →
  ...
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert
  intro eUnary wUnary dUnary tUnary rUnary sUnary hUnary cUnary nUnary precisionRoute
    ledgerRoute telescopingRoute handoffRoute sealRoute replayRoute scopeRoute publicRoute
  ...
  have publicUnary : UnaryHistory publicRead :=
    unary_cont_closed scopeUnary nUnary publicRoute
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont E W precisionRead ∧ Cont precisionRead D ledgerRead ∧
              Cont ledgerRead T telescopingRead ∧ Cont telescopingRead R handoffRead ∧
                Cont handoffRead S sealRead ∧ Cont sealRead C replayRead ∧
                  Cont replayRead H scopeRead ∧ Cont scopeRead N publicRead ∧
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, precisionRoute, ledgerRoute, telescopingRoute, handoffRoute,
          sealRoute, replayRoute, scopeRoute, publicRoute, provenancePkg, namePkg⟩
  }
  exact
    ⟨cert, precisionUnary, ledgerUnary, telescopingUnary, handoffUnary, sealUnary,
      replayUnary, scopeUnary, publicUnary⟩
