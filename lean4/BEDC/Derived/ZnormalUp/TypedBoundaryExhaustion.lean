...
theorem ZnormalPacket_typed_boundary_exhaustion [AskSetup] [PackageSetup]
  ...
  exact ⟨cert, terminalReadTerminal, continuationReadUnary, downstreamUnary⟩

theorem ZnormalPacket_fuel_normality_nonescape [AskSetup] [PackageSetup]
    {typed fuel terminal normal continuation transports routes provenance name terminalRead normalRead
      consumer : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    ZnormalPacket typed fuel terminal normal continuation transports routes provenance name
        bundle pkg →
        ...
        Cont terminalRead normal normalRead →
          Cont normalRead routes consumer →
            PkgSig bundle consumer pkg →
              hsame terminalRead terminal ∧ UnaryHistory normalRead ∧ UnaryHistory consumer ∧
  ...
  intro packet typedFuelTerminalRead terminalReadNormalRead normalReadRoutesConsumer consumerPkg
  obtain ⟨typedUnary, fuelUnary, _terminalUnary, normalUnary, _continuationUnary,
  ...
  exact
    ⟨terminalReadTerminal, normalReadUnary, consumerUnary, terminalReadNormalRead,
      normalReadRoutesConsumer, provenancePkg, consumerPkg⟩
