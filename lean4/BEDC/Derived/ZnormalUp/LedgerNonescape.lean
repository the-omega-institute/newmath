...
theorem ZnormalPacket_ledger_nonescape_certificate [AskSetup] [PackageSetup]
  ...
  intro packet provenanceNameLedgerRead ledgerReadPkg
  ...
  have cert :
    ...
    exact {
      core := {
        carrier_inhabited := Exists.intro ledgerRead ledgerReadSource
        equiv_refl := by
          intro row _source
  ...
  exact
    ⟨cert, provenanceUnary, nameUnary, ledgerReadUnary, provenancePkg, namePkg,
      ledgerReadPkg⟩
