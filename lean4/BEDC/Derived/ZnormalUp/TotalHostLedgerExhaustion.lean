...
theorem ZnormalPacket_total_host_ledger_exhaustion [AskSetup] [PackageSetup]
    {typed fuel terminal normal continuation transports routes provenance name terminalRead
      normalRead ledger : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    ZnormalPacket typed fuel terminal normal continuation transports routes provenance name
        bundle pkg →
      Cont typed fuel terminalRead →
        Cont terminalRead normal normalRead →
          Cont normalRead transports ledger →
            PkgSig bundle ledger pkg →
              SemanticNameCert
                  (fun row : BHist => hsame row ledger ∧ UnaryHistory row)
                  (fun row : BHist =>
                    hsame row typed ∨ hsame row fuel ∨ hsame row terminalRead ∨
                      hsame row normalRead ∨ hsame row ledger)
                  (fun row : BHist => hsame row ledger ∧ PkgSig bundle ledger pkg)
                  hsame ∧
                UnaryHistory terminalRead ∧ UnaryHistory normalRead ∧
                  UnaryHistory ledger ∧ PkgSig bundle provenance pkg := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro packet typedFuelTerminalRead terminalReadNormalRead normalReadTransportsLedger ledgerPkg
  ...
  have ledgerUnary : UnaryHistory ledger :=
    unary_cont_closed normalReadUnary transportsUnary normalReadTransportsLedger
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row ledger ∧ UnaryHistory row)
          (fun row : BHist =>
  ...
  exact ⟨cert, terminalReadUnary, normalReadUnary, ledgerUnary, provenancePkg⟩
