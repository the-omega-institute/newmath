...
theorem RealClassifierTheoremCheckTargetHandoff [AskSetup] [PackageSetup]
    {X Y SX SY RX RY W D C E H K P N windowRead classifierRead sealRead auditRead
      densityRead targetRead : BHist}
                          ...
                          hsame row X ∨ hsame row Y ∨ hsame row SX ∨ hsame row SY ∨
                            hsame row RX ∨ hsame row RY ∨ hsame row W ∨ hsame row D ∨
                              hsame row C ∨ hsame row E ∨ hsame row H ∨ hsame row K ∨
                                hsame row P ∨ hsame row N ∨ hsame row targetRead)
                        (fun row : BHist =>
                            ...
                            Cont windowRead C classifierRead ∧
                              Cont classifierRead E sealRead ∧ Cont sealRead K auditRead ∧
                                Cont auditRead P densityRead ∧
                                  Cont densityRead N targetRead ∧
                                    PkgSig bundle targetRead pkg)
                        hsame ∧ UnaryHistory targetRead := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier windowRoute classifierRoute sealRoute auditRoute densityRoute targetRoute
    targetPkg
  obtain ⟨_xUnary, _yUnary, _sxUnary, _syUnary, _rxUnary, _ryUnary, wUnary,
    dUnary, cUnary, eUnary, _hUnary, kUnary, pUnary, nUnary, _sealPkg⟩ :=
    carrier
  ...
  have targetUnary : UnaryHistory targetRead :=
    unary_cont_closed densityUnary nUnary targetRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row targetRead ∧ UnaryHistory row)
          (fun row : BHist =>
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont W D windowRead ∧ Cont windowRead C classifierRead ∧
              Cont classifierRead E sealRead ∧ Cont sealRead K auditRead ∧
                Cont auditRead P densityRead ∧ Cont densityRead N targetRead ∧
                  PkgSig bundle targetRead pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, windowRoute, classifierRoute, sealRoute, auditRoute,
          densityRoute, targetRoute, targetPkg⟩
  }
