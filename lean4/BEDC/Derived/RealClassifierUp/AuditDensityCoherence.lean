...
theorem RealClassifierAuditDensityCoherence [AskSetup] [PackageSetup]
    {X Y SX SY RX RY W D C E H K P N windowRead classifierRead sealRead auditRead
      densityRead : BHist}
                            ...
                            hsame row C ∨ hsame row E ∨ hsame row H ∨ hsame row K ∨
                              hsame row P ∨ hsame row N ∨ hsame row windowRead ∨
                                hsame row classifierRead ∨ hsame row sealRead ∨
                                  hsame row auditRead ∨ hsame row densityRead)
                      (fun row : BHist =>
                        UnaryHistory row ∧ Cont W D windowRead ∧
                          Cont windowRead C classifierRead ∧
                            Cont classifierRead E sealRead ∧ Cont sealRead K auditRead ∧
                              Cont auditRead P densityRead ∧ PkgSig bundle densityRead pkg)
                      hsame ∧ UnaryHistory windowRead ∧ UnaryHistory classifierRead ∧
                    UnaryHistory sealRead ∧ UnaryHistory auditRead ∧
                  UnaryHistory densityRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier windowRoute classifierRoute sealRoute auditRoute densityRoute densityPkg
  obtain ⟨_xUnary, _yUnary, _sxUnary, _syUnary, _rxUnary, _ryUnary, wUnary,
    dUnary, cUnary, eUnary, _hUnary, kUnary, pUnary, _nUnary, _sealPkg⟩ :=
    carrier
  ...
  have densityUnary : UnaryHistory densityRead :=
    unary_cont_closed auditUnary pUnary densityRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row densityRead ∧ UnaryHistory row)
          (fun row : BHist =>
                ...
                hsame row C ∨ hsame row E ∨ hsame row H ∨ hsame row K ∨
                  hsame row P ∨ hsame row N ∨ hsame row windowRead ∨
                    hsame row classifierRead ∨ hsame row sealRead ∨
                      hsame row auditRead ∨ hsame row densityRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont W D windowRead ∧ Cont windowRead C classifierRead ∧
              Cont classifierRead E sealRead ∧ Cont sealRead K auditRead ∧
                Cont auditRead P densityRead ∧ PkgSig bundle densityRead pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, windowRoute, classifierRoute, sealRoute, auditRoute, densityRoute,
          densityPkg⟩
  }
  exact ⟨cert, windowUnary, classifierUnary, sealUnary, auditUnary, densityUnary⟩
