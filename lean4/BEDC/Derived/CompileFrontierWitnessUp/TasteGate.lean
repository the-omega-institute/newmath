...
def CompileFrontierWitnessCarrier [AskSetup] [PackageSetup]
    (F T S A B H C P N : BHist) (bundle : ProbeBundle ProbeName) (pkg : Pkg) :
    Prop :=
...
theorem CompileFrontierWitnessCarrier_namecert_obligations [AskSetup] [PackageSetup]
            ...
            SemanticNameCert
                ...
                (fun row : BHist =>
                  UnaryHistory row ∧ Cont S A auditRead ∧ Cont auditRead B boundaryRead ∧
                    PkgSig bundle N pkg)
                hsame ∧
              UnaryHistory auditRead ∧ UnaryHistory boundaryRead := by
  -- BEDC touchpoint anchor: CompileFrontierWitnessCarrier BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert
  intro carrier auditRoute boundaryRoute namePkg
  obtain ⟨_fUnary, _tUnary, sUnary, aUnary, bUnary, _hUnary, _cUnary, _pUnary,
    nUnary, _pPkg, _storedNamePkg⟩ := carrier
