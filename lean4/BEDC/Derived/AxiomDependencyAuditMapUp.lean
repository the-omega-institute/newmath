...
theorem AxiomDependencyAuditMapCarrier_obligation_closure [AskSetup] [PackageSetup]
    {K M W A L H C P N modeRead witnessAxiomRead ledgerRead publicRead auditRead
                      ...
                      PkgSig bundle dependencyRead pkg ->
                        SemanticNameCert
                          ...
                          SemanticNameCert
                              ...
                              (fun row : BHist =>
                                hsame row K ∨ hsame row M ∨ hsame row W ∨
                                  hsame row A ∨ hsame row L ∨ hsame row H ∨
  ...
  -- BEDC touchpoint anchor: AxiomDependencyAuditMapCarrier BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier modeRoute witnessAxiomRoute ledgerRoute publicRoute auditRoute
    dependencyRoute witnessAxiomPkg auditPkg dependencyPkg
  obtain ⟨kUnary, mUnary, wUnary, aUnary, lUnary, hUnary, _cUnary, _pUnary,
    nUnary, _claimModeLedger, _ledgerAxiomTransport, _transportConsumerProvenance,
    _carrierPkg⟩ := carrier
  have modeUnary : UnaryHistory modeRead :=
    unary_cont_closed mUnary wUnary modeRoute
  ...
  have auditCert :
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, auditRoute, auditPkg⟩
  }
  have dependencyCert :
      SemanticNameCert
          (fun row : BHist => hsame row dependencyRead ∧ UnaryHistory row)
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, dependencyRoute, dependencyPkg⟩
  }
  exact
    ⟨auditCert, dependencyCert, witnessAxiomUnary, ledgerUnary, publicUnary⟩
