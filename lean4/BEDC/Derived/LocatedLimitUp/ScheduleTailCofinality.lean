...
theorem LocatedLimitScheduleTailCofinality [AskSetup] [PackageSetup]
    {S M T Q E _H _C P _N scheduleRead readbackRead sealRead tailRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    Cont S M scheduleRead ->
      Cont scheduleRead T readbackRead ->
        Cont readbackRead Q sealRead ->
                    ...
                    UnaryHistory Q ->
                      UnaryHistory E ->
                        SemanticNameCert
                            ...
                            (fun row : BHist =>
                              hsame row S ∨ hsame row M ∨ hsame row T ∨ hsame row Q ∨
                                hsame row E ∨ hsame row scheduleRead ∨
  ...
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro scheduleCont readbackCont sealCont tailCont pkgP unaryS unaryM unaryT unaryQ unaryE
  have scheduleUnary : UnaryHistory scheduleRead :=
    unary_cont_closed unaryS unaryM scheduleCont
  have readbackUnary : UnaryHistory readbackRead :=
    unary_cont_closed scheduleUnary unaryT readbackCont
  ...
  exact ⟨cert, scheduleUnary, readbackUnary, sealUnary, tailUnary⟩
