import BEDC.Derived.LawCertificateUp.TasteGate
import BEDC.FKernel.Ask
import BEDC.FKernel.Bundle
import BEDC.FKernel.Cont
import BEDC.FKernel.NameCert
import BEDC.FKernel.Package

namespace BEDC.Derived.LawCertificateUp

open BEDC.FKernel.Ask
open BEDC.FKernel.Bundle
open BEDC.FKernel.Cont
open BEDC.FKernel.Hist
open BEDC.FKernel.NameCert
open BEDC.FKernel.Package

theorem LawCertificateBridgeCarrierRoute [AskSetup] [PackageSetup]
    {F P C S E Ld H R Q N stabilityRead scopedRead transported publicRead bridgeRead :
      BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    lawCertificateFields (LawCertificateUp.mk F P C S E Ld H R Q N) =
        [F, P, C, S, E, Ld, H, R, Q, N] →
      Cont F P C →
        Cont S E stabilityRead →
          Cont stabilityRead Ld scopedRead →
            Cont H R transported →
              Cont transported N publicRead →
                Cont publicRead N bridgeRead →
                  PkgSig bundle N pkg →
                    PkgSig bundle publicRead pkg →
                      PkgSig bundle bridgeRead pkg →
                        SemanticNameCert
                            (fun row : BHist =>
                              hsame row bridgeRead ∧
                                ∃ packet : LawCertificateUp,
                                  packet = LawCertificateUp.mk F P C S E Ld H R Q N ∧
                                    lawCertificateFields packet =
                                      [F, P, C, S, E, Ld, H, R, Q, N])
                            (fun row : BHist =>
                              Cont F P C ∧ Cont S E stabilityRead ∧
                                Cont stabilityRead Ld scopedRead ∧
                                  Cont transported N publicRead ∧
                                    Cont publicRead N row)
                            (fun row : BHist =>
                              hsame row bridgeRead ∧ Cont H R transported ∧
                                PkgSig bundle N pkg ∧ PkgSig bundle publicRead pkg ∧
                                  PkgSig bundle bridgeRead pkg)
                            hsame ∧
                          Cont publicRead N bridgeRead := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle PkgSig SemanticNameCert hsame
  intro fieldsExact sourceClassifier stabilityRoute scopedRoute transportedRoute publicRoute
    bridgeRoute packageName packagePublic packageBridge
  constructor
  · exact {
      core := {
        carrier_inhabited := by
          exact
            ⟨bridgeRead, hsame_refl bridgeRead,
              LawCertificateUp.mk F P C S E Ld H R Q N, rfl, fieldsExact⟩
        equiv_refl := by
          intro row _source
          exact hsame_refl row
        equiv_symm := by
          intro _row _row' sameRows
          exact hsame_symm sameRows
        equiv_trans := by
          intro _row _middle _row' sameLeft sameRight
          exact hsame_trans sameLeft sameRight
        carrier_respects_equiv := by
          intro row row' sameRows source
          exact
            ⟨hsame_trans (hsame_symm sameRows) source.left,
              source.right⟩
      }
      pattern_sound := by
        intro row source
        exact
          ⟨sourceClassifier, stabilityRoute, scopedRoute, publicRoute,
            cont_result_hsame_transport bridgeRoute (hsame_symm source.left)⟩
      ledger_sound := by
        intro row source
        exact
          ⟨source.left, transportedRoute, packageName, packagePublic, packageBridge⟩
    }
  · exact bridgeRoute

end BEDC.Derived.LawCertificateUp
