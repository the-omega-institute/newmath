...
theorem CauchyOscillationCarrier_ledger_nonescape [AskSetup] [PackageSetup]
                ...
                SemanticNameCert
                    ...
                    (fun row : BHist =>
                      hsame row tailWindow ∨ hsame row modulus ∨ hsame row tolerance ∨
                        hsame row ledger ∨ hsame row ledgerRead ∨ hsame row sealRead)
                    (fun row : BHist =>
                      UnaryHistory row ∧ Cont modulus tolerance ledgerRead ∧
                        Cont ledgerRead sealRow sealRead ∧ PkgSig bundle provenance pkg ∧
                          PkgSig bundle nameCert pkg)
                    hsame ∧
  ...
  intro carrier _tailModulusTolerance ledgerRoute sealRoute provenancePkg nameCertPkg
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, ledgerRoute, sealRoute, provenancePkg, nameCertPkg⟩
  }
  exact ⟨cert, ledgerReadUnary, sealReadUnary⟩
