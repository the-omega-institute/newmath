...
theorem SetlikeMembershipSourceAdmission [AskSetup] [PackageSetup] (S : SetlikeUp)
    {M Q I R E H C P N membershipRead transportedRead replayRead namedRead : BHist}
            ...
            UnaryHistory C ->
              UnaryHistory N ->
                Cont M Q membershipRead ->
                  Cont membershipRead H transportedRead ->
  ...
  intro fields rowsM rowsQ rowsH rowsC rowsN membershipRoute transportRoute replayRoute
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row M ∨ hsame row Q ∨ hsame row H ∨ hsame row C ∨ hsame row P ∨
              hsame row N ∨ hsame row membershipRead ∨ hsame row transportedRead ∨
                hsame row replayRead ∨ hsame row namedRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont M Q membershipRead ∧
              Cont membershipRead H transportedRead ∧ Cont transportedRead C replayRead ∧
                Cont replayRead N namedRead ∧ PkgSig bundle P pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, membershipRoute, transportRoute, replayRoute, namedRoute, packageRead⟩
  }
  exact ⟨cert, membershipUnary, transportedUnary, replayUnary, namedUnary⟩
