...
theorem SetlikeFirstOrderMembershipSubstitutionBoundary [AskSetup] [PackageSetup] (S : SetlikeUp)
    {M Q I R E H C P N membershipRead subsetRead comprehensionRead extensionalRead
      substitutionRead namedRead : BHist}
                      ...
                      Cont Q I subsetRead ->
                        Cont R E comprehensionRead ->
                          Cont membershipRead subsetRead extensionalRead ->
                            Cont extensionalRead H substitutionRead ->
                              Cont substitutionRead N namedRead ->
                                PkgSig bundle P pkg ->
                                  SemanticNameCert
  ...
  intro fields rowsM rowsQ rowsI rowsR rowsE rowsH rowsN membershipRoute subsetRoute
    comprehensionRoute extensionalRoute substitutionRoute namedRoute packageRead
  ...
  have substitutionUnary : UnaryHistory substitutionRead :=
    unary_cont_closed extensionalUnary rowsH substitutionRoute
  have namedUnary : UnaryHistory namedRead :=
    unary_cont_closed substitutionUnary rowsN namedRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row namedRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row M ∨ hsame row Q ∨ hsame row I ∨ hsame row R ∨ hsame row E ∨
              hsame row extensionalRead ∨ hsame row substitutionRead ∨ hsame row namedRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont M Q membershipRead ∧ Cont Q I subsetRead ∧
              Cont R E comprehensionRead ∧ Cont membershipRead subsetRead extensionalRead ∧
                Cont extensionalRead H substitutionRead ∧ Cont substitutionRead N namedRead ∧
                  PkgSig bundle P pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, membershipRoute, subsetRoute, comprehensionRoute, extensionalRoute,
          substitutionRoute, namedRoute, packageRead⟩
  }
  exact ⟨cert, substitutionUnary, namedUnary⟩
