...
theorem SetlikeSubsetComprehensionStability [AskSetup] [PackageSetup] (S : SetlikeUp)
    {M Q I R E H C P N membershipRead implicationRead comprehensionRead namedRead : BHist}
                ...
                Cont M Q membershipRead →
                  Cont membershipRead I implicationRead →
                    Cont implicationRead R comprehensionRead →
                      Cont comprehensionRead N namedRead →
                        PkgSig bundle P pkg →
                          SemanticNameCert
                            ...
                            UnaryHistory membershipRead ∧ UnaryHistory implicationRead ∧
                              UnaryHistory comprehensionRead ∧ UnaryHistory namedRead := by
  -- BEDC touchpoint anchor: SetlikeUp setlikeFields BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro fields unaryM unaryQ unaryI unaryR unaryN membershipRoute implicationRoute
  ...
  have implicationUnary : UnaryHistory implicationRead :=
    unary_cont_closed membershipUnary unaryI implicationRoute
  have comprehensionUnary : UnaryHistory comprehensionRead :=
    unary_cont_closed implicationUnary unaryR comprehensionRoute
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont M Q membershipRead ∧
              Cont membershipRead I implicationRead ∧ Cont implicationRead R comprehensionRead ∧
                Cont comprehensionRead N namedRead ∧ PkgSig bundle P pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, membershipRoute, implicationRoute, comprehensionRoute, namedRoute,
          packageRead⟩
  }
  exact ⟨cert, membershipUnary, implicationUnary, comprehensionUnary, namedUnary⟩
