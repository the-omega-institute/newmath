...
theorem SetlikeFiniteFamilyMembershipRoute [AskSetup] [PackageSetup] (S : SetlikeUp)
    {M Q I R E H C P N membershipRead subsetRead familyRead transportedRead replayRead
      namedRead : BHist}
                  ...
                  UnaryHistory N ->
                    Cont M Q membershipRead ->
                      Cont membershipRead I subsetRead ->
                        Cont subsetRead R familyRead ->
                          Cont familyRead H transportedRead ->
                                      ...
                                      (fun row : BHist =>
                                        hsame row M ∨ hsame row Q ∨ hsame row I ∨
                                          hsame row R ∨ hsame row membershipRead ∨
                                            hsame row subsetRead ∨ hsame row familyRead ∨
                                              hsame row namedRead)
                                      (fun row : BHist =>
                                          ...
                                          Cont membershipRead I subsetRead ∧
                                            Cont subsetRead R familyRead ∧
                                              Cont replayRead N namedRead ∧
                                                PkgSig bundle P pkg)
                                      hsame ∧
                                    UnaryHistory membershipRead ∧ UnaryHistory subsetRead ∧
                                      UnaryHistory familyRead ∧ UnaryHistory transportedRead ∧
  ...
  intro fields rowsM rowsQ rowsI rowsR rowsH rowsC rowsN membershipRoute subsetRoute
    familyRoute transportRoute replayRoute namedRoute packageRead
  ...
  have familyUnary : UnaryHistory familyRead :=
    unary_cont_closed subsetUnary rowsR familyRoute
  have transportedUnary : UnaryHistory transportedRead :=
    unary_cont_closed familyUnary rowsH transportRoute
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row M ∨ hsame row Q ∨ hsame row I ∨ hsame row R ∨
              hsame row membershipRead ∨ hsame row subsetRead ∨ hsame row familyRead ∨
                hsame row namedRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont M Q membershipRead ∧
              Cont membershipRead I subsetRead ∧ Cont subsetRead R familyRead ∧
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, membershipRoute, subsetRoute, familyRoute, namedRoute, packageRead⟩
  }
  exact
    ⟨cert, membershipUnary, subsetUnary, familyUnary, transportedUnary, replayUnary, namedUnary⟩
