...
theorem SetlikeRootTypeModelMembershipFactorization [AskSetup] [PackageSetup] (S : SetlikeUp)
    {M Q I R E H C P N membershipRead subsetRead comprehensionRead extensionalRead
      transportRead replayRead provenanceRead namedRead typeModelRead : BHist}
                                  ...
                                  Cont transportRead C replayRead ->
                                    Cont replayRead P provenanceRead ->
                                      Cont provenanceRead N namedRead ->
                                        Cont namedRead Q typeModelRead ->
                                          PkgSig bundle P pkg ->
                                            SemanticNameCert
                                                (fun row : BHist =>
                                                  hsame row typeModelRead ∧
                                                    UnaryHistory row)
                                                (fun row : BHist =>
                                                                ...
                                                                Cont replayRead P
                                                                  ...
                                                                  Cont provenanceRead N
                                                                    ...
                                                                    Cont namedRead Q
                                                                      typeModelRead ∧
                                                                      PkgSig bundle P pkg)
                                                hsame ∧ UnaryHistory typeModelRead := by
  ...
  intro fields rowsM rowsQ rowsI rowsR rowsE rowsH rowsC rowsP rowsN membershipRoute
    ...
    namedRoute typeModelRoute packageRead
  ...
  have typeModelUnary : UnaryHistory typeModelRead :=
    unary_cont_closed namedUnary rowsQ typeModelRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row typeModelRead ∧ UnaryHistory row)
          (fun row : BHist =>
              ...
              Cont R E comprehensionRead ∧ Cont membershipRead subsetRead extensionalRead ∧
                Cont extensionalRead H transportRead ∧ Cont transportRead C replayRead ∧
                  Cont replayRead P provenanceRead ∧ Cont provenanceRead N namedRead ∧
                    Cont namedRead Q typeModelRead ∧ PkgSig bundle P pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro typeModelRead
        ⟨hsame_refl typeModelRead, typeModelUnary⟩
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, membershipRoute, subsetRoute, comprehensionRoute, extensionalRoute,
          transportRoute, replayRoute, provenanceRoute, namedRoute, typeModelRoute,
          packageRead⟩
  }
  exact ⟨cert, typeModelUnary⟩
