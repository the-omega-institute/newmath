...
theorem SetlikeTypelikeSignatureScope [AskSetup] [PackageSetup] (S : SetlikeUp)
    {M Q I R E H C P N membershipReplay subsetReplay comprehensionReplay extensionalReplay
      routeRead signatureRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    setlikeFields S = [M, Q, I, R, E, H, C, P, N] ->
      UnaryHistory M ->
        UnaryHistory Q ->
                  ...
                  Cont M Q membershipReplay ->
                    Cont Q I subsetReplay ->
                      Cont R E comprehensionReplay ->
                        Cont membershipReplay subsetReplay extensionalReplay ->
                          Cont extensionalReplay comprehensionReplay routeRead ->
                            Cont routeRead H signatureRead ->
                              PkgSig bundle P pkg ->
                                SemanticNameCert
                                    (fun row : BHist =>
                                      hsame row signatureRead ∧ UnaryHistory row)
                                    (fun row : BHist =>
                                      hsame row M ∨ hsame row Q ∨ hsame row I ∨
                                      ...
                                      UnaryHistory comprehensionReplay ∧
                                        UnaryHistory extensionalReplay ∧
                                          UnaryHistory routeRead ∧
                                            UnaryHistory signatureRead := by
  -- BEDC touchpoint anchor: SetlikeUp setlikeFields BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro fields rowsM rowsQ rowsI rowsR rowsE rowsH membershipRoute subsetRoute
    comprehensionRoute extensionalRoute routeReadRoute signatureRoute packageRead
  have _acceptedFields : setlikeFields S = [M, Q, I, R, E, H, C, P, N] := fields
  have membershipUnary : UnaryHistory membershipReplay :=
    unary_cont_closed rowsM rowsQ membershipRoute
  have subsetUnary : UnaryHistory subsetReplay :=
  ...
  have signatureUnary : UnaryHistory signatureRead :=
    unary_cont_closed routeUnary rowsH signatureRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row signatureRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row M ∨ hsame row Q ∨ hsame row I ∨ hsame row R ∨ hsame row E ∨
              hsame row routeRead ∨ hsame row signatureRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont extensionalReplay comprehensionReplay routeRead ∧
              Cont routeRead H signatureRead ∧ PkgSig bundle P pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro signatureRead ⟨hsame_refl signatureRead, signatureUnary⟩
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, routeReadRoute, signatureRoute, packageRead⟩
  }
  exact
    ⟨cert, membershipUnary, subsetUnary, comprehensionUnary, extensionalUnary, routeUnary,
      signatureUnary⟩

theorem SetlikeTypedComprehensionKernelEnvelope [AskSetup] [PackageSetup] (S : SetlikeUp)
    {M Q I R E H C P N membershipReplay subsetReplay comprehensionReplay typedEnvelopeRead
      namedRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    setlikeFields S = [M, Q, I, R, E, H, C, P, N] ->
      UnaryHistory M ->
                  ...
                  Cont M Q membershipReplay ->
                    Cont Q I subsetReplay ->
                      Cont subsetReplay R comprehensionReplay ->
                        Cont comprehensionReplay H typedEnvelopeRead ->
                          Cont typedEnvelopeRead N namedRead ->
                            PkgSig bundle P pkg ->
                              SemanticNameCert
                                  (fun row : BHist => hsame row namedRead ∧ UnaryHistory row)
                                  (fun row : BHist =>
                                    hsame row M ∨ hsame row Q ∨ hsame row I ∨
                                      hsame row R ∨ hsame row typedEnvelopeRead ∨
                                        hsame row namedRead)
                                  (fun row : BHist =>
                                    UnaryHistory row ∧ Cont M Q membershipReplay ∧
                                      Cont Q I subsetReplay ∧
                                ...
                                UnaryHistory membershipReplay ∧
                                  UnaryHistory subsetReplay ∧
                                    UnaryHistory comprehensionReplay ∧
                                      UnaryHistory typedEnvelopeRead ∧
                                        UnaryHistory namedRead := by
  -- BEDC touchpoint anchor: SetlikeUp setlikeFields BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro fields rowsM rowsQ rowsI rowsR rowsH rowsN membershipRoute subsetRoute
    comprehensionRoute typedEnvelopeRoute namedRoute packageRead
  have _acceptedFields : setlikeFields S = [M, Q, I, R, E, H, C, P, N] := fields
  have membershipUnary : UnaryHistory membershipReplay :=
    unary_cont_closed rowsM rowsQ membershipRoute
  have subsetUnary : UnaryHistory subsetReplay :=
  ...
  have typedEnvelopeUnary : UnaryHistory typedEnvelopeRead :=
    unary_cont_closed comprehensionUnary rowsH typedEnvelopeRoute
  have namedUnary : UnaryHistory namedRead :=
    unary_cont_closed typedEnvelopeUnary rowsN namedRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row namedRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row M ∨ hsame row Q ∨ hsame row I ∨ hsame row R ∨
              hsame row typedEnvelopeRead ∨ hsame row namedRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont M Q membershipReplay ∧ Cont Q I subsetReplay ∧
              Cont subsetReplay R comprehensionReplay ∧
                Cont comprehensionReplay H typedEnvelopeRead ∧
                  Cont typedEnvelopeRead N namedRead ∧ PkgSig bundle P pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro namedRead ⟨hsame_refl namedRead, namedUnary⟩
      equiv_refl := by
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, membershipRoute, subsetRoute, comprehensionRoute,
          typedEnvelopeRoute, namedRoute, packageRead⟩
  }
  exact
    ⟨cert, membershipUnary, subsetUnary, comprehensionUnary, typedEnvelopeUnary, namedUnary⟩
