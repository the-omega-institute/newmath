...
theorem SetlikeRootMembershipFamilyExhaustion [AskSetup] [PackageSetup] (S : SetlikeUp)
    {M Q I R E H C P N membershipReplay subsetReplay comprehensionReplay extensionalReplay
      routeRead firstOrderRead modelRead typeRead : BHist}
                                  ...
                                  Cont membershipReplay C modelRead ->
                                    Cont membershipReplay P typeRead ->
                                      PkgSig bundle N pkg ->
                                        SemanticNameCert
                                            (fun row : BHist =>
                                              hsame row routeRead ∨ hsame row firstOrderRead ∨
                                                hsame row modelRead ∨ hsame row typeRead)
                                            (fun row : BHist =>
  ...
  have cert :
      SemanticNameCert
          ...
          hsame := {
    core := {
      carrier_inhabited := Exists.intro routeRead (Or.inl (hsame_refl routeRead))
      equiv_refl := by
        intro row _source
      ...
      carrier_respects_equiv := by
        ...
        cases source with
        | inl routeSource =>
            exact Or.inl (hsame_trans (hsame_symm sameRows) routeSource)
        | inr tail =>
            cases tail with
            | inl firstSource =>
                exact Or.inr (Or.inl (hsame_trans (hsame_symm sameRows) firstSource))
            | inr tail =>
                cases tail with
                | inl modelSource =>
                    exact Or.inr
                      (Or.inr (Or.inl (hsame_trans (hsame_symm sameRows) modelSource)))
                | inr typeSource =>
                    exact Or.inr
                      (Or.inr (Or.inr (hsame_trans (hsame_symm sameRows) typeSource)))
    }
    pattern_sound := by
      intro _row source
      cases source with
      | inl routeSource =>
          exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl routeSource)))))
      | inr tail =>
          cases tail with
          | inl firstSource =>
              exact Or.inr
                (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl firstSource))))))
          | inr tail =>
              cases tail with
              | inl modelSource =>
                  exact Or.inr
                    (Or.inr
                      (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl modelSource)))))))
              | inr typeSource =>
                  exact Or.inr
                    (Or.inr
                      (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr typeSource)))))))
    ledger_sound := by
      intro _row source
      cases source with
      | inl routeSource =>
          exact
            ⟨unary_transport routeUnary (hsame_symm routeSource), membershipRoute,
              packageRead⟩
      | inr tail =>
          cases tail with
          | inl firstSource =>
  ...
  exact ⟨cert, routeUnary, firstOrderUnary, modelUnary, typeUnary⟩
