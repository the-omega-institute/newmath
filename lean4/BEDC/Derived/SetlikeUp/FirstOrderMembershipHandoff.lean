...
theorem SetlikeFirstOrderMembershipHandoff [AskSetup] [PackageSetup] (S : SetlikeUp)
    {M Q I R E H C P N membershipRead subsetRead comprehensionRead extensionalRead
      firstOrderRead namedRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    setlikeFields S = [M, Q, I, R, E, H, C, P, N] →
      UnaryHistory M → UnaryHistory Q → UnaryHistory I → UnaryHistory R →
        UnaryHistory E → UnaryHistory H → UnaryHistory C → UnaryHistory N →
          Cont M Q membershipRead → Cont Q I subsetRead → Cont R E comprehensionRead →
            Cont membershipRead subsetRead extensionalRead →
              Cont extensionalRead H firstOrderRead →
                Cont firstOrderRead N namedRead → PkgSig bundle P pkg →
                  SemanticNameCert
                    (fun row : BHist => hsame row namedRead ∧ UnaryHistory row)
                    (fun row : BHist =>
                      hsame row M ∨ hsame row Q ∨ hsame row I ∨ hsame row R ∨
                        hsame row E ∨ hsame row firstOrderRead ∨ hsame row namedRead)
                    (fun row : BHist =>
                      UnaryHistory row ∧ Cont M Q membershipRead ∧
                        Cont Q I subsetRead ∧ Cont R E comprehensionRead ∧
                          Cont membershipRead subsetRead extensionalRead ∧
                            Cont extensionalRead H firstOrderRead ∧
                              Cont firstOrderRead N namedRead ∧ PkgSig bundle P pkg)
                    hsame ∧ UnaryHistory firstOrderRead ∧ UnaryHistory namedRead := by
  -- BEDC touchpoint anchor: SetlikeUp setlikeFields BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro fields unaryM unaryQ unaryI unaryR unaryE unaryH _unaryC unaryN membershipRoute
    subsetRoute comprehensionRoute extensionalRoute firstOrderRoute namedRoute packageRead
  ...
  have comprehensionUnary : UnaryHistory comprehensionRead :=
    unary_cont_closed unaryR unaryE comprehensionRoute
  ...
  have firstOrderUnary : UnaryHistory firstOrderRead :=
    unary_cont_closed extensionalUnary unaryH firstOrderRoute
  have namedUnary : UnaryHistory namedRead :=
    unary_cont_closed firstOrderUnary unaryN namedRoute
  have cert :
      SemanticNameCert
        (fun row : BHist => hsame row namedRead ∧ UnaryHistory row)
        (fun row : BHist =>
          hsame row M ∨ hsame row Q ∨ hsame row I ∨ hsame row R ∨ hsame row E ∨
            hsame row firstOrderRead ∨ hsame row namedRead)
        (fun row : BHist =>
          UnaryHistory row ∧ Cont M Q membershipRead ∧ Cont Q I subsetRead ∧
            Cont R E comprehensionRead ∧ Cont membershipRead subsetRead extensionalRead ∧
              Cont extensionalRead H firstOrderRead ∧ Cont firstOrderRead N namedRead ∧
                PkgSig bundle P pkg)
        hsame := {
    core := {
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, membershipRoute, subsetRoute, comprehensionRoute, extensionalRoute,
          firstOrderRoute, namedRoute, packageRead⟩
  }
  exact ⟨cert, firstOrderUnary, namedUnary⟩
