...
theorem SetlikeMembershipClassifierTransport [AskSetup] [PackageSetup] (S : SetlikeUp)
    {M Q I R E H C P N membershipReplay subsetReplay transportedReplay namedReplay : BHist}
                    ...
                    Cont M Q membershipReplay ->
                      Cont Q I subsetReplay ->
                        Cont membershipReplay H transportedReplay ->
                          Cont transportedReplay N namedReplay ->
                            PkgSig bundle P pkg ->
                              SemanticNameCert
                                ...
                                UnaryHistory membershipReplay ∧ UnaryHistory subsetReplay ∧
                                  UnaryHistory transportedReplay ∧ UnaryHistory namedReplay := by
  -- BEDC touchpoint anchor: SetlikeUp setlikeFields BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro fields rowsM rowsQ rowsI rowsH _rowsC _rowsP rowsN membershipRoute subsetRoute
    transportedRoute namedRoute packageRead
  ...
  have namedUnary : UnaryHistory namedReplay :=
    unary_cont_closed transportedUnary rowsN namedRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row namedReplay ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row M ∨ hsame row Q ∨ hsame row I ∨ hsame row H ∨ hsame row C ∨
              hsame row P ∨ hsame row N ∨ hsame row membershipReplay ∨
                hsame row subsetReplay ∨ hsame row transportedReplay ∨
                  hsame row namedReplay)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont M Q membershipReplay ∧ Cont Q I subsetReplay ∧
              Cont membershipReplay H transportedReplay ∧ Cont transportedReplay N namedReplay ∧
                PkgSig bundle P pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, membershipRoute, subsetRoute, transportedRoute, namedRoute, packageRead⟩
  }
  exact ⟨cert, membershipUnary, subsetUnary, transportedUnary, namedUnary⟩
