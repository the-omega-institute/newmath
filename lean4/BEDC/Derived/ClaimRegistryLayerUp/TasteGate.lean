...
theorem ClaimRegistryLayer_bridge_route_certificate [AskSetup] [PackageSetup]
    {E R B C F V H P N bridgeRead : BHist} {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
  ...
  intro bridgePkg entryRefusalBridge
  have cert :
      SemanticNameCert
        (fun row : BHist => hsame row bridgeRead)
        (fun row : BHist =>
              ...
              (Or.inr
                (Or.inr
                  (Or.inr
                    (Or.inr
                      (Or.inr
                        (Or.inr entryRefusalBridge))))))))
    ledger_sound := by
      intro _row source
      exact ⟨source, bridgePkg⟩
  }
  exact ⟨rfl, cert⟩
