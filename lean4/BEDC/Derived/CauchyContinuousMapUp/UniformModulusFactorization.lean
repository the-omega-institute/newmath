...
namespace BEDC.Derived.CauchyContinuousMapUp.UniformModulusFactorization
...
theorem CauchyContinuousMap_uniform_modulus_factorization [AskSetup] [PackageSetup]
    (M : CauchyContinuousMapUp) {imageRead sealRead boundaryRead modulusRead uniformRead
      publicRead : BHist}
          ...
          Cont sealRead M.replay boundaryRead →
            Cont boundaryRead M.localName modulusRead →
              Cont modulusRead M.transport uniformRead →
                Cont uniformRead M.provenance publicRead →
                          ...
                          hsame row M.windows ∨ hsame row M.imageReadback ∨
                            hsame row M.toleranceLedger ∨ hsame row M.realSealHandoff ∨
                              hsame row M.replay ∨ hsame row M.localName ∨
                                hsame row M.transport ∨ hsame row M.provenance ∨
  ...
  intro packet imageRoute sealRoute boundaryRoute modulusRoute uniformRoute publicRoute
    publicPkg
  ...
  have publicUnary : UnaryHistory publicRead :=
    unary_cont_closed uniformUnary provenanceUnary publicRoute
  have sourcePublic :
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, imageRoute, sealRoute, boundaryRoute, modulusRoute,
          uniformRoute, publicRoute, publicPkg⟩
  }
  exact
    ⟨cert, imageUnary, sealUnaryRead, boundaryUnary, modulusUnary, uniformUnary,
      publicUnary⟩

end BEDC.Derived.CauchyContinuousMapUp.UniformModulusFactorization
