...
theorem SimplicialComplexUnion_finite_face_closed_dimension_grading
    ...
    (dimMonoL :
      forall {alpha beta : BHist},
        SimplexL alpha -> SimplexL beta -> Face alpha beta -> dim alpha <= dim beta) :
    (forall {alpha beta : BHist}, SimplexK alpha ∨ SimplexL alpha -> Face alpha beta ->
      SimplexK beta ∨ SimplexL beta -> dim alpha <= dim beta) ∧
      (forall {rho tau sigma : BHist}, SimplexK sigma ∨ SimplexL sigma ->
        Face tau sigma -> Face rho tau ->
          dim rho <= dim tau ∧ dim tau <= dim sigma ∧ dim rho <= dim sigma) := by
  constructor
  · intro alpha beta simplexAlpha faceAlphaBeta simplexBeta
  ...
  · intro rho tau sigma simplexSigma faceTauSigma faceRhoTau
    exact SimplicialComplexUnion_face_dimension_grading simplices listed faceClosedK
