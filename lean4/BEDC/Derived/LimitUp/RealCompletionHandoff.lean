...
theorem LimitRealCompletionHandoff [AskSetup] [PackageSetup]
    {S R D A T C H P N windowRead toleranceRead sealRead handoffRead replayRead : BHist}
  ...
  intro fieldRows sUnary rUnary dUnary aUnary tUnary cUnary windowRoute toleranceRoute
    sealRoute handoffRoute replayRoute provenancePkg namePkg
  cases fieldRows
  ...
  have replayUnary : UnaryHistory replayRead :=
    unary_cont_closed handoffUnary cUnary replayRoute
  constructor
  · exact {
      core := {
      ...
      ledger_sound := by
        intro _row source
        exact
          ⟨source.right, windowRoute, toleranceRoute, sealRoute, handoffRoute, replayRoute,
