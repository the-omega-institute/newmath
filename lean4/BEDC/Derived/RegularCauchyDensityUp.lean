...
theorem RegularCauchyDensityCarrier_namecert_obligations [AskSetup] [PackageSetup]
    {Q S R A E H C P N windowRead readbackRead sealRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    RegularCauchyDensityCarrier Q S R A E H C P N bundle pkg ->
      Cont S R windowRead ->
          ...
          Cont readbackRead E sealRead ->
            PkgSig bundle sealRead pkg ->
              SemanticNameCert
                  ...
                  UnaryHistory windowRead ∧ UnaryHistory readbackRead ∧
                    UnaryHistory sealRead := by
  -- BEDC touchpoint anchor: RegularCauchyDensityCarrier BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro carrier windowRoute readbackRoute sealRoute sealPkg
  obtain ⟨_qUnary, sUnary, rUnary, aUnary, eUnary, _hUnary, _cUnary, _pUnary,
    _nUnary, _carrierPkg⟩ := carrier
  have windowUnary : UnaryHistory windowRead :=
  ...
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row sealRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row Q ∨ hsame row S ∨ hsame row R ∨ hsame row A ∨ hsame row E ∨
