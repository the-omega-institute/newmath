...
theorem RegularCauchyReciprocalReadbackBudgetExactness
    (x : TasteGate.RegularCauchyReciprocalUp) :
    ∃ Q A M W D B T E H C P N : BHist,
      x = TasteGate.RegularCauchyReciprocalUp.mk Q A M W D B T E H C P N ∧
        List.Mem B (TasteGate.regularCauchyReciprocalFields x) ∧
          List.Mem T (TasteGate.regularCauchyReciprocalFields x) ∧
            List.Mem E (TasteGate.regularCauchyReciprocalFields x) ∧
              TasteGate.regularCauchyReciprocalFromEventFlow
                  (TasteGate.regularCauchyReciprocalToEventFlow x) =
                some x ∧
                TasteGate.regularCauchyReciprocalEncodeBHist BHist.Empty =
                  ([] : List BMark) := by
  -- BEDC touchpoint anchor: BHist BMark
      ...
      exact
        ⟨Q, A, M, W, D, B, T, E, H, C, P, N, rfl,
          ...
          List.Mem.tail Q
            (List.Mem.tail A
              (List.Mem.tail M
                (List.Mem.tail W
                  (List.Mem.tail D
                    (List.Mem.tail B (List.Mem.head [E, H, C, P, N])))))),
          List.Mem.tail Q
            (List.Mem.tail A
              (List.Mem.tail M
                (List.Mem.tail W
          ...
          by
            change
              TasteGate.regularCauchyReciprocalFromEventFlow
                  (TasteGate.regularCauchyReciprocalToEventFlow
                    (TasteGate.RegularCauchyReciprocalUp.mk Q A M W D B T E H C P N)) =
                some
                  (TasteGate.RegularCauchyReciprocalUp.mk
                    Q A M W D B T E H C P N)
            exact
              TasteGate.regularCauchyReciprocal_round_trip
                (TasteGate.RegularCauchyReciprocalUp.mk Q A M W D B T E H C P N),
          rfl⟩
