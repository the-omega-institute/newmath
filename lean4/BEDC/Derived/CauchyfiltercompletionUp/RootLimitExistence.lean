...
theorem CauchyFilterCompletionPacket_root_cauchy_filter_limit_existence [AskSetup]
    [PackageSetup]
    {filter windows tolerance readback sealRow transport replay provenance name limitRead :
      BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    CauchyFilterCompletionPacket filter windows tolerance readback sealRow transport replay
        provenance name bundle pkg →
      Cont filter sealRow limitRead →
            ...
            (fun row : BHist =>
              CauchyFilterCompletionPacket filter windows tolerance readback sealRow transport
                ...
                (hsame row filter ∨ hsame row windows ∨ hsame row tolerance ∨
                  hsame row readback ∨ hsame row sealRow ∨ hsame row limitRead))
            (fun _row : BHist =>
              Cont filter windows tolerance ∧ Cont tolerance readback sealRow ∧
                Cont filter sealRow limitRead ∧ Cont transport replay provenance ∧
                  PkgSig bundle provenance pkg)
            (fun row : BHist => UnaryHistory row ∧ PkgSig bundle limitRead pkg)
            hsame := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg UnaryHistory Cont PkgSig hsame SemanticNameCert
  intro packet filterSeal limitPkg
  ...
  have limitUnary : UnaryHistory limitRead :=
    unary_cont_closed filterUnary sealUnary filterSeal
  have sourceFilter :
      (fun row : BHist =>
        CauchyFilterCompletionPacket filter windows tolerance readback sealRow transport replay
            provenance name bundle pkg ∧
          (hsame row filter ∨ hsame row windows ∨ hsame row tolerance ∨
            hsame row readback ∨ hsame row sealRow ∨ hsame row limitRead)) filter := by
                      ...
                      | inr rest =>
                          ...
                          | inr rest =>
                              ...
                              | inr sameLimit =>
                                  exact
                                    Or.inr <| Or.inr <| Or.inr <| Or.inr <|
                                      Or.inr (hsame_trans (hsame_symm sameRows) sameLimit)
    }
    pattern_sound := by
      intro _row _source
      exact ⟨filterWindows, toleranceReadback, filterSeal, transportReplay, provenancePkg⟩
    ledger_sound := by
      intro row source
      cases source with
      | intro _sourcePacket sourceRows =>
          cases sourceRows with
          | inl sameFilter =>
              exact ⟨unary_transport filterUnary (hsame_symm sameFilter), limitPkg⟩
          | inr rest =>
              cases rest with
              | inl sameWindows =>
                  exact ⟨unary_transport windowsUnary (hsame_symm sameWindows), limitPkg⟩
              | inr rest =>
                  cases rest with
                  | inl sameTolerance =>
                      exact
                  ...
                  | inr rest =>
                      ...
                      | inr rest =>
                          cases rest with
                          | inl sameSeal =>
                              exact ⟨unary_transport sealUnary (hsame_symm sameSeal), limitPkg⟩
                          | inr sameLimit =>
                              exact
                                ⟨unary_transport limitUnary (hsame_symm sameLimit), limitPkg⟩
  }
