...
theorem CauchyFilterCompletionRootFilterRoute [AskSetup] [PackageSetup]
    {filter windows tolerance readback sealRow transport replay provenance name completionRead :
      BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    CauchyFilterCompletionPacket filter windows tolerance readback sealRow transport replay
        provenance name bundle pkg →
      Cont sealRow provenance completionRead →
        PkgSig bundle completionRead pkg →
          SemanticNameCert
            (fun row : BHist =>
              CauchyFilterCompletionPacket filter windows tolerance readback sealRow transport
                ...
                (hsame row filter ∨ hsame row windows ∨ hsame row tolerance ∨
                  hsame row readback ∨ hsame row sealRow ∨ hsame row completionRead))
            (fun _row : BHist =>
              Cont filter windows tolerance ∧ Cont tolerance readback sealRow ∧
                Cont transport replay provenance ∧ PkgSig bundle provenance pkg)
  ...
  have sourceFilter :
      (fun row : BHist =>
        CauchyFilterCompletionPacket filter windows tolerance readback sealRow transport replay
            provenance name bundle pkg ∧
          (hsame row filter ∨ hsame row windows ∨ hsame row tolerance ∨
            hsame row readback ∨ hsame row sealRow ∨ hsame row completionRead)) filter := by
    ...
    ledger_sound := by
      ...
      | intro _sourcePacket sourceRows =>
          ...
          | inr rest =>
              ...
              | inl sameWindows =>
                  exact
                    ⟨unary_transport windowsUnary (hsame_symm sameWindows), completionPkg⟩
              | inr rest =>
                  cases rest with
                  | inl sameTolerance =>
                      exact
                  ...
                  | inr rest =>
                      ...
                      | inr rest =>
                          ...
                          | inr sameCompletion =>
                              exact
                                ⟨unary_transport completionUnary (hsame_symm sameCompletion),
                                  completionPkg⟩
  }
