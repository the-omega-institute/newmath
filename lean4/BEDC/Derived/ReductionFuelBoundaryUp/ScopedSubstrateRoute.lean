...
theorem ReductionFuelBoundaryScopedSubstrateRoute [AskSetup] [PackageSetup]
  ...
  have cert :
    ...
    core := {
      carrier_inhabited :=
        Exists.intro hostExport ⟨hsame_refl hostExport, hostExportUnary⟩
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, hostFuelRoute, endpointRoute, timeoutRoute, auditRoute,
          replayRoute, hostExportRoute, provenancePkg, namePkg⟩
  }
  exact ⟨cert, hostExportUnary⟩
