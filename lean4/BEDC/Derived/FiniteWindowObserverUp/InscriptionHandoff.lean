...
theorem FiniteWindowObserverInscriptionHandoff [AskSetup] [PackageSetup]
            ...
            Cont acceptedRead D handoffRead →
              PkgSig bundle handoffRead pkg →
                SemanticNameCert
                    ...
                    (fun row : BHist =>
                      hsame row handoffRead ∧ Cont O F selectRead ∧
                        Cont selectRead I checkerRead ∧
                          Cont checkerRead K acceptedRead ∧
                            Cont acceptedRead D handoffRead)
                    (fun row : BHist =>
  ...
  have cert :
      SemanticNameCert
        ...
        (fun row : BHist =>
          hsame row handoffRead ∧ Cont O F selectRead ∧
            Cont selectRead I checkerRead ∧ Cont checkerRead K acceptedRead ∧
              Cont acceptedRead D handoffRead)
    ...
    pattern_sound := by
      intro _row source
      exact ⟨source.left, selectRoute, checkerRoute, acceptedRoute, handoffRoute⟩
    ledger_sound := by
      intro _row source
