...
theorem IdealIntersection_closure_rows
    ...
    (_carrierZero : Carrier zero)
    (_carrierAdd : forall {x y : BHist}, Carrier x -> Carrier y -> Carrier (add x y))
    (_carrierNeg : forall {x : BHist}, Carrier x -> Carrier (neg x))
    (_carrierMul : forall {x y : BHist}, Carrier x -> Carrier y -> Carrier (mul x y))
    ...
    (_J_support : forall {x : BHist}, J x -> Carrier x)
    (J_zero : J zero)
...
theorem IdealSum_closure_rows
    ...
    (I_neg : forall {x : BHist}, I x -> I (neg x))
    (I_mul : forall {x y : BHist}, I x -> I y -> I (mul x y))
    (_I_transport : forall {x y : BHist}, I x -> Classifier x y -> I y)
    (I_absorb : forall {r x : BHist}, Carrier r -> I x -> I (mul r x) ∧ I (mul x r))
    ...
    (J_neg : forall {x : BHist}, J x -> J (neg x))
    (J_mul : forall {x y : BHist}, J x -> J y -> J (mul x y))
    (_J_transport : forall {x y : BHist}, J x -> Classifier x y -> J y)
    (J_absorb : forall {r x : BHist}, Carrier r -> J x -> J (mul r x) ∧ J (mul x r))
    (addCongr :
