...
theorem MetacicNormalizationAuditResidualFuelDecreases [AskSetup] [PackageSetup]
    {kernel normalizer frontier sn confluence audit ledger transport replay provenance localName
      residualRead spentFuel : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    MetacicNormalizationAuditUp kernel normalizer frontier sn confluence audit ledger transport
      ...
      Cont audit replay residualRead ->
        Cont residualRead sn spentFuel ->
          PkgSig bundle spentFuel pkg ->
            SemanticNameCert
                (fun row : BHist => hsame row spentFuel ∧ UnaryHistory row ∧
                  PkgSig bundle row pkg)
                (fun row : BHist =>
                  hsame row audit ∨ hsame row replay ∨ hsame row residualRead ∨
                    hsame row sn ∨ hsame row spentFuel ∨ hsame row provenance)
                (fun row : BHist =>
                  hsame row spentFuel ∧ Cont audit replay residualRead ∧
                    Cont residualRead sn spentFuel ∧ PkgSig bundle provenance pkg)
                hsame ∧ UnaryHistory residualRead ∧ UnaryHistory spentFuel := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier auditReplayResidual residualSnSpent spentPkg
  obtain ⟨_kernelUnary, _normalizerUnary, _frontierUnary, snUnary, _confluenceUnary,
  ...
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row spentFuel ∧ UnaryHistory row ∧
            PkgSig bundle row pkg)
          (fun row : BHist =>
            hsame row audit ∨ hsame row replay ∨ hsame row residualRead ∨ hsame row sn ∨
              hsame row spentFuel ∨ hsame row provenance)
          (fun row : BHist =>
            hsame row spentFuel ∧ Cont audit replay residualRead ∧
              Cont residualRead sn spentFuel ∧ PkgSig bundle provenance pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro spentFuel
        ⟨hsame_refl spentFuel, spentUnary, spentPkg⟩
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
    ...
    ledger_sound := by
      intro _row sourceRow
      exact ⟨sourceRow.left, auditReplayResidual, residualSnSpent, provenancePkg⟩
  }
  exact ⟨cert, residualUnary, spentUnary⟩

theorem MetacicNormalizationAuditPublicRoute [AskSetup] [PackageSetup]
    {kernel normalizer frontier sn confluence audit ledger transport replay provenance
        ...
        PkgSig bundle publicRead pkg ->
          SemanticNameCert
              ...
              (fun row : BHist =>
                hsame row kernel ∨ hsame row normalizer ∨ hsame row frontier ∨
                  hsame row sn ∨ hsame row confluence ∨ hsame row audit ∨
                    hsame row ledger ∨ hsame row publicRead ∨ hsame row provenance ∨
                      hsame row localName)
              (fun row : BHist =>
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row kernel ∨ hsame row normalizer ∨ hsame row frontier ∨ hsame row sn ∨
              hsame row confluence ∨ hsame row audit ∨ hsame row ledger ∨
                hsame row publicRead ∨ hsame row provenance ∨ hsame row localName)
          (fun row : BHist =>
            hsame row publicRead ∧ Cont audit replay publicRead ∧
              PkgSig bundle provenance pkg ∧ PkgSig bundle localName pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro publicRead
        ⟨hsame_refl publicRead, publicUnary, publicPkg⟩
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
    ...
    ledger_sound := by
      intro _row sourceRow
      exact ⟨sourceRow.left, auditReplayPublic, provenancePkg, localNamePkg⟩
  }
