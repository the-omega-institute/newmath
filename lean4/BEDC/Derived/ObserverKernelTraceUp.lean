...
theorem ObserverKernelTrace_mature_consumer_coverage
    {trace transport route probe signature ledger provenance localName consumer bridgeRead
      finalRead : BHist}
    (traceReplay : Cont trace ledger route)
    (consumerRoute : Cont route provenance consumer)
    (bridgeRoute : Cont consumer localName bridgeRead)
    (finalRoute : Cont bridgeRead ledger finalRead) :
    SemanticNameCert
      (fun row : BHist =>
        hsame row finalRead ∧
          ∃ packet : ObserverKernelTraceUp,
      ...
      (fun row : BHist =>
        Cont trace ledger route ∧ Cont route provenance consumer ∧
          Cont consumer localName bridgeRead ∧ Cont bridgeRead ledger finalRead ∧
            hsame row finalRead ∧ hsame probe probe ∧ hsame signature signature)
      (fun row : BHist =>
        hsame row finalRead ∧ hsame ledger ledger ∧ hsame provenance provenance ∧
          hsame localName localName)
      hsame := by
  -- BEDC touchpoint anchor: BHist hsame Cont SemanticNameCert
  refine
  ...
  · exact
      Exists.intro finalRead
        (And.intro (hsame_refl finalRead)
          (Exists.intro
            (ObserverKernelTraceUp.mk trace transport route probe signature ledger provenance
              localName)
            rfl))
