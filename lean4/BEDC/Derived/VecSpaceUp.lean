...
theorem VecSpaceSingletonSmul_image_semanticNameCert :
    SemanticNameCert VecSpaceSingletonCarrier
      (fun h : BHist =>
        exists r m : BHist, VecSpaceSingletonCarrier m ∧ hsame h (VecSpaceSingletonSmul r m))
      VecSpaceSingletonCarrier VecSpaceSingletonClassifier := by
  have emptyCarrier : VecSpaceSingletonCarrier BHist.Empty := hsame_refl BHist.Empty
  exact {
    core := {
    ...
    pattern_sound := by
      intro h carrier
      exact Exists.intro BHist.Empty (Exists.intro h (And.intro carrier carrier))
    ledger_sound := by
      intro _h carrier
      exact carrier
  }
...
theorem VecSpaceSingletonSmul_inverse_readback_classifier_iff {r m n : BHist}
    (p : FieldSingletonNonZero r) :
    VecSpaceSingletonCarrier m ->
      hsame (VecSpaceSingletonSmul (FieldSingletonInv r p) (VecSpaceSingletonSmul r m))
        BHist.Empty ∧
        (VecSpaceSingletonClassifier
          (VecSpaceSingletonSmul (FieldSingletonInv r p) (VecSpaceSingletonSmul r m)) n <->
            VecSpaceSingletonClassifier m n) := by
  intro carrierM
  have actionCarrier :
      VecSpaceSingletonCarrier
        (VecSpaceSingletonSmul (FieldSingletonInv r p) (VecSpaceSingletonSmul r m)) :=
    hsame_refl BHist.Empty
  constructor
  · exact actionCarrier
  · constructor
