...
theorem CauchyNetLimitCarrier_abstract_completion_bridge [AskSetup] [PackageSetup]
    {K W R D A H C P N windowRead readbackRead toleranceRead sealRead completionRead : BHist}
                ...
                PkgSig bundle P pkg ->
                  PkgSig bundle completionRead pkg ->
                    SemanticNameCert
                        ...
                        (fun row : BHist =>
                          hsame row K ∨ hsame row W ∨ hsame row R ∨ hsame row D ∨
                            hsame row A ∨ Cont sealRead C completionRead)
                        (fun row : BHist =>
  ...
  intro carrier windowRoute readbackRoute toleranceRoute sealRoute completionRoute
    provenancePkg completionPkg
  obtain ⟨kUnary, wUnary, rUnary, dUnary, aUnary, _hUnary, cUnary, _pUnary,
    _nUnary⟩ := carrier
  have windowUnary : UnaryHistory windowRead :=
    unary_cont_closed kUnary wUnary windowRoute
  have readbackUnary : UnaryHistory readbackRead :=
  ...
  have completionUnary : UnaryHistory completionRead :=
    unary_cont_closed sealUnary cUnary completionRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row completionRead ∧ UnaryHistory row)
          (fun row : BHist =>
    ...
    exact {
      ...
      ledger_sound := by
        ...
        exact ⟨provenancePkg, completionPkg, sourceRow.left⟩
    }
  exact
    ⟨cert, windowUnary, readbackUnary, toleranceUnary, sealUnary, completionUnary⟩
