...
theorem BaireCategoryCarrier_namecert_export_surface [AskSetup] [PackageSetup]
  ...
  have cert :
      SemanticNameCert
          (fun row : BHist =>
            hsame row denseRead ∨ hsame row threadRead ∨ hsame row limitRead ∨ hsame row M)
          (fun row : BHist => UnaryHistory row)
          (fun row : BHist => PkgSig bundle P pkg ∨ PkgSig bundle row pkg)
          hsame := {
    core := {
      ...
      carrier_respects_equiv := by
        ...
        | inr rest =>
            ...
            | inr rest =>
                ...
                | inl sameLimit =>
                    exact Or.inr
                      (Or.inr (Or.inl (hsame_trans (hsame_symm sameRows) sameLimit)))
                | inr sameMetric =>
                    exact Or.inr
                      (Or.inr (Or.inr (hsame_trans (hsame_symm sameRows) sameMetric)))
    }
    pattern_sound := by
      intro _row source
      cases source with
  ...
  exact ⟨cert, denseReadUnary, threadReadUnary, limitReadUnary, rfl⟩
