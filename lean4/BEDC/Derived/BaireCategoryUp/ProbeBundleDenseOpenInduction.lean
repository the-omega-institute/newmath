...
theorem BaireCategoryCarrier_probe_bundle_dense_open_induction [AskSetup] [PackageSetup]
    {B M D O R T H C P N baseRead denseRead refinedRead threadRead : BHist}
                          ...
                          UnaryHistory T ->
                            SemanticNameCert
                              ...
                              UnaryHistory baseRead ∧ UnaryHistory denseRead ∧
                                UnaryHistory refinedRead ∧ UnaryHistory threadRead ∧
                                  Cont B M baseRead ∧ Cont D O denseRead ∧
                                    Cont denseRead R refinedRead ∧
                                      Cont refinedRead T threadRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro baseRoute denseRoute refinedRoute threadRoute packageP packageThread baseUnary
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row B ∨ hsame row M ∨ hsame row D ∨ hsame row O ∨
              hsame row R ∨ hsame row T ∨ hsame row threadRead)
          (fun row : BHist =>
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, packageP, packageThread⟩
  }
  exact
    ⟨cert, baseReadUnary, denseReadUnary, refinedReadUnary, threadReadUnary, baseRoute,
      denseRoute, refinedRoute, threadRoute⟩
