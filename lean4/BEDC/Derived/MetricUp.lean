...
theorem MetricDistanceWitness_prefix_independent_symmetric_zero_depth_empty_boundary_forward
    {p q p' q' x y dxy dyx : BHist} :
    MetricDistanceWitness (append p x) (append y q) (append (append p dxy) q) ->
      MetricDistanceWitness (append p' y) (append x q') (append (append p' dyx) q') ->
        MetricDistanceDepth dxy = 0 ->
          MetricDistanceWitness (append p BHist.Empty) (append BHist.Empty q)
            (append (append p BHist.Empty) q) := by
  ...
  have reverseCentral :=
    (MetricDistanceWitness_visible_context_iff (p := p') (q := q') (x := y) (y := x)
      (d := dyx)).mp reverse
  have sameDistances : hsame dxy dyx :=
    MetricDistanceWitness_symmetric_classifier forwardCentral.2.2 reverseCentral.2.2
  cases dxy with
  | Empty =>
      cases sameDistances
      have boundary :=
      ...
      exact
        (MetricDistanceWitness_visible_context_empty_distance_iff (p := p) (q := q)
          ...
          (And.intro boundary.1
            (And.intro boundary.2.1
...
theorem MetricDistanceWitness_visible_context_endpoint_hsame_result_deterministic
    {p q x x' y y' d d' : BHist} :
    hsame x x' -> hsame y y' ->
      MetricDistanceWitness (append p x) (append y q) (append (append p d) q) ->
        MetricDistanceWitness (append p x') (append y' q) (append (append p d') q) ->
          hsame d d' := by
