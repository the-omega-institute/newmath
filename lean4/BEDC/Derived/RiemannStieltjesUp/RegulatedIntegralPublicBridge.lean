import BEDC.Derived.RiemannStieltjesUp.TasteGate

namespace BEDC.Derived.RiemannStieltjesUp

open BEDC.FKernel.Ask
open BEDC.FKernel.Bundle
open BEDC.FKernel.Cont
open BEDC.FKernel.Hist
open BEDC.FKernel.NameCert
open BEDC.FKernel.Package
open BEDC.FKernel.Unary

theorem RiemannStieltjesCarrier_regulated_integral_public_bridge [AskSetup] [PackageSetup]
    {regulated variation tagged step handoff sealRow transportRow replayRow provenance nameRow
      publicRead bridgeRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    RiemannStieltjesCarrier regulated variation tagged step handoff sealRow transportRow
      replayRow provenance nameRow bundle pkg ->
      Cont tagged step publicRead ->
        Cont publicRead handoff bridgeRead ->
          PkgSig bundle bridgeRead pkg ->
            SemanticNameCert
                (fun row : BHist => hsame row bridgeRead ∧ UnaryHistory row)
                (fun row : BHist =>
                  hsame row regulated ∨ hsame row variation ∨ hsame row tagged ∨
                    hsame row step ∨ hsame row handoff ∨ hsame row sealRow ∨
                      hsame row publicRead ∨ hsame row bridgeRead)
                (fun row : BHist =>
                  UnaryHistory row ∧ Cont tagged step publicRead ∧
                    Cont publicRead handoff bridgeRead ∧ PkgSig bundle bridgeRead pkg)
                hsame ∧ UnaryHistory publicRead ∧ UnaryHistory bridgeRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont UnaryHistory PkgSig hsame SemanticNameCert
  intro carrier publicRoute bridgeRoute bridgePkg
  obtain ⟨_regulatedUnary, _variationUnary, taggedUnary, stepUnary, handoffUnary,
    _sealUnary, _transportUnary, _replayUnary, _provenanceUnary, _regulatedRoute,
    _handoffRoute, _replayRoute, _namePkg⟩ := carrier
  have publicUnary : UnaryHistory publicRead :=
    unary_cont_closed taggedUnary stepUnary publicRoute
  have bridgeUnary : UnaryHistory bridgeRead :=
    unary_cont_closed publicUnary handoffUnary bridgeRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row bridgeRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row regulated ∨ hsame row variation ∨ hsame row tagged ∨
              hsame row step ∨ hsame row handoff ∨ hsame row sealRow ∨
                hsame row publicRead ∨ hsame row bridgeRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont tagged step publicRead ∧
              Cont publicRead handoff bridgeRead ∧ PkgSig bundle bridgeRead pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro bridgeRead ⟨hsame_refl bridgeRead, bridgeUnary⟩
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
        intro _row _other sameRows
        exact hsame_symm sameRows
      equiv_trans := by
        intro _row _middle _other sameLeft sameRight
        exact hsame_trans sameLeft sameRight
      carrier_respects_equiv := by
        intro _row _other sameRows source
        exact
          ⟨hsame_trans (hsame_symm sameRows) source.left,
            unary_transport source.right sameRows⟩
    }
    pattern_sound := by
      intro _row source
      exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr source.left))))))
    ledger_sound := by
      intro _row source
      exact ⟨source.right, publicRoute, bridgeRoute, bridgePkg⟩
  }
  exact ⟨cert, publicUnary, bridgeUnary⟩

end BEDC.Derived.RiemannStieltjesUp
