...
theorem HyperspaceFiniteCoverSelectorRefusal [AskSetup] [PackageSetup]
      ...
      Cont K0 K1 subsetRead ->
        Cont N0 N1 netRead ->
          Cont subsetRead netRead finiteCoverRead ->
            Cont finiteCoverRead C refusalRead ->
                ...
                SemanticNameCert
                    ...
                    (fun row : BHist =>
                      UnaryHistory row ∧ PkgSig bundle P pkg ∧
                        PkgSig bundle refusalRead pkg)
                    hsame ∧
                  UnaryHistory finiteCoverRead ∧ UnaryHistory refusalRead := by
  -- BEDC touchpoint anchor: BHist HyperspaceCarrier Cont ProbeBundle Pkg SemanticNameCert
  intro carrier subsetRoute netRoute finiteCoverRoute refusalRoute refusalPkg
  ...
  have refusalUnary : UnaryHistory refusalRead :=
    unary_cont_closed finiteCoverUnary cUnary refusalRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row refusalRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row X ∨ hsame row K0 ∨ hsame row K1 ∨ hsame row N0 ∨
              hsame row N1 ∨ hsame row finiteCoverRead ∨ hsame row refusalRead)
          (fun row : BHist =>
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, provenancePkg, refusalPkg⟩
  }
  exact ⟨cert, finiteCoverUnary, refusalUnary⟩
