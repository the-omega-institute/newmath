...
theorem Hyperspace_compact_subset_root_admission [AskSetup] [PackageSetup]
    {X K0 K1 N0 N1 D0 D1 R Hs C P M subsetRead netRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    HyperspaceCarrier X K0 K1 N0 N1 D0 D1 R Hs C P M bundle pkg →
      Cont K0 K1 subsetRead →
        Cont N0 N1 netRead →
          SemanticNameCert
              (fun row : BHist => hsame row subsetRead ∧ UnaryHistory row)
              (fun row : BHist =>
                hsame row X ∨ hsame row K0 ∨ hsame row K1 ∨ hsame row N0 ∨
                  hsame row N1 ∨ hsame row subsetRead ∨ hsame row netRead)
              (fun row : BHist => hsame row subsetRead ∧ PkgSig bundle P pkg)
              hsame ∧
            UnaryHistory subsetRead ∧ UnaryHistory netRead := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle Pkg SemanticNameCert hsame
  intro carrier subsetRoute netRoute
  ...
  have cert :
    ...
    core := {
      carrier_inhabited :=
        Exists.intro subsetRead ⟨hsame_refl subsetRead, subsetUnary⟩
      equiv_refl := by
        intro row _source
  ...
  exact ⟨cert, subsetUnary, netUnary⟩
