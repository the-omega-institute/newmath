...
theorem HyperspaceRootHausdorffFiniteWindow [AskSetup] [PackageSetup]
    {X K0 K1 N0 N1 D0 D1 R Hs C P M subsetRead netRead distanceRead toleranceRead :
      BHist}
      ...
      Cont K0 K1 subsetRead →
        Cont N0 N1 netRead →
          Cont D0 D1 distanceRead →
            Cont distanceRead R toleranceRead →
                      ...
                      hsame row X ∨ hsame row K0 ∨ hsame row K1 ∨ hsame row N0 ∨
                        hsame row N1 ∨ hsame row D0 ∨ hsame row D1 ∨ hsame row R ∨
                          hsame row subsetRead ∨ hsame row netRead ∨
                            hsame row distanceRead ∨ hsame row toleranceRead)
                    (fun row : BHist =>
                      UnaryHistory row ∧ PkgSig bundle P pkg ∧
                        PkgSig bundle toleranceRead pkg)
                    hsame ∧
                  hsame distanceRead (append D0 D1) ∧ UnaryHistory subsetRead ∧
                    UnaryHistory netRead ∧ UnaryHistory distanceRead ∧
                      UnaryHistory toleranceRead := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle PkgSig SemanticNameCert hsame
  intro carrier subsetRoute netRoute distanceRoute toleranceRoute tolerancePkg
  ...
  have toleranceUnary : UnaryHistory toleranceRead :=
    unary_cont_closed distanceUnary rUnary toleranceRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row toleranceRead ∧ UnaryHistory row)
          (fun row : BHist =>
  ...
  exact
    ⟨cert, distanceRoute, subsetUnary, netUnary, distanceUnary, toleranceUnary⟩
