...
theorem HyperspaceFiniteCoverLedger [AskSetup] [PackageSetup]
      ...
      Cont K0 K1 subsetRead →
        Cont N0 N1 netRead →
          Cont subsetRead netRead finiteCoverRead →
            Cont finiteCoverRead C cauchyRead →
                ...
                SemanticNameCert
                    ...
                    (fun row : BHist =>
                      hsame row X ∨ hsame row K0 ∨ hsame row K1 ∨ hsame row N0 ∨
                        hsame row N1 ∨ hsame row finiteCoverRead ∨
                  ...
                  UnaryHistory subsetRead ∧ UnaryHistory netRead ∧
                    UnaryHistory finiteCoverRead ∧ UnaryHistory cauchyRead := by
  -- BEDC touchpoint anchor: BHist HyperspaceCarrier Cont ProbeBundle Pkg SemanticNameCert
  intro carrier subsetRoute netRoute finiteCoverRoute cauchyRoute cauchyPkg
  ...
  have cauchyUnary : UnaryHistory cauchyRead :=
    unary_cont_closed finiteCoverUnary cUnary cauchyRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row cauchyRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row X ∨ hsame row K0 ∨ hsame row K1 ∨ hsame row N0 ∨
              hsame row N1 ∨ hsame row finiteCoverRead ∨ hsame row cauchyRead)
          (fun row : BHist =>
            UnaryHistory row ∧ PkgSig bundle P pkg ∧ PkgSig bundle cauchyRead pkg)
          hsame := {
    core := {
      carrier_inhabited :=
        Exists.intro cauchyRead ⟨hsame_refl cauchyRead, cauchyUnary⟩
      equiv_refl := by
        intro row _source
  ...
  exact ⟨cert, subsetUnary, netUnary, finiteCoverUnary, cauchyUnary⟩

theorem HyperspaceFiniteCoverPublicRouteLedger [AskSetup] [PackageSetup]
    {X K0 K1 N0 N1 D0 D1 R Hs C P M subsetRead netRead finiteCoverRead
