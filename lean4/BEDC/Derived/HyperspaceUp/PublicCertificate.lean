...
theorem HyperspacePublicCertificate [AskSetup] [PackageSetup]
    {X K0 K1 N0 N1 D0 D1 R Hs C P M compactRead hausdorffRead vietorisRead
      completionRead replayRead localRead publicRead : BHist}
  ...
  intro carrier compactRoute hausdorffRoute vietorisRoute completionRoute replayRoute
    localRoute publicRoute _completionPkg _localPkg publicPkg
      ...
      core := {
        carrier_inhabited :=
          Exists.intro publicRead
            ⟨Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (hsame_refl publicRead))))),
              publicUnary⟩
        equiv_refl := by
          intro row _source
  ...
  · exact
      ⟨compactUnary, hausdorffUnary, vietorisUnary, completionUnary, replayUnary,
        localUnary, publicUnary⟩
