...
theorem BoundedRealSequenceCarrier_regular_readback_stability [AskSetup] [PackageSetup]
                          ...
                          SemanticNameCert
                              ...
                              (fun row : BHist =>
                                hsame row S ∨ hsame row W ∨ hsame row Q ∨ hsame row R ∨
                                  hsame row I ∨ hsame row sealRead)
                              (fun row : BHist =>
                                hsame row sealRead ∧ PkgSig bundle P pkg ∧
  ...
  intro carrier carrier' _sameS _sameW _sameQ _sameR _sameI sourceWindow
    windowReadback readbackSeal sealPkg
  obtain ⟨SUnary, WUnary, QUnary, RUnary, _IUnary, _HUnary, _CUnary, _PUnary,
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row S ∨ hsame row W ∨ hsame row Q ∨ hsame row R ∨
              hsame row I ∨ hsame row sealRead)
          (fun row : BHist =>
            hsame row sealRead ∧ PkgSig bundle P pkg ∧ PkgSig bundle P' pkg ∧
              PkgSig bundle sealRead pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.left, carrierPkg, carrierPkg', sealPkg⟩
  }
