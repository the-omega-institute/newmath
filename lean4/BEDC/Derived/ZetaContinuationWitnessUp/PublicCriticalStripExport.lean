...
theorem ZetaContinuationWitnessPacket_public_critical_strip_export [AskSetup]
    [PackageSetup]
    {basic eta analytic pole functional zeroLedger gamma transports routes provenance name
      criticalRead sourceLock : BHist}
                        ...
                        (fun row : BHist =>
                          Cont transports routes provenance ∧ PkgSig bundle name pkg ∧
                            PkgSig bundle provenance pkg ∧ PkgSig bundle criticalRead pkg ∧
                              PkgSig bundle sourceLock pkg ∧ hsame row criticalRead)
                        hsame ∧
  ...
  intro packet routesUnary nameUnary routesNameCritical _basicAnalyticSource criticalPkg
    sourceLockPkg
  have projected :=
  ...
  have cert :
    ...
    · intro _row source
      exact
        ⟨transportsRoutesProvenance, namePkg, provenancePkg, criticalPkg, sourceLockPkg,
          source.right⟩
