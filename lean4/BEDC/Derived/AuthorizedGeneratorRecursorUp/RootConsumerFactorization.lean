...
theorem AuthorizedGeneratorRecursorRootConsumerFactorization [AskSetup] [PackageSetup]
    {signature eliminator motive branches descent output audit transport routes provenance gap name
      branchRead publicRead outputRead auditRead boundaryRead : BHist}
  ...
  intro signatureEliminatorBranches branchesDescentBranchRead branchReadRoutesPublicRead
    ...
    _signatureUnary _eliminatorUnary _motiveUnary branchesUnary routesUnary outputUnary
    auditUnary gapUnary nameUnary provenancePkg
  ...
  have cert :
      SemanticNameCert
        ...
        (fun row : BHist =>
          hsame row name ∧ Cont signature eliminator branches ∧
            Cont branches descent branchRead ∧ Cont branchRead routes publicRead ∧
    ...
    pattern_sound := by
      intro _row source
      exact
        ⟨source.left, signatureEliminatorBranches, branchesDescentBranchRead,
          branchReadRoutesPublicRead, outputAuditOutputRead, outputReadNameAuditRead,
          gapNameBoundaryRead⟩
    ledger_sound := by
      intro _row source
