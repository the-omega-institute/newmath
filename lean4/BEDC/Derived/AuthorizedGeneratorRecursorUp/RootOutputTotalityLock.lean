...
theorem AuthorizedGeneratorRecursorRootOutputTotalityLock [AskSetup] [PackageSetup]
              ...
              Cont G N boundaryRead ->
                PkgSig bundle publicRead pkg ->
                  SemanticNameCert
                      ...
                      hsame ∧ UnaryHistory publicRead ∧ UnaryHistory boundaryRead := by
  ...
  rcases carrier with
    ⟨unaryI, unaryE, _unaryM, _unaryB, unaryD, unaryO, _unaryA, _unaryH,
      unaryC, _unaryP, unaryG, unaryN, _carrierBranch, _carrierDescent, _carrierOutput,
  ...
  have cert :
    ...
    exact {
      ...
      pattern_sound := by
        intro _row source
        exact
          ⟨source.left, branchRoute, descentRoute, outputRoute, publicRoute,
            boundaryRoute⟩
      ledger_sound := by
        intro _row source
        exact ⟨source.left, provenancePkg, publicPkg⟩
    }
  exact ⟨cert, publicUnary, boundaryUnary⟩
