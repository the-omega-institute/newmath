...
theorem AuthorizedGeneratorRecursor_output_boundary_totality [AskSetup] [PackageSetup]
          ...
          Cont G N boundaryRead ->
            PkgSig bundle publicRead pkg ->
              (SemanticNameCert
                ...
                (fun row : BHist =>
                  hsame row N ∧ Cont O A outputRead ∧ Cont outputRead C publicRead ∧
  ...
  rcases carrier with
    ⟨_unaryI, _unaryE, _unaryM, _unaryB, _unaryD, unaryO, unaryA, _unaryH,
      unaryC, _unaryP, unaryG, unaryN, _carrierBranch, _carrierDescent,
  ...
  have cert :
    ...
    exact {
      ...
      pattern_sound := by
        intro _row source
        exact ⟨source.left, outputRoute, publicRoute, boundaryRoute, transportSame⟩
      ledger_sound := by
        intro _row source
        exact ⟨source.left, provenancePkg, publicPkg⟩
    }
  exact ⟨cert, outputUnary, publicUnary, boundaryUnary⟩
