...
theorem MetricTriangleUniformEstimateCarrier_source_target_two_triangle_route
    ...
    {sourceMetric targetMetric graph left right center sourceBoundLeft sourceBoundRight
      ...
      localName sourceTriangleRead uniformRead : BHist}
    ...
    Cont sourceBoundLeft sourceBoundRight sourceTriangleRead ->
    Cont sourceTriangleRead targetTriangle uniformRead ->
    PkgSig bundle uniformRead pkg ->
      SemanticNameCert
        (fun row : BHist => hsame row uniformRead ∧ UnaryHistory row)
        (fun row : BHist =>
          hsame row sourceBoundLeft ∨ hsame row sourceBoundRight ∨
            hsame row targetTriangle ∨ hsame row uniformRead)
        (fun row : BHist => hsame row uniformRead ∧ PkgSig bundle uniformRead pkg)
        hsame ∧ UnaryHistory sourceTriangleRead ∧ UnaryHistory uniformRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert
  intro carrier sourceTriangleRoute uniformRoute uniformPkg
  ...
  have sourceTriangleUnary : UnaryHistory sourceTriangleRead :=
    unary_cont_closed sourceBoundLeftUnary sourceBoundRightUnary sourceTriangleRoute
  have uniformUnary : UnaryHistory uniformRead :=
    unary_cont_closed sourceTriangleUnary targetTriangleUnary uniformRoute
  have cert :
      SemanticNameCert
        (fun row : BHist => hsame row uniformRead ∧ UnaryHistory row)
        (fun row : BHist =>
  ...
  exact ⟨cert, sourceTriangleUnary, uniformUnary⟩
