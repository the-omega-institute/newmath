...
theorem MetricTriangleUniformEstimateCarrier_public_closure_package [AskSetup] [PackageSetup]
    {sourceMetric targetMetric graph left right center sourceBoundLeft sourceBoundRight
      ...
      localName handoff consumer exportRow : BHist}
    ...
    MetricTriangleUniformEstimateCarrier sourceMetric targetMetric graph left right center
      ...
      Cont precision targetTriangle handoff ->
        Cont handoff localName consumer ->
          Cont sourceBoundLeft targetTriangle exportRow ->
            PkgSig bundle exportRow pkg ->
              PkgSig bundle consumer pkg ->
                SemanticNameCert
                    (fun row : BHist => hsame row consumer ∧ UnaryHistory row)
                    (fun row : BHist =>
                      hsame row sourceBoundLeft ∨ hsame row targetTriangle ∨
                        hsame row exportRow ∨ hsame row consumer)
  ...
  intro carrier precisionTriangleHandoff handoffLocalConsumer sourceTriangleExport
    _exportPkg consumerPkg
  ...
  have _exportUnary : UnaryHistory exportRow :=
    unary_cont_closed sourceBoundLeftUnary targetTriangleUnary sourceTriangleExport
  have sourceConsumer :
  ...
  have cert :
      SemanticNameCert
        (fun row : BHist => hsame row consumer ∧ UnaryHistory row)
        (fun row : BHist =>
          hsame row sourceBoundLeft ∨ hsame row targetTriangle ∨ hsame row exportRow ∨
            hsame row consumer)
        (fun row : BHist => hsame row consumer ∧ PkgSig bundle consumer pkg)
        hsame := by
    exact {
