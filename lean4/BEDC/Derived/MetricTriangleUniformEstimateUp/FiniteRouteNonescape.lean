...
theorem MetricTriangleUniformEstimateCarrier_finite_route_nonescape
    ...
    MetricTriangleUniformEstimateCarrier sourceMetric targetMetric graph left right center
      ...
      PkgSig bundle consumer pkg ->
        SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧
              (hsame row center ∨ hsame row precision ∨ hsame row sourceBoundLeft ∨
                hsame row sourceExtraction ∨ hsame row targetTriangle ∨ hsame row exportRow ∨
                  hsame row consumer))
          (fun _row : BHist =>
            Cont center precision centerPrecision ∧
              Cont centerPrecision sourceBoundLeft sourceExtraction ∧
                Cont sourceExtraction targetTriangle exportRow ∧
                  Cont exportRow localName consumer ∧ PkgSig bundle localName pkg ∧
  ...
  intro carrier centerPrecisionRow sourceExtractionRow exportRowRoute exportConsumerRoute
  ...
  have centerPrecisionUnary : UnaryHistory centerPrecision :=
    unary_cont_closed centerUnary precisionUnary centerPrecisionRow
  have sourceExtractionUnary : UnaryHistory sourceExtraction :=
    unary_cont_closed centerPrecisionUnary sourceBoundLeftUnary sourceExtractionRow
  have exportRowUnary : UnaryHistory exportRow :=
    unary_cont_closed sourceExtractionUnary targetTriangleUnary exportRowRoute
  have consumerUnary : UnaryHistory consumer :=
    unary_cont_closed exportRowUnary localNameUnary exportConsumerRoute
  have cert :
      SemanticNameCert
        (fun row : BHist =>
          MetricTriangleUniformEstimateCarrier sourceMetric targetMetric graph left right center
    ...
    exact {
      core := {
        ...
        carrier_respects_equiv := by
          ...
          exact
            And.intro sourceRow.left (hsame_trans (hsame_symm sameRows) sourceRow.right)
      }
      pattern_sound := by
        ...
        exact
          And.intro
            ...
            (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr sourceRow.right))))))
      ledger_sound := by
        intro _row _sourceRow
        exact
          ⟨centerPrecisionRow, sourceExtractionRow, exportRowRoute, exportConsumerRoute,
            localNamePkg, exportPkg, consumerPkg⟩
    }
  exact ⟨cert, sourceExtractionUnary, exportRowUnary, consumerUnary⟩
