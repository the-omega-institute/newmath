...
theorem MetricTriangleUniformEstimateCarrier_cont_route_scope [AskSetup] [PackageSetup]
              ...
              Cont handoff localName consumer ->
                PkgSig bundle consumer pkg ->
                  SemanticNameCert
                    ...
                    (fun row : BHist => hsame row handoff ∨ hsame row consumer)
                    (fun _row : BHist =>
                      Cont left center sourceBoundLeft ∧
                        Cont sourceBoundLeft precision sourceBoundRight ∧
                          Cont targetBoundLeft targetBoundRight targetTriangle ∧
                            Cont precision targetTriangle handoff ∧
                              Cont handoff localName consumer ∧ PkgSig bundle consumer pkg)
                    hsame := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig SemanticNameCert hsame
  intro carrier leftCenterSourceBound sourceTriangle targetTriangleRoute
  ...
  exact {
    ...
    ledger_sound := by
      intro _row _source
      exact
        ⟨leftCenterSourceBound, sourceTriangle, targetTriangleRoute,
          precisionTriangleHandoff, handoffLocalConsumer, consumerPkg⟩
  }
