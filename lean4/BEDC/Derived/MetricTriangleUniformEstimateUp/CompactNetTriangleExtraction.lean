...
theorem MetricTriangleUniformEstimateCompactNetTriangleExtraction
    [AskSetup] [PackageSetup]
    {sourceMetric targetMetric graph left right center sourceBoundLeft sourceBoundRight
      precision targetBoundLeft targetBoundRight targetTriangle transport route provenance
      localName sourceRead targetRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    MetricTriangleUniformEstimateCarrier sourceMetric targetMetric graph left right center
      sourceBoundLeft sourceBoundRight precision targetBoundLeft targetBoundRight targetTriangle
      transport route provenance localName bundle pkg ->
      Cont sourceBoundLeft precision sourceRead ->
        Cont targetBoundLeft targetBoundRight targetRead ->
          SemanticNameCert
              (fun row : BHist => hsame row targetRead ∧ UnaryHistory row)
              (fun row : BHist => hsame row targetRead ∧ Cont sourceBoundLeft precision sourceRead)
              (fun row : BHist =>
                hsame row targetRead ∧ Cont targetBoundLeft targetBoundRight targetRead)
              hsame ∧ UnaryHistory sourceRead ∧ UnaryHistory targetRead := by
  -- BEDC touchpoint anchor: BHist Cont hsame SemanticNameCert UnaryHistory
  intro carrier sourceRoute targetRoute
  obtain ⟨_sourceMetricUnary, _targetMetricUnary, _graphUnary, _leftUnary, _rightUnary,
    _centerUnary, sourceBoundLeftUnary, _sourceBoundRightUnary, precisionUnary,
    targetBoundLeftUnary, targetBoundRightUnary, _targetTriangleUnary, _transportUnary,
    _routeUnary, _provenanceUnary, _localNameUnary, _leftCenterSourceBound,
    _rightCenterSourceBound, _targetTriangleRoute, _routeProvenanceLocalName,
    _localNamePkg, _transportTargetTriangle⟩ := carrier
  have sourceReadUnary : UnaryHistory sourceRead :=
    unary_cont_closed sourceBoundLeftUnary precisionUnary sourceRoute
  have targetReadUnary : UnaryHistory targetRead :=
    unary_cont_closed targetBoundLeftUnary targetBoundRightUnary targetRoute
  ...
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row targetRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row targetRead ∧ Cont sourceBoundLeft precision sourceRead)
          (fun row : BHist =>
            hsame row targetRead ∧ Cont targetBoundLeft targetBoundRight targetRead)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.left, targetRoute⟩
  }
  exact ⟨cert, sourceReadUnary, targetReadUnary⟩
