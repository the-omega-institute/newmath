...
theorem AxiomRefusalLedger_noninternalization [AskSetup] [PackageSetup]
    {A S Q F R G H C P N nameRead alternativeRead auditRead acceptedRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    UnaryHistory A →
      UnaryHistory S →
        UnaryHistory Q →
                              ...
                              PkgSig bundle P pkg →
                                SemanticNameCert
                                    (fun row : BHist => hsame row acceptedRead ∧
                                      UnaryHistory row)
                                    (fun row : BHist =>
                                      hsame row A ∨ hsame row S ∨ hsame row Q ∨
                                        hsame row F ∨ hsame row R ∨ hsame row G ∨
                                          hsame row auditRead ∨ hsame row acceptedRead)
                                    (fun row : BHist =>
                                      UnaryHistory row ∧ Cont A S nameRead ∧
                                        Cont Q F alternativeRead ∧ Cont R G auditRead ∧
                                          Cont auditRead C acceptedRead ∧
                                            PkgSig bundle P pkg)
                                    hsame ∧
  ...
  intro unaryA unaryS unaryQ unaryF unaryR unaryG unaryC _unaryN nameRoute
    alternativeRoute auditRoute acceptedRoute _transportEmpty provenancePkg
  have auditReadUnary : UnaryHistory auditRead :=
    unary_cont_closed unaryR unaryG auditRoute
  have acceptedReadUnary : UnaryHistory acceptedRead :=
    unary_cont_closed auditReadUnary unaryC acceptedRoute
  have sourceAccepted :
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row A ∨ hsame row S ∨ hsame row Q ∨ hsame row F ∨ hsame row R ∨
              hsame row G ∨ hsame row auditRead ∨ hsame row acceptedRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont A S nameRead ∧ Cont Q F alternativeRead ∧
              Cont R G auditRead ∧ Cont auditRead C acceptedRead ∧ PkgSig bundle P pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, nameRoute, alternativeRoute, auditRoute, acceptedRoute,
          provenancePkg⟩
  }
  exact ⟨cert, acceptedReadUnary⟩

theorem AxiomRefusalLedger_gate_readback [AskSetup] [PackageSetup]
    {A S Q F R G H C P N nameRead alternativeRead auditRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    UnaryHistory A →
      UnaryHistory S →
  ...
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro unaryA unaryS unaryQ unaryF unaryR unaryG _unaryN nameRoute alternativeRoute
    auditRoute transportEmpty provenancePkg
  have nameReadUnary : UnaryHistory nameRead :=
    unary_cont_closed unaryA unaryS nameRoute
  have alternativeReadUnary : UnaryHistory alternativeRead :=
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row A ∨ hsame row S ∨ hsame row Q ∨ hsame row F ∨ hsame row R ∨
              hsame row G ∨ hsame row auditRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont A S nameRead ∧ Cont Q F alternativeRead ∧
              Cont R G auditRead ∧ hsame H BHist.Empty ∧ PkgSig bundle P pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, nameRoute, alternativeRoute, auditRoute, transportEmpty,
          provenancePkg⟩
  }
  exact ⟨cert, nameReadUnary, alternativeReadUnary, auditReadUnary⟩
