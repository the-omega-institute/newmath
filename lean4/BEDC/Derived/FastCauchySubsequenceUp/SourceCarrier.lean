...
theorem FastCauchySubsequenceSourceCarrier [AskSetup] [PackageSetup]
    {S M Q F R W E H C P N modulusRead selectorRead sourceRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    FastCauchySubsequenceCarrier S M Q F R W E H C P N bundle pkg →
      Cont S M modulusRead →
        Cont modulusRead Q selectorRead →
          Cont selectorRead F sourceRead →
              ...
              SemanticNameCert
                  ...
                  (fun row : BHist =>
                    UnaryHistory row ∧ Cont S M modulusRead ∧
                      Cont modulusRead Q selectorRead ∧ Cont selectorRead F sourceRead ∧
                        PkgSig bundle sourceRead pkg)
  ...
  intro carrier modulusRoute selectorRoute sourceRoute sourcePkg
  obtain ⟨unaryS, unaryM, unaryQ, unaryF, _unaryR, _unaryW, _unaryE, _unaryH,
  ...
  have sourceUnary : UnaryHistory sourceRead :=
    unary_cont_closed selectorUnary unaryF sourceRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row sourceRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row S ∨ hsame row M ∨ hsame row Q ∨ hsame row F ∨
              hsame row H ∨ hsame row C ∨ hsame row P ∨ hsame row N ∨
                hsame row modulusRead ∨ hsame row selectorRead ∨ hsame row sourceRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont S M modulusRead ∧
              Cont modulusRead Q selectorRead ∧ Cont selectorRead F sourceRead ∧
                PkgSig bundle sourceRead pkg)
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, modulusRoute, selectorRoute, sourceRoute, sourcePkg⟩
  }
  exact ⟨cert, modulusUnary, selectorUnary, sourceUnary⟩
