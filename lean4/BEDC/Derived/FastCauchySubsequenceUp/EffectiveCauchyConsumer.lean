...
theorem FastCauchySubsequenceEffectiveCauchyConsumer [AskSetup] [PackageSetup]
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row sourceRow
      exact
        ⟨sourceRow.right, selectorRoute, fastRoute, regularRoute, realRoute, realPkg⟩
  }
  exact ⟨cert, modulusUnary, selectorUnary, fastUnary, regularUnary, realUnary⟩
