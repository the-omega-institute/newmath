...
theorem FastCauchySubsequenceUniformTailWindow [AskSetup] [PackageSetup]
                  ...
                  SemanticNameCert
                      ...
                      (fun row : BHist =>
                        hsame row S ∨ hsame row M ∨ hsame row Q ∨ hsame row F ∨
                          hsame row R ∨ hsame row W ∨ hsame row tailRead)
                      (fun row : BHist =>
                          ...
                          Cont modulusRead Q selectorRead ∧
                            Cont selectorRead F fastRead ∧
                              Cont fastRead R regularRead ∧
                                Cont regularRead W tailRead ∧
                                  PkgSig bundle tailRead pkg)
                      hsame ∧ UnaryHistory regularRead ∧ UnaryHistory tailRead := by
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, modulusRoute, selectorRoute, fastRoute, regularRoute, tailRoute,
          tailPkg⟩
  }
  exact ⟨cert, regularUnary, tailUnary⟩
