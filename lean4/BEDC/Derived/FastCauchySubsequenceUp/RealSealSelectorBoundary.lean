...
theorem FastCauchySubsequenceRealSealSelectorBoundary [AskSetup] [PackageSetup]
  ...
  intro carrier modulusRoute selectorRoute fastRoute regularRoute readbackRoute realRoute realPkg
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row M ∨ hsame row Q ∨ hsame row F ∨ hsame row R ∨ hsame row W ∨
              hsame row E ∨ hsame row selectorRead ∨ hsame row fastRead ∨
                hsame row regularRead ∨ hsame row readbackRead ∨ hsame row realRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont modulusRead Q selectorRead ∧
              Cont selectorRead F fastRead ∧ Cont fastRead R regularRead ∧
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, selectorRoute, fastRoute, regularRoute, readbackRoute,
          realRoute, realPkg⟩
  }
  exact ⟨cert, selectorUnary, fastUnary, regularUnary, readbackUnary, realUnary⟩
