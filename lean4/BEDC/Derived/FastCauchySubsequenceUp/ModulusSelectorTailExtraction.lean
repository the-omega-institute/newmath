...
theorem FastCauchySubsequenceModulusSelectorTailExtraction [AskSetup] [PackageSetup]
            ...
            Cont fastRead R regularRead →
              PkgSig bundle P pkg →
                SemanticNameCert
                  ...
                  UnaryHistory fastRead ∧ UnaryHistory regularRead := by
  ...
  intro carrier modulusRoute selectorRoute fastRoute regularRoute provenancePkg
  obtain ⟨unaryS, unaryM, unaryQ, unaryF, unaryR, _unaryW, _unaryE, _unaryH,
    _unaryC, _unaryP, _unaryN, _carrierProvenancePkg, _localNamePkg⟩ := carrier
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row S ∨ hsame row M ∨ hsame row Q ∨ hsame row F ∨
              hsame row H ∨ hsame row C ∨ hsame row P ∨ hsame row N ∨ hsame row fastRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont S M modulusRead ∧ Cont modulusRead Q selectorRead ∧
              Cont selectorRead F fastRead ∧ PkgSig bundle P pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro fastRead ⟨hsame_refl fastRead, fastUnary⟩
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, modulusRoute, selectorRoute, fastRoute, provenancePkg⟩
  }
  exact ⟨cert, modulusUnary, selectorUnary, fastUnary, regularUnary⟩
