...
theorem FastCauchySubsequenceTailRealSealCofinality [AskSetup] [PackageSetup]
          ...
          Cont selectorRead F fastRead ->
            Cont fastRead R regularRead ->
              Cont regularRead W tailRead ->
                Cont tailRead E realRead ->
                  Cont realRead N namedRead ->
                    PkgSig bundle namedRead pkg ->
                      SemanticNameCert
                          (fun row : BHist => hsame row namedRead ∧ UnaryHistory row)
                          (fun row : BHist =>
                            hsame row Q ∨ hsame row M ∨ hsame row W ∨ hsame row E ∨
                              hsame row namedRead)
                          (fun row : BHist =>
                            UnaryHistory row ∧ Cont modulusRead Q selectorRead ∧
                              Cont regularRead W tailRead ∧ Cont tailRead E realRead ∧
                                PkgSig bundle namedRead pkg)
                          hsame ∧ UnaryHistory selectorRead ∧ UnaryHistory tailRead ∧
  ...
  intro carrier modulusRoute selectorRoute fastRoute regularRoute tailRoute realRoute
    namedRoute namedPkg
  ...
  have realUnary : UnaryHistory realRead :=
    unary_cont_closed tailUnary unaryE realRoute
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, selectorRoute, tailRoute, realRoute, namedPkg⟩
  }
  exact ⟨cert, selectorUnary, tailUnary, realUnary, namedUnary⟩
