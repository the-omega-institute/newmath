...
theorem FastCauchySubsequenceSelectorWindowCoverage [AskSetup] [PackageSetup]
                ...
                SemanticNameCert
                    ...
                    (fun row : BHist =>
                      hsame row M ∨ hsame row Q ∨ hsame row F ∨ hsame row R ∨
                        hsame row selectorRead ∨ hsame row regularRead)
                    (fun row : BHist =>
                  ...
                  UnaryHistory regularRead ∧ Cont selectorRead F fastRead ∧
                    Cont fastRead R regularRead := by
  -- BEDC touchpoint anchor: FastCauchySubsequenceCarrier BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro carrier modulusRoute selectorRoute fastRoute regularRoute regularPkg
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, selectorRoute, fastRoute, regularRoute, regularPkg⟩
  }
  exact ⟨cert, selectorUnary, fastUnary, regularUnary, fastRoute, regularRoute⟩
