...
theorem FastCauchySubsequenceCofinalTailStability [AskSetup] [PackageSetup]
    {S M Q F R W E H C P N sourceRead modulusRead selectorRead fastRead regularRead
      tailRead : BHist}
                  ...
                  SemanticNameCert
                      ...
                      (fun row : BHist =>
                        hsame row M ∨ hsame row Q ∨ hsame row F ∨ hsame row R ∨
                          hsame row W ∨ hsame row selectorRead ∨
                            hsame row regularRead ∨ hsame row tailRead)
                      (fun row : BHist =>
                    ...
                    UnaryHistory regularRead ∧ UnaryHistory tailRead := by
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row M ∨ hsame row Q ∨ hsame row F ∨ hsame row R ∨ hsame row W ∨
              hsame row selectorRead ∨ hsame row regularRead ∨ hsame row tailRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont modulusRead Q selectorRead ∧
              Cont selectorRead F fastRead ∧ Cont fastRead R regularRead ∧
                Cont regularRead W tailRead ∧ PkgSig bundle tailRead pkg)
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, selectorRoute, fastRoute, regularRoute, tailRoute, tailPkg⟩
  }
  exact ⟨cert, selectorUnary, fastUnary, regularUnary, tailUnary⟩
