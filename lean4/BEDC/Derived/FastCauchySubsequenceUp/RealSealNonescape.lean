...
theorem FastCauchySubsequenceRealSealNonescape [AskSetup] [PackageSetup]
                                  ...
                                  Cont fastRead R regularRead ∧
                                    Cont regularRead W readbackRead ∧
                                      Cont readbackRead E realRead ∧
                                        Cont realRead N namedRead ∧
                                          PkgSig bundle namedRead pkg)
                          hsame ∧ UnaryHistory realRead ∧ UnaryHistory namedRead := by
  ...
  have namedUnary : UnaryHistory namedRead :=
    unary_cont_closed realUnary unaryN nameRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row namedRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row S ∨ hsame row M ∨ hsame row Q ∨ hsame row F ∨ hsame row R ∨
              hsame row W ∨ hsame row E ∨ hsame row realRead ∨ hsame row namedRead)
          (fun row : BHist =>
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, modulusRoute, selectorRoute, fastRoute, regularRoute,
          readbackRoute, realRoute, nameRoute, namedPkg⟩
  }
  exact ⟨cert, realUnary, namedUnary⟩
