...
theorem FastCauchySubsequenceModulusSelectorObligation [AskSetup] [PackageSetup]
                ...
                PkgSig bundle readbackRead pkg ->
                  SemanticNameCert
                    ...
                    UnaryHistory selectorRead ∧ UnaryHistory fastRead ∧
                      UnaryHistory regularRead ∧ UnaryHistory readbackRead := by
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont S M modulusRead ∧ Cont modulusRead Q selectorRead ∧
              Cont selectorRead F fastRead ∧ Cont fastRead R regularRead ∧
                Cont regularRead W readbackRead ∧ PkgSig bundle readbackRead pkg)
          hsame := {
    core := {
      carrier_inhabited :=
  ...
  exact ⟨cert, selectorUnary, fastUnary, regularUnary, readbackUnary⟩
