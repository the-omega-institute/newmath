...
theorem FastCauchySubsequenceDyadicTailBudget [AskSetup] [PackageSetup]
    {S M Q F R W E H C P N modulusRead selectorRead fastRead regularRead tailBudget :
      BHist}
        ...
        Cont modulusRead Q selectorRead ->
          Cont selectorRead F fastRead ->
            Cont fastRead R regularRead ->
              Cont regularRead W tailBudget ->
                PkgSig bundle tailBudget pkg ->
                  SemanticNameCert
                      (fun row : BHist => hsame row tailBudget ∧ UnaryHistory row)
                      (fun row : BHist =>
                        hsame row M ∨ hsame row Q ∨ hsame row F ∨ hsame row R ∨
                          hsame row W ∨ hsame row tailBudget)
                      (fun row : BHist =>
                          ...
                          Cont modulusRead Q selectorRead ∧
                            Cont selectorRead F fastRead ∧
                              Cont fastRead R regularRead ∧
                                Cont regularRead W tailBudget ∧
                                  PkgSig bundle tailBudget pkg)
                      hsame ∧ UnaryHistory tailBudget := by
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont S M modulusRead ∧ Cont modulusRead Q selectorRead ∧
              Cont selectorRead F fastRead ∧ Cont fastRead R regularRead ∧
                Cont regularRead W tailBudget ∧ PkgSig bundle tailBudget pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro tailBudget ⟨hsame_refl tailBudget, tailUnary⟩
      equiv_refl := by
        intro row _source
