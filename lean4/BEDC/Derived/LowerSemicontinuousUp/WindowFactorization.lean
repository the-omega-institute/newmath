...
theorem LowerSemicontinuousBaireWindowFactorization [AskSetup] [PackageSetup]
    {X F E W R O H C P N scheduleRead readbackRead epigraphRead locatedRead
      scopeRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    lowerSemicontinuousRootFields (LowerSemicontinuousUp.mk X F E W R O H C P N) =
        [X, F, E, W, R, O, H, C, P, N] ->
                                ...
                                SemanticNameCert
                                    ...
                                    (fun row : BHist =>
                                      hsame row W ∨ hsame row R ∨ hsame row E ∨
                                        hsame row O ∨ hsame row scopeRead)
                                    (fun row : BHist =>
                                  ...
                                  UnaryHistory readbackRead ∧ UnaryHistory epigraphRead ∧
                                    UnaryHistory locatedRead ∧ UnaryHistory scopeRead := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle Pkg SemanticNameCert UnaryHistory hsame
  intro fields xUnary wUnary rUnary eUnary oUnary nUnary scheduleRoute readbackRoute
    epigraphRoute locatedRoute scopeRoute provenancePkg namePkg
  have _acceptedFields :
      lowerSemicontinuousRootFields (LowerSemicontinuousUp.mk X F E W R O H C P N) =
        [X, F, E, W, R, O, H, C, P, N] := fields
  ...
  have scopeUnary : UnaryHistory scopeRead :=
    unary_cont_closed locatedUnary nUnary scopeRoute
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont X W scheduleRead ∧ Cont scheduleRead R readbackRead ∧
              Cont readbackRead E epigraphRead ∧ Cont epigraphRead O locatedRead ∧
                Cont locatedRead N scopeRead ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
          hsame := {
    core := {
  ...
  exact
    ⟨cert, scheduleUnary, readbackUnary, epigraphUnary, locatedUnary, scopeUnary⟩
