...
theorem LowerSemicontinuousPublicEpigraphExport [AskSetup] [PackageSetup]
    {X F E W R O H C P N windowRead epigraphRead locatedRead transportRead replayRead
      ...
      UnaryHistory W -> UnaryHistory R -> UnaryHistory E -> UnaryHistory O ->
        UnaryHistory H -> UnaryHistory C -> UnaryHistory N ->
          Cont W R windowRead -> Cont windowRead E epigraphRead ->
            Cont epigraphRead O locatedRead -> Cont locatedRead H transportRead ->
              Cont transportRead C replayRead -> Cont replayRead N realSeal ->
                Cont realSeal N publicRead -> PkgSig bundle P pkg ->
                  PkgSig bundle N pkg -> PkgSig bundle publicRead pkg ->
                    SemanticNameCert
                        (fun row : BHist =>
                          (hsame row realSeal ∨ hsame row publicRead) ∧ UnaryHistory row)
                        (fun row : BHist =>
                          hsame row W ∨ hsame row R ∨ hsame row E ∨ hsame row O ∨
                            hsame row H ∨ hsame row C ∨ hsame row P ∨ hsame row N ∨
                        ...
                        hsame ∧ UnaryHistory realSeal ∧ UnaryHistory publicRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert
  intro fields wUnary rUnary eUnary oUnary hUnary cUnary nUnary windowRoute epigraphRoute
    locatedRoute transportRoute replayRoute sealRoute publicRoute provenancePkg namePkg
    publicPkg
  have _acceptedFields :
      lowerSemicontinuousRootFields (LowerSemicontinuousUp.mk X F E W R O H C P N) =
        [X, F, E, W, R, O, H, C, P, N] := fields
  ...
  have publicUnary : UnaryHistory publicRead :=
    unary_cont_closed realSealUnary nUnary publicRoute
  have cert :
      SemanticNameCert
          (fun row : BHist =>
            (hsame row realSeal ∨ hsame row publicRead) ∧ UnaryHistory row)
    ...
    core := {
      carrier_inhabited := Exists.intro publicRead
        ⟨Or.inr (hsame_refl publicRead), publicUnary⟩
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, windowRoute, epigraphRoute, locatedRoute, transportRoute, replayRoute,
          sealRoute, publicRoute, provenancePkg, namePkg, publicPkg⟩
  }
