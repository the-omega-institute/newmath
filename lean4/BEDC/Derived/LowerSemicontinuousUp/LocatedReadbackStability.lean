...
theorem LowerSemicontinuousLocatedReadbackStability [AskSetup] [PackageSetup]
    {X F E W R O H C P N windowRead epigraphRead locatedRead transportRead realRead :
      BHist}
                    ...
                    Cont windowRead E epigraphRead ->
                      Cont epigraphRead O locatedRead ->
                        Cont locatedRead H transportRead ->
                          Cont transportRead C realRead ->
                            PkgSig bundle P pkg ->
                              PkgSig bundle N pkg ->
                                SemanticNameCert
                                    (fun row : BHist =>
                                      hsame row locatedRead ∧ UnaryHistory row)
                                    (fun row : BHist =>
                                      hsame row W ∨ hsame row R ∨ hsame row E ∨
                                        hsame row O ∨ hsame row locatedRead)
                                    (fun row : BHist =>
                                      ...
                                      hsame row E ∨ hsame row W ∨ hsame row R ∨
                                        hsame row O ∨ hsame row H ∨ hsame row C ∨
                                          hsame row P ∨ hsame row N ∨
                                            hsame row locatedRead ∨ hsame row realRead)
                                    (fun row : BHist =>
                                      UnaryHistory row ∧ Cont W R windowRead ∧
                                        Cont windowRead E epigraphRead ∧
                                          Cont epigraphRead O locatedRead ∧
  ...
  intro fields wUnary rUnary eUnary oUnary hUnary cUnary windowRoute epigraphRoute
    locatedRoute transportRoute realRoute provenancePkg namePkg
  ...
  have realUnary : UnaryHistory realRead :=
    unary_cont_closed transportUnary cUnary realRoute
  have locatedCert :
      SemanticNameCert
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, windowRoute, epigraphRoute, locatedRoute⟩
  }
  have realCert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont W R windowRead ∧ Cont windowRead E epigraphRead ∧
              Cont epigraphRead O locatedRead ∧ Cont locatedRead H transportRead ∧
                Cont transportRead C realRead ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, windowRoute, epigraphRoute, locatedRoute, transportRoute, realRoute,
          provenancePkg, namePkg⟩
  }
  exact ⟨locatedCert, realCert⟩
