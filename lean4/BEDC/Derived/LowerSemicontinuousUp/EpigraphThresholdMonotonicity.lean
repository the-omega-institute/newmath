...
theorem LowerSemicontinuousEpigraphThresholdMonotonicity [AskSetup] [PackageSetup]
  ...
  intro fieldRows qUnary eqUnary oqUnary bqUnary hqUnary cqUnary thresholdRoute
    boundaryRoute stableRoute replayRoute refinedRoute pPkg nPkg
  cases fieldRows
      ...
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row q ∨ hsame row Eq ∨ hsame row Oq ∨ hsame row Bq ∨
              hsame row Hq ∨ hsame row Cq ∨ hsame row refinedRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont q Eq thresholdRead ∧
