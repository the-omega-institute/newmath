...
theorem LowerSemicontinuousRootEpigraphBasis [AskSetup] [PackageSetup]
    ...
    lowerSemicontinuousRootEpigraphFields
      ...
      UnaryHistory X -> UnaryHistory W -> UnaryHistory R -> UnaryHistory E ->
        UnaryHistory O -> Cont X W windowRead -> Cont windowRead R epigraphRead ->
          Cont epigraphRead E realSeal -> PkgSig bundle P pkg ->
            PkgSig bundle N pkg ->
              SemanticNameCert
                (fun row : BHist => hsame row realSeal ∧ UnaryHistory row)
                (fun row : BHist =>
                  hsame row X ∨ hsame row W ∨ hsame row R ∨ hsame row E ∨
                    hsame row O ∨ hsame row realSeal)
                (fun row : BHist =>
                  UnaryHistory row ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
                hsame := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro fields xUnary wUnary rUnary eUnary _oUnary windowRoute epigraphRoute realSealRoute
  ...
  have realSealUnary : UnaryHistory realSeal :=
    unary_cont_closed epigraphUnary eUnary realSealRoute
  exact {
    core := {
