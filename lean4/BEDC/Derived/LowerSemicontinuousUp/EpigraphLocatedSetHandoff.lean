...
theorem LowerSemicontinuousEpigraphLocatedSetHandoff [AskSetup] [PackageSetup]
  ...
  intro fields xUnary wUnary rUnary eUnary oUnary hUnary cUnary scheduleRoute readbackRoute
    epigraphRoute locatedRoute transportRoute handoffRoute provenancePkg namePkg
  ...
  have sourceAtHandoff :
      (fun row : BHist => hsame row handoffRead ∧ UnaryHistory row) handoffRead :=
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, scheduleRoute, readbackRoute, epigraphRoute, locatedRoute,
          transportRoute, handoffRoute, provenancePkg, namePkg⟩
  }
  exact
