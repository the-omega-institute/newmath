...
theorem LowerSemicontinuousRealSealNonescape [AskSetup] [PackageSetup]
    {X F E W R O H C P N windowRead epigraphRead locatedRead replayRead realSeal : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    lowerSemicontinuousRootFields (LowerSemicontinuousUp.mk X F E W R O H C P N) =
        [X, F, E, W, R, O, H, C, P, N] ->
  ...
  intro fields wUnary rUnary eUnary oUnary cUnary nUnary windowRoute epigraphRoute
    locatedRoute replayRoute sealRoute provenancePkg namePkg
  have _acceptedFields :
      lowerSemicontinuousRootFields (LowerSemicontinuousUp.mk X F E W R O H C P N) =
        [X, F, E, W, R, O, H, C, P, N] := fields
  have windowUnary : UnaryHistory windowRead :=
  ...
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row realSeal ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row E ∨ hsame row W ∨ hsame row R ∨ hsame row O ∨ hsame row C ∨
              hsame row P ∨ hsame row N ∨ Cont W R windowRead ∨
                Cont windowRead E epigraphRead ∨ Cont epigraphRead O locatedRead ∨
                  Cont locatedRead C replayRead ∨ Cont replayRead N realSeal)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont W R windowRead ∧ Cont windowRead E epigraphRead ∧
              Cont epigraphRead O locatedRead ∧ Cont locatedRead C replayRead ∧
                Cont replayRead N realSeal ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
          hsame := {
    core := {
