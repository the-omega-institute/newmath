...
theorem LowerSemicontinuousEpigraphDependencyRoute [AskSetup] [PackageSetup]
  ...
  intro fields wUnary rUnary eUnary oUnary nUnary pUnary windowRoute epigraphRoute
    locatedRoute realSealRoute seriesRoute epigraphConsumerRoute dependencyRoute _pPkg _nPkg
    dependencyPkg
  ...
  have cert :
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont W R windowRead ∧ Cont windowRead E epigraphRead ∧
              Cont epigraphRead O locatedRead ∧ Cont locatedRead N realSeal ∧
                Cont E O epigraphConsumer ∧ Cont epigraphConsumer N dependencyRead ∧
                  PkgSig bundle dependencyRead pkg)
          hsame := {
    core := {
      carrier_inhabited :=
    ...
    pattern_sound := by
      intro _row _source
      exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr dependencyRoute)))))
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, windowRoute, epigraphRoute, locatedRoute, realSealRoute,
          epigraphConsumerRoute, dependencyRoute, dependencyPkg⟩
  }
