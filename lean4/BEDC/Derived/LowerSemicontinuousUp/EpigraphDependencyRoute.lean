...
theorem LowerSemicontinuousEpigraphDependencyRoute [AskSetup] [PackageSetup]
  ...
  intro fields wUnary rUnary eUnary oUnary nUnary pUnary windowRoute epigraphRoute
    locatedRoute realSealRoute seriesRoute dependencyRoute _pPkg _nPkg dependencyPkg
  ...
  have dependencyUnary : UnaryHistory dependencyRead :=
    unary_cont_closed epigraphUnary seriesUnary dependencyRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row dependencyRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row E ∨ hsame row O ∨ hsame row seriesRead ∨
              Cont epigraphRead seriesRead dependencyRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont windowRead E epigraphRead ∧
              Cont epigraphRead O locatedRead ∧ Cont realSeal P seriesRead ∧
                Cont epigraphRead seriesRead dependencyRead ∧
                  PkgSig bundle dependencyRead pkg)
          hsame := {
    core := {
    ...
    pattern_sound := by
      ...
      right
      right
      exact dependencyRoute
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, epigraphRoute, locatedRoute, seriesRoute, dependencyRoute,
          dependencyPkg⟩
  }
