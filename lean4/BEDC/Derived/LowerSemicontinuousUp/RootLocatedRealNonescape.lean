...
theorem LowerSemicontinuousRootLocatedRealNonescape [AskSetup] [PackageSetup]
  ...
  have sourceAtReal :
      (fun row : BHist => hsame row realRead ∧ UnaryHistory row) realRead :=
  ...
  exact ⟨cert, windowUnary, epigraphUnary, locatedUnary, transportUnary, realUnary⟩
