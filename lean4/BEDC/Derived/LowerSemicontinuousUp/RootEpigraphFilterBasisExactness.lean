...
theorem LowerSemicontinuousRootEpigraphFilterBasisExactness [AskSetup] [PackageSetup]
    {X F E W R O H C P N leftThreshold rightThreshold commonThreshold filterRead
      namedRead : BHist}
                ...
                SemanticNameCert
                  ...
                  (fun row : BHist =>
                    hsame row W ∨ hsame row R ∨ hsame row E ∨ hsame row O ∨
                      hsame row commonThreshold ∨ hsame row filterRead ∨ hsame row namedRead)
                  (fun row : BHist =>
