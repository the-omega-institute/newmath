...
theorem LowerSemicontinuousRealSeriesSiblingRoute [AskSetup] [PackageSetup]
    {X F E W R O H C P N windowRead epigraphRead locatedRead realSeal seriesRead : BHist}
                      ...
                      Cont epigraphRead O locatedRead ->
                        Cont locatedRead N realSeal ->
                          Cont realSeal P seriesRead ->
                            PkgSig bundle P pkg ->
                              PkgSig bundle N pkg ->
                                PkgSig bundle seriesRead pkg ->
                                  SemanticNameCert
                                      (fun row : BHist =>
                                        hsame row seriesRead ∧ UnaryHistory row)
                                      (fun row : BHist =>
  ...
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row seriesRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row W ∨ hsame row R ∨ hsame row E ∨ hsame row O ∨ hsame row N ∨
              hsame row P ∨ Cont realSeal P seriesRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont W R windowRead ∧ Cont windowRead E epigraphRead ∧
              Cont epigraphRead O locatedRead ∧ Cont locatedRead N realSeal ∧
                Cont realSeal P seriesRead ∧ PkgSig bundle seriesRead pkg)
          hsame := {
    core := {
      carrier_inhabited :=
        Exists.intro seriesRead ⟨hsame_refl seriesRead, seriesUnary⟩
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, windowRoute, epigraphRoute, locatedRoute, realSealRoute,
          seriesRoute, seriesPkg⟩
  }
  exact ⟨cert, seriesUnary⟩
