...
theorem LowerSemicontinuousPublicRouteExactnessTriad [AskSetup] [PackageSetup]
    {X F E W R O H C P N windowRead epigraphRead locatedRead realSeal publicRead :
      BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    lowerSemicontinuousRootFields (LowerSemicontinuousUp.mk X F E W R O H C P N) =
        [X, F, E, W, R, O, H, C, P, N] ->
                              ...
                              PkgSig bundle publicRead pkg ->
                                SemanticNameCert
                                  ...
                                  UnaryHistory windowRead ∧ UnaryHistory epigraphRead ∧
                                    UnaryHistory locatedRead ∧ UnaryHistory realSeal ∧
                                      UnaryHistory publicRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert
  intro fields wUnary rUnary eUnary oUnary cUnary _nUnary pUnary windowRoute epigraphRoute
    locatedRoute sealRoute publicRoute publicPkg
  have _acceptedFields :
      lowerSemicontinuousRootFields (LowerSemicontinuousUp.mk X F E W R O H C P N) =
        [X, F, E, W, R, O, H, C, P, N] := fields
  have windowUnary : UnaryHistory windowRead :=
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont W R windowRead ∧ Cont windowRead E epigraphRead ∧
              Cont epigraphRead O locatedRead ∧ Cont locatedRead C realSeal ∧
                Cont realSeal P publicRead ∧ PkgSig bundle publicRead pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, windowRoute, epigraphRoute, locatedRoute, sealRoute, publicRoute,
          publicPkg⟩
  }
  exact
    ⟨cert, windowUnary, epigraphUnary, locatedUnary, realSealUnary, publicUnary⟩
