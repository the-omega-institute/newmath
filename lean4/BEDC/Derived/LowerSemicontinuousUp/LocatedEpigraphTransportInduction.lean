import BEDC.Derived.LowerSemicontinuousUp.LocatedEpigraphTransport

namespace BEDC.Derived.LowerSemicontinuousUp

open BEDC.FKernel.Ask
open BEDC.FKernel.Bundle
open BEDC.FKernel.Cont
open BEDC.FKernel.Hist
open BEDC.FKernel.NameCert
open BEDC.FKernel.Package
open BEDC.FKernel.Unary

theorem LowerSemicontinuousLocatedEpigraphTransportInduction [AskSetup] [PackageSetup]
    {X F E W R O H C P N windowRead epigraphRead locatedRead transportRead replayRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    lowerSemicontinuousRootFields (LowerSemicontinuousUp.mk X F E W R O H C P N) =
        [X, F, E, W, R, O, H, C, P, N] ->
      UnaryHistory W -> UnaryHistory R -> UnaryHistory E -> UnaryHistory O ->
        UnaryHistory H -> UnaryHistory C -> Cont W R windowRead ->
          Cont windowRead E epigraphRead -> Cont epigraphRead O locatedRead ->
            Cont locatedRead H transportRead -> Cont transportRead C replayRead ->
              PkgSig bundle P pkg -> PkgSig bundle N pkg ->
                SemanticNameCert
                    (fun row : BHist =>
                      (hsame row locatedRead ∨ hsame row transportRead ∨
                          hsame row replayRead) ∧
                        UnaryHistory row)
                    (fun row : BHist =>
                      hsame row W ∨ hsame row R ∨ hsame row E ∨ hsame row O ∨
                        hsame row H ∨ hsame row C ∨ Cont epigraphRead O locatedRead ∨
                          Cont locatedRead H transportRead ∨ Cont transportRead C replayRead)
                    (fun row : BHist =>
                      UnaryHistory row ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
                    hsame ∧
                  UnaryHistory locatedRead ∧ UnaryHistory transportRead ∧
                    UnaryHistory replayRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro fields wUnary rUnary eUnary oUnary hUnary cUnary windowRoute epigraphRoute
    locatedRoute transportRoute replayRoute provenancePkg namePkg
  have _acceptedFields :
      lowerSemicontinuousRootFields (LowerSemicontinuousUp.mk X F E W R O H C P N) =
        [X, F, E, W, R, O, H, C, P, N] := fields
  have windowUnary : UnaryHistory windowRead :=
    unary_cont_closed wUnary rUnary windowRoute
  have epigraphUnary : UnaryHistory epigraphRead :=
    unary_cont_closed windowUnary eUnary epigraphRoute
  have locatedUnary : UnaryHistory locatedRead :=
    unary_cont_closed epigraphUnary oUnary locatedRoute
  have transportUnary : UnaryHistory transportRead :=
    unary_cont_closed locatedUnary hUnary transportRoute
  have replayUnary : UnaryHistory replayRead :=
    unary_cont_closed transportUnary cUnary replayRoute
  have cert :
      SemanticNameCert
          (fun row : BHist =>
            (hsame row locatedRead ∨ hsame row transportRead ∨ hsame row replayRead) ∧
              UnaryHistory row)
          (fun row : BHist =>
            hsame row W ∨ hsame row R ∨ hsame row E ∨ hsame row O ∨ hsame row H ∨
              hsame row C ∨ Cont epigraphRead O locatedRead ∨
                Cont locatedRead H transportRead ∨ Cont transportRead C replayRead)
          (fun row : BHist =>
            UnaryHistory row ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro locatedRead
        ⟨Or.inl (hsame_refl locatedRead), locatedUnary⟩
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
        intro _row _other sameRows
        exact hsame_symm sameRows
      equiv_trans := by
        intro _row _middle _other sameLeft sameRight
        exact hsame_trans sameLeft sameRight
      carrier_respects_equiv := by
        intro _row _other sameRows source
        constructor
        · cases source.left with
          | inl sameLocated =>
              exact Or.inl (hsame_trans (hsame_symm sameRows) sameLocated)
          | inr transportOrReplay =>
              cases transportOrReplay with
              | inl sameTransport =>
                  exact Or.inr (Or.inl (hsame_trans (hsame_symm sameRows) sameTransport))
              | inr sameReplay =>
                  exact Or.inr (Or.inr (hsame_trans (hsame_symm sameRows) sameReplay))
        · exact unary_transport source.right sameRows
    }
    pattern_sound := by
      intro _row source
      cases source.left with
      | inl _sameLocated =>
          exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl locatedRoute))))))
      | inr transportOrReplay =>
          cases transportOrReplay with
          | inl _sameTransport =>
              exact Or.inr
                (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl transportRoute)))))))
          | inr _sameReplay =>
              exact Or.inr
                (Or.inr
                  (Or.inr
                    (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr replayRoute)))))))
    ledger_sound := by
      intro _row source
      exact ⟨source.right, provenancePkg, namePkg⟩
  }
  exact ⟨cert, locatedUnary, transportUnary, replayUnary⟩

end BEDC.Derived.LowerSemicontinuousUp
