...
def lowerSemicontinuousRootEpigraphFields : LowerSemicontinuousUp → List BHist
  -- BEDC touchpoint anchor: BHist BMark
  | LowerSemicontinuousUp.mk X F E W R O H C P N => [X, F, W, R, E, O, H, C, P, N]

theorem LowerSemicontinuousRootEpigraph_carrier [AskSetup] [PackageSetup]
  ...
  intro rootFields epigraphFields rowsW rowsR rowsE rowsO windowRoute epigraphRoute
    locatedRoute packageRead
...
theorem LowerSemicontinuousClosedSuperlevelThresholdExhaustion [AskSetup] [PackageSetup]
                          ...
                          PkgSig bundle Pq pkg ->
                            PkgSig bundle Nq pkg ->
                              SemanticNameCert
                                      ...
                                      hsame row Bq ∨ hsame row Hq ∨ hsame row Cq ∨
                                        hsame row Pq ∨ hsame row Nq ∨
                                          hsame row thresholdRead ∨
                                            hsame row boundaryRead ∨ hsame row stableRead ∨
                                              hsame row replayRead ∨ hsame row exhaustedRead)
                                  (fun row : BHist =>
                                    UnaryHistory row ∧ Cont q Eq thresholdRead ∧
                                      Cont thresholdRead Oq boundaryRead ∧
                                        Cont boundaryRead Bq stableRead ∧
                                  ...
                                  hsame ∧
                                UnaryHistory thresholdRead ∧ UnaryHistory boundaryRead ∧
                                  UnaryHistory stableRead ∧ UnaryHistory replayRead ∧
                                    UnaryHistory exhaustedRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Cont PkgSig SemanticNameCert UnaryHistory
  intro qUnary eqUnary oqUnary bqUnary hqUnary cqUnary thresholdRoute boundaryRoute
    stableRoute replayRoute exhaustedRoute provenancePkg namePkg
  have thresholdUnary : UnaryHistory thresholdRead :=
    unary_cont_closed qUnary eqUnary thresholdRoute
  have boundaryUnary : UnaryHistory boundaryRead :=
    unary_cont_closed thresholdUnary oqUnary boundaryRoute
  ...
  have exhaustedUnary : UnaryHistory exhaustedRead :=
    unary_cont_closed replayUnary cqUnary exhaustedRoute
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont q Eq thresholdRead ∧ Cont thresholdRead Oq boundaryRead ∧
              Cont boundaryRead Bq stableRead ∧ Cont stableRead Hq replayRead ∧
                Cont replayRead Cq exhaustedRead ∧ PkgSig bundle Pq pkg ∧
                  PkgSig bundle Nq pkg)
          hsame := {
    core := {
      carrier_inhabited :=
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, thresholdRoute, boundaryRoute, stableRoute, replayRoute,
          exhaustedRoute, provenancePkg, namePkg⟩
  }
  exact
