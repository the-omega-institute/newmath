...
theorem LowerSemicontinuousRootRealSealBoundary [AskSetup] [PackageSetup]
    ...
    lowerSemicontinuousRootFields (LowerSemicontinuousUp.mk X F E W R O H C P N) =
      ...
      UnaryHistory W -> UnaryHistory R -> UnaryHistory E -> UnaryHistory O ->
        Cont W R windowRead -> Cont windowRead E epigraphRead ->
          Cont epigraphRead O realSeal -> PkgSig bundle P pkg ->
            PkgSig bundle N pkg ->
              SemanticNameCert
  ...
  intro fields wUnary rUnary eUnary oUnary windowRoute epigraphRoute realSealRoute
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row W ∨ hsame row R ∨ hsame row E ∨ hsame row O ∨
              hsame row windowRead ∨ hsame row epigraphRead ∨ hsame row realSeal)
          (fun row : BHist =>
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, windowRoute, epigraphRoute, realSealRoute, provenancePkg, namePkg⟩
  }
  exact ⟨cert, windowUnary, epigraphUnary, realSealUnary⟩

theorem LowerSemicontinuousRealSealBoundary [AskSetup] [PackageSetup]
