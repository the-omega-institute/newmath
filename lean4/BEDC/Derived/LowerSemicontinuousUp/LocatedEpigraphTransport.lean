...
theorem LowerSemicontinuousLocatedEpigraphTransport [AskSetup] [PackageSetup]
        ...
        UnaryHistory C -> Cont W R windowRead -> Cont windowRead E epigraphRead ->
          Cont epigraphRead O locatedRead -> Cont locatedRead C replayRead ->
            PkgSig bundle P pkg -> PkgSig bundle N pkg ->
              SemanticNameCert
                  (fun row : BHist =>
                    (hsame row locatedRead ∨ hsame row replayRead) ∧ UnaryHistory row)
                  (fun row : BHist =>
                    hsame row E ∨ hsame row O ∨ Cont epigraphRead O locatedRead ∨
                      Cont locatedRead C replayRead)
                  (fun row : BHist =>
                    UnaryHistory row ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
                  hsame ∧ UnaryHistory locatedRead ∧ UnaryHistory replayRead := by
  ...
  have cert :
    ...
    pattern_sound := by
      ...
      cases source.left with
      | inl _sameLocated =>
          exact Or.inr (Or.inr (Or.inl locatedRoute))
      | inr _sameReplay =>
          exact Or.inr (Or.inr (Or.inr replayRoute))
    ledger_sound := by
      intro _row source
      exact ⟨source.right, provenancePkg, namePkg⟩
  }
  exact ⟨cert, locatedUnary, replayUnary⟩
