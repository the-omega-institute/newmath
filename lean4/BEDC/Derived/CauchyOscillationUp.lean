...
theorem CauchyOscillationCarrier_tail_window_carrier_obligation [AskSetup] [PackageSetup]
    ...
    CauchyOscillationCarrier tailWindow modulus tolerance ledger sealRow transport routes provenance
      ...
      Cont tailWindow modulus windowRead ->
        Cont windowRead tolerance ledgerRead ->
          Cont ledgerRead sealRow routes ->
            Cont routes nameCert sealRead ->
              PkgSig bundle sealRead pkg ->
                SemanticNameCert
                  ...
                  (fun row : BHist =>
                    UnaryHistory row ∧ Cont tailWindow modulus windowRead ∧
                      Cont windowRead tolerance ledgerRead ∧ PkgSig bundle sealRead pkg)
                  hsame ∧ UnaryHistory windowRead ∧ UnaryHistory ledgerRead ∧
                    UnaryHistory sealRead := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle Pkg SemanticNameCert hsame UnaryHistory
  intro carrier tailWindowModulusWindowRead windowReadToleranceLedgerRead _ledgerReadSealRoute
    routesNameCertSealRead sealReadPkg
  obtain ⟨tailWindowUnary, modulusUnary, toleranceUnary, _ledgerUnary, _sealUnary,
    _transportUnary, routesUnary, _provenanceUnary, nameCertUnary, _tailWindowModulus,
    _modulusToleranceLedger, _ledgerSealRoute, _routesNameCert, _provenancePkg⟩ := carrier
  have windowReadUnary : UnaryHistory windowRead :=
    unary_cont_closed tailWindowUnary modulusUnary tailWindowModulusWindowRead
  have ledgerReadUnary : UnaryHistory ledgerRead :=
    unary_cont_closed windowReadUnary toleranceUnary windowReadToleranceLedgerRead
  have sealReadUnary : UnaryHistory sealRead :=
    unary_cont_closed routesUnary nameCertUnary routesNameCertSealRead
  have sourceLedger :
  ...
  have cert :
    ...
    exact {
      ...
      ledger_sound := by
        intro _row source
        exact
          ⟨source.right, tailWindowModulusWindowRead, windowReadToleranceLedgerRead,
            sealReadPkg⟩
    }
  exact ⟨cert, windowReadUnary, ledgerReadUnary, sealReadUnary⟩
