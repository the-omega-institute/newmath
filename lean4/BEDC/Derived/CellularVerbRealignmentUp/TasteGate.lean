...
theorem CellularVerbRealignmentNameCert_obligations
    {B O M H E K L C P N markRead histRead extRead contRead namedRead : BHist} :
    UnaryHistory B → UnaryHistory O → UnaryHistory M → UnaryHistory H → UnaryHistory E →
      UnaryHistory K → UnaryHistory L → UnaryHistory N → Cont B O markRead →
        Cont markRead M histRead → Cont histRead E extRead → Cont extRead K contRead →
            ...
            SemanticNameCert
                ...
                (fun row : BHist =>
                  UnaryHistory row ∧ Cont B O markRead ∧ Cont markRead M histRead ∧
                    Cont histRead E extRead ∧ Cont extRead K contRead ∧
                      Cont contRead N namedRead)
                hsame ∧
