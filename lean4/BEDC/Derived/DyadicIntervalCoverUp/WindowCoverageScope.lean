...
theorem DyadicIntervalCoverWindowCoverageScope [AskSetup] [PackageSetup]
    {L U M R V W Q A H C P N endpointRead windowRead coverRead membershipRead sealRead
      namedRead : BHist}
                ...
                Cont sealRead N namedRead →
                  PkgSig bundle namedRead pkg →
                    SemanticNameCert
                          ...
                          hsame row L ∨ hsame row U ∨ hsame row M ∨ hsame row R ∨
                            hsame row V ∨ hsame row W ∨ hsame row Q ∨ hsame row A ∨
                              hsame row endpointRead ∨ hsame row windowRead ∨
                                hsame row coverRead ∨ hsame row membershipRead ∨
                      ...
                      UnaryHistory coverRead ∧ UnaryHistory membershipRead ∧
                        UnaryHistory sealRead ∧ UnaryHistory namedRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro surface endpointRoute windowRoute coverRoute membershipRoute sealRoute namedRoute
    namedPkg
  have unaryL : UnaryHistory L := surface.left
  have unaryU : UnaryHistory U := surface.right.left
  have unaryM : UnaryHistory M := surface.right.right.left
  have unaryR : UnaryHistory R := surface.right.right.right.left
  have unaryV : UnaryHistory V := surface.right.right.right.right.left
  have unaryW : UnaryHistory W := surface.right.right.right.right.right.left
  have unaryQ : UnaryHistory Q := surface.right.right.right.right.right.right.left
  have unaryA : UnaryHistory A := surface.right.right.right.right.right.right.right.left
  ...
  have membershipUnary : UnaryHistory membershipRead :=
    unary_cont_closed coverUnary unaryV membershipRoute
  have sealUnary : UnaryHistory sealRead :=
    unary_cont_closed membershipUnary unaryA sealRoute
  ...
  have cert :
          ...
          (fun row : BHist =>
            hsame row L ∨ hsame row U ∨ hsame row M ∨ hsame row R ∨ hsame row V ∨
              hsame row W ∨ hsame row Q ∨ hsame row A ∨ hsame row endpointRead ∨
                hsame row windowRead ∨ hsame row coverRead ∨ hsame row membershipRead ∨
                  hsame row sealRead ∨ hsame row namedRead)
          (fun row : BHist =>
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, endpointRoute, windowRoute, coverRoute, membershipRoute, sealRoute,
          namedRoute, namedPkg⟩
  }
  exact
    ⟨cert, endpointUnary, windowUnary, coverUnary, membershipUnary, sealUnary, namedUnary⟩
