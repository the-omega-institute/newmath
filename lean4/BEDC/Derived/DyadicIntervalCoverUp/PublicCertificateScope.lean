...
theorem DyadicIntervalCoverPublicCertificateScope [AskSetup] [PackageSetup]
  ...
  intro surface endpointRoute windowRoute readbackRoute coverRoute sealRoute publicRoute
    publicPkg
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont L U endpointRead ∧ Cont W Q windowRead ∧
              Cont windowRead R readbackRead ∧ Cont readbackRead V coverRead ∧
                Cont coverRead A sealRead ∧ Cont sealRead N publicRead ∧
                  PkgSig bundle publicRead pkg)
          hsame := {
    core := {
