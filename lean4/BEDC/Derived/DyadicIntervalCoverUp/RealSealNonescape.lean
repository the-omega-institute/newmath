...
theorem DyadicIntervalCoverRealSealNonescape [AskSetup] [PackageSetup]
    {L U M R V W Q A H C P N endpointRead radiusRead membershipRead sealRead
      namedRead : BHist}
                  ...
                  SemanticNameCert
                      ...
                      (fun row : BHist =>
                        hsame row L ∨ hsame row U ∨ hsame row M ∨ hsame row R ∨
                          hsame row V ∨ hsame row A ∨ hsame row N ∨
                            hsame row endpointRead ∨ hsame row radiusRead ∨
                              hsame row membershipRead ∨ hsame row sealRead ∨
                                hsame row namedRead)
                      (fun row : BHist =>
                        UnaryHistory row ∧ Cont L U endpointRead ∧
                          Cont M R radiusRead ∧ Cont radiusRead V membershipRead ∧
                            Cont membershipRead A sealRead ∧ Cont sealRead N namedRead ∧
                              PkgSig bundle namedRead pkg)
                      hsame ∧ UnaryHistory endpointRead ∧ UnaryHistory radiusRead ∧
                    UnaryHistory membershipRead ∧ UnaryHistory sealRead ∧
                      UnaryHistory namedRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro surface endpointRoute radiusRoute membershipRoute sealRoute namedRoute namedPkg
  ...
  have radiusUnary : UnaryHistory radiusRead :=
    unary_cont_closed mUnary rUnary radiusRoute
  have membershipUnary : UnaryHistory membershipRead :=
    unary_cont_closed radiusUnary vUnary membershipRoute
  ...
  have cert :
          ...
          (fun row : BHist =>
            hsame row L ∨ hsame row U ∨ hsame row M ∨ hsame row R ∨ hsame row V ∨
              hsame row A ∨ hsame row N ∨ hsame row endpointRead ∨
                hsame row radiusRead ∨ hsame row membershipRead ∨
                  hsame row sealRead ∨ hsame row namedRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont L U endpointRead ∧ Cont M R radiusRead ∧
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, endpointRoute, radiusRoute, membershipRoute, sealRoute, namedRoute,
          namedPkg⟩
  }
  exact ⟨cert, endpointUnary, radiusUnary, membershipUnary, sealUnary, namedUnary⟩
