...
theorem DyadicIntervalCoverBridgeBudget [AskSetup] [PackageSetup]
    {L U M R V W Q A H C P N endpointRead windowRead readbackRead coverRead sealRead
      subcoverRead namedRead publicRead budgetRead : BHist}
          ...
          (fun row : BHist =>
            hsame row L ∨ hsame row U ∨ hsame row M ∨ hsame row R ∨ hsame row V ∨
              hsame row W ∨ hsame row Q ∨ hsame row A ∨ hsame row H ∨ hsame row C ∨
                hsame row P ∨ hsame row N ∨ hsame row publicRead ∨ hsame row budgetRead)
          (fun row : BHist =>
              ...
              Cont windowRead R readbackRead ∧ Cont readbackRead V coverRead ∧
                Cont coverRead A sealRead ∧ Cont endpointRead sealRead subcoverRead ∧
                  Cont subcoverRead N namedRead ∧ Cont namedRead P publicRead ∧
                    Cont publicRead N budgetRead ∧ PkgSig bundle budgetRead pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, endpointRoute, windowRoute, readbackRoute, coverRoute, sealRoute,
          subcoverRoute, namedRoute, publicRoute, budgetRoute, budgetPkg⟩
  }
  exact ⟨cert, publicUnary, budgetUnary⟩
