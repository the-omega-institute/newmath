...
theorem DyadicIntervalCoverBridgeReadbackFactorization [AskSetup] [PackageSetup]
    {L U M R V W Q A H C P N endpointRead windowRead readbackRead coverRead sealRead
      subcoverRead namedRead publicRead budgetRead bridgeRead factorRead : BHist}
                                                  ...
                                                  Cont publicRead N budgetRead ∧
                                                    Cont budgetRead C bridgeRead ∧
                                                      Cont bridgeRead Q factorRead ∧
                                                        PkgSig bundle factorRead pkg)
                                  hsame ∧
                                UnaryHistory bridgeRead ∧ UnaryHistory factorRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro surface endpointRoute windowRoute readbackRoute coverRoute sealRoute subcoverRoute
    namedRoute publicRoute budgetRoute bridgeRoute factorRoute factorPkg
  ...
  have factorUnary : UnaryHistory factorRead :=
    unary_cont_closed bridgeUnary qUnary factorRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row factorRead ∧ UnaryHistory row)
          (fun row : BHist =>
                ...
                Cont coverRead A sealRead ∧ Cont endpointRead sealRead subcoverRead ∧
                  Cont subcoverRead N namedRead ∧ Cont namedRead P publicRead ∧
                    Cont publicRead N budgetRead ∧ Cont budgetRead C bridgeRead ∧
                      Cont bridgeRead Q factorRead ∧ PkgSig bundle factorRead pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro factorRead ⟨hsame_refl factorRead, factorUnary⟩
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, endpointRoute, windowRoute, readbackRoute, coverRoute, sealRoute,
          subcoverRoute, namedRoute, publicRoute, budgetRoute, bridgeRoute, factorRoute,
          factorPkg⟩
  }
  exact ⟨cert, bridgeUnary, factorUnary⟩
