...
theorem RealSequenceAlgebraBoundedConsumer (x : RealSequenceAlgebraUp) :
    ∃ outputSeal : BHist,
      outputSeal ∈ realSequenceAlgebraFields x ∧
        ∃ dyadicLedger : BHist,
          dyadicLedger ∈ realSequenceAlgebraFields x ∧
            SemanticNameCert
              ...
              List.Mem.tail _ <|
                List.Mem.tail _ <|
                  List.Mem.tail _ <|
                    List.Mem.tail _ <|
                      List.Mem.tail _ <| List.Mem.head _
      refine ⟨outputSeal, outputMember, dyadicLedger, dyadicLedgerMember, ?_⟩
      refine
        { core :=
            { carrier_inhabited := ?_
              equiv_refl := ?_
