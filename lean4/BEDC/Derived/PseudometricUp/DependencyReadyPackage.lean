...
theorem PseudometricCarrier_dependency_ready_package [AskSetup] [PackageSetup]
    ...
    PseudometricCarrier point distance dyadic stream readback sealRow zeroRow transport replay
      ...
      Cont distance stream distanceRead → Cont distanceRead sealRow sealedDistance →
        Cont zeroRow transport zeroBoundaryRead → PkgSig bundle sealedDistance pkg →
          PkgSig bundle zeroBoundaryRead pkg →
            UnaryHistory point ∧ UnaryHistory distance ∧ UnaryHistory dyadic ∧
              UnaryHistory stream ∧ UnaryHistory readback ∧ UnaryHistory sealRow ∧
                UnaryHistory zeroRow ∧ UnaryHistory distanceRead ∧
  ...
  obtain ⟨pointUnary, distanceUnary, dyadicUnary, streamUnary, readbackUnary, sealUnary,
  ...
  exact ⟨pointUnary, distanceUnary, dyadicUnary, streamUnary, readbackUnary, sealUnary,
    zeroUnary, distanceReadUnary, sealedDistanceUnary, zeroBoundaryUnary,
    streamReadbackDyadic, dyadicSealZero, distanceRoute, sealedRoute, zeroBoundaryRoute,
    localNamePkg, sealedPkg, zeroBoundaryPkg⟩
