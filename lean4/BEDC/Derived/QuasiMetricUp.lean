...
theorem QuasiMetricCarrier_scope_tightening [AskSetup] [PackageSetup]
    {source points distance zero triangle ball filter net uniformReflection transport replay
      provenance localName directedRead ballRead completionRead scopedRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    QuasiMetricCarrier source points distance zero triangle ball filter net uniformReflection
        transport replay provenance localName bundle pkg →
                ...
                PkgSig bundle scopedRead pkg →
                  SemanticNameCert
                    ...
                    UnaryHistory directedRead ∧ UnaryHistory ballRead ∧
                      UnaryHistory completionRead ∧ UnaryHistory scopedRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier localNameUnary distancePointRead directedBallRead ballUniformRead
    completionLocalRead scopedPkg
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont distance points directedRead ∧
              Cont directedRead ball ballRead ∧ Cont ball uniformReflection completionRead ∧
                Cont completionRead localName scopedRead ∧ PkgSig bundle scopedRead pkg)
          hsame := {
    core := {
...
theorem QuasiMetricCarrier_directed_distance_scope_upgrade [AskSetup] [PackageSetup]
    {source points distance zero triangle ball filter net uniformReflection transport replay
      provenance localName directedRead ballRead completionRead scopedRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    QuasiMetricCarrier source points distance zero triangle ball filter net uniformReflection
                          ...
                          Cont directedRead ball ballRead ∧
                            Cont ball uniformReflection completionRead ∧
                              Cont completionRead localName scopedRead ∧
                                PkgSig bundle scopedRead pkg)
                      hsame ∧
                    UnaryHistory distance ∧ UnaryHistory directedRead ∧ UnaryHistory ballRead ∧
                      UnaryHistory completionRead ∧ UnaryHistory scopedRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier localNameUnary distancePointRead directedBallRead ballUniformRead
    completionLocalRead scopedPkg
  ...
  exact
    ⟨cert, distanceUnary, directedReadUnary, ballReadUnary, completionReadUnary,
      scopedReadUnary⟩
