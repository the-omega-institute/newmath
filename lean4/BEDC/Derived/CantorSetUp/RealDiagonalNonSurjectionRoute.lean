...
theorem CantorSetRealDiagonalNonSurjectionRoute [AskSetup] [PackageSetup]
        ...
        Cont gapRead D endpointRead →
          Cont endpointRead R regularRead →
            Cont regularRead E sealedRead →
              Cont sealedRead H diagonalRead →
                PkgSig bundle P pkg →
                  PkgSig bundle N pkg →
                    UnaryHistory T →
                      UnaryHistory G →
                            ...
                            UnaryHistory R →
                              UnaryHistory E →
                                UnaryHistory H →
                                  SemanticNameCert
                                      (fun row : BHist =>
                                        hsame row diagonalRead ∧ UnaryHistory row)
                                      (fun row : BHist =>
                                        hsame row T ∨ hsame row G ∨ hsame row I ∨
                                          hsame row D ∨ hsame row R ∨ hsame row E ∨
                                            hsame row H ∨ hsame row diagonalRead)
                                      (fun row : BHist =>
                                        UnaryHistory row ∧ Cont sealedRead H diagonalRead ∧
                                          PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
                                      hsame ∧
                                    UnaryHistory prefixRead ∧ UnaryHistory gapRead ∧
                                      UnaryHistory endpointRead ∧ UnaryHistory regularRead ∧
                                        UnaryHistory sealedRead ∧ UnaryHistory diagonalRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro prefixRoute gapRoute endpointRoute regularRoute sealedRoute diagonalRoute pkgP pkgN
    unaryT unaryG unaryI unaryD unaryR unaryE unaryH
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, diagonalRoute, pkgP, pkgN⟩
  }
  exact
    ⟨cert, prefixUnary, gapUnary, endpointUnary, regularUnary, sealedUnary, diagonalUnary⟩
