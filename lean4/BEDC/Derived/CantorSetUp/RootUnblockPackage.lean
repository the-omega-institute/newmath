...
theorem CantorSetCarrier_root_unblock_package [AskSetup] [PackageSetup]
    {T G I D R E H K P N prefixRead gapRead endpointRead regularRead sealedRead boundedRead
      finiteNetRead : BHist}
  ...
  intro prefixRoute gapRoute endpointRoute regularRoute sealRoute boundedRoute finiteRoute
  ...
  have finiteUnary : UnaryHistory finiteNetRead :=
    unary_cont_closed boundedUnary unaryK finiteRoute
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg ∧
              Cont sealedRead H boundedRead ∧ Cont boundedRead K finiteNetRead)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro finiteNetRead ⟨hsame_refl finiteNetRead, finiteUnary⟩
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, pkgP, pkgN, boundedRoute, finiteRoute⟩
  }
  exact
