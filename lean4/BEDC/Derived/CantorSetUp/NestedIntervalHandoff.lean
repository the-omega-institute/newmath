...
theorem CantorSetCarrier_nested_interval_handoff [AskSetup] [PackageSetup]
    {T G I D R E H K P N endpointRead realRead : BHist}
  ...
  intro intervalRoute endpointRoute realRoute packageN prefixUnary gapUnary endpointUnary
    realUnary
  ...
  have cert :
      SemanticNameCert
          ...
          hsame := {
    core := {
      carrier_inhabited := Exists.intro I ⟨hsame_refl I, intervalUnary⟩
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, intervalRoute, endpointRoute, realRoute, packageN⟩
  }
  exact ⟨cert, intervalUnary, endpointReadUnary, realReadUnary⟩
