...
theorem ClosedSubstitutionSealPrimitiveScope [AskSetup] [PackageSetup]
    {T D V Q L M A K substitutionRead shiftRead auditRead handoffRead : BHist}
                          ...
                          Cont auditRead K handoffRead ->
                            PkgSig bundle handoffRead pkg ->
                              SemanticNameCert
                                  ...
                                  (fun row : BHist =>
                                    hsame row T ∨ hsame row D ∨ hsame row V ∨
                                      hsame row Q ∨ hsame row L ∨ hsame row M ∨
                                        hsame row A ∨ hsame row K ∨ hsame row handoffRead)
                                  (fun row : BHist =>
                                      ...
                                      Cont substitutionRead M shiftRead ∧
                                        Cont shiftRead A auditRead ∧
                                          Cont auditRead K handoffRead ∧
                                            PkgSig bundle handoffRead pkg)
                                  hsame ∧
                                UnaryHistory substitutionRead ∧ UnaryHistory shiftRead ∧
                                  UnaryHistory auditRead ∧ UnaryHistory handoffRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro _tUnary _dUnary _vUnary qUnary lUnary mUnary aUnary kUnary qToL subToM
    shiftToA auditToK handoffPkg
  ...
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row handoffRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row T ∨ hsame row D ∨ hsame row V ∨ hsame row Q ∨ hsame row L ∨
              hsame row M ∨ hsame row A ∨ hsame row K ∨ hsame row handoffRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont Q L substitutionRead ∧
          ...
          hsame := {
    core := {
      carrier_inhabited := ⟨handoffRead, hsame_refl handoffRead, handoffUnary⟩
      equiv_refl := by
        intro row _source
  ...
  exact ⟨cert, substitutionUnary, shiftUnary, auditUnary, handoffUnary⟩
