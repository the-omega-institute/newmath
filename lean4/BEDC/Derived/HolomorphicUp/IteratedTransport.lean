...
theorem IteratedStrictCplxDiff_hsame_transport_unary_readback {seed seed' h h' : BHist}
    {n : Nat} :
    hsame seed seed' -> hsame h h' -> IteratedStrictCplxDiff seed n h ->
      IteratedStrictCplxDiff seed' n h' ∧ (UnaryHistory seed' -> UnaryHistory h') := by
  intro sameSeed sameH diff
  have transported : IteratedStrictCplxDiff seed' n h' := by
    induction n generalizing seed seed' h h' with
    | zero =>
        exact hsame_trans (hsame_symm sameSeed) (hsame_trans diff sameH)
    | succ n ih =>
        ...
        | intro previous rest =>
            cases rest with
            | intro step data =>
                have previousTransported : IteratedStrictCplxDiff seed' n previous :=
                  ih sameSeed (hsame_refl previous) data.left
                have stepTransported : Cont previous step h' :=
                  cont_hsame_transport (hsame_refl previous) (hsame_refl step) sameH
  ...
  exact And.intro transported
    (fun seedUnary => IteratedStrictCplxDiff_unary_of_seed seedUnary transported)
