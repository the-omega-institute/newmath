...
theorem FilterLimitBasisWindowInduction [AskSetup] [PackageSetup]
    ...
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    FilterLimitBasisCarrier Q F L W R D E H C P N bundle pkg -> Cont Q F basis ->
      Cont basis L limit -> Cont limit W window -> Cont window R readback ->
        Cont readback D tolerance -> Cont tolerance E realSeal -> Cont H C replayRead ->
          PkgSig bundle P pkg -> PkgSig bundle replayRead pkg ->
            SemanticNameCert (fun row : BHist => hsame row realSeal ∧ UnaryHistory row)
              (fun row : BHist => hsame row Q ∨ hsame row F ∨ hsame row L ∨ hsame row W ∨
                hsame row R ∨ hsame row D ∨ hsame row E ∨ hsame row realSeal ∨
                  hsame row replayRead)
              (fun row : BHist => UnaryHistory row ∧ Cont Q F basis ∧ Cont basis L limit ∧
                Cont limit W window ∧ Cont window R readback ∧ Cont readback D tolerance ∧
                  Cont tolerance E realSeal ∧ Cont H C replayRead ∧ PkgSig bundle P pkg ∧
                    PkgSig bundle replayRead pkg) hsame ∧ UnaryHistory realSeal ∧
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, qf, basisLimit, limitWindow, windowRead, readTolerance, toleranceSeal,
          hc, pkgP, pkgReplay⟩
  }
  exact ⟨cert, realSealUnary, replayReadUnary⟩
