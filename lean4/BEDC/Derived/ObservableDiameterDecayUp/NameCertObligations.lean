...
theorem ObservableDiameterDecayNamecertObligations [AskSetup] [PackageSetup]
    {compactNet pointwise radius triangle modulus transport replay provenance localName
      compactPoint radiusTriangle triangleModulus finalRead : BHist}
                  ...
                  Cont compactNet pointwise compactPoint ->
                    Cont radius triangle radiusTriangle ->
                      Cont radiusTriangle modulus triangleModulus ->
                        Cont compactPoint triangleModulus finalRead ->
                            ...
                            PkgSig bundle localName pkg ->
                              SemanticNameCert
                                  ...
                                  (fun row : BHist =>
                                    hsame row compactNet ∨ hsame row pointwise ∨
                                      hsame row radius ∨ hsame row triangle ∨
                                        hsame row modulus ∨ hsame row finalRead)
                                  (fun row : BHist =>
                                ...
                                UnaryHistory compactPoint ∧ UnaryHistory radiusTriangle ∧
                                  UnaryHistory triangleModulus ∧ UnaryHistory finalRead := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro compactUnary pointwiseUnary radiusUnary triangleUnary modulusUnary _transportUnary
    _replayUnary compactPointRoute radiusTriangleRoute triangleModulusRoute finalRoute
  ...
  have finalUnary : UnaryHistory finalRead :=
    unary_cont_closed compactPointUnary triangleModulusUnary finalRoute
  have sourceFinal :
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row compactNet ∨ hsame row pointwise ∨ hsame row radius ∨
              hsame row triangle ∨ hsame row modulus ∨ hsame row finalRead)
          (fun row : BHist =>
            UnaryHistory row ∧ PkgSig bundle provenance pkg ∧
              PkgSig bundle localName pkg ∧ Cont compactPoint triangleModulus finalRead)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, provenancePkg, localNamePkg, finalRoute⟩
  }
  exact ⟨cert, compactPointUnary, radiusTriangleUnary, triangleModulusUnary, finalUnary⟩
