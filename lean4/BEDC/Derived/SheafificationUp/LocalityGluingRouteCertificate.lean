...
theorem SheafificationLocalityGluingRouteCertificate [AskSetup] [PackageSetup]
    {C T J P L G S H R Q N localityRead gluingRead targetRead replayRead : BHist}
                      ...
                      (fun row : BHist =>
                        UnaryHistory row ∧ Cont P L localityRead ∧
                          Cont localityRead G gluingRead ∧ Cont gluingRead S targetRead ∧
                            Cont targetRead R replayRead ∧ PkgSig bundle Q pkg ∧
                              PkgSig bundle N pkg)
                      hsame ∧
                    UnaryHistory localityRead ∧ UnaryHistory gluingRead ∧
                      UnaryHistory targetRead ∧ UnaryHistory replayRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert
  intro carrier localityRoute gluingRoute targetRoute replayRoute provenancePkg namePkg
  obtain ⟨_cUnary, _tUnary, _jUnary, pUnary, lUnary, gUnary, sUnary, hUnary,
    rUnary, qUnary, nUnary, _carrierProvenancePkg, _carrierNamePkg⟩ := carrier
  ...
  have replayUnary : UnaryHistory replayRead :=
    unary_cont_closed targetUnary rUnary replayRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row replayRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row P ∨ hsame row L ∨ hsame row G ∨ hsame row S ∨ hsame row H ∨
              hsame row R ∨ hsame row Q ∨ hsame row N ∨ hsame row replayRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont P L localityRead ∧ Cont localityRead G gluingRead ∧
              Cont gluingRead S targetRead ∧ Cont targetRead R replayRead ∧
    ...
    pattern_sound := by
      ...
      exact Or.inr <| Or.inr <| Or.inr <| Or.inr <| Or.inr <| Or.inr <|
        Or.inr <| Or.inr <| hsame_refl replayRead
    ledger_sound := by
      intro row sourceRow
      exact
        ⟨sourceRow.right, localityRoute, gluingRoute, targetRoute, replayRoute,
          provenancePkg, namePkg⟩
  }
  exact ⟨cert, localityUnary, gluingUnary, targetUnary, replayUnary⟩
