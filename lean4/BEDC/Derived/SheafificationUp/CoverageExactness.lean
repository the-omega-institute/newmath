...
theorem SheafificationCoverageExactness [AskSetup] [PackageSetup]
              ...
              SemanticNameCert
                  ...
                  (fun row : BHist =>
                    hsame row C ∨ hsame row T ∨ hsame row J ∨ hsame row P ∨
                      hsame row coverWindow ∨ hsame row restrictionRead ∨
                ...
                UnaryHistory coverWindow ∧ UnaryHistory restrictionRead ∧
                  UnaryHistory localityRead ∧ PkgSig bundle N pkg := by
  ...
  have sourceLocality : hsame localityRead localityRead ∧ UnaryHistory localityRead :=
    ⟨hsame_refl localityRead, localityReadUnary⟩
  have cert :
      SemanticNameCert
        (fun row : BHist => hsame row localityRead ∧ UnaryHistory row)
        (fun row : BHist =>
          hsame row C ∨ hsame row T ∨ hsame row J ∨ hsame row P ∨
            hsame row coverWindow ∨ hsame row restrictionRead ∨ hsame row localityRead)
        (fun row : BHist =>
          UnaryHistory row ∧ Cont C T coverWindow ∧ Cont J P restrictionRead ∧
            Cont coverWindow restrictionRead localityRead ∧ PkgSig bundle localityRead pkg)
        hsame := {
    core := {
      carrier_inhabited := Exists.intro localityRead sourceLocality
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, coverRoute, restrictionRoute, localityRoute, localityPkg⟩
  }
  exact
    ⟨cert, coverWindowUnary, restrictionReadUnary, localityReadUnary, namePkg⟩
