...
theorem RegularCauchyRingCarrier_componentwise_ring_laws [AskSetup] [PackageSetup]
    {A B WA WB DA DB S G M L RS RG RM RL ES EG EM EL H C P N ringLawRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    RegularCauchyRingCarrier A B WA WB DA DB S G M L RS RG RM RL ES EG EM EL H C P N
        bundle pkg →
      Cont S ES RS →
        Cont G EG RG →
          Cont M EM RM →
            Cont L EL RL →
              Cont RS RM ringLawRead →
                PkgSig bundle ringLawRead pkg →
                  SemanticNameCert
                      (fun row : BHist => hsame row ringLawRead ∧ UnaryHistory row)
                      (fun row : BHist =>
                        hsame row S ∨ hsame row G ∨ hsame row M ∨ hsame row L ∨
                          hsame row RS ∨ hsame row RG ∨ hsame row RM ∨ hsame row RL ∨
                            hsame row ringLawRead)
                      (fun row : BHist => UnaryHistory row ∧ PkgSig bundle ringLawRead pkg)
                      hsame ∧
                    UnaryHistory ringLawRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert
  intro carrier sumSeal negSeal productSeal scaleSeal ringLawRoute ringLawPkg
  obtain ⟨_unaryA, _unaryB, _unaryWA, _unaryWB, _unaryDA, _unaryDB, sUnary, gUnary,
    mUnary, lUnary, _carrierUnaryRS, _carrierUnaryRG, _carrierUnaryRM, _carrierUnaryRL,
    esUnary, egUnary, emUnary, elUnary, _unaryH, _unaryC, _unaryP, _unaryN,
    _sourceWindowA, _sourceWindowB, _transportReplay, _provenancePkg, _namePkg⟩ :=
      carrier
  have rsUnary : UnaryHistory RS := unary_cont_closed sUnary esUnary sumSeal
  have rgUnary : UnaryHistory RG := unary_cont_closed gUnary egUnary negSeal
  have rmUnary : UnaryHistory RM := unary_cont_closed mUnary emUnary productSeal
  have rlUnary : UnaryHistory RL := unary_cont_closed lUnary elUnary scaleSeal
  have ringLawUnary : UnaryHistory ringLawRead :=
    unary_cont_closed rsUnary rmUnary ringLawRoute
  constructor
  · exact {
      core := {
        carrier_inhabited :=
          Exists.intro ringLawRead ⟨hsame_refl ringLawRead, ringLawUnary⟩
        equiv_refl := by
          intro row _source
          exact hsame_refl row
        equiv_symm := by
