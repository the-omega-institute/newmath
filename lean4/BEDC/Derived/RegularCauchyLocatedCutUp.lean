...
inductive RegularCauchyLocatedCutUp : Type where
  -- BEDC touchpoint anchor: BHist Cont hsame NameCert
  | mk : (S Q D L R H C P N : BHist) → RegularCauchyLocatedCutUp

def regularCauchyLocatedCutFields :
    RegularCauchyLocatedCutUp → List BHist
  -- BEDC touchpoint anchor: BHist Cont hsame NameCert
  | RegularCauchyLocatedCutUp.mk S Q D L R H C P N =>
      [S, Q, D, L, R, H, C, P, N]

theorem RegularCauchyLocatedCutCarrier_namecert_obligations [AskSetup] [PackageSetup]
    (K : RegularCauchyLocatedCutUp)
    {S Q D L R H C P N windowRead rationalRead dyadicRead cutRead realSeal : BHist}
                          ...
                          SemanticNameCert
                              ...
                              (fun row : BHist =>
                                hsame row S ∨ hsame row Q ∨ hsame row D ∨
                                  hsame row L ∨ hsame row R ∨ Cont S Q windowRead ∨
                                    Cont windowRead D dyadicRead ∨
                                      Cont dyadicRead L cutRead ∨
                                        Cont cutRead R realSeal)
                              (fun row : BHist =>
                            ...
                            UnaryHistory windowRead ∧ UnaryHistory dyadicRead ∧
                              UnaryHistory cutRead ∧ UnaryHistory realSeal := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert UnaryHistory
  intro fieldsEq streamUnary rationalUnary dyadicUnary cutUnary realUnary
    streamRationalWindow windowDyadicRead dyadicCutRead cutRealSeal provenancePkg
  cases K with
  | mk KS KQ KD KL KR KH KC KP KN =>
          ...
          SemanticNameCert
              ...
              (fun row : BHist =>
                hsame row KS ∨ hsame row KQ ∨ hsame row KD ∨ hsame row KL ∨
                  hsame row KR ∨ Cont KS KQ windowRead ∨
                    Cont windowRead KD dyadicRead ∨ Cont dyadicRead KL cutRead ∨
                      Cont cutRead KR realSeal)
              (fun row : BHist => PkgSig bundle KP pkg ∧ hsame row realSeal)
              hsame := {
        core := {
          carrier_inhabited :=
      ...
      exact ⟨cert, windowUnary, dyadicReadUnary, cutReadUnary, realSealUnary⟩
