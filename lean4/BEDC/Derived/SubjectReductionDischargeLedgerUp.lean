...
theorem SubjectReductionDischargeLedgerNonescape
    (x : SubjectReductionDischargeLedgerUp) :
    ∃ beta appArg lambdaDomain piDomain route transport replay provenance name : BHist,
      x = SubjectReductionDischargeLedgerUp.mk beta appArg lambdaDomain piDomain route
          transport replay provenance name ∧
        SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row name ∧ Cont BHist.Empty beta beta ∧ Cont BHist.Empty appArg appArg ∧
              Cont BHist.Empty lambdaDomain lambdaDomain ∧ Cont BHist.Empty piDomain piDomain)
          hsame := by
  -- BEDC touchpoint anchor: BHist hsame Cont SemanticNameCert
  cases x with
  | mk beta appArg lambdaDomain piDomain route transport replay provenance name =>
      ...
      exact {
        ...
        ledger_sound := by
          intro _row source
          exact
            ⟨source, cont_left_unit beta, cont_left_unit appArg,
              cont_left_unit lambdaDomain, cont_left_unit piDomain⟩
      }
