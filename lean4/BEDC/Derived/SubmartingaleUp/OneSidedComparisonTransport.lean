...
theorem SubmartingaleCarrier_one_sided_comparison_transport [AskSetup] [PackageSetup]
  ...
  intro carrier comparisonRoute sameTransport replayRoute _provenancePkg localNamePkg
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont endpoint expectation comparisonRead ∧
              Cont transportedRead replay replayRead ∧ PkgSig bundle localName pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, comparisonRoute, replayRoute, localNamePkg⟩
  }
