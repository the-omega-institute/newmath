...
theorem BishopCompletionModulusCarrier_bridge_facing_real_seal_boundary [AskSetup] [PackageSetup]
    ...
    BishopCompletionModulusCarrier M S n k W D R E H C P N bundle pkg ->
      Cont R E sealRead ->
        UnaryHistory R ∧ UnaryHistory E ∧ UnaryHistory sealRead ∧ Cont M n k ∧
          Cont S k W ∧ Cont W D R ∧ Cont R E sealRead ∧ hsame sealRead H ∧
  ...
  obtain ⟨_mUnary, _sUnary, _nUnary, _kUnary, _wUnary, _dUnary, rUnary, eUnary,
    _hUnary, _cUnary, _pUnary, _localNameUnary, modulusRoute, windowRoute,
      handoffRoute, carrierSealRoute, provenancePkg, localNamePkg⟩ := carrier
  have sealUnary : UnaryHistory sealRead :=
    unary_cont_closed rUnary eUnary sealRoute
  have sameSeal : hsame sealRead H :=
    cont_respects_hsame (hsame_refl R) (hsame_refl E) sealRoute carrierSealRoute
  exact
    ⟨rUnary, eUnary, sealUnary, modulusRoute, windowRoute, handoffRoute, sealRoute,
      sameSeal, provenancePkg, localNamePkg⟩
