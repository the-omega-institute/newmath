...
theorem LayeredRelationGateCarrierObligation [AskSetup] [PackageSetup]
    {A B L P N F G H C Pi sourceRead layerRead refusalRead verdictRead carrierRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    UnaryHistory A →
      UnaryHistory B →
        UnaryHistory L →
  ...
  intro aUnary bUnary lUnary pUnary nUnary fUnary gUnary _piUnary sourceRoute
    layerRoute refusalRoute verdictRoute carrierRoute packageRead
  have sourceUnary : UnaryHistory sourceRead :=
    unary_cont_closed aUnary bUnary sourceRoute
  have layerUnary : UnaryHistory layerRead :=
    unary_cont_closed sourceUnary lUnary layerRoute
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont A B sourceRead ∧ Cont sourceRead L layerRead ∧
              Cont P N refusalRead ∧ Cont refusalRead F verdictRead ∧
                Cont verdictRead G carrierRead ∧ PkgSig bundle Pi pkg)
          hsame := {
    core := {
  ...
  exact ⟨cert, sourceUnary, layerUnary, refusalUnary, verdictUnary, carrierUnary⟩
