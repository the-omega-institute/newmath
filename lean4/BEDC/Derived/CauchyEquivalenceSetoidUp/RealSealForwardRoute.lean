...
theorem CauchyEquivalenceSetoidRealSealForwardRoute [AskSetup] [PackageSetup]
    {s0 s1 r0 r1 dyadic test sealRow transport replay provenance name sealRead realRead :
      BHist}
            ...
            SemanticNameCert
                ...
                (fun row : BHist =>
                  UnaryHistory row ∧ Cont test sealRow sealRead ∧
                    Cont sealRead replay realRead ∧ PkgSig bundle realRead pkg)
                hsame ∧ UnaryHistory sealRead ∧ UnaryHistory realRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro carrier sealRoute realRoute realPkg
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row sourceRow
      exact ⟨sourceRow.right, sealRoute, realRoute, realPkg⟩
  }
  exact ⟨cert, sealReadUnary, realReadUnary⟩
