...
theorem CauchyEquivalenceSetoidRealSeparabilityStableDensity [AskSetup] [PackageSetup]
    {s0 s1 r0 r1 dyadic test sealRow transport replay provenance name densityRead
      sealRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    CauchyEquivalenceSetoidCarrier s0 s1 r0 r1 dyadic test sealRow transport replay
        provenance name bundle pkg →
      Cont test sealRow sealRead →
        Cont sealRead transport densityRead →
            ...
            SemanticNameCert
                ...
                (fun row : BHist =>
                  UnaryHistory row ∧ Cont test sealRow sealRead ∧
                    Cont sealRead transport densityRead ∧ PkgSig bundle densityRead pkg)
                hsame ∧ UnaryHistory sealRead ∧ UnaryHistory densityRead := by
  ...
  have densityUnary : UnaryHistory densityRead :=
    unary_cont_closed sealReadUnary transportUnary densityRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row densityRead ∧ UnaryHistory row)
          (fun row : BHist =>
  ...
  exact ⟨cert, sealReadUnary, densityUnary⟩
