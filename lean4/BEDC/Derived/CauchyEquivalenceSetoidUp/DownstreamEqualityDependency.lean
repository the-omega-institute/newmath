...
theorem CauchyEquivalenceSetoidDownstreamEqualityDependency [AskSetup] [PackageSetup]
    ...
    CauchyEquivalenceSetoidCarrier s0 s1 r0 r1 dyadic test sealRow transport replay
      ...
      Cont test sealRow sealRead ->
        Cont sealRead replay equalityRead ->
          Cont equalityRead name downstreamRead ->
                    ...
                    (fun row : BHist =>
                      hsame row s0 ∨ hsame row s1 ∨ hsame row r0 ∨ hsame row r1 ∨
                        hsame row dyadic ∨ hsame row test ∨ hsame row sealRead ∨
                          hsame row equalityRead ∨ hsame row downstreamRead)
                    (fun row : BHist =>
  ...
  intro carrier sealRoute equalityRoute downstreamRoute _equalityPkg downstreamPkg
  obtain ⟨_s0Unary, _s1Unary, _r0Unary, _r1Unary, _dyadicUnary, testUnary, sealUnary,
    _transportUnary, replayUnary, provenanceUnary, nameUnary, _leftTransport, _rightReplay,
    _dyadicSeal, provenancePkg, _namePkg⟩ := carrier
  ...
  have downstreamUnary : UnaryHistory downstreamRead :=
    unary_cont_closed equalityReadUnary nameUnary downstreamRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row downstreamRead ∧ UnaryHistory row)
          (fun row : BHist =>
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont test sealRow sealRead ∧
              Cont sealRead replay equalityRead ∧ Cont equalityRead name downstreamRead ∧
    ...
    ledger_sound := by
      intro _row sourceRow
      exact
        ⟨sourceRow.right, sealRoute, equalityRoute, downstreamRoute, provenancePkg,
          downstreamPkg⟩
  }
  exact ⟨cert, sealReadUnary, equalityReadUnary, downstreamUnary⟩
