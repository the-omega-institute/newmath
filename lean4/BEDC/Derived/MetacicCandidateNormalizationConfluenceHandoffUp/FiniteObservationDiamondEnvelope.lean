...
theorem FiniteObservationMetaCICDiamondEnvelopeHandoff [AskSetup] [PackageSetup]
    {A K N F C D B T R P L endpointRead residualRead deciderRead blockedRead joinRead :
      BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    BEDC.Derived.MetacicCandidateNormalizationConfluenceHandoffCarrier A K N F C D B T R P L
        bundle pkg →
                ...
                PkgSig bundle blockedRead pkg →
                  PkgSig bundle joinRead pkg →
                    SemanticNameCert
                        ...
                        (fun row : BHist =>
                          hsame row K ∨ hsame row N ∨ hsame row C ∨ hsame row D ∨
                            hsame row B ∨ hsame row endpointRead ∨
                              hsame row residualRead ∨ hsame row deciderRead ∨
                                hsame row blockedRead ∨ hsame row joinRead)
                        (fun row : BHist =>
                          UnaryHistory row ∧ Cont K N endpointRead ∧
                            Cont endpointRead C residualRead ∧
                              Cont residualRead D deciderRead ∧
                        ...
                        hsame ∧
                      UnaryHistory endpointRead ∧ UnaryHistory residualRead ∧
                        UnaryHistory deciderRead ∧ UnaryHistory blockedRead ∧
                          UnaryHistory joinRead := by
  -- BEDC touchpoint anchor: MetacicCandidateNormalizationConfluenceHandoffCarrier BHist Cont ProbeBundle Pkg PkgSig hsame SemanticNameCert UnaryHistory
  intro carrier endpointRoute residualRoute deciderRoute blockedRoute joinRoute _blockedPkg
    joinPkg
  obtain ⟨_auditUnary, candidateUnary, normalEndpointUnary, frontierUnary,
    confluenceUnary, decidabilityUnary, blockedUnary, _transportUnary, _replayUnary,
    _provenanceUnary, _localNameUnary, _provenancePkg⟩ := carrier
  have endpointReadUnary : UnaryHistory endpointRead :=
    unary_cont_closed candidateUnary normalEndpointUnary endpointRoute
  ...
  have joinReadUnary : UnaryHistory joinRead :=
    unary_cont_closed blockedReadUnary frontierUnary joinRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row joinRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row K ∨ hsame row N ∨ hsame row C ∨ hsame row D ∨
              hsame row B ∨ hsame row endpointRead ∨ hsame row residualRead ∨
                hsame row deciderRead ∨ hsame row blockedRead ∨ hsame row joinRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont K N endpointRead ∧
              Cont endpointRead C residualRead ∧
                Cont residualRead D deciderRead ∧
                  Cont deciderRead B blockedRead ∧
                    Cont blockedRead F joinRead ∧ PkgSig bundle joinRead pkg)
          hsame := {
    core := {
      carrier_inhabited :=
        Exists.intro joinRead ⟨hsame_refl joinRead, joinReadUnary⟩
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
  ...
  exact
    ⟨cert, endpointReadUnary, residualReadUnary, deciderReadUnary, blockedReadUnary,
      joinReadUnary⟩
