...
theorem ClosedSubstitutionBoundary_substitute_inertness_route
                        ...
                        hsame H BHist.Empty →
                          PkgSig bundle P pkg →
                            SemanticNameCert
                              ...
                              (fun row : BHist =>
                                hsame row N ∧ Cont K Q substituteRead ∧
                                  Cont substituteRead C publicRead ∧ hsame H BHist.Empty)
                              (fun row : BHist => hsame row N ∧ PkgSig bundle P pkg)
                              hsame := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont SemanticNameCert hsame
  intro _unaryT _unaryD _unaryV _unaryK _unaryR _unaryQ _unaryC unaryN
    substituteRoute publicRoute emptyTransport pkgSig
  ...
  exact {
    ...
    pattern_sound := by
      intro _row source
      exact ⟨source.left, substituteRoute, publicRoute, emptyTransport⟩
    ledger_sound := by
      intro _row source
