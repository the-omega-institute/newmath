...
theorem MetaCICNormalizationFrontierKernelNonescape [AskSetup] [PackageSetup]
    {candidate closedCandidate finished endpoint obstruction transport replay provenance
      localRow candidateRead finishedRead endpointRead kernelRead : BHist}
      ...
      Cont candidate closedCandidate candidateRead ->
        Cont finished endpoint finishedRead ->
          Cont endpoint replay endpointRead ->
            Cont endpointRead localRow kernelRead ->
              PkgSig bundle kernelRead pkg ->
                SemanticNameCert
                    ...
                    (fun row : BHist => PkgSig bundle kernelRead pkg ∧ hsame row kernelRead)
                    hsame ∧
                  UnaryHistory candidateRead ∧ UnaryHistory finishedRead ∧
                    UnaryHistory endpointRead ∧ UnaryHistory kernelRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg SemanticNameCert hsame Cont
  intro carrier candidateClosedRead finishedEndpointRead endpointReplayRead endpointLocalKernel
    kernelPkg
  ...
  have kernelReadUnary : UnaryHistory kernelRead :=
    unary_cont_closed endpointReadUnary localRowUnary endpointLocalKernel
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row kernelRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row candidateRead ∨ hsame row finishedRead ∨ hsame row endpointRead ∨
              hsame row obstruction ∨ hsame row kernelRead)
          (fun row : BHist => PkgSig bundle kernelRead pkg ∧ hsame row kernelRead)
          hsame := by
    exact {
      core := {
      ...
      ledger_sound := by
        intro _row source
        exact ⟨kernelPkg, source.left⟩
    }
  exact ⟨cert, candidateReadUnary, finishedReadUnary, endpointReadUnary, kernelReadUnary⟩
