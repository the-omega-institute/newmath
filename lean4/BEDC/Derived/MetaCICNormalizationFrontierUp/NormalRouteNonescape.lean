...
theorem MetaCICNormalizationFrontierCarrier_normal_route_nonescape [AskSetup] [PackageSetup]
    {candidate closedCandidate finished endpoint obstruction transport replay provenance
      localRow candidateRead finishedRead normalRead : BHist}
    ...
    MetaCICNormalizationFrontierCarrier candidate closedCandidate finished endpoint obstruction
      ...
      Cont candidate closedCandidate candidateRead ->
        Cont finished endpoint finishedRead ->
          Cont candidateRead finishedRead normalRead ->
              ...
              SemanticNameCert
                  ...
                  (fun row : BHist =>
                    hsame row candidateRead ∨ hsame row finishedRead ∨
                      hsame row obstruction ∨ hsame row normalRead)
                  (fun row : BHist => PkgSig bundle normalRead pkg ∧ hsame row normalRead)
                  hsame ∧
                UnaryHistory candidateRead ∧ UnaryHistory finishedRead ∧
                  UnaryHistory normalRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg SemanticNameCert hsame Cont
  intro carrier candidateClosedRead finishedEndpointRead candidateFinishedNormal normalPkg
  ...
  have normalReadUnary : UnaryHistory normalRead :=
    unary_cont_closed candidateReadUnary finishedReadUnary candidateFinishedNormal
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row normalRead ∧ UnaryHistory row)
          (fun row : BHist =>
    ...
    exact {
      ...
      ledger_sound := by
        intro _row source
        exact ⟨normalPkg, source.left⟩
    }
  exact ⟨cert, candidateReadUnary, finishedReadUnary, normalReadUnary⟩
