...
theorem MetaCICNormalizationFrontierRootObstructionPreservingNamecertTriad
    [AskSetup] [PackageSetup]
    {candidate closedCandidate finished endpoint obstruction transport replay provenance localRow
      candidateRead endpointRead obstructionRead : BHist}
              ...
              UnaryHistory candidateRead ∧ UnaryHistory endpointRead ∧
                UnaryHistory obstructionRead ∧
                  SemanticNameCert
                    ...
                    (fun row : BHist =>
                      PkgSig bundle obstructionRead pkg ∧ hsame row obstructionRead)
                    hsame := by
  ...
  have cert :
      SemanticNameCert
        ...
        (fun row : BHist => PkgSig bundle obstructionRead pkg ∧ hsame row obstructionRead)
        hsame := by
    exact {
      core := {
        carrier_inhabited :=
      ...
      ledger_sound := by
        intro _row source
        exact ⟨obstructionReadPkg, source.left⟩
    }
  exact ⟨candidateReadUnary, endpointReadUnary, obstructionReadUnary, cert⟩
