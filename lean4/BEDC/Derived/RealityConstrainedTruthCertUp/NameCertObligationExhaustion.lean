...
theorem RealityConstrainedTruthCertNameCertObligationExhaustion [AskSetup] [PackageSetup]
    {S Sigma K T U D I L F N sourceSig ledgerFailure exportRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    Cont S Sigma sourceSig ->
      Cont L F ledgerFailure ->
        Cont ledgerFailure N exportRead ->
                  ...
                  Cont ledgerFailure N exportRead /\
                    PkgSig bundle exportRead pkg /\
                      SemanticNameCert
                        ...
                        (fun row : BHist =>
                          hsame row sourceSig \/ hsame row ledgerFailure \/
                            hsame row exportRead)
                        (fun row : BHist =>
                          hsame row exportRead /\ PkgSig bundle exportRead pkg)
                        hsame := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle Pkg SemanticNameCert hsame
  intro sourceRoute ledgerRoute exportRoute exportPkg
  have sourceExport :
  ...
  have cert :
      SemanticNameCert
        (fun row : BHist => hsame row exportRead)
        (fun row : BHist =>
          hsame row sourceSig \/ hsame row ledgerFailure \/ hsame row exportRead)
  ...
  exact ⟨rfl, sourceRoute, ledgerRoute, exportRoute, exportPkg, cert⟩
