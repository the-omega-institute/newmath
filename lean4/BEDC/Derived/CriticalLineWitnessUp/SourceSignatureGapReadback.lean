...
theorem CriticalLineWitnessCarrier_source_signature_gap_readback
    {Z S M R Q H C P N signatureRead gapRead readback : BHist} :
        ...
        Cont signatureRead Q gapRead ->
          Cont gapRead N readback ->
            SemanticNameCert
                ...
                (fun row : BHist =>
                  hsame row readback ∧ Cont Z H signatureRead ∧
                    Cont signatureRead Q gapRead)
                (fun row : BHist => hsame row readback ∧ Cont gapRead N readback)
                hsame ∧
              UnaryHistory signatureRead ∧ UnaryHistory gapRead ∧ UnaryHistory readback ∧
                hsame H (append Z S) ∧ Cont M R Q ∧ Cont Q H C ∧ Cont C P N := by
  -- BEDC touchpoint anchor: BHist Cont hsame SemanticNameCert UnaryHistory
  intro packet signatureRoute gapRoute readbackRoute
  ...
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row readback ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row readback ∧ Cont Z H signatureRead ∧ Cont signatureRead Q gapRead)
          (fun row : BHist => hsame row readback ∧ Cont gapRead N readback)
          hsame := {
    core := {
  ...
  exact
    ⟨cert, signatureUnary, gapUnary, readbackUnary, sameH, routeQ, routeC, routeN⟩
