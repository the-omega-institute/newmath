...
theorem CriticalLineWitnessCarrier_source_ledger_exhaustion
    {Z S M R Q H C P N sourceRead ledgerRead downstreamRead : BHist} :
    CriticalLineWitnessCarrier Z S M R Q H C P N ->
      Cont Z S sourceRead ->
        Cont sourceRead H ledgerRead ->
                ...
                (fun row : BHist =>
                  hsame row Z ∨ hsame row S ∨ hsame row M ∨ hsame row R ∨
                    hsame row Q ∨ hsame row H ∨ hsame row C ∨ hsame row P ∨
                      hsame row N ∨ hsame row sourceRead ∨ hsame row ledgerRead ∨
                        hsame row downstreamRead)
                (fun row : BHist =>
                  UnaryHistory row ∧ Cont Z S sourceRead ∧
                    Cont sourceRead H ledgerRead ∧ Cont ledgerRead Q downstreamRead ∧
                      hsame H (append Z S))
                hsame ∧
  ...
  have ledgerUnary : UnaryHistory ledgerRead :=
    unary_cont_closed sourceUnary unaryH ledgerRoute
  have downstreamUnary : UnaryHistory downstreamRead :=
    unary_cont_closed ledgerUnary unaryQ downstreamRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row downstreamRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row Z ∨ hsame row S ∨ hsame row M ∨ hsame row R ∨ hsame row Q ∨
              hsame row H ∨ hsame row C ∨ hsame row P ∨ hsame row N ∨
                hsame row sourceRead ∨ hsame row ledgerRead ∨ hsame row downstreamRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont Z S sourceRead ∧ Cont sourceRead H ledgerRead ∧
              Cont ledgerRead Q downstreamRead ∧ hsame H (append Z S))
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, sourceRoute, ledgerRoute, downstreamRoute, sameH⟩
  }
  exact ⟨cert, sourceUnary, ledgerUnary, downstreamUnary⟩
