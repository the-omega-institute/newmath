...
theorem CriticalLineWitnessCarrier_rh_consumer_refusal_exactness
      ...
      Cont N Q refusalRead →
        Cont refusalRead C consumerRead →
          SemanticNameCert
              ...
              (fun row : BHist => hsame row consumerRead ∧ hsame H (append Z S))
              (fun row : BHist =>
                hsame row consumerRead ∧ Cont N Q refusalRead ∧
                  Cont refusalRead C consumerRead)
              hsame ∧
            UnaryHistory Z ∧ UnaryHistory S ∧ UnaryHistory M ∧ UnaryHistory R ∧
              UnaryHistory Q ∧ UnaryHistory C ∧ UnaryHistory N ∧
                UnaryHistory refusalRead ∧ UnaryHistory consumerRead ∧
                  hsame H (append Z S) ∧ Cont M R Q ∧ Cont Q H C ∧
                    Cont C P N ∧ Cont N Q refusalRead ∧
                      Cont refusalRead C consumerRead := by
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist => hsame row consumerRead ∧ hsame H (append Z S))
          (fun row : BHist =>
            hsame row consumerRead ∧ Cont N Q refusalRead ∧ Cont refusalRead C consumerRead)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.left, refusalRoute, consumerRoute⟩
  }
  exact
    ⟨cert, unaryZ, unaryS, unaryM, unaryR, unaryQ, unaryC, unaryN, refusalUnary,
      consumerUnary, sameH, routeQ, routeC, routeN, refusalRoute, consumerRoute⟩
