...
theorem CriticalLineWitnessCarrier_rh_consumer_source_nonescape
    {Z S M R Q H C P N sourceRead lockedRead rhRead : BHist} :
        ...
        Cont sourceRead Q lockedRead ->
          Cont lockedRead N rhRead ->
            SemanticNameCert
                ...
                (fun row : BHist =>
                  hsame row sourceRead ∨ hsame row lockedRead ∨ hsame row rhRead)
                (fun row : BHist =>
                  UnaryHistory row ∧ Cont Z S sourceRead ∧
                    Cont sourceRead Q lockedRead ∧ Cont lockedRead N rhRead ∧
                      hsame H (append Z S))
                hsame ∧
              UnaryHistory sourceRead ∧ UnaryHistory lockedRead ∧ UnaryHistory rhRead := by
      ...
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont Z S sourceRead ∧ Cont sourceRead Q lockedRead ∧
              Cont lockedRead N rhRead ∧ hsame H (append Z S))
          hsame := {
    core := {
  ...
  exact ⟨cert, sourceUnary, lockedUnary, rhUnary⟩
