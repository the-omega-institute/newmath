...
theorem CriticalLineWitnessCarrier_root_rh_boundary_formal_target
        ...
        Cont stripRead Q modulusRead ->
          Cont N Q refusalRead ->
            Cont refusalRead C boundaryRead ->
              Cont boundaryRead P publicRead ->
                SemanticNameCert
                    ...
                    (fun row : BHist =>
                      UnaryHistory row ∧ Cont boundaryRead P publicRead ∧
                        hsame H (append Z S))
                    hsame ∧
  ...
  have publicUnary : UnaryHistory publicRead :=
    unary_cont_closed boundaryUnary unaryP publicRoute
  have sourceAtPublic :
  ...
  have cert :
          ...
          (fun row : BHist =>
            hsame row Z ∨ hsame row S ∨ hsame row M ∨ hsame row R ∨
              hsame row Q ∨ hsame row H ∨ hsame row C ∨ hsame row P ∨
                hsame row N ∨ hsame row publicRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont boundaryRead P publicRead ∧ hsame H (append Z S))
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, publicRoute, sameH⟩
  }
  exact ⟨cert, publicUnary, sameH⟩
