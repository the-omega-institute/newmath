...
theorem CriticalLineWitnessCarrier_supplied_zero_strip_transport_lock {Z S M R Q H C P N : BHist} :
    CriticalLineWitnessCarrier Z S M R Q H C P N ->
      UnaryHistory Z ∧ UnaryHistory S ∧ hsame H (append Z S) ∧ Cont Q H C ∧
        Cont C P N := by
  -- BEDC touchpoint anchor: BHist hsame Cont UnaryHistory
  intro packet
  obtain ⟨unaryZ, unaryS, _unaryM, _unaryR, _unaryP, sameH, _routeQ, routeC, routeN⟩ :=
    packet
  exact ⟨unaryZ, unaryS, sameH, routeC, routeN⟩

theorem CriticalLineWitnessCarrier_downstream_refusal_factorization
    ...
    CriticalLineWitnessCarrier Z S M R Q H C P N -> Cont N Q refusalRead ->
      Cont refusalRead C downstreamRead ->
        SemanticNameCert
            ...
            (fun row : BHist => hsame row downstreamRead ∧ Cont refusalRead C downstreamRead)
            hsame ∧
          UnaryHistory refusalRead ∧ UnaryHistory downstreamRead := by
  ...
  have downstreamUnary : UnaryHistory downstreamRead :=
    unary_cont_closed refusalUnary unaryC downstreamRoute
  ...
  exact ⟨cert, refusalUnary, downstreamUnary⟩
