...
theorem CriticalLineWitnessCarrier_root_refusal_consumer_exactness
    {Z S M R Q H C P N rootRead refusalRead consumerRead : BHist} :
    CriticalLineWitnessCarrier Z S M R Q H C P N ->
      Cont (append Z S) Q rootRead ->
        Cont rootRead N refusalRead ->
          Cont refusalRead C consumerRead ->
            SemanticNameCert
                (fun row : BHist => hsame row consumerRead ∧ UnaryHistory row)
                (fun row : BHist =>
                  hsame row consumerRead ∧ Cont (append Z S) Q rootRead)
                (fun row : BHist =>
              ...
              UnaryHistory Z ∧ UnaryHistory S ∧ UnaryHistory Q ∧ UnaryHistory N ∧
                UnaryHistory rootRead ∧ UnaryHistory refusalRead ∧
                  UnaryHistory consumerRead ∧ hsame H (append Z S) ∧ Cont M R Q ∧
                    Cont Q H C ∧ Cont C P N ∧ Cont (append Z S) Q rootRead ∧
                      Cont rootRead N refusalRead ∧
                        Cont refusalRead C consumerRead := by
  -- BEDC touchpoint anchor: BHist Cont hsame SemanticNameCert UnaryHistory
  intro packet rootRoute refusalRoute consumerRoute
  ...
  have refusalUnary : UnaryHistory refusalRead :=
    unary_cont_closed rootUnary unaryN refusalRoute
  ...
  exact
    ⟨cert, unaryZ, unaryS, unaryQ, unaryN, rootUnary, refusalUnary, consumerUnary,
      sameH, routeQ, routeC, routeN, rootRoute, refusalRoute, consumerRoute⟩
