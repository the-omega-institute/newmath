...
theorem CriticalLineWitnessRootVisibleModulusConsumerTotality
    {Z S M R Q H C P N zeroStripRead modulusRead consumerRead : BHist} :
  ...
  have consumerCert :
    ...
    core := {
      carrier_inhabited :=
        Exists.intro consumerRead ⟨hsame_refl consumerRead, unaryConsumerRead⟩
      equiv_refl := by
        intro row _source
  ...
  exact
    ⟨consumerCert, unaryZ, unaryS, unaryM, unaryR, unaryQ, unaryN, unaryZeroStripRead,
      unaryModulusRead, unaryConsumerRead, sameH, routeQ, routeC, routeN⟩
