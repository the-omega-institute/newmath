...
theorem CriticalLineWitnessCarrier_modulus_ledger_nonescape
    ...
    CriticalLineWitnessCarrier Z S M R Q H C P N ->
      Cont M Q ledgerRead ->
        SemanticNameCert
            ...
            (fun row : BHist =>
              hsame row M ∨ hsame row Q ∨ hsame row R ∨ hsame row H ∨ hsame row C ∨
                hsame row P ∨ hsame row N ∨ hsame row ledgerRead)
            (fun row : BHist => hsame row ledgerRead ∧ Cont M Q ledgerRead)
            hsame ∧
          UnaryHistory ledgerRead ∧ Cont M Q ledgerRead ∧ hsame H (append Z S) := by
  -- BEDC touchpoint anchor: BHist Cont hsame SemanticNameCert UnaryHistory
  ...
  exact ⟨cert, ledgerUnary, ledgerRoute, sameH⟩
