...
theorem CriticalLineWitnessCarrier_root_modulus_ledger
    {Z S M R Q H C P N modulusRead comparisonRead ledgerRead : BHist} :
    CriticalLineWitnessCarrier Z S M R Q H C P N ->
      Cont M R modulusRead ->
        Cont modulusRead Q comparisonRead ->
            ...
            SemanticNameCert
                ...
                (fun row : BHist =>
                  hsame row M ∨ hsame row R ∨ hsame row Q ∨ hsame row H ∨
                    hsame row C ∨ hsame row P ∨ hsame row N ∨ hsame row ledgerRead)
                (fun row : BHist =>
                  hsame row ledgerRead ∧ Cont comparisonRead H ledgerRead)
                hsame ∧
              UnaryHistory M ∧ UnaryHistory R ∧ UnaryHistory Q ∧ UnaryHistory H ∧
                UnaryHistory C ∧ UnaryHistory N ∧ UnaryHistory modulusRead ∧
                  UnaryHistory comparisonRead ∧ UnaryHistory ledgerRead ∧
                    hsame H (append Z S) ∧ Cont M R Q ∧ Cont M R modulusRead ∧
                      Cont modulusRead Q comparisonRead ∧
                        Cont comparisonRead H ledgerRead ∧ Cont Q H C ∧
                          Cont C P N := by
  -- BEDC touchpoint anchor: BHist Cont hsame SemanticNameCert UnaryHistory
  intro packet modulusRoute comparisonRoute ledgerRoute
  obtain ⟨unaryZ, unaryS, unaryM, unaryR, unaryP, sameH, routeQ, routeC, routeN⟩ :=
    packet
  have unaryQ : UnaryHistory Q :=
    unary_cont_closed unaryM unaryR routeQ
  ...
  have unaryComparisonRead : UnaryHistory comparisonRead :=
    unary_cont_closed unaryModulusRead unaryQ comparisonRoute
  have unaryLedgerRead : UnaryHistory ledgerRead :=
    unary_cont_closed unaryComparisonRead unaryH ledgerRoute
  have sourceAtLedger : hsame ledgerRead ledgerRead ∧ UnaryHistory ledgerRead :=
    ⟨hsame_refl ledgerRead, unaryLedgerRead⟩
  have cert :
      SemanticNameCert
  ...
  exact
    ⟨cert, unaryM, unaryR, unaryQ, unaryH, unaryC, unaryN, unaryModulusRead,
      unaryComparisonRead, unaryLedgerRead, sameH, routeQ, modulusRoute, comparisonRoute,
