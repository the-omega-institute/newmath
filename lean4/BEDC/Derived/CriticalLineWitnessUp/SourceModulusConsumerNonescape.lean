...
theorem CriticalLineWitnessCarrier_source_modulus_consumer_nonescape
            ...
            SemanticNameCert
                ...
                (fun row : BHist =>
                  UnaryHistory row ∧ Cont Z S sourceRead ∧
                    Cont sourceRead Q modulusRead ∧ Cont modulusRead N consumerRead ∧
                      hsame H (append Z S))
                hsame ∧
  ...
  have consumerUnary : UnaryHistory consumerRead :=
    unary_cont_closed modulusUnary unaryN consumerRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row consumerRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row sourceRead ∨ hsame row modulusRead ∨ hsame row consumerRead ∨
              hsame row N)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont Z S sourceRead ∧ Cont sourceRead Q modulusRead ∧
              Cont modulusRead N consumerRead ∧ hsame H (append Z S))
          hsame := {
    core := {
