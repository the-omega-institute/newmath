...
theorem field_square_nonzero_iff_factor_nonzero
    {add mul : BHist -> BHist -> BHist} {neg : BHist -> BHist} {one : BHist}
    {NonZero : BHist -> Prop} {inv : (a : BHist) -> NonZero a -> BHist}
    (addAssoc : forall x y z : BHist, hsame (add (add x y) z) (add x (add y z)))
    (zeroLeft : forall x : BHist, hsame (add BHist.Empty x) x)
    ...
    forall a : BHist, NonZero (mul a a) <-> NonZero a := by
  intro a
  constructor
  · intro squareNonzero
    exact apartToNonzero
      (field_product_nonzero_excludes_zero_factors addAssoc zeroLeft negLeft
        addCongr mulCongr leftDistrib rightDistrib nonzeroTransport
        nonzeroEmptyAbsurd (a := a) (b := a) squareNonzero).left
  · intro factorNonzero
    exact field_product_nonzero_of_nonzero_factors addAssoc zeroLeft negLeft assocC
      ...
      nonzeroTransport nonzeroEmptyAbsurd apartToNonzero factorNonzero factorNonzero
