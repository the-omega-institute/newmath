...
theorem field_affine_inverse_coefficients_derived {mul : BHist -> BHist -> BHist}
    {one : BHist} {NonZero : BHist -> Prop} {inv : (a : BHist) -> NonZero a -> BHist}
    ...
    (nonzeroOfApartEmpty : forall x : BHist, (hsame x BHist.Empty -> False) -> NonZero x)
    {a b : BHist} (pa : NonZero a) (pb : NonZero b) :
    ∃ pA : NonZero (inv a pa), ∃ pB : NonZero (inv b pb),
      hsame (inv (inv a pa) pA) a ∧ hsame (inv (inv b pb) pB) b := by
  let pA : NonZero (inv a pa) :=
    field_inverse_nonzero_from_one_apartness
  ...
  have sameB : hsame (inv (inv b pb) pB) b := by
    exact BEDC.Derived.GroupUp.group_left_right_inverse_uniqueness
      ...
      mulRightId
      mulCongr
      (mulLeftInv (inv b pb) pB)
      (mulLeftInv b pb)
  exact Exists.intro pA (Exists.intro pB (And.intro sameA sameB))
