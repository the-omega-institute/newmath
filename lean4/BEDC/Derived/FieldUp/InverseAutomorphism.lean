  ...
  exact Exists.intro pA
    (And.intro
      (BEDC.Derived.GroupUp.group_left_right_inverse_uniqueness
        assocC leftId rightId mulCongr (leftInv (inv a pa) pA) (leftInv a pa))
      (Iff.intro
        (fun sameInv =>
          field_inverse_cancel_from_apartness
            assocC leftId rightId mulCongr leftInv rightInv pa pb sameInv)
        (fun sameAB =>
          field_inverse_congruence_from_apartness
