...
theorem LowerRealUpperRealDualityBoundary [AskSetup] [PackageSetup]
    {L0 W R E H C P N lowerRead rationalRead realRead boundaryRead : BHist}
                            ...
                            SemanticNameCert
                                ...
                                (fun row : BHist =>
                                  hsame row L0 ∨ hsame row W ∨ hsame row R ∨
                                    hsame row E ∨ hsame row N ∨ hsame row lowerRead ∨
  ...
  have boundaryUnary : UnaryHistory boundaryRead :=
    unary_cont_closed realUnary nUnary boundaryRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row boundaryRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row L0 ∨ hsame row W ∨ hsame row R ∨ hsame row E ∨ hsame row N ∨
              hsame row lowerRead ∨ hsame row rationalRead ∨ hsame row realRead ∨
                hsame row boundaryRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont L0 W lowerRead ∧ Cont lowerRead R rationalRead ∧
              Cont rationalRead E realRead ∧ Cont realRead N boundaryRead ∧
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, lowerRoute, rationalRoute, realRoute, boundaryRoute, pPkg,
          boundaryPkg⟩
  }
  exact ⟨cert, lowerUnary, rationalUnary, realUnary, boundaryUnary⟩
