...
theorem AxisZeckendorfCannotClaimRegistryPacket_horizon_refusal_coverage
    [AskSetup] [PackageSetup] {a b c d e f g h p n horizon audit : BHist}
      ...
      (hsame horizon e ∨ hsame horizon f ∨ hsame horizon g) →
        Cont h p audit →
          PkgSig bundle audit pkg →
            UnaryHistory horizon ∧ Cont e f h ∧ hsame p n ∧ PkgSig bundle p pkg ∧
              PkgSig bundle audit pkg := by
  -- BEDC touchpoint anchor: BHist UnaryHistory hsame Cont ProbeBundle PkgSig
  intro packet horizonSource _auditRoute auditPkg
  ...
  exact ⟨horizonUnary, routeEF, sameProvenanceName, provenancePkg, auditPkg⟩
