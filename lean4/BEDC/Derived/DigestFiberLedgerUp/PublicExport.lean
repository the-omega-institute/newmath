...
theorem DigestFiberLedgerPublicExport
    {visible fiber gap refusal transport continuation provenance name publicRead : BHist} :
    Cont visible fiber publicRead →
                  ...
                  UnaryHistory provenance →
                    UnaryHistory name →
                      SemanticNameCert
                          ...
                          (fun row : BHist =>
                            UnaryHistory row ∧ Cont visible fiber publicRead)
                          hsame ∧
                        UnaryHistory publicRead ∧
                          digestFiberLedgerFields
                              (DigestFiberLedgerUp.mk visible fiber gap refusal transport
                                continuation provenance name) =
                            [visible, fiber, gap, refusal, transport, continuation,
                              provenance, name] := by
  -- BEDC touchpoint anchor: DigestFiberLedgerUp BHist Cont SemanticNameCert hsame UnaryHistory
  intro publicRoute visibleUnary fiberUnary _gapUnary _refusalUnary _transportUnary
    _continuationUnary _provenanceUnary _nameUnary
  have publicUnary : UnaryHistory publicRead :=
    unary_cont_closed visibleUnary fiberUnary publicRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row publicRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row visible ∨ hsame row fiber ∨ hsame row gap ∨ hsame row refusal ∨
              hsame row transport ∨ hsame row continuation ∨ hsame row provenance ∨
                hsame row name ∨ hsame row publicRead)
          (fun row : BHist => UnaryHistory row ∧ Cont visible fiber publicRead)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, publicRoute⟩
  }
  exact ⟨cert, publicUnary, rfl⟩
