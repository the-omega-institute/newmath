...
theorem LatticeDirected_commutativity_from_bounds
    ...
    (antisymm :
      forall {a b : BHist}, Carrier a -> Carrier b -> Le a b -> Le b a -> Classifier a b)
    (meet_carrier :
  ...
  have carrierJoinYX : Carrier (join y x) := join_carrier carrierY carrierX
  have joinXY_le_joinYX : Le (join x y) (join y x) :=
    join_least carrierJoinYX carrierX carrierY
      (join_upper_right carrierY carrierX)
      (join_upper_left carrierY carrierX)
  have joinYX_le_joinXY : Le (join y x) (join x y) :=
    join_least carrierJoinXY carrierY carrierX
      (join_upper_right carrierX carrierY)
      (join_upper_left carrierX carrierY)
  exact And.intro
    (antisymm carrierMeetXY carrierMeetYX meetXY_le_meetYX meetYX_le_meetXY)
    (antisymm carrierJoinXY carrierJoinYX joinXY_le_joinYX joinYX_le_joinXY)
