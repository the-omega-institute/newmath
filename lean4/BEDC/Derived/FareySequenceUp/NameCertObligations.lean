...
theorem FareySequenceCarrier_admission [AskSetup] [PackageSetup]
    {B A M L T S D Q W R G E H C P N routeRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
                  ...
                  hsame row T ∨ hsame row S ∨ hsame row D ∨ hsame row Q ∨
                    hsame row W ∨ hsame row R ∨ hsame row G ∨ hsame row E ∨
                      hsame row H ∨ hsame row C ∨ hsame row P ∨ hsame row N ∨
                        hsame row routeRead)
              (fun row : BHist =>
                UnaryHistory row ∧ Cont B A routeRead ∧ PkgSig bundle N pkg)
              hsame ∧
            UnaryHistory routeRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro carrier boundaryRoute packageN
  obtain ⟨boundaryUnary, adjacencyUnary, _mediantUnary, _levelUnary, _toleranceUnary,
    _sternBrocotUnary, _densityUnary, _rationalUnary, _windowUnary, _readbackUnary,
    _approximationUnary, _sealUnary, _transportUnary, _replayUnary, _provenanceUnary,
    _nameUnary, _adjacencyEmpty, _sternBrocotEmpty, _mediantEmpty, _approximationEmpty,
    _sealEmpty, _provenancePkg⟩ := carrier
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, boundaryRoute, packageN⟩
  }
