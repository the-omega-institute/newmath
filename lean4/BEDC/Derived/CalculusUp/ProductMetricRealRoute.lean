...
theorem CalculusProductMetricRealRoute [AskSetup] [PackageSetup]
    {cauchyProduct metric stream regSeq dyadic real productRead streamRead regularRead
      dyadicRead modulusRead terminalRead realRouteRead provenance : BHist}
                          ...
                          Cont modulusRead real terminalRead ->
                            Cont terminalRead real realRouteRead ->
                              PkgSig bundle provenance pkg ->
                                SemanticNameCert
                                    (fun row : BHist =>
                                      hsame row realRouteRead ∧ UnaryHistory row)
                                    (fun row : BHist =>
  ...
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row realRouteRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row cauchyProduct ∨ hsame row metric ∨ hsame row stream ∨
              hsame row regSeq ∨ hsame row dyadic ∨ hsame row modulusRead ∨
                hsame row terminalRead ∨ hsame row realRouteRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont regularRead dyadic dyadicRead ∧
              Cont dyadicRead metric modulusRead ∧ Cont modulusRead real terminalRead ∧
                Cont terminalRead real realRouteRead ∧ PkgSig bundle provenance pkg)
          hsame := {
    core := {
      carrier_inhabited :=
        Exists.intro realRouteRead ⟨hsame_refl realRouteRead, realRouteReadUnary⟩
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, dyadicRoute, modulusRoute, terminalRoute, realRoute, provenancePkg⟩
  }
  exact
    ⟨cert, productReadUnary, streamReadUnary, regularReadUnary, dyadicReadUnary,
      modulusReadUnary, terminalReadUnary, realRouteReadUnary⟩
