...
theorem CalculusRootChainRuleContinuousMapRegSeqRatScope [AskSetup] [PackageSetup]
    {C D Q Y R H T P N outerGraph innerGraph derivativeOuter derivativeInner rationalRead
      dyadicRead realRead replayRead : BHist}
                  ...
                  Cont outerGraph C innerGraph ->
                    Cont innerGraph D derivativeOuter ->
                      Cont derivativeOuter D derivativeInner ->
                        Cont derivativeInner Q rationalRead ->
  ...
  intro unaryC unaryD unaryQ unaryY unaryR unaryT outerRoute innerRoute
    derivativeOuterRoute derivativeInnerRoute rationalRoute dyadicRoute realRoute replayRoute
  ...
  have rationalUnary : UnaryHistory rationalRead :=
    unary_cont_closed derivativeInnerUnary unaryQ rationalRoute
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont C C outerGraph ∧ Cont outerGraph C innerGraph ∧
              Cont innerGraph D derivativeOuter ∧ Cont derivativeOuter D derivativeInner ∧
                Cont derivativeInner Q rationalRead ∧ Cont rationalRead Y dyadicRead ∧
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, outerRoute, innerRoute, derivativeOuterRoute, derivativeInnerRoute,
          rationalRoute, dyadicRoute, realRoute, replayRoute, transportSame, provenancePkg,
          namePkg⟩
  }
