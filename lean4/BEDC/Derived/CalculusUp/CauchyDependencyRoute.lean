...
theorem CalculusCauchyDependencyRoute [AskSetup] [PackageSetup]
    {real limit continuous derivative integral readback transport replay provenance localName
      derivativeRead integralRead cauchyWindow realSeal limitConsumer : BHist}
                        ...
                        UnaryHistory row ∧ Cont derivativeRead integralRead cauchyWindow ∧
                          Cont cauchyWindow real realSeal ∧
                            Cont realSeal limit limitConsumer ∧
                              PkgSig bundle provenance pkg)
                      hsame ∧
                    UnaryHistory cauchyWindow ∧ UnaryHistory realSeal ∧
                      UnaryHistory limitConsumer := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro carrier derivativeRoute integralRoute cauchyRoute realSealRoute limitConsumerRoute
    provenancePkg
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont derivativeRead integralRead cauchyWindow ∧
              Cont cauchyWindow real realSeal ∧ Cont realSeal limit limitConsumer ∧
                PkgSig bundle provenance pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro limitConsumer sourceAtLimit
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, cauchyRoute, realSealRoute, limitConsumerRoute, provenancePkg⟩
  }
  exact ⟨cert, cauchyWindowUnary, realSealUnary, limitConsumerUnary⟩
