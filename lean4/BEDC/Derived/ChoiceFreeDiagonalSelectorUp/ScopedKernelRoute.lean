...
theorem ChoiceFreeDiagonalSelectorCarrier_scoped_kernel_route [AskSetup] [PackageSetup]
    {epsilon window stream readback realSeal transport replay provenance localName windowRead
      witnessRead sealRead ledgerRead namedRead : BHist}
  ...
  intro carrier epsilonWindow windowStream witnessReadback sealReal ledgerReplay namedPkg
  obtain ⟨epsilonUnary, windowUnary, streamUnary, readbackUnary, realSealUnary,
    _transportUnary, replayUnary, _provenanceUnary, _localNameUnary, _storedWindowRoute,
  ...
  have namedReadUnary : UnaryHistory namedRead :=
    unary_cont_closed ledgerReadUnary replayUnary ledgerReplay
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row namedRead ∧ UnaryHistory row)
          (fun row : BHist =>
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont epsilon window windowRead ∧
              Cont windowRead stream witnessRead ∧ Cont witnessRead readback sealRead ∧
                Cont sealRead realSeal ledgerRead ∧ Cont ledgerRead replay namedRead ∧
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, epsilonWindow, windowStream, witnessReadback, sealReal,
          ledgerReplay, provenancePkg, namedPkg⟩
  }
  exact
