...
namespace BEDC.Derived.ChoiceFreeDiagonalSelectorUp.PublicHandoff
...
theorem ChoiceFreeDiagonalSelectorPublicHandoff [AskSetup] [PackageSetup]
    {epsilon window stream readback realSeal transport replay provenance localName windowRead
      witnessRead sealRead ledgerRead consumerRead publicRead : BHist}
          ...
          Cont witnessRead readback sealRead ->
            Cont sealRead realSeal ledgerRead ->
              Cont ledgerRead localName consumerRead ->
                Cont consumerRead provenance publicRead ->
                          ...
                          (fun row : BHist =>
                            hsame row epsilon ∨ hsame row window ∨ hsame row stream ∨
                              hsame row readback ∨ hsame row realSeal ∨
                                hsame row windowRead ∨ hsame row witnessRead ∨
  ...
  intro carrier epsilonWindow windowStream witnessReadback sealReal ledgerLocal consumerPublic
  ...
  have publicReadUnary : UnaryHistory publicRead :=
    unary_cont_closed consumerReadUnary provenanceUnary consumerPublic
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row publicRead ∧ UnaryHistory row)
          (fun row : BHist =>
            ...
            UnaryHistory row ∧ Cont epsilon window windowRead ∧
              Cont windowRead stream witnessRead ∧ Cont witnessRead readback sealRead ∧
                Cont sealRead realSeal ledgerRead ∧ Cont ledgerRead localName consumerRead ∧
                  Cont consumerRead provenance publicRead ∧ PkgSig bundle provenance pkg ∧
                    PkgSig bundle publicRead pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, epsilonWindow, windowStream, witnessReadback, sealReal,
          ledgerLocal, consumerPublic, provenancePkg, publicPkg⟩
  }
  exact
    ⟨cert, windowReadUnary, witnessReadUnary, sealReadUnary, ledgerReadUnary,
      consumerReadUnary, publicReadUnary⟩

end BEDC.Derived.ChoiceFreeDiagonalSelectorUp.PublicHandoff
