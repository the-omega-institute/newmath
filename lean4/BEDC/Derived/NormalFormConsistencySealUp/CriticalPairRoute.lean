...
theorem NormalFormConsistencySealObligationClosureRoute [AskSetup] [PackageSetup]
    {T F N K X H C P L candidateRead residualRead joinRead scheduleRead
      closureRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    normalFormConsistencySealFields (NormalFormConsistencySealUp.mk T F N K X H C P L) =
        [T, F, N, K, X, H, C, P, L] ->
      UnaryHistory K ->
        UnaryHistory X ->
          UnaryHistory H ->
            UnaryHistory C ->
              UnaryHistory L ->
                Cont K X candidateRead ->
                  Cont candidateRead C residualRead ->
                    Cont residualRead H joinRead ->
                      Cont joinRead C scheduleRead ->
                        Cont scheduleRead L closureRead ->
                          PkgSig bundle P pkg ->
                            PkgSig bundle L pkg ->
                              SemanticNameCert
                                  (fun row : BHist =>
                                  ...
                                  (fun row : BHist =>
                                    UnaryHistory row ∧ Cont K X candidateRead ∧
                                      Cont candidateRead C residualRead ∧
                                        Cont residualRead H joinRead ∧
                                          Cont joinRead C scheduleRead ∧
                                            Cont scheduleRead L closureRead ∧
  ...
  intro fieldsExact unaryK unaryX unaryH unaryC unaryL candidateRoute residualRoute
    joinRoute scheduleRoute closureRoute provenancePkg localNamePkg
  cases fieldsExact
  have candidateUnary : UnaryHistory candidateRead :=
    unary_cont_closed unaryK unaryX candidateRoute
  have residualUnary : UnaryHistory residualRead :=
    unary_cont_closed candidateUnary unaryC residualRoute
  have joinUnary : UnaryHistory joinRead :=
    unary_cont_closed residualUnary unaryH joinRoute
  have scheduleUnary : UnaryHistory scheduleRead :=
    unary_cont_closed joinUnary unaryC scheduleRoute
  have closureUnary : UnaryHistory closureRead :=
    unary_cont_closed scheduleUnary unaryL closureRoute
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont K X candidateRead ∧
              Cont candidateRead C residualRead ∧ Cont residualRead H joinRead ∧
                Cont joinRead C scheduleRead ∧ Cont scheduleRead L closureRead ∧
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, candidateRoute, residualRoute, joinRoute, scheduleRoute,
