...
theorem commring_signed_square_empty_zero_exact {add mul : BHist -> BHist -> BHist}
    {neg : BHist -> BHist}
    (addAssoc : forall x y z : BHist, hsame (add (add x y) z) (add x (add y z)))
    (addComm : forall x y : BHist, hsame (add x y) (add y x))
    (zeroLeft : forall x : BHist, hsame (add BHist.Empty x) x)
  ...
  intro a b
  have squareExact :=
    commring_signed_product_empty_zero_exact addAssoc addComm zeroLeft negLeft
      addCongr mulComm mulCongr leftDistrib (a := a) (b := b) (c := a) (d := b)
  have crossToDouble :
      hsame (add (mul a b) (mul b a)) (add (mul a b) (mul a b)) := by
    exact addCongr (hsame_refl (mul a b)) (mulComm b a)
  constructor
  · intro squareEmpty
    exact hsame_trans (squareExact.mp squareEmpty) crossToDouble
  · intro doubleCross
    exact squareExact.mpr (hsame_trans doubleCross (hsame_symm crossToDouble))
