...
theorem CoveringDimensionFiniteCoverRootMetricCompletionRoute [AskSetup] [PackageSetup]
    {compactMetric epsilonNet cover refinement orderBound lebesgue transport replay provenance
      localName metricRead completionRead namedRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    CoveringDimensionCarrier compactMetric epsilonNet cover refinement orderBound lebesgue
        transport replay provenance localName bundle pkg →
      Cont cover refinement metricRead →
        Cont metricRead orderBound completionRead →
          Cont completionRead localName namedRead →
  ...
  intro carrier metricRoute completionRoute namedRoute namedPkg
  ...
  have metricUnary : UnaryHistory metricRead :=
    unary_cont_closed coverUnary refinementUnary metricRoute
  have completionUnary : UnaryHistory completionRead :=
    unary_cont_closed metricUnary orderUnary completionRoute
  have namedUnary : UnaryHistory namedRead :=
    unary_cont_closed completionUnary localNameUnary namedRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row namedRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row compactMetric ∨ hsame row epsilonNet ∨ hsame row cover ∨
              hsame row refinement ∨ hsame row orderBound ∨ hsame row lebesgue ∨
                hsame row metricRead ∨ hsame row completionRead ∨ hsame row namedRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont cover refinement metricRead ∧
              Cont metricRead orderBound completionRead ∧
                Cont completionRead localName namedRead ∧
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, metricRoute, completionRoute, namedRoute, provenancePkg, namedPkg⟩
  }
  exact ⟨cert, metricUnary, completionUnary, namedUnary⟩
