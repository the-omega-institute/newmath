...
theorem CoveringDimensionFiniteRefinementWindow [AskSetup] [PackageSetup]
            ...
            SemanticNameCert
                ...
                (fun row : BHist =>
                  UnaryHistory row ∧ Cont cover refinement refinementRead ∧
                    Cont refinementRead orderBound orderRead ∧ PkgSig bundle orderRead pkg)
                hsame ∧
  ...
  intro carrier coverRefinementRead refinementOrderRead orderReadPkg
  ...
  have orderReadUnary : UnaryHistory orderRead :=
    unary_cont_closed refinementReadUnary orderUnary refinementOrderRead
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row orderRead ∧ UnaryHistory row)
          (fun row : BHist =>
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, coverRefinementRead, refinementOrderRead, orderReadPkg⟩
  }
