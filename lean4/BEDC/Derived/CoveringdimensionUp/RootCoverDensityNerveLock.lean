...
theorem CoveringDimensionRootCoverDensityNerveLock [AskSetup] [PackageSetup]
          ...
          Cont densityRead localName nerveRead →
            PkgSig bundle nerveRead pkg →
              SemanticNameCert
                  ...
                  (fun row : BHist =>
                    hsame row cover ∨ hsame row orderBound ∨ hsame row rootRead ∨
                      hsame row densityRead ∨ hsame row nerveRead)
                  (fun row : BHist =>
                    UnaryHistory row ∧ Cont cover orderBound rootRead ∧
                      Cont rootRead provenance densityRead ∧
                        Cont densityRead localName nerveRead ∧ PkgSig bundle nerveRead pkg)
                  hsame ∧
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, coverOrderRoot, rootDensity, densityNerve, nervePkg⟩
  }
