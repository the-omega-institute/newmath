...
theorem RealNameClassifierScopeRoute [AskSetup] [PackageSetup]
    {source stream rat dyadic tolerance refinement sealRow transport replay provenance
      localName scopedRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    BEDC.Derived.RealNameClassifierUp source stream rat dyadic tolerance refinement sealRow
      transport replay provenance localName bundle pkg ->
      Cont source stream scopedRead ->
        PkgSig bundle scopedRead pkg ->
          SemanticNameCert
              ...
              (fun row : BHist =>
                UnaryHistory row ∧ Cont source stream scopedRead ∧
  ...
  intro carrier scopeRoute scopedPkg
  obtain ⟨sourceUnary, streamUnary, _ratUnary, _dyadicUnary, _toleranceUnary,
    _refinementUnary, _sealUnary, _transportUnary, _replayUnary, _provenanceUnary,
  ...
  have scopedUnary : UnaryHistory scopedRead :=
    unary_cont_closed sourceUnary streamUnary scopeRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row scopedRead ∧ UnaryHistory row)
          (fun row : BHist =>
    ...
    ledger_sound := by
      intro _row sourceData
      exact ⟨sourceData.right, scopeRoute, provenancePkg, localNamePkg, scopedPkg⟩
  }
