...
theorem MetaCICCriticalPathL10BoundaryConsumer [AskSetup] [PackageSetup]
    {strongNorm normalForm obstruction handoff dischargeSocket transport route provenance
      ...
      Cont route localName dyadicBudget ->
        Cont dyadicBudget route streamSchedule ->
          Cont streamSchedule normalForm regReadback ->
            Cont regReadback provenance realSeal ->
              Cont realSeal localName statusRead ->
                Cont statusRead obstruction boundaryConsumer ->
                  PkgSig bundle statusRead pkg ->
                    PkgSig bundle boundaryConsumer pkg ->
                      SemanticNameCert
                          (fun row : BHist => hsame row boundaryConsumer ∧ UnaryHistory row)
                          (fun row : BHist =>
                            hsame row obstruction ∨ hsame row dischargeSocket ∨
                              hsame row dyadicBudget ∨ hsame row streamSchedule ∨
                                hsame row regReadback ∨ hsame row realSeal ∨
                                  hsame row boundaryConsumer)
                          (fun row : BHist =>
                            UnaryHistory row ∧ Cont statusRead obstruction boundaryConsumer ∧
                              PkgSig bundle boundaryConsumer pkg)
                          hsame ∧
                        UnaryHistory boundaryConsumer ∧ PkgSig bundle provenance pkg := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro packet routeLocalNameBudget budgetRouteSchedule scheduleNormalFormReadback
    readbackProvenanceSeal sealLocalNameStatus statusObstructionBoundary _statusPkg
    boundaryPkg
  ...
  have boundaryConsumerUnary : UnaryHistory boundaryConsumer :=
    unary_cont_closed statusReadUnary obstructionUnary statusObstructionBoundary
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row boundaryConsumer ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row obstruction ∨ hsame row dischargeSocket ∨ hsame row dyadicBudget ∨
              hsame row streamSchedule ∨ hsame row regReadback ∨ hsame row realSeal ∨
                hsame row boundaryConsumer)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont statusRead obstruction boundaryConsumer ∧
              PkgSig bundle boundaryConsumer pkg)
    ...
    core := {
      carrier_inhabited :=
        Exists.intro boundaryConsumer ⟨hsame_refl boundaryConsumer, boundaryConsumerUnary⟩
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, statusObstructionBoundary, boundaryPkg⟩
  }
  exact ⟨cert, boundaryConsumerUnary, provenancePkg⟩
