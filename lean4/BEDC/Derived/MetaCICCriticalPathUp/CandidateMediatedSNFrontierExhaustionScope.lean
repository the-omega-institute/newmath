...
theorem MetaCICCriticalPathCandidateMediatedSNFrontierExhaustionScope [AskSetup]
    [PackageSetup]
    {strongNorm normalForm obstruction unblock discharge handoff continuation provenance
      localName dyadic stream regseq realSeal frontier residualSocket confluenceBudget
      decidabilityRead publicRead scopeRead : BHist}
          ...
          Cont residualSocket handoff confluenceBudget →
            Cont confluenceBudget realSeal decidabilityRead →
              Cont decidabilityRead localName publicRead →
                Cont publicRead provenance scopeRead →
                    ...
                    SemanticNameCert
                      ...
                      UnaryHistory frontier ∧ UnaryHistory residualSocket ∧
                        UnaryHistory confluenceBudget ∧ UnaryHistory decidabilityRead ∧
                          UnaryHistory publicRead ∧ UnaryHistory scopeRead := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro ledger continuationLocalFrontier frontierDischargeResidual residualHandoffConfluence
    confluenceRealSealDecidability decidabilityLocalPublic publicProvenanceScope scopePkg
  ...
  have scopeUnary : UnaryHistory scopeRead :=
    unary_cont_closed publicUnary provenanceUnary publicProvenanceScope
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row scopeRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row dyadic ∨ hsame row stream ∨ hsame row regseq ∨
              hsame row realSeal ∨ hsame row frontier ∨ hsame row residualSocket ∨
                hsame row confluenceBudget ∨ hsame row decidabilityRead ∨
                  hsame row publicRead ∨ hsame row scopeRead)
          (fun row : BHist =>
            UnaryHistory row ∧ PkgSig bundle scopeRead pkg ∧
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, scopePkg, realSealPkg, frontierDischargeResidual⟩
  }
  exact
    ⟨cert, frontierUnary, residualUnary, confluenceUnary, decidabilityUnary,
      publicUnary, scopeUnary⟩
