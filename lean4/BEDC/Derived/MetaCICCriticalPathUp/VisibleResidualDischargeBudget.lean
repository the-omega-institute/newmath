...
theorem MetaCICCriticalPathVisibleResidualDischargeBudget [AskSetup] [PackageSetup]
          ...
          Cont candidateResidual obstruction budgetRead →
            Cont budgetRead normalForm diamondRead →
              Cont diamondRead transport checkerRead →
                Cont checkerRead dischargeSocket dischargeRead →
                        ...
                        (fun row : BHist =>
                          hsame row candidateResidual ∨ hsame row budgetRead ∨
                            hsame row diamondRead ∨ hsame row checkerRead ∨
                              hsame row dischargeRead ∨ hsame row dischargeSocket ∨
  ...
  intro packet routeLocalSource sourceSocketCandidate candidateObstructionBudget
    budgetNormalDiamond diamondTransportChecker checkerSocketDischarge dischargePkg
  ...
  have dischargeUnary : UnaryHistory dischargeRead :=
    unary_cont_closed checkerUnary dischargeSocketUnary checkerSocketDischarge
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row dischargeRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row candidateResidual ∨ hsame row budgetRead ∨
              hsame row diamondRead ∨ hsame row checkerRead ∨ hsame row dischargeRead ∨
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, dischargePkg, provenancePkg⟩
  }
  exact
    ⟨cert, candidateUnary, budgetUnary, diamondUnary, checkerUnary, dischargeUnary⟩
