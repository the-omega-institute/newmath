...
theorem MetaCICCriticalPathResidualWitnessConsumption [AskSetup] [PackageSetup]
    {strongNorm normalForm obstruction unblock discharge handoff continuation provenance
      localName dyadic stream regseq realSeal diamondRead budgetRead endpointRead frontierRead
      residualRead witnessRead consumerRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    MetaCICCriticalPathOpenPhaseSourceLedger strongNorm normalForm obstruction unblock
        discharge handoff continuation provenance localName dyadic stream regseq realSeal
        bundle pkg →
            ...
            Cont endpointRead handoff frontierRead →
              Cont frontierRead discharge residualRead →
                Cont residualRead obstruction witnessRead →
                  Cont witnessRead provenance consumerRead →
                    PkgSig bundle consumerRead pkg →
                      SemanticNameCert
                          (fun row : BHist => hsame row consumerRead ∧ UnaryHistory row)
                          (fun row : BHist =>
                            hsame row residualRead ∨ hsame row witnessRead ∨
                              hsame row frontierRead ∨ hsame row consumerRead ∨
                                hsame row dyadic ∨ hsame row stream ∨ hsame row regseq ∨
                                  hsame row realSeal)
                          (fun row : BHist =>
                            UnaryHistory row ∧ Cont residualRead obstruction witnessRead ∧
                              PkgSig bundle consumerRead pkg ∧ PkgSig bundle realSeal pkg)
                          hsame ∧
                        UnaryHistory witnessRead ∧ UnaryHistory consumerRead := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro ledger continuationLocalDiamond diamondObstructionBudget budgetRealEndpoint
    endpointHandoffFrontier frontierDischargeResidual residualObstructionWitness
    witnessProvenanceConsumer consumerPkg
  ...
  have consumerUnary : UnaryHistory consumerRead :=
    unary_cont_closed witnessUnary provenanceUnary witnessProvenanceConsumer
  have sourceConsumer :
      (fun row : BHist => hsame row consumerRead ∧ UnaryHistory row) consumerRead := by
    exact ⟨hsame_refl consumerRead, consumerUnary⟩
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row consumerRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row residualRead ∨ hsame row witnessRead ∨ hsame row frontierRead ∨
              hsame row consumerRead ∨ hsame row dyadic ∨ hsame row stream ∨
                hsame row regseq ∨ hsame row realSeal)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont residualRead obstruction witnessRead ∧
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, residualObstructionWitness, consumerPkg, realSealPkg⟩
  }
  exact ⟨cert, witnessUnary, consumerUnary⟩
