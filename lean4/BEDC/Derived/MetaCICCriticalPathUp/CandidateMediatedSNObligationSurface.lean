...
theorem MetaCICConfluenceCandidateMediatedSNObligationSurface [AskSetup] [PackageSetup]
    {strongNorm normalForm obstruction unblock discharge handoff continuation provenance
      localName dyadic stream regseq realSeal obligationRead confluenceRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    MetaCICCriticalPathOpenPhaseSourceLedger strongNorm normalForm obstruction unblock
        discharge handoff continuation provenance localName dyadic stream regseq realSeal
        bundle pkg →
      Cont continuation realSeal obligationRead →
        Cont obligationRead handoff confluenceRead →
            ...
            SemanticNameCert
                ...
                (fun row : BHist =>
                  UnaryHistory row ∧ PkgSig bundle confluenceRead pkg ∧
                    PkgSig bundle realSeal pkg)
                hsame ∧
              UnaryHistory obligationRead ∧ UnaryHistory confluenceRead ∧
                PkgSig bundle realSeal pkg := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro ledger continuationRealSealObligation obligationHandoffConfluence confluencePkg
  ...
  obtain ⟨_strongNormUnary, _normalFormUnary, _obstructionUnary, _unblockUnary,
    ...
    _provenancePkg⟩ := packet
  have obligationUnary : UnaryHistory obligationRead :=
    unary_cont_closed continuationUnary realSealUnary continuationRealSealObligation
  have confluenceUnary : UnaryHistory confluenceRead :=
    unary_cont_closed obligationUnary handoffUnary obligationHandoffConfluence
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, confluencePkg, realSealPkg⟩
  }
  exact ⟨cert, obligationUnary, confluenceUnary, realSealPkg⟩
