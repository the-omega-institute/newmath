...
theorem MetaCICCriticalPathCandidateMediatedSNClosedNormalFormReadback
    [AskSetup] [PackageSetup]
    {strongNorm normalForm obstruction unblock discharge handoff continuation provenance
      localName dyadic stream regseq realSeal frontier closedRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    MetaCICCriticalPathOpenPhaseSourceLedger strongNorm normalForm obstruction unblock
        discharge handoff continuation provenance localName dyadic stream regseq realSeal
        bundle pkg →
      Cont continuation localName frontier →
        Cont frontier normalForm closedRead →
          PkgSig bundle closedRead pkg →
            SemanticNameCert
                (fun row : BHist => hsame row closedRead ∧ UnaryHistory row)
                (fun row : BHist =>
                  hsame row strongNorm ∨ hsame row normalForm ∨ hsame row frontier ∨
                    hsame row closedRead)
                (fun row : BHist =>
                  UnaryHistory row ∧ PkgSig bundle closedRead pkg ∧
                    Cont frontier normalForm closedRead)
                hsame ∧
              UnaryHistory frontier ∧ UnaryHistory closedRead := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro ledger continuationLocalFrontier frontierNormalFormRead closedReadPkg
  obtain ⟨packet, _dyadicUnary, _streamUnary, _regseqUnary, _realSealUnary,
    _dyadicStreamRegseq, _regseqRealSealHandoff, _realSealPkg⟩ := ledger
  ...
  have closedReadUnary : UnaryHistory closedRead :=
    unary_cont_closed frontierUnary normalFormUnary frontierNormalFormRead
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row closedRead ∧ UnaryHistory row)
          (fun row : BHist =>
    ...
    core := {
      carrier_inhabited :=
        Exists.intro closedRead ⟨hsame_refl closedRead, closedReadUnary⟩
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, closedReadPkg, frontierNormalFormRead⟩
  }
  exact ⟨cert, frontierUnary, closedReadUnary⟩

theorem MetaCICCriticalPathCandidateMediatedSNNormalEndpointBudget [AskSetup]
    [PackageSetup]
    {strongNorm normalForm obstruction unblock discharge handoff continuation provenance
      localName dyadic stream regseq realSeal candidateRead endpointRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    MetaCICCriticalPathOpenPhaseSourceLedger strongNorm normalForm obstruction unblock
        discharge handoff continuation provenance localName dyadic stream regseq realSeal
        bundle pkg →
      Cont continuation normalForm candidateRead →
        Cont candidateRead realSeal endpointRead →
          PkgSig bundle endpointRead pkg →
            SemanticNameCert
                (fun row : BHist => hsame row endpointRead ∧ UnaryHistory row)
                (fun row : BHist =>
                  hsame row strongNorm ∨ hsame row normalForm ∨ hsame row candidateRead ∨
                    hsame row endpointRead ∨ hsame row realSeal)
                (fun row : BHist =>
                  UnaryHistory row ∧ Cont continuation normalForm candidateRead ∧
                    Cont candidateRead realSeal endpointRead ∧ PkgSig bundle endpointRead pkg)
                hsame ∧
              UnaryHistory candidateRead ∧ UnaryHistory endpointRead := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro ledger continuationNormalCandidate candidateRealEndpoint endpointPkg
  ...
  have candidateUnary : UnaryHistory candidateRead :=
    unary_cont_closed continuationUnary normalFormUnary continuationNormalCandidate
  have endpointUnary : UnaryHistory endpointRead :=
    unary_cont_closed candidateUnary realSealUnary candidateRealEndpoint
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row endpointRead ∧ UnaryHistory row)
          (fun row : BHist =>
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, continuationNormalCandidate, candidateRealEndpoint, endpointPkg⟩
  }
  exact ⟨cert, candidateUnary, endpointUnary⟩
