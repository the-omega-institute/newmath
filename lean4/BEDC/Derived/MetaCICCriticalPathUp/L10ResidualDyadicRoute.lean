...
theorem MetaCICCriticalPathL10ResidualDyadicRoute [AskSetup] [PackageSetup]
    {strongNorm normalForm obstruction handoff dischargeSocket transport route provenance
      localName dyadicBudget streamSchedule regReadback realSeal residualRead : BHist}
  ...
  intro packet routeLocalNameBudget budgetRouteSchedule scheduleNormalFormReadback
    readbackProvenanceSeal sealObstructionResidual residualReadPkg
  ...
  have residualReadUnary : UnaryHistory residualRead :=
    unary_cont_closed realSealUnary obstructionUnary sealObstructionResidual
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row residualRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row dyadicBudget ∨ hsame row streamSchedule ∨
              hsame row regReadback ∨ hsame row realSeal ∨ hsame row residualRead)
          (fun row : BHist =>
            UnaryHistory row ∧ PkgSig bundle residualRead pkg ∧
              PkgSig bundle provenance pkg)
          hsame := {
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, residualReadPkg, provenancePkg⟩
  }
