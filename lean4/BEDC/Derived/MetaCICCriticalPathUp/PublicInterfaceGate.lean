...
theorem MetaCICCriticalPathBridgeRoute [AskSetup] [PackageSetup]
    {strongNorm normalForm obstruction handoff dischargeSocket transport route provenance
      localName l10Read candidateFrontier residualSocket confluenceBudget bridgeRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    MetaCICCriticalPathPacket strongNorm normalForm obstruction handoff dischargeSocket transport
        route provenance localName bundle pkg →
      Cont route provenance l10Read →
        Cont l10Read handoff candidateFrontier →
          Cont candidateFrontier dischargeSocket residualSocket →
            Cont residualSocket obstruction confluenceBudget →
              Cont confluenceBudget localName bridgeRead →
                      ...
                      (fun row : BHist =>
                        UnaryHistory row ∧ PkgSig bundle bridgeRead pkg ∧
                          PkgSig bundle provenance pkg ∧
                            Cont candidateFrontier dischargeSocket residualSocket)
                      hsame ∧
                    UnaryHistory l10Read ∧ UnaryHistory candidateFrontier ∧
                      UnaryHistory residualSocket ∧ UnaryHistory confluenceBudget ∧
                        UnaryHistory bridgeRead := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro packet routeProvenance l10Handoff candidateDischarge residualObstruction
    confluenceLocalName bridgePkg
  obtain ⟨_strongNormUnary, _normalFormUnary, obstructionUnary, handoffUnary,
    dischargeSocketUnary, _transportUnary, routeUnary, provenanceUnary, localNameUnary,
    _strongNormNormalFormRoute, _handoffObstructionSocket, _transportLocalName,
    provenancePkg⟩ := packet
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row l10Read ∨ hsame row candidateFrontier ∨ hsame row residualSocket ∨
              hsame row confluenceBudget ∨ hsame row bridgeRead)
          (fun row : BHist =>
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, bridgePkg, provenancePkg, candidateDischarge⟩
  }
  exact ⟨cert, l10Unary, candidateUnary, residualUnary, confluenceUnary, bridgeUnary⟩
