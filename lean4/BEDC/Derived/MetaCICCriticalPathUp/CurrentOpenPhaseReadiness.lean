...
theorem MetaCICCriticalPathCurrentOpenPhaseReadiness [AskSetup] [PackageSetup]
      ...
      Cont realSeal provenance completionFrontier ->
        PkgSig bundle completionFrontier pkg ->
          SemanticNameCert
              ...
              (fun row : BHist =>
                PkgSig bundle realSeal pkg ∧ PkgSig bundle completionFrontier pkg ∧
                  (hsame row dyadic ∨ hsame row stream ∨ hsame row regseq ∨
                    hsame row realSeal ∨ hsame row completionFrontier))
              hsame ∧
            UnaryHistory completionFrontier := by
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨realSealPkg, completionFrontierPkg,
          Or.inr (Or.inr (Or.inr (Or.inr source.left)))⟩
  }
  exact ⟨cert, completionFrontierUnary⟩
