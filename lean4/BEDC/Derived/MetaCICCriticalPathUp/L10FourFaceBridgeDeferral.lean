...
theorem MetaCICCriticalPathL10FourFaceBridgeDeferral [AskSetup] [PackageSetup]
    {strongNorm normalForm obstruction handoff dischargeSocket transport route provenance
      localName dyadicBudget streamSchedule regReadback realSeal bridgeRead : BHist}
        ...
        Cont dyadicBudget route streamSchedule →
          Cont streamSchedule normalForm regReadback →
            Cont regReadback provenance realSeal →
              Cont realSeal localName bridgeRead →
                PkgSig bundle bridgeRead pkg →
                  SemanticNameCert
                      (fun row : BHist => hsame row bridgeRead ∧ UnaryHistory row)
                      (fun row : BHist =>
                        hsame row dyadicBudget ∨ hsame row streamSchedule ∨
                          hsame row regReadback ∨ hsame row realSeal ∨ hsame row bridgeRead)
                      (fun row : BHist =>
  ...
  intro packet routeLocalNameBudget budgetRouteSchedule scheduleNormalReadback
    readbackProvenanceSeal sealLocalNameBridge bridgeReadPkg
  ...
  have bridgeReadUnary : UnaryHistory bridgeRead :=
    unary_cont_closed realSealUnary localNameUnary sealLocalNameBridge
  have sourceBridgeRead :
      (fun row : BHist => hsame row bridgeRead ∧ UnaryHistory row) bridgeRead := by
    exact ⟨hsame_refl bridgeRead, bridgeReadUnary⟩
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row bridgeRead ∧ UnaryHistory row)
          (fun row : BHist =>
          ...
          hsame := {
    core := {
      carrier_inhabited := Exists.intro bridgeRead sourceBridgeRead
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, bridgeReadPkg, sealLocalNameBridge⟩
  }
