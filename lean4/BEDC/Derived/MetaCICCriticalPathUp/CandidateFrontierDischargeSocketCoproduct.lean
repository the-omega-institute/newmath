...
theorem MetaCICCriticalPathCandidateFrontierDischargeSocketCoproduct [AskSetup]
    [PackageSetup]
    {strongNorm normalForm obstruction unblock discharge handoff continuation provenance
      localName dyadic stream regseq realSeal candidateRead frontierRead socketRead
      confluenceRead : BHist}
      ...
      Cont continuation localName candidateRead →
        Cont candidateRead discharge frontierRead →
          Cont frontierRead obstruction socketRead →
            Cont frontierRead handoff confluenceRead →
              PkgSig bundle socketRead pkg →
                PkgSig bundle confluenceRead pkg →
                  SemanticNameCert
                      (fun row : BHist =>
                        (hsame row candidateRead ∨ hsame row frontierRead ∨
                            hsame row socketRead ∨ hsame row confluenceRead ∨
                    ...
                    UnaryHistory candidateRead ∧ UnaryHistory frontierRead ∧
                      UnaryHistory socketRead ∧ UnaryHistory confluenceRead := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro ledger continuationLocalNameCandidate candidateDischargeFrontier
    frontierObstructionSocket frontierHandoffConfluence socketPkg confluencePkg
  ...
  have sourceCandidate :
      (fun row : BHist =>
        (hsame row candidateRead ∨ hsame row frontierRead ∨ hsame row socketRead ∨
            hsame row confluenceRead ∨ hsame row dyadic ∨ hsame row stream ∨
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, socketPkg, confluencePkg, realSealPkg⟩
  }
  exact ⟨cert, candidateUnary, frontierUnary, socketUnary, confluenceUnary⟩
