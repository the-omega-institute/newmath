...
theorem MetaCICCriticalPathRealCompletionSourceBudget [AskSetup] [PackageSetup]
    {strongNorm normalForm obstruction unblock discharge handoff continuation provenance
      localName dyadic stream regseq realSeal completionRead sourceRead : BHist}
              ...
              SemanticNameCert
                  ...
                  (fun row : BHist =>
                    UnaryHistory row ∧ PkgSig bundle completionRead pkg ∧
                      PkgSig bundle sourceRead pkg ∧ PkgSig bundle realSeal pkg)
                  hsame ∧
                UnaryHistory sourceRead ∧ UnaryHistory completionRead ∧
                  PkgSig bundle realSeal pkg := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle Pkg PkgSig SemanticNameCert hsame
  intro ledger realSealProvenanceCompletion dyadicStreamSource completionPkg sourcePkg
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, completionPkg, sourcePkg, realSealPkg⟩
  }
  exact ⟨cert, sourceUnary, completionUnary, realSealPkg⟩
