...
theorem MetaCICCriticalPathL10NormalizationDischargeBudget [AskSetup] [PackageSetup]
    {strongNorm normalForm obstruction handoff dischargeSocket transport route provenance
      localName dyadicBudget streamSchedule regRead realSeal socketRead : BHist}
                    ...
                    SemanticNameCert
                        (fun row : BHist =>
                          hsame row dyadicBudget ∨ hsame row streamSchedule ∨
                            hsame row regRead ∨ hsame row realSeal ∨ hsame row socketRead)
  ...
  intro packet routeLocalNameBudget budgetRouteSchedule scheduleProvenanceRead
    readLocalNameSeal handoffObstructionSocket realSealPkg socketReadPkg
  ...
  have sourceBudget :
      (fun row : BHist =>
        hsame row dyadicBudget ∨ hsame row streamSchedule ∨ hsame row regRead ∨
          hsame row realSeal ∨ hsame row socketRead) dyadicBudget := by
        ...
        | inr rest =>
            ...
            | inr rest =>
                ...
                | inr rest =>
                    ...
                    | inr sourceSocket =>
                        exact Or.inr (Or.inr (Or.inr
                          (Or.inr (hsame_trans (hsame_symm sameRows) sourceSocket))))
    }
    pattern_sound := by
      intro _row source
      cases source with
  ...
  exact ⟨cert, realSealUnary, socketReadUnary, provenancePkg⟩
