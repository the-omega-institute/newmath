...
theorem MetaCICCriticalPathResidualSubstitutionFrontier [AskSetup] [PackageSetup]
    {strongNorm normalForm obstruction handoff dischargeSocket transport route provenance
      localName closedRead residualRead mediatedRead frontierRead : BHist}
      ...
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row closedRead ∨ hsame row residualRead ∨ hsame row mediatedRead ∨
              hsame row frontierRead ∨ hsame row provenance)
          (fun row : BHist =>
            UnaryHistory row ∧ PkgSig bundle frontierRead pkg ∧
              PkgSig bundle provenance pkg)
          hsame := {
  ...
  exact ⟨cert, closedUnary, residualUnary, mediatedUnary, frontierUnary⟩
