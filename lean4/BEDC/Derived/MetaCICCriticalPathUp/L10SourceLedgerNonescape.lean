...
theorem MetaCICCriticalPathL10SourceLedgerNonescape [AskSetup] [PackageSetup]
    {strongNorm normalForm obstruction unblock discharge handoff continuation provenance
      localName dyadic stream regseq realSeal frontierRead ledgerRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    MetaCICCriticalPathOpenPhaseSourceLedger strongNorm normalForm obstruction unblock
        discharge handoff continuation provenance localName dyadic stream regseq realSeal
        bundle pkg ->
      Cont continuation handoff frontierRead ->
        Cont frontierRead realSeal ledgerRead ->
          PkgSig bundle frontierRead pkg ->
            SemanticNameCert
                (fun row : BHist => hsame row ledgerRead ∧ UnaryHistory row)
                (fun row : BHist =>
                  hsame row dyadic ∨ hsame row stream ∨ hsame row regseq ∨
                    hsame row realSeal ∨ hsame row frontierRead ∨ hsame row ledgerRead)
                (fun row : BHist =>
                  UnaryHistory row ∧ Cont continuation handoff frontierRead ∧
                    Cont frontierRead realSeal ledgerRead ∧ PkgSig bundle realSeal pkg)
                hsame ∧ UnaryHistory ledgerRead ∧ PkgSig bundle realSeal pkg := by
  -- BEDC touchpoint anchor: BHist Cont PkgSig ProbeBundle Pkg SemanticNameCert hsame
  intro ledger continuationHandoffFrontier frontierRealSealLedger _frontierPkg
  ...
  have ledgerUnary : UnaryHistory ledgerRead :=
    unary_cont_closed frontierUnary realSealUnary frontierRealSealLedger
  have sourceLedger :
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, continuationHandoffFrontier, frontierRealSealLedger,
          realSealPkg⟩
  }
  exact ⟨cert, ledgerUnary, realSealPkg⟩
