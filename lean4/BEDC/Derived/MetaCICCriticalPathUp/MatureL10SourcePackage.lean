...
theorem MetaCICCriticalPathMatureL10SourcePackage [AskSetup] [PackageSetup]
          ...
          Cont frontierRead obstruction socketRead →
            Cont socketRead realSeal maturityRead →
              PkgSig bundle maturityRead pkg →
                SemanticNameCert
                    (fun row : BHist =>
                      (hsame row maturityRead ∨ hsame row dyadic ∨ hsame row stream ∨
                      ...
                      hsame row candidateRead ∨ hsame row frontierRead ∨
                        hsame row socketRead ∨ hsame row maturityRead ∨
                          hsame row dyadic ∨ hsame row stream ∨ hsame row regseq ∨
                            hsame row realSeal)
                    (fun row : BHist =>
                      UnaryHistory row ∧ PkgSig bundle maturityRead pkg ∧
  ...
  intro ledger continuationLocalNameCandidate candidateHandoffFrontier
    frontierObstructionSocket socketRealSealMaturity maturityPkg
  ...
  have sourceMaturity :
      (fun row : BHist =>
        (hsame row maturityRead ∨ hsame row dyadic ∨ hsame row stream ∨
          hsame row regseq ∨ hsame row realSeal) ∧ UnaryHistory row) maturityRead := by
    exact ⟨Or.inl (hsame_refl maturityRead), maturityUnary⟩
  have cert :
      SemanticNameCert
          (fun row : BHist =>
            (hsame row maturityRead ∨ hsame row dyadic ∨ hsame row stream ∨
    ...
    pattern_sound := by
      ...
      | inl sameMaturity =>
          exact Or.inr (Or.inr (Or.inr (Or.inl sameMaturity)))
      | inr rest =>
          cases rest with
          | inl sameDyadic =>
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, maturityPkg, realSealPkg⟩
  }
  exact ⟨cert, candidateUnary, frontierUnary, socketUnary, maturityUnary⟩
