...
theorem MetaCICCriticalPathParallelDiamondCandidateFrontierFormalTarget [AskSetup]
    [PackageSetup]
    {strongNorm normalForm obstruction unblock discharge handoff continuation provenance
      localName dyadic stream regseq realSeal candidateRead frontierRead residualRead
      diamondRead l10Read endpoint maturityRead readback targetRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    MetaCICCriticalPathOpenPhaseSourceLedger strongNorm normalForm obstruction unblock
        discharge handoff continuation provenance localName dyadic stream regseq realSeal
        bundle pkg →
                        ...
                        PkgSig bundle targetRead pkg →
                          SemanticNameCert
                              ...
                              (fun row : BHist =>
                                hsame row candidateRead ∨ hsame row frontierRead ∨
                                  hsame row residualRead ∨ hsame row diamondRead ∨
                                    hsame row l10Read ∨ hsame row readback ∨
                                      hsame row targetRead)
                              (fun row : BHist =>
                                UnaryHistory row ∧ Cont readback provenance targetRead ∧
                                  PkgSig bundle targetRead pkg)
                              hsame ∧
  ...
  intro ledger continuationLocalCandidate candidateHandoffFrontier frontierDischargeResidual
    residualObstructionDiamond diamondRealSealL10 l10ProvenanceEndpoint
    endpointRealSealMaturity maturityHandoffReadback readbackProvenanceTarget targetPkg
  obtain ⟨packet, _dyadicUnary, _streamUnary, _regseqUnary, realSealUnary,
    _dyadicStreamRegseq, _regseqRealSealHandoff, _realSealPkg⟩ := ledger
  obtain ⟨_strongNormUnary, _normalFormUnary, obstructionUnary, _unblockUnary,
    dischargeUnary, handoffUnary, continuationUnary, provenanceUnary, localNameUnary,
  ...
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row candidateRead ∨ hsame row frontierRead ∨
              hsame row residualRead ∨ hsame row diamondRead ∨
                hsame row l10Read ∨ hsame row readback ∨ hsame row targetRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont readback provenance targetRead ∧
              PkgSig bundle targetRead pkg)
          hsame := {
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, readbackProvenanceTarget, targetPkg⟩
  }
  exact ⟨cert, targetUnary⟩

theorem MetaCICParallelDiamondFrontierReadinessEnvelope [AskSetup] [PackageSetup]
    {strongNorm normalForm obstruction unblock discharge handoff continuation provenance
      localName dyadic stream regseq realSeal candidateRead frontierRead residualRead
      diamondRead l10Read endpoint maturityRead readback targetRead envelopeRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    MetaCICCriticalPathOpenPhaseSourceLedger strongNorm normalForm obstruction unblock
        discharge handoff continuation provenance localName dyadic stream regseq realSeal
        bundle pkg →
                            ...
                            SemanticNameCert
                                ...
                                (fun row : BHist =>
                                  hsame row frontierRead ∨ hsame row residualRead ∨
                                    hsame row diamondRead ∨ hsame row l10Read ∨
                                      hsame row readback ∨ hsame row targetRead ∨
  ...
  intro ledger continuationLocalCandidate candidateHandoffFrontier frontierDischargeResidual
    ...
    targetObstructionEnvelope envelopePkg
  obtain ⟨packet, _dyadicUnary, _streamUnary, _regseqUnary, realSealUnary,
    _dyadicStreamRegseq, _regseqRealSealHandoff, _realSealPkg⟩ := ledger
  obtain ⟨_strongNormUnary, _normalFormUnary, obstructionUnary, _unblockUnary,
    dischargeUnary, handoffUnary, continuationUnary, provenanceUnary, localNameUnary,
  ...
  have sourceEnvelope :
      (fun row : BHist => hsame row envelopeRead ∧ UnaryHistory row) envelopeRead := by
    exact ⟨hsame_refl envelopeRead, envelopeUnary⟩
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row envelopeRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row frontierRead ∨ hsame row residualRead ∨ hsame row diamondRead ∨
              hsame row l10Read ∨ hsame row readback ∨ hsame row targetRead ∨
                hsame row envelopeRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont targetRead obstruction envelopeRead ∧
              PkgSig bundle envelopeRead pkg)
          hsame := {
    core := {
      carrier_inhabited := Exists.intro envelopeRead sourceEnvelope
      equiv_refl := by
        intro row _source
        exact hsame_refl row
      equiv_symm := by
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, targetObstructionEnvelope, envelopePkg⟩
  }
