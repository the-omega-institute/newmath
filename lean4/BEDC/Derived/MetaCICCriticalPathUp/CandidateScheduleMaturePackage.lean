...
theorem MetaCICCriticalPathL10ResidualDiamondSourceExhaustion [AskSetup] [PackageSetup]
                    ...
                    SemanticNameCert
                        ...
                        (fun row : BHist =>
                          hsame row dyadic ∨ hsame row stream ∨ hsame row regseq ∨
                            hsame row realSeal ∨ hsame row localDiamond ∨ hsame row endpoint)
                        (fun row : BHist =>
                          UnaryHistory row ∧ Cont frontier handoff localDiamond ∧
                            Cont localDiamond realSeal l10 ∧ PkgSig bundle endpoint pkg ∧
                              PkgSig bundle realSeal pkg)
                        hsame ∧
                      UnaryHistory dyadic ∧ UnaryHistory stream ∧ UnaryHistory regseq ∧
                        UnaryHistory realSeal ∧ UnaryHistory endpoint := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro ledger continuationLocalSchedule scheduleDischargeResidual residualObstructionFrontier
    frontierHandoffLocalDiamond localDiamondRealSealL10 l10ProvenanceEndpoint endpointPkg
  ...
  have localDiamondUnary : UnaryHistory localDiamond :=
    unary_cont_closed frontierUnary handoffUnary frontierHandoffLocalDiamond
  have l10Unary : UnaryHistory l10 :=
    unary_cont_closed localDiamondUnary realSealUnary localDiamondRealSealL10
  have endpointUnary : UnaryHistory endpoint :=
    unary_cont_closed l10Unary provenanceUnary l10ProvenanceEndpoint
  have sourceEndpoint :
      (fun row : BHist => hsame row endpoint ∧ UnaryHistory row) endpoint := by
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, frontierHandoffLocalDiamond, localDiamondRealSealL10, endpointPkg,
          realSealPkg⟩
  }
  exact ⟨cert, dyadicUnary, streamUnary, regseqUnary, realSealUnary, endpointUnary⟩
