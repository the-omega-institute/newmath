...
theorem MetaCICCriticalPathL10FourFaceClosureReadiness [AskSetup] [PackageSetup]
                      ...
                      SemanticNameCert
                          ...
                          (fun row : BHist =>
                            UnaryHistory row ∧ PkgSig bundle closureRead pkg ∧
                              Cont statusRead provenance closureRead)
                          hsame ∧
                        UnaryHistory closureRead ∧ PkgSig bundle provenance pkg := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle Pkg SemanticNameCert hsame UnaryHistory
  intro packet routeLocalNameBudget budgetRouteSchedule scheduleNormalFormReadback
    readbackProvenanceSeal sealLocalNameStatus statusProvenanceClosure closurePkg
    provenancePkgInput
  ...
  have cert :
    ...
    core := {
      carrier_inhabited :=
        Exists.intro closureRead ⟨hsame_refl closureRead, closureReadUnary⟩
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, closurePkg, statusProvenanceClosure⟩
  }
  exact ⟨cert, closureReadUnary, provenancePkgInput⟩
