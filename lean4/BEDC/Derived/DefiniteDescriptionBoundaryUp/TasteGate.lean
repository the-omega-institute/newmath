...
theorem DefiniteDescriptionBoundaryScopedKernelRoute
    {D E U S H C P N witness boundary stable named : BHist} :
    Cont D E witness →
      Cont E U boundary → Cont U S stable → Cont stable N named →
        SemanticNameCert
          (fun row : BHist =>
            hsame row named ∧
              ∃ packet : DefiniteDescriptionBoundaryUp,
                packet = DefiniteDescriptionBoundaryUp.mk D E U S H C P N)
          (fun row : BHist =>
            Cont D E witness ∧ Cont E U boundary ∧ Cont U S stable ∧ Cont stable N row)
          (fun row : BHist =>
            hsame row named ∧ hsame H H ∧ hsame C C ∧ hsame P P ∧ hsame N N)
          hsame := by
  -- BEDC touchpoint anchor: BHist Cont hsame SemanticNameCert
  intro descriptionExistence existenceBoundary uniquenessStable stableNamed
  exact {
    core := {
      ...
      equiv_symm := by intro _row _row' sameRows; exact hsame_symm sameRows
      equiv_trans := by
        intro _row _row' _row'' sameLeft sameRight
        exact hsame_trans sameLeft sameRight
      carrier_respects_equiv := by intro _row _row' sameRows source; exact ⟨hsame_trans
        (hsame_symm sameRows) source.left, source.right⟩
    }
    pattern_sound := by
      intro row source
      exact
        ⟨descriptionExistence, existenceBoundary, uniquenessStable,
          cont_result_hsame_transport stableNamed (hsame_symm source.left)⟩
    ledger_sound := by
      intro _row source
      exact ⟨source.left, hsame_refl H, hsame_refl C, hsame_refl P, hsame_refl N⟩
  }
