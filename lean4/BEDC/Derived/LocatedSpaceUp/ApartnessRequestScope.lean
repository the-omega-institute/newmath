...
theorem LocatedSpaceCarrier_apartness_request_scope [AskSetup] [PackageSetup]
    {X R A G W Q E H C P N requestRead gapRead windowRead readbackRead sealRead : BHist}
  ...
  intro rows requestRoute gapRoute windowRoute readbackRoute sealRoute namePkg
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, requestRoute, gapRoute, namePkg⟩
  }
  exact ⟨cert, gapUnary, sealUnary⟩
