...
theorem PolishSpaceCompleteSeparableObservationLock [AskSetup] [PackageSetup]
    {M K D S R W H C G N completionRead densityRead streamRead observationRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    PolishSpaceCarrier M K D S R W H C G N bundle pkg →
      Cont M K completionRead →
        Cont M D densityRead →
          Cont completionRead densityRead streamRead →
            Cont streamRead W observationRead →
              PkgSig bundle G pkg →
                PkgSig bundle N pkg →
                  SemanticNameCert
                      ...
                      (fun row : BHist =>
                        hsame row M ∨ hsame row K ∨ hsame row D ∨ hsame row S ∨
                          hsame row R ∨ hsame row W ∨ hsame row streamRead ∨
                            hsame row observationRead)
                      (fun row : BHist =>
                        UnaryHistory row ∧ Cont completionRead densityRead streamRead ∧
                          Cont streamRead W observationRead ∧ PkgSig bundle G pkg ∧
                            PkgSig bundle N pkg)
                      hsame ∧
  ...
  intro carrier completionRoute densityRoute streamRoute observationRoute provenancePkg localNamePkg
  ...
  have streamUnary : UnaryHistory streamRead :=
    unary_cont_closed completionUnary densityUnary streamRoute
  have observationUnary : UnaryHistory observationRead :=
    unary_cont_closed streamUnary WUnary observationRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row observationRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row M ∨ hsame row K ∨ hsame row D ∨ hsame row S ∨ hsame row R ∨
              hsame row W ∨ hsame row streamRead ∨ hsame row observationRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont completionRead densityRead streamRead ∧
              Cont streamRead W observationRead ∧ PkgSig bundle G pkg ∧
                PkgSig bundle N pkg)
    ...
    core := {
      carrier_inhabited :=
        Exists.intro observationRead ⟨hsame_refl observationRead, observationUnary⟩
      equiv_refl := by
        intro row _source
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, streamRoute, observationRoute, provenancePkg, localNamePkg⟩
  }
  exact ⟨cert, completionUnary, densityUnary, observationUnary⟩
