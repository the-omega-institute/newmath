...
theorem PolishSpaceObligationClosureWitness [AskSetup] [PackageSetup]
                  ...
                  PkgSig bundle G pkg →
                    PkgSig bundle N pkg →
                      SemanticNameCert
                          ...
                          (fun row : BHist =>
                            UnaryHistory row ∧ Cont streamWindow R readbackRead ∧
                              Cont readbackRead W frontierRead ∧
                                Cont frontierRead N closureRead ∧ PkgSig bundle G pkg ∧
                                  PkgSig bundle N pkg)
                          hsame ∧
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, readbackRoute, frontierRoute, closureRoute, provenancePkg,
          localNamePkg⟩
  }
