...
theorem PolishSpaceCompleteSeparableUnblockPackage [AskSetup] [PackageSetup]
    {metric complete separable stream readback ledger transport replay completionRead denseRead
      rootRead finalRead provenance localName : BHist}
                                ...
                                SemanticNameCert
                                  ...
                                  UnaryHistory completionRead ∧
                                    UnaryHistory denseRead ∧
                                      UnaryHistory rootRead ∧ UnaryHistory finalRead := by
  ...
  intro metricUnary completeUnary separableUnary _streamUnary readbackUnary ledgerUnary
    ...
    replayReadbackRoot rootLedgerFinal provenancePkg localNamePkg
      ...
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row metric ∨ hsame row complete ∨ hsame row separable ∨
              hsame row stream ∨ hsame row readback ∨ hsame row ledger ∨ hsame row finalRead)
          (fun row : BHist =>
  ...
  exact ⟨cert, completionUnary, denseUnary, rootUnary, finalUnary⟩

theorem PolishSpaceCompleteSeparableSharedSourceRoute [AskSetup] [PackageSetup]
    {metric complete separable stream readback ledger transport completionRead denseRead
      sharedRead finalRead provenance localName : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    UnaryHistory metric →
      UnaryHistory complete →
        UnaryHistory separable →
                  ...
                  Cont metric complete completionRead →
                    Cont metric separable denseRead →
                      Cont completionRead denseRead sharedRead →
                        Cont sharedRead ledger finalRead →
                          PkgSig bundle provenance pkg →
                            PkgSig bundle localName pkg →
                              SemanticNameCert
                                  (fun row : BHist => hsame row finalRead ∧ UnaryHistory row)
                                  (fun row : BHist =>
                                    hsame row metric ∨ hsame row complete ∨
                                      hsame row separable ∨ hsame row sharedRead ∨
                                        hsame row finalRead)
                                  (fun row : BHist =>
                                    UnaryHistory row ∧ PkgSig bundle provenance pkg ∧
                                      PkgSig bundle localName pkg)
                                ...
                                UnaryHistory completionRead ∧
                                  UnaryHistory denseRead ∧
                                    UnaryHistory sharedRead ∧ UnaryHistory finalRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro metricUnary completeUnary separableUnary _streamUnary _readbackUnary ledgerUnary
    _transportUnary metricCompleteRead metricSeparableRead completionDenseShared
    sharedLedgerFinal provenancePkg localNamePkg
  have completionUnary : UnaryHistory completionRead :=
    unary_cont_closed metricUnary completeUnary metricCompleteRead
  have denseUnary : UnaryHistory denseRead :=
    unary_cont_closed metricUnary separableUnary metricSeparableRead
  have sharedUnary : UnaryHistory sharedRead :=
    unary_cont_closed completionUnary denseUnary completionDenseShared
  have finalUnary : UnaryHistory finalRead :=
    unary_cont_closed sharedUnary ledgerUnary sharedLedgerFinal
  have sourceFinal :
      (fun row : BHist => hsame row finalRead ∧ UnaryHistory row) finalRead := by
    exact ⟨hsame_refl finalRead, finalUnary⟩
  have cert :
      SemanticNameCert
          ...
          (fun row : BHist =>
            hsame row metric ∨ hsame row complete ∨ hsame row separable ∨
              hsame row sharedRead ∨ hsame row finalRead)
          (fun row : BHist =>
            UnaryHistory row ∧ PkgSig bundle provenance pkg ∧ PkgSig bundle localName pkg)
          hsame := {
    core := {
