...
theorem PolishSpaceRegSeqRatRealHandoff [AskSetup] [PackageSetup]
    {metric complete separable stream readback ledger transport replay completionRead denseRead
      densityRead finalRead realSeal provenance localName : BHist}
                      ...
                      Cont ledger transport replay →
                        Cont replay readback densityRead →
                          Cont densityRead metric finalRead →
                            Cont finalRead readback realSeal →
                                  ...
                                  SemanticNameCert
                                      ...
                                      (fun row : BHist =>
                                        hsame row stream ∨ hsame row readback ∨
                                          hsame row finalRead ∨ hsame row realSeal)
                                      (fun row : BHist =>
                                    ...
                                    UnaryHistory completionRead ∧
                                      UnaryHistory denseRead ∧
                                        UnaryHistory densityRead ∧
                                          UnaryHistory finalRead ∧ UnaryHistory realSeal := by
  ...
  intro metricUnary completeUnary separableUnary streamUnary readbackUnary ledgerUnary
    ...
    replayReadbackDensity densityMetricFinal finalReadbackRealSeal provenancePkg localNamePkg
  ...
  have realSealUnary : UnaryHistory realSeal :=
    unary_cont_closed finalUnary readbackUnary finalReadbackRealSeal
  have sourceRealSeal :
  ...
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row realSeal ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row stream ∨ hsame row readback ∨ hsame row finalRead ∨ hsame row realSeal)
          (fun row : BHist =>
  ...
  exact ⟨cert, completionUnary, denseUnary, densityUnary, finalUnary, realSealUnary⟩
