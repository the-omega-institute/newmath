...
theorem PolishSpaceRootCarrier_rows_exposed [AskSetup] [PackageSetup]
    ...
    PolishSpaceRootUnblockSurface metric complete separable stream readback ledger transport
      ...
      Cont replay localName rootRead →
              ...
              (fun row : BHist =>
                hsame row metric ∨ hsame row complete ∨ hsame row separable ∨
                  hsame row stream ∨ hsame row readback ∨ hsame row ledger ∨
                    hsame row replay ∨ hsame row localName ∨ hsame row rootRead)
              (fun row : BHist =>
                UnaryHistory row ∧ PkgSig bundle provenance pkg ∧
                  PkgSig bundle localName pkg ∧ PkgSig bundle rootRead pkg)
              hsame ∧
            UnaryHistory rootRead := by
  -- BEDC touchpoint anchor: PolishSpaceRootUnblockSurface BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro surface rootRoute rootPkg
  obtain ⟨_metricUnary, _completeUnary, _separableUnary, _streamUnary, _readbackUnary,
    ledgerUnary, transportUnary, localNameUnary, _metricCompleteRoute, _metricSeparableRoute,
    replayRoute, provenancePkg, localNamePkg⟩ := surface
  have replayUnary : UnaryHistory replay :=
    unary_cont_closed ledgerUnary transportUnary replayRoute
  have rootUnary : UnaryHistory rootRead :=
    unary_cont_closed replayUnary localNameUnary rootRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row rootRead ∧ UnaryHistory row)
          (fun row : BHist =>
