...
theorem PolishSpaceDenseScheduleRootObligationCert [AskSetup] [PackageSetup]
    {metric separable stream readback ledger transport denseRead finalRead provenance
      localName : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    UnaryHistory metric →
      UnaryHistory separable →
            ...
            UnaryHistory ledger →
              UnaryHistory transport →
                Cont metric separable denseRead →
                  Cont denseRead stream finalRead →
                    PkgSig bundle provenance pkg →
                      PkgSig bundle localName pkg →
                        SemanticNameCert
                            ...
                            (fun row : BHist =>
                              hsame row metric ∨ hsame row separable ∨ hsame row stream ∨
                                hsame row readback ∨ hsame row ledger ∨ hsame row finalRead)
                            (fun row : BHist =>
                              UnaryHistory row ∧ PkgSig bundle provenance pkg ∧
                                PkgSig bundle localName pkg)
                            hsame ∧
  ...
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro metricUnary separableUnary streamUnary _readbackUnary _ledgerUnary _transportUnary
    metricSeparableDense denseStreamFinal provenancePkg localNamePkg
  have denseUnary : UnaryHistory denseRead :=
    unary_cont_closed metricUnary separableUnary metricSeparableDense
  have finalUnary : UnaryHistory finalRead :=
    unary_cont_closed denseUnary streamUnary denseStreamFinal
  have sourceFinal :
  ...
  exact ⟨cert, denseUnary, finalUnary⟩
