...
theorem PolishspaceCompleteSeparableObligationClosure [AskSetup] [PackageSetup]
    {M K D S R W H C G N completionRead denseRead streamWindow readbackRead observationRead
      closureRead : BHist}
        ...
        Cont M D denseRead ->
          Cont denseRead S streamWindow ->
            Cont streamWindow R readbackRead ->
              Cont readbackRead W observationRead ->
                Cont observationRead H closureRead ->
                  PkgSig bundle G pkg ->
                    PkgSig bundle N pkg ->
                      SemanticNameCert
                        ...
                        UnaryHistory completionRead ∧ UnaryHistory denseRead ∧
                          UnaryHistory streamWindow ∧ UnaryHistory readbackRead ∧
                            UnaryHistory observationRead ∧ UnaryHistory closureRead := by
  -- BEDC touchpoint anchor: PolishSpaceCarrier BHist Cont ProbeBundle PkgSig SemanticNameCert hsame UnaryHistory
  intro carrier completionRoute denseRoute streamRoute readbackRoute observationRoute
    closureRoute provenancePkg localNamePkg
  obtain ⟨MUnary, KUnary, DUnary, SUnary, RUnary, WUnary, HUnary, _CUnary,
  ...
  have observationUnary : UnaryHistory observationRead :=
    unary_cont_closed readbackUnary WUnary observationRoute
  have closureUnary : UnaryHistory closureRead :=
    unary_cont_closed observationUnary HUnary closureRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row closureRead ∧ UnaryHistory row)
          (fun row : BHist =>
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont denseRead S streamWindow ∧
              Cont streamWindow R readbackRead ∧ Cont readbackRead W observationRead ∧
                Cont observationRead H closureRead ∧ PkgSig bundle G pkg ∧
                  PkgSig bundle N pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact
        ⟨source.right, streamRoute, readbackRoute, observationRoute, closureRoute,
          provenancePkg, localNamePkg⟩
  }
  exact
    ⟨cert, completionUnary, denseUnary, streamUnary, readbackUnary, observationUnary,
      closureUnary⟩
