...
theorem ClosedTermSubstitutionBoundaryRootAuditReplayTotality [AskSetup] [PackageSetup]
            ...
            Cont route audit consumer ->
              Cont audit route auditReplay ->
                Cont auditReplay consumer auditTerminal ->
                  Cont auditTerminal audit publicRead ->
                    PkgSig bundle publicRead pkg ->
                      UnaryHistory ledger ∧ UnaryHistory audit ∧ UnaryHistory route ∧
                        UnaryHistory consumer ∧ UnaryHistory auditReplay ∧
                          UnaryHistory auditTerminal ∧ UnaryHistory publicRead ∧
                            Cont route audit consumer ∧ Cont auditTerminal audit publicRead ∧
                              PkgSig bundle publicRead pkg := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont UnaryHistory PkgSig
  intro classifier shiftSubstitutionLedger substitutionDepthAudit ledgerAuditRoute
    routeAuditConsumer auditRouteReplay replayConsumerTerminal terminalAuditPublic publicPkg
  ...
  have publicReadUnary : UnaryHistory publicRead :=
    unary_cont_closed auditTerminalUnary auditUnary terminalAuditPublic
  exact
    ⟨ledgerUnary, auditUnary, routeUnary, consumerUnary, auditReplayUnary,
      auditTerminalUnary, publicReadUnary, routeAuditConsumer, terminalAuditPublic, publicPkg⟩
