...
theorem ClosedTermSubstitutionBoundaryRouteSaturationStrictObstruction [AskSetup]
    [PackageSetup]
    {source value depth shift substitution ledger audit route consumer obstruction : BHist}
        ...
        Cont substitution depth audit →
          Cont ledger audit route →
            Cont route audit consumer →
              Cont consumer route obstruction →
                      ...
                      (fun row : BHist =>
                        hsame row source ∨ hsame row value ∨ hsame row depth ∨
                          hsame row ledger ∨ hsame row audit ∨ hsame row route ∨
                            hsame row consumer ∨ hsame row obstruction)
                      (fun row : BHist =>
                        UnaryHistory row ∧ Cont ledger audit route ∧
                          Cont route audit consumer ∧ Cont consumer route obstruction ∧
                            PkgSig bundle obstruction pkg)
                      hsame ∧
  ...
  intro classifier shiftSubstitutionLedger substitutionDepthAudit ledgerAuditRoute
    routeAuditConsumer consumerRouteObstruction obstructionPkg
  ...
  have obstructionUnary : UnaryHistory obstruction :=
    unary_cont_closed consumerUnary routeUnary consumerRouteObstruction
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row obstruction ∧ UnaryHistory row)
          (fun row : BHist =>
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont ledger audit route ∧ Cont route audit consumer ∧
              Cont consumer route obstruction ∧ PkgSig bundle obstruction pkg)
          hsame := {
    core := {
      carrier_inhabited :=
    ...
    ledger_sound := by
      intro _row sourceData
      exact
        ⟨sourceData.right, ledgerAuditRoute, routeAuditConsumer, consumerRouteObstruction,
          obstructionPkg⟩
  }
