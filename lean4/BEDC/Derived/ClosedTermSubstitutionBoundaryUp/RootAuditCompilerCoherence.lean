...
theorem ClosedTermSubstitutionBoundaryRootAuditCompilerCoherence [AskSetup] [PackageSetup]
  ...
  intro classifier shiftSubstitutionLedger substitutionDepthAudit ledgerAuditRoute
    routeAuditCompilerRead auditRouteReplay replayCompilerHandoff handoffPkg
  ...
  have cert :
    ...
    pattern_sound := by
      intro _row source
      exact
        ⟨cont_result_hsame_transport replayCompilerHandoff (hsame_symm source),
          handoffPkg⟩
    ledger_sound := by
      intro _row source
      exact ⟨unary_transport handoffUnary (hsame_symm source), handoffPkg⟩
  }
  exact ⟨cert, compilerReadUnary, auditReplayUnary, handoffUnary⟩
