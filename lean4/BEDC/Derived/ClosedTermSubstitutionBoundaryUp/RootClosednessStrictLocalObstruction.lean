import BEDC.Derived.ClosedTermSubstitutionBoundaryUp.RootClosednessGate
...
theorem ClosedTermSubstitutionBoundaryRootClosednessStrictLocalObstruction [AskSetup]
    [PackageSetup]
        ...
        Cont substitution depth audit ->
          Cont ledger audit route ->
            Cont route audit consumer ->
              Cont consumer ledger obstruction ->
                  ...
                  SemanticNameCert
                      ...
                      (fun row : BHist =>
                        UnaryHistory row ∧ Cont ledger audit route ∧
                          Cont route audit consumer ∧ Cont consumer ledger obstruction ∧
                            PkgSig bundle obstruction pkg)
                      hsame ∧
  ...
  intro classifier shiftSubstitutionLedger substitutionDepthAudit ledgerAuditRoute
    routeAuditConsumer consumerLedgerObstruction obstructionPkg
  ...
  have obstructionUnary : UnaryHistory obstruction :=
    unary_cont_closed consumerUnary ledgerUnary consumerLedgerObstruction
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row obstruction ∧ UnaryHistory row)
          (fun row : BHist =>
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont ledger audit route ∧ Cont route audit consumer ∧
              Cont consumer ledger obstruction ∧ PkgSig bundle obstruction pkg)
          hsame := {
    core := {
      carrier_inhabited :=
    ...
    ledger_sound := by
      intro _row sourceData
      exact
        ⟨sourceData.right, ledgerAuditRoute, routeAuditConsumer,
          consumerLedgerObstruction, obstructionPkg⟩
  }
