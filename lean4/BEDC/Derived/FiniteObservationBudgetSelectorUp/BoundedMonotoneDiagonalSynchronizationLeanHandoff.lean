...
theorem FiniteObservationBudgetSelectorCarrier_bounded_monotone_diagonal_synchronization_lean_handoff
    [AskSetup] [PackageSetup]
    {B S W D R E H C P N terminal boundedSeal diagonalRead handoff : BHist}
                ...
                SemanticNameCert
                    ...
                    (fun row : BHist =>
                      hsame row terminal ∨ hsame row boundedSeal ∨ hsame row diagonalRead ∨
                        hsame row handoff)
                    (fun row : BHist => PkgSig bundle handoff pkg ∧ hsame row handoff)
                    hsame ∧
                  UnaryHistory terminal ∧ UnaryHistory boundedSeal ∧
                    UnaryHistory diagonalRead ∧ UnaryHistory handoff := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg SemanticNameCert hsame Cont
  intro carrier readEndpoint terminalEndpoint terminalBounded diagonalEndpoint handoffPkg
  obtain ⟨unaryB, unaryS, unaryD, unaryE, _unaryH, budgetRoute, windowRoute,
    _regularRoute, _sameEndpoint⟩ := carrier
  ...
  exact ⟨cert, terminalUnary, boundedSealUnary, diagonalReadUnary, handoffUnary⟩
