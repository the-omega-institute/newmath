...
theorem MetaCICRedexFrontierSNObstructionRetention [AskSetup] [PackageSetup]
    {B A L P O E H C G N obstructionRead betaRead handoffRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    MetaCICRedexFrontierCarrier B A L P O E H C G N →
      Cont O E obstructionRead →
        Cont B E betaRead →
          Cont obstructionRead C handoffRead →
            PkgSig bundle handoffRead pkg →
              UnaryHistory O ∧ UnaryHistory obstructionRead ∧ UnaryHistory handoffRead ∧
                hsame H (append B O) ∧
                  List.Mem (metaCICRedexFrontierEncodeBHist O)
                    (metaCICRedexFrontierToEventFlow
                      (MetaCICRedexFrontierUp.mk B A L P O E H C G N)) ∧
                    Cont O E obstructionRead ∧ Cont obstructionRead C handoffRead ∧
                      PkgSig bundle handoffRead pkg := by
  -- BEDC touchpoint anchor: MetaCICRedexFrontierCarrier BHist BMark Cont ProbeBundle PkgSig
  intro carrier obstructionRoute _betaRoute handoffRoute handoffPkg
  obtain ⟨_bUnary, _aUnary, _lUnary, _pUnary, oUnary, eUnary, gUnary, boundarySame,
    carrierBetaRoute, _carrierArgRoute, carrierProvenanceRoute⟩ := carrier
  ...
  have handoffUnary : UnaryHistory handoffRead :=
    unary_cont_closed obstructionUnary cUnary handoffRoute
  ...
  exact
    ⟨oUnary, obstructionUnary, handoffUnary, boundarySame, obstructionListed,
      obstructionRoute, handoffRoute, handoffPkg⟩
