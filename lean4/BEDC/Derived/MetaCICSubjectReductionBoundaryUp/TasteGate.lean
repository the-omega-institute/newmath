...
theorem MetaCICSubjectReductionBoundaryCarrier_nonescape [AskSetup] [PackageSetup]
    {beta app lam pi preservation obstruction audit transport route provenance name consumer : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    Cont beta app preservation ->
      Cont lam pi preservation ->
        Cont obstruction audit route ->
          Cont route transport consumer ->
            PkgSig bundle consumer pkg ->
              SemanticNameCert
                (fun row : BHist =>
                  hsame row consumer /\
                    exists packet : MetaCICSubjectReductionBoundaryUp,
                ...
                (fun row : BHist =>
                  Cont beta app preservation /\ Cont lam pi preservation /\
                    Cont obstruction audit route /\ Cont route transport consumer /\
                      hsame row consumer)
                (fun row : BHist => hsame row consumer /\ PkgSig bundle consumer pkg)
                hsame := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig hsame SemanticNameCert
  intro betaApp lamPi obstructionAudit routeTransport consumerPkg
  exact {
    core := {
      carrier_inhabited :=
    ...
    pattern_sound := by
      ...
      exact ⟨betaApp, lamPi, obstructionAudit, routeTransport, sourceRow.left⟩
    ledger_sound := by
      intro _row sourceRow
