...
theorem NormedSpaceCarrier_real_norm_metric_factorization [AskSetup] [PackageSetup]
        ...
        Cont normRead M metricRead ->
          PkgSig bundle metricRead pkg ->
            SemanticNameCert
                ...
                (fun row : BHist =>
                  hsame row V ∨ hsame row N ∨ hsame row M ∨ hsame row normRead ∨
                    hsame row metricRead)
                (fun row : BHist =>
                  UnaryHistory row ∧ Cont V N normRead ∧ Cont normRead M metricRead ∧
                    PkgSig bundle metricRead pkg)
                hsame ∧
              UnaryHistory normRead ∧ UnaryHistory metricRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont UnaryHistory PkgSig hsame SemanticNameCert
  intro carrier normRoute metricRoute metricPkg
  obtain ⟨vUnary, _rUnary, nUnary, mUnary, _qUnary, _hUnary, _tUnary, _pUnary,
  ...
  have cert :
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, normRoute, metricRoute, metricPkg⟩
  }
  exact ⟨cert, normUnary, metricUnary⟩
