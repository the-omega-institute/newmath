...
theorem QuotientSoundnessBoundary_psame_consumer_closure [AskSetup] [PackageSetup]
  ...
  obtain ⟨eUnary, _aUnary, tUnary, vUnary, hUnary, cUnary, _pUnary, _nUnary, eAV,
    eTH, _hCN, _pPkg, _nPkg, _hN⟩ := carrier
  ...
  exact {
    ...
    pattern_sound := by
      intro row source
      exact
        ⟨eAV, eTH, source.right.left, source.right.right.left,
    ...
    ledger_sound := by
      ...
      exact
        ⟨unary_transport consumerUnary (hsame_symm source.right.right.right.right),
          refusalPkg, transportPkg, consumerPkg⟩
  }
