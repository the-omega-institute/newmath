...
theorem BanachSpaceMatureLinearCompletionExport [AskSetup] [PackageSetup]
    {V N M Q S R E Z H C P L normRead metricRead cauchyRead completionRead
      toleranceRead separatedRead transportRead replayRead scalarName matureRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    UnaryHistory V ->
      UnaryHistory N ->
        UnaryHistory M ->
                                    ...
                                    Cont toleranceRead Z separatedRead ->
                                      Cont separatedRead H transportRead ->
                                        Cont transportRead C replayRead ->
                                          Cont replayRead L matureRead ->
                                            Cont toleranceRead E scalarName ->
                                              PkgSig bundle P pkg ->
  ...
  intro unaryV unaryN unaryM unaryQ unaryS unaryR unaryE unaryZ unaryH unaryC unaryL
    ...
    transportRoute replayRoute matureRoute scalarRoute _provenancePkg maturePkg
  have normUnary : UnaryHistory normRead :=
  ...
  have matureUnary : UnaryHistory matureRead :=
    unary_cont_closed replayUnary unaryL matureRoute
  have scalarUnary : UnaryHistory scalarName :=
    unary_cont_closed toleranceUnary unaryE scalarRoute
  have sourceMature :
          ...
          (fun row : BHist =>
            UnaryHistory row ∧ Cont V N normRead ∧ Cont normRead M metricRead ∧
              Cont Q S cauchyRead ∧ Cont cauchyRead R completionRead ∧
                Cont completionRead E toleranceRead ∧ Cont toleranceRead Z separatedRead ∧
                  Cont separatedRead H transportRead ∧ Cont transportRead C replayRead ∧
                    Cont replayRead L matureRead ∧ PkgSig bundle matureRead pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, normRoute, metricRoute, cauchyRoute, completionRoute,
          toleranceRoute, separatedRoute, transportRoute, replayRoute, matureRoute,
          maturePkg⟩
  }
  exact
    ⟨cert, normUnary, metricUnary, cauchyUnary, completionUnary, toleranceUnary,
      separatedUnary, transportUnary, replayUnary, scalarUnary, matureUnary⟩
