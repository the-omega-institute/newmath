...
theorem group_centralizer_normalizer_orbit_endpoint_transport_iff
  ...
  have _invCongr : forall {u v : BHist}, hsame u v -> hsame (inv u) (inv v) :=
    group_inverse_congruence_from_laws assocC leftId rightId mulCongr leftInv rightInv
  have conjCongrRight :
  ...
  · intro orbitXY
    ...
    | intro s data =>
        have sameEndpoint :
            hsame (mul (mul s x') (inv s)) y' := by
          exact hsame_trans (conjCongrRight (hsame_symm sameXX'))
            (hsame_trans data.right.right.right sameYY')
        exact Exists.intro s
          (And.intro data.left (And.intro centralX' (And.intro centralY' sameEndpoint)))
  · intro orbitX'Y'
    cases orbitX'Y' with
    | intro s data =>
        have sameEndpoint :
            hsame (mul (mul s x) (inv s)) y := by
          exact hsame_trans (conjCongrRight sameXX')
            (hsame_trans data.right.right.right (hsame_symm sameYY'))
        exact Exists.intro s
          (And.intro data.left (And.intro centralX (And.intro centralY sameEndpoint)))
