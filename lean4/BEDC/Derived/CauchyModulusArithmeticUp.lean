...
theorem CauchyModulusArithmeticCarrier_real_seal_schedule_nonescape [AskSetup]
    [PackageSetup]
    {stream0 stream1 modulus0 modulus1 meet sum product dyadic window readback sealRow transport
      replay provenance localName scheduleRead sumRead productRead sealRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    CauchyModulusArithmeticCarrier stream0 stream1 modulus0 modulus1 meet sum product dyadic
        window readback sealRow transport replay provenance localName bundle pkg ->
      Cont meet dyadic scheduleRead ->
        Cont scheduleRead sum sumRead ->
          Cont scheduleRead product productRead ->
            Cont sumRead productRead sealRead ->
              PkgSig bundle sealRead pkg ->
                UnaryHistory scheduleRead ∧ UnaryHistory sumRead ∧
                  UnaryHistory productRead ∧ UnaryHistory sealRead ∧
                    Cont meet dyadic scheduleRead ∧ Cont scheduleRead sum sumRead ∧
                      Cont scheduleRead product productRead ∧
                        Cont sumRead productRead sealRead ∧ Cont window readback sealRow ∧
                          Cont transport replay provenance ∧ PkgSig bundle localName pkg ∧
                            PkgSig bundle sealRead pkg := by
  -- BEDC touchpoint anchor: BHist Cont ProbeBundle Pkg PkgSig UnaryHistory
  intro carrier scheduleRoute sumReadRoute productReadRoute sealReadRoute sealReadPackage
  obtain
    ⟨_stream0Unary, _stream1Unary, _modulus0Unary, _modulus1Unary, meetUnary,
      sumUnary, productUnary, dyadicUnary, _windowUnary, _readbackUnary, _sealUnary,
      _transportUnary, _meetRoute, _sumRoute, _productRoute, windowReadbackSeal,
  ...
  have scheduleUnary : UnaryHistory scheduleRead :=
    unary_cont_closed meetUnary dyadicUnary scheduleRoute
  have sumReadUnary : UnaryHistory sumRead :=
    unary_cont_closed scheduleUnary sumUnary sumReadRoute
  have productReadUnary : UnaryHistory productRead :=
    unary_cont_closed scheduleUnary productUnary productReadRoute
  have sealReadUnary : UnaryHistory sealRead :=
    unary_cont_closed sumReadUnary productReadUnary sealReadRoute
  exact
    ⟨scheduleUnary, sumReadUnary, productReadUnary, sealReadUnary, scheduleRoute,
      sumReadRoute, productReadRoute, sealReadRoute, windowReadbackSeal,
      transportReplayProvenance, localPackage, sealReadPackage⟩
...
theorem CauchyModulusArithmeticCarrier_real_seal_distribution [AskSetup] [PackageSetup]
    {stream0 stream1 modulus0 modulus1 meet sum product dyadic window readback sealRow transport
      replay provenance localName sumRead productRead sealRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    CauchyModulusArithmeticCarrier stream0 stream1 modulus0 modulus1 meet sum product dyadic
        window readback sealRow transport replay provenance localName bundle pkg ->
      Cont meet dyadic sumRead ->
        Cont meet dyadic productRead ->
          Cont sumRead productRead sealRead ->
            PkgSig bundle sealRead pkg ->
              UnaryHistory meet ∧ UnaryHistory dyadic ∧ UnaryHistory sumRead ∧
                UnaryHistory productRead ∧ UnaryHistory sealRead ∧
                  Cont meet dyadic sumRead ∧ Cont meet dyadic productRead ∧
                    Cont sumRead productRead sealRead ∧ Cont window readback sealRow ∧
                      Cont transport replay provenance ∧ PkgSig bundle localName pkg ∧
                        PkgSig bundle sealRead pkg := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig UnaryHistory
  intro carrier sumReadRoute productReadRoute sealReadRoute sealReadPackage
  obtain
    ⟨_stream0Unary, _stream1Unary, _modulus0Unary, _modulus1Unary, meetUnary,
      _sumUnary, _productUnary, dyadicUnary, _windowUnary, _readbackUnary,
      _sealRowUnary, _transportUnary, _meetRoute, _sumRoute, _productRoute,
      windowReadbackSeal, transportReplayProvenance, localPackage⟩ := carrier
  have sumReadUnary : UnaryHistory sumRead :=
    unary_cont_closed meetUnary dyadicUnary sumReadRoute
  have productReadUnary : UnaryHistory productRead :=
    unary_cont_closed meetUnary dyadicUnary productReadRoute
  have sealReadUnary : UnaryHistory sealRead :=
    unary_cont_closed sumReadUnary productReadUnary sealReadRoute
  exact
    ⟨meetUnary, dyadicUnary, sumReadUnary, productReadUnary, sealReadUnary,
      sumReadRoute, productReadRoute, sealReadRoute, windowReadbackSeal,
      transportReplayProvenance, localPackage, sealReadPackage⟩

theorem CauchyModulusArithmeticCarrier_dyadic_sum_threshold_obligation [AskSetup] [PackageSetup]
    {stream0 stream1 modulus0 modulus1 meet sum product dyadic window readback sealRow transport replay provenance localName tailWindow sumThreshold : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    CauchyModulusArithmeticCarrier stream0 stream1 modulus0 modulus1 meet sum product dyadic window readback sealRow transport replay provenance localName bundle pkg ->
      Cont modulus0 meet tailWindow -> Cont meet dyadic sumThreshold -> PkgSig bundle sumThreshold pkg ->
        UnaryHistory modulus0 ∧ UnaryHistory modulus1 ∧ UnaryHistory meet ∧ UnaryHistory dyadic ∧ UnaryHistory window ∧ UnaryHistory readback ∧ UnaryHistory tailWindow ∧ UnaryHistory sumThreshold ∧ Cont modulus0 meet tailWindow ∧ Cont meet dyadic sum ∧ Cont meet dyadic sumThreshold ∧ Cont window readback sealRow ∧ PkgSig bundle localName pkg ∧ PkgSig bundle sumThreshold pkg := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont PkgSig UnaryHistory
  intro carrier tailRoute thresholdRoute thresholdPackage
  obtain
    ⟨_stream0Unary, _stream1Unary, modulus0Unary, modulus1Unary, meetUnary,
      _sumUnary, _productUnary, dyadicUnary, windowUnary, readbackUnary, _sealRowUnary,
      _transportUnary, _meetRoute, sumRoute, _productRoute, windowReadbackSeal,
      _transportReplayProvenance, localPackage⟩ := carrier
  exact
    ⟨modulus0Unary, modulus1Unary, meetUnary, dyadicUnary, windowUnary, readbackUnary,
      unary_cont_closed modulus0Unary meetUnary tailRoute,
      unary_cont_closed meetUnary dyadicUnary thresholdRoute, tailRoute, sumRoute,
      thresholdRoute, windowReadbackSeal, localPackage, thresholdPackage⟩
