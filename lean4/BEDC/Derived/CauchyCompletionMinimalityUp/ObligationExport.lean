...
theorem CauchyCompletionMinimalityObligationExport [AskSetup] [PackageSetup]
              ...
              Cont comparedRead replay replayRead →
                Cont replayRead provenance provenanceRead →
                  Cont provenanceRead name nameRead →
                    Cont nameRead replay exportRead →
  ...
  intro carrier denseRoute universalRoute extensionRoute comparedRoute replayRoute
    provenanceRoute nameRoute exportRoute _provenancePkg _namePkg exportPkg
  obtain ⟨_sourceUnary, completionUnary, embeddingUnary, universalUnary, extensionUnary,
    separatedUnary, _transportUnary, replayUnary, _provenanceUnary, nameUnary,
  ...
  have provenanceReadUnary : UnaryHistory provenanceRead :=
    unary_cont_closed replayReadUnary _provenanceUnary provenanceRoute
  ...
  have exportReadUnary : UnaryHistory exportRead :=
    unary_cont_closed nameReadUnary replayUnary exportRoute
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row exportRead ∧ UnaryHistory row)
          (fun row : BHist =>
                ...
                Cont universalRead extension extensionRead ∧
                  Cont extension separated comparedRead ∧
                    Cont comparedRead replay replayRead ∧
                      Cont replayRead provenance provenanceRead ∧
                        Cont provenanceRead name nameRead ∧
                          Cont nameRead replay exportRead ∧ PkgSig bundle exportRead pkg)
          hsame := {
    core := {
      carrier_inhabited :=
    ...
    ledger_sound := by
      ...
      exact
        ⟨source.right, denseRoute, universalRoute, extensionRoute, comparedRoute,
          replayRoute, provenanceRoute, nameRoute, exportRoute, exportPkg⟩
  }
