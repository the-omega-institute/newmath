...
theorem SequentiallyCompleteMetricCompletionHandoffObligations [AskSetup] [PackageSetup]
                                  ...
                                  Cont replayRead N completionRead ->
                                    PkgSig bundle P pkg ->
                                      PkgSig bundle N pkg ->
                                        SemanticNameCert
                                            (fun row : BHist =>
                                              (hsame row regularRead ∨ hsame row limitRead ∨
  ...
  · exact {
        ...
        carrier_respects_equiv := by
          ...
          · cases source.left with
            ...
            | inr rest =>
                cases rest with
                | inl sameLimit =>
                    exact Or.inr (Or.inl (hsame_trans (hsame_symm sameRows) sameLimit))
                | inr sameCompletion =>
                    exact Or.inr
      ...
      pattern_sound := by
        intro _row _source
        exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr completionRoute))))
      ledger_sound := by
        intro _row source
        exact
