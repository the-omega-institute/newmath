...
theorem SequentiallyCompleteMetricRegularLimitExtraction [AskSetup] [PackageSetup]
  ...
  · exact {
      ...
      pattern_sound := by
        ...
        | inr rest =>
            cases rest with
            | inl sameD =>
                exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl sameD)))))
            | inr _sameCompletion =>
      ...
      ledger_sound := by
        ...
        exact
          ⟨source.right, sequenceRoute, regularRoute, modulusRoute, limitRoute,
            distanceRoute, completionRoute, provenancePkg, namePkg⟩
    }
