...
theorem SequentiallyCompleteMetricRootModulusCoverage [AskSetup] [PackageSetup]
                                ...
                                SemanticNameCert
                                    ...
                                    (fun row : BHist =>
                                      hsame row M ∨ hsame row D ∨
                                        hsame row modulusRead ∨ hsame row distanceRead)
                                    (fun row : BHist =>
                                      UnaryHistory row ∧ Cont sequenceRead M modulusRead ∧
                                        Cont limitRead D distanceRead ∧ PkgSig bundle P pkg ∧
                                          PkgSig bundle N pkg)
                                    hsame ∧
                                  UnaryHistory modulusRead ∧ UnaryHistory distanceRead ∧
                                    UnaryHistory replayRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro fieldRows xUnary sUnary mUnary lUnary dUnary cUnary sequenceRoute modulusRoute
  ...
  have cert :
      SemanticNameCert
          (fun row : BHist => hsame row modulusRead ∧ UnaryHistory row)
          (fun row : BHist =>
            hsame row M ∨ hsame row D ∨ hsame row modulusRead ∨ hsame row distanceRead)
          (fun row : BHist =>
            UnaryHistory row ∧ Cont sequenceRead M modulusRead ∧
              Cont limitRead D distanceRead ∧ PkgSig bundle P pkg ∧ PkgSig bundle N pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      intro _row source
      exact ⟨source.right, modulusRoute, distanceRoute, provenancePkg, namePkg⟩
  }
  exact ⟨cert, modulusUnary, distanceUnary, replayUnary⟩
