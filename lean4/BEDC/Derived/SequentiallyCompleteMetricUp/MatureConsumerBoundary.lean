...
theorem SequentiallyCompleteMetricMatureConsumerBoundary [AskSetup] [PackageSetup]
    {X S M L D H C P N sequenceRead modulusRead limitRead distanceRead replayRead
      completionRead matureRead : BHist}
                          ...
                          Cont limitRead D distanceRead →
                            Cont distanceRead C replayRead →
                              Cont replayRead N completionRead →
                                Cont completionRead C matureRead →
                                  PkgSig bundle P pkg →
                                    PkgSig bundle N pkg →
                                      SemanticNameCert
                                          (fun row : BHist =>
  ...
  intro fieldRows xUnary sUnary mUnary lUnary dUnary cUnary nUnary sequenceRoute
    modulusRoute limitRoute distanceRoute replayRoute completionRoute matureRoute
            ...
            UnaryHistory row ∧ Cont X S sequenceRead ∧ Cont sequenceRead M modulusRead ∧
              Cont modulusRead L limitRead ∧ Cont limitRead D distanceRead ∧
                Cont distanceRead C replayRead ∧ Cont replayRead N completionRead ∧
                  Cont completionRead C matureRead ∧ PkgSig bundle P pkg ∧
                    PkgSig bundle N pkg)
          hsame := {
    core := {
    ...
    ledger_sound := by
      ...
      exact
        ⟨sourceRow.right, sequenceRoute, modulusRoute, limitRoute, distanceRoute,
          replayRoute, completionRoute, matureRoute, provenancePkg, namePkg⟩
  }
