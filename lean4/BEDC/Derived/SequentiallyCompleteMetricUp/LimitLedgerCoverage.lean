...
theorem SequentiallyCompleteMetricLimitLedgerCoverage [AskSetup] [PackageSetup]
                              ...
                              Cont replayRead N namedRead ->
                                PkgSig bundle P pkg ->
                                  PkgSig bundle N pkg ->
                                    SemanticNameCert
                                        (fun row : BHist =>
                                          hsame row distanceRead ∧ UnaryHistory row)
                                        (fun row : BHist =>
                                          hsame row M ∨ hsame row D ∨
                                            hsame row limitRead ∨
                                              hsame row distanceRead ∨
                                                hsame row namedRead ∨
                                                  Cont limitRead D distanceRead)
                                        (fun row : BHist =>
                                          UnaryHistory row ∧
                                              ...
                                              Cont modulusRead L limitRead ∧
                                                Cont limitRead D distanceRead ∧
                                                  PkgSig bundle P pkg ∧
                                                    PkgSig bundle N pkg)
                                        hsame ∧
                                      UnaryHistory distanceRead ∧ UnaryHistory namedRead := by
  ...
  · exact {
      ...
      ledger_sound := by
        intro _row source
        exact ⟨source.right, modulusRoute, limitRoute, distanceRoute, provenancePkg,
          namePkg⟩
    }
  · exact And.intro distanceUnary namedUnary
