...
theorem SequentiallyCompleteMetricCompletionRouteExhaustion [AskSetup] [PackageSetup]
  ...
  · exact {
      core := {
        carrier_inhabited :=
          Exists.intro completionRead (And.intro (hsame_refl completionRead) completionUnary)
        equiv_refl := by
          intro row _source
...
theorem SequentiallyCompleteMetricCompletionRoute [AskSetup] [PackageSetup]
    {X S M L D H C P N windowRead modulusRead limitRead completionRead : BHist}
    {bundle : ProbeBundle ProbeName} {pkg : Pkg} :
    SequentiallyCompleteMetricCarrier X S M L D H C P N bundle pkg ->
      Cont X S windowRead ->
        Cont windowRead M modulusRead ->
          Cont modulusRead L limitRead ->
                  ...
                  SemanticNameCert
                      ...
                      (fun row : BHist =>
                        UnaryHistory row ∧ Cont X S windowRead ∧
                          Cont windowRead M modulusRead ∧ Cont modulusRead L limitRead ∧
                            Cont limitRead N completionRead ∧ PkgSig bundle P pkg ∧
                              PkgSig bundle completionRead pkg)
                      hsame ∧
                    UnaryHistory completionRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro carrier windowRoute modulusRoute limitRoute completionRoute provenancePkg completionPkg
  obtain ⟨xUnary, sUnary, mUnary, lUnary, _dUnary, _hUnary, _cUnary, _pUnary, nUnary,
  ...
  have modulusUnary : UnaryHistory modulusRead :=
    unary_cont_closed windowUnary mUnary modulusRoute
  have limitUnary : UnaryHistory limitRead :=
    unary_cont_closed modulusUnary lUnary limitRoute
  have completionUnary : UnaryHistory completionRead :=
  ...
  · exact {
      ...
      ledger_sound := by
        intro _row source
        exact ⟨source.right, windowRoute, modulusRoute, limitRoute, completionRoute,
