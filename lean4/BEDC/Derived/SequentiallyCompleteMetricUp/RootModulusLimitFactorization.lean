...
theorem SequentiallyCompleteMetricRootModulusLimitFactorization [AskSetup] [PackageSetup]
                        ...
                        Cont modulusRead L limitRead ->
                          Cont limitRead D distanceRead ->
                            Cont distanceRead C replayRead ->
                              Cont replayRead N factorRead ->
                                PkgSig bundle P pkg ->
                                  PkgSig bundle N pkg ->
                                    SemanticNameCert
                                        (fun row : BHist =>
                                          hsame row factorRead ∧ UnaryHistory row)
                                        (fun row : BHist =>
                                                  ...
                                                  Cont distanceRead C replayRead ∧
                                                    Cont replayRead N factorRead ∧
                                                      PkgSig bundle P pkg ∧
                                                        PkgSig bundle N pkg)
                                        hsame ∧
                                      UnaryHistory factorRead := by
  -- BEDC touchpoint anchor: BHist ProbeBundle Pkg Cont hsame SemanticNameCert UnaryHistory
  intro fieldRows xUnary sUnary mUnary lUnary dUnary cUnary nUnary sequenceRoute
    modulusRoute limitRoute distanceRoute replayRoute factorRoute provenancePkg namePkg
  cases fieldRows
  ...
  have factorUnary : UnaryHistory factorRead :=
    unary_cont_closed replayUnary nUnary factorRoute
  constructor
  · exact {
      core := {
        carrier_inhabited :=
          Exists.intro factorRead ⟨hsame_refl factorRead, factorUnary⟩
        equiv_refl := by
          intro row _source
      ...
      ledger_sound := by
        ...
        exact
          ⟨source.right, sequenceRoute, modulusRoute, limitRoute, distanceRoute,
            replayRoute, factorRoute, provenancePkg, namePkg⟩
    }
  · exact factorUnary
